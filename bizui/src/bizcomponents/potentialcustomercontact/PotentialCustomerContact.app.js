...
import styles from './PotentialCustomerContact.app.less'
...
class PotentialCustomerContactBizApp extends React.PureComponent {
constructor(props) {
    super(props)
     this.state = {
 ...
 getNavMenuItems = (targetObject, style, customTheme) => {
    ...
    return (
	  <Menu
       ...
       <Menu.Item key="workbench">
        <Link to={`/potentialCustomerContact/${this.props.potentialCustomerContact.id}/workbench`}><Icon type="solution" style={{marginRight:"20px"}}/><span>工作台</span></Link>
      </Menu.Item>
        ...
        {filteredMenuItemsGroup(targetObject,this).map((groupedMenuItem,index)=>{
          return(
    <SubMenu id={`submenu-vg${index}`}  key={`vg${index}`} title={<span><Icon type={viewGroupIconNameOf('potential_customer_contact',`${groupedMenuItem.viewGroup}`)} style={{marginRight:"20px"}} /><span>{viewGroupDisplayNameOf('potential_customer_contact',`${groupedMenuItem.viewGroup}`)}</span></span>} >
  ...
  getRequestTypeStepForm = () => {
    ...
    return connect(state => ({
      selectedRows: state._potentialCustomerContact.selectedRows,
      role: "cq",
      currentUpdateIndex: state._potentialCustomerContact.currentUpdateIndex,
      owner: { type: '_potentialCustomerContact', id: state._potentialCustomerContact.id, listName: 'nolist', ref:state._potentialCustomerContact, listDisplayName: appLocaleName(userContext,"List") }, // this is for model namespace and
  ...
  buildRouters = () =>{
    const {PotentialCustomerContactWorkbench} = GlobalComponents

    const {PotentialCustomerContactDashboard} = GlobalComponents
  	const {PotentialCustomerContactPermission} = GlobalComponents
  	const {PotentialCustomerContactProfile} = GlobalComponents


    const routers=[
    {path:"/potentialCustomerContact/:id/workbench", component: PotentialCustomerContactWorkbench},
    
  	{path:"/potentialCustomerContact/:id/profile", component: PotentialCustomerContactProfile},
  	{path:"/potentialCustomerContact/:id/permission", component: PotentialCustomerContactPermission},
  ...
  handleOpenChange = (openKeys) => {
   ...
   render() {
    ...
    const {searchLocalData}=GlobalComponents.PotentialCustomerContactBase
    ...
    const renderMenuSwitch=()=>{
     ...
     <Layout>
      ...
      {this.state.showSearch&&(
        ...
        <div style={{backgroundColor:'black'}}  onClick={()=>hideSearchResult()}  >{searchLocalData(this.props.potentialCustomerContact,this.state.searchKeyword)}</div>
       ...
       <Sider
        ...
        >
         ...
         {this.getNavMenuItems(this.props.potentialCustomerContact,"inline","dark")}
