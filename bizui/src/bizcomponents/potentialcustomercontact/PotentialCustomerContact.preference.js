...
const internalImageListOf = (potentialCustomerContact) =>{
...
const internalSettingListOf = (potentialCustomerContact) =>{
	const userContext = null
	const optionList = [ 
	]
...
const handleTransferSearch =(targetComponent,filterKey,newRequest)=>{
  ...
  const pageNo = 1;
  const candidateReferenceService = PotentialCustomerContactService[candidateServiceName] 
...
const executeTrans = (potentialCustomerContact,targetComponent) =>{
  ...
  validateFieldsAndScroll((error, values) => {
    ...
    const id=potentialCustomerContact.id;
    const serviceNameToCall = transferServiceName;

    const payload = {parameters,id,serviceNameToCall}
    
    //targetComponent.setState({transferModalVisiable:false})
    dispatch({type:"_potentialCustomerContact/doJob",payload: payload})
...
const buildTransferModal = (potentialCustomerContact,targetComponent) => {
...
<Modal title={transferModalTitle}
          ...
          onOk={()=>executeTrans(potentialCustomerContact,targetComponent)}
...
const internalRenderExtraFooter = (potentialCustomerContact) =>{
	return null
}
...
<Description term="潜在的客户">{potentialCustomerContact.potentialCustomer==null?appLocaleName(userContext,"NotAssigned"):potentialCustomerContact.potentialCustomer.displayName}
 <Icon type="swap" onClick={()=>
  showTransferModel(targetComponent,"潜在的客户","potentialCustomer","requestCandidatePotentialCustomer",
	      "transferToAnotherPotentialCustomer","anotherPotentialCustomerId",potentialCustomerContact.potentialCustomer?potentialCustomerContact.potentialCustomer.id:"")} 
  style={{fontSize: 20,color:"red"}} />
</Description>
<Description term="城市合伙人">{potentialCustomerContact.cityPartner==null?appLocaleName(userContext,"NotAssigned"):potentialCustomerContact.cityPartner.displayName}
 <Icon type="swap" onClick={()=>
  showTransferModel(targetComponent,"城市合伙人","cityPartner","requestCandidateCityPartner",
	      "transferToAnotherCityPartner","anotherCityPartnerId",potentialCustomerContact.cityPartner?potentialCustomerContact.cityPartner.id:"")} 
  style={{fontSize: 20,color:"red"}} />
</Description>
<Description term="接触">{potentialCustomerContact.contactTo==null?appLocaleName(userContext,"NotAssigned"):potentialCustomerContact.contactTo.displayName}
 <Icon type="swap" onClick={()=>
  showTransferModel(targetComponent,"接触","potentialCustomerContactPerson","requestCandidateContactTo",
	      "transferToAnotherContactTo","anotherContactToId",potentialCustomerContact.contactTo?potentialCustomerContact.contactTo.id:"")} 
  style={{fontSize: 20,color:"red"}} />
</Description>
...
class PotentialCustomerContactPreference extends Component {
...
}))(Form.create()(PotentialCustomerContactPreference))
