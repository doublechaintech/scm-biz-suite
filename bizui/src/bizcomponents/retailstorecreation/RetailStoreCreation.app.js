...
import styles from './RetailStoreCreation.app.less'
 ...
 getNavMenuItems = (targetObject, style, customTheme) => {
    ...
    return (
	  <Menu
       ...
       <Menu.Item key="workbench">
        <Link to={`/retailStoreCreation/${this.props.retailStoreCreation.id}/workbench`}><Icon type="solution" style={{marginRight:"20px"}}/><span>工作台</span></Link>
      </Menu.Item>
        ...
        {filteredMenuItemsGroup(targetObject,this).map((groupedMenuItem,index)=>{
          return(
    <SubMenu id={`submenu-vg${index}`}  key={`vg${index}`} title={<span><Icon type={viewGroupIconNameOf('retail_store_creation',`${groupedMenuItem.viewGroup}`)} style={{marginRight:"20px"}} /><span>{viewGroupDisplayNameOf('retail_store_creation',`${groupedMenuItem.viewGroup}`)}</span></span>} >
  ...
  getRetailStoreSearch = () => {
    ...
    return connect(state => ({
      rule: state.rule,
      name: window.mtrans('retail_store','retail_store_creation.retail_store_list',false),
      role: "retailStore",
      ...
      returnURL: `/retailStoreCreation/${state._retailStoreCreation.id}/workbench`,
      ...
      expandForm: state._retailStoreCreation.expandForm,
      loading: state._retailStoreCreation.loading,
      partialList: state._retailStoreCreation.partialList,
  ...
  getRetailStoreCreateForm = () => {
    ...
    return connect(state => ({
      ...
      metaInfo: state._retailStoreCreation.retailStoreListMetaInfo,
      count: state._retailStoreCreation.retailStoreCount,
      returnURL: `/retailStoreCreation/${state._retailStoreCreation.id}/list`,
      currentPage: state._retailStoreCreation.retailStoreCurrentPageNumber,
      searchFormParameters: state._retailStoreCreation.retailStoreSearchFormParameters,
      loading: state._retailStoreCreation.loading,
      owner: { type: '_retailStoreCreation', id: state._retailStoreCreation.id || this.getOwnerId(), referenceName: 'creation', listName: 'retailStoreList', ref:state._retailStoreCreation, listDisplayName: appLocaleName(userContext,"List")}, // this is for model namespace and
  ...
  getRetailStoreUpdateForm = () => {
    ...
    return connect(state => ({
      ...
      role: "retailStore",
      currentUpdateIndex: state._retailStoreCreation.currentUpdateIndex || 0,
      owner: { type: '_retailStoreCreation', id: state._retailStoreCreation.id || this.getOwnerId(), listName: 'retailStoreList', ref:state._retailStoreCreation, listDisplayName: appLocaleName(userContext,"List") }, // this is for model namespace and
  ...
  getRequestTypeStepForm = () => {
    ...
    return connect(state => ({
      ...
      owner: { type: '_retailStoreCreation', id: state._retailStoreCreation.id, listName: 'nolist', ref:state._retailStoreCreation, listDisplayName: appLocaleName(userContext,"List") }, // this is for model namespace and
  ...
  buildRouters = () =>{
    const {RetailStoreCreationWorkbench} = GlobalComponents

    const {RetailStoreCreationDashboard} = GlobalComponents
  	const {RetailStoreCreationPermission} = GlobalComponents
  	const {RetailStoreCreationProfile} = GlobalComponents


    const routers=[
    {path:"/retailStoreCreation/:id/workbench", component: RetailStoreCreationWorkbench},
    
  	{path:"/retailStoreCreation/:id/profile", component: RetailStoreCreationProfile},
  	{path:"/retailStoreCreation/:id/permission", component: RetailStoreCreationPermission},



  	{path:"/retailStoreCreation/:id/list/retailStoreList", component: this.getRetailStoreSearch()},
  	{path:"/retailStoreCreation/:id/list/retailStoreCreateForm", component: this.getRetailStoreCreateForm()},
  	{path:"/retailStoreCreation/:id/list/retailStoreUpdateForm", component: this.getRetailStoreUpdateForm()},
  ...
  handleOpenChange = (openKeys) => {
   ...
   render() {
    ...
    const {searchLocalData}=GlobalComponents.RetailStoreCreationBase
    ...
    const renderMenuSwitch=()=>{
     ...
     <Layout>
      ...
      {this.state.showSearch&&(
        ...
        <div style={{backgroundColor:'black'}}  onClick={()=>hideSearchResult()}  >{searchLocalData(this.props.retailStoreCreation,this.state.searchKeyword)}</div>
       ...
       <Sider
        ...
        >
         ...
         {this.getNavMenuItems(this.props.retailStoreCreation,"inline","dark")}
