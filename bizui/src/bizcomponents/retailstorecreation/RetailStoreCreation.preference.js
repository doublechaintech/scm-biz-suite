...
const internalImageListOf = (retailStoreCreation) =>{
...
const internalSettingListOf = (retailStoreCreation) =>{
	const userContext = null
	const optionList = [ 
	]
...
const handleTransferSearch =(targetComponent,filterKey,newRequest)=>{
  ...
  const pageNo = 1;
  const candidateReferenceService = RetailStoreCreationService[candidateServiceName] 
...
const executeTrans = (retailStoreCreation,targetComponent) =>{
...
const buildTransferModal = (retailStoreCreation,targetComponent) => {
...
<Modal title={transferModalTitle}
          ...
          onOk={()=>executeTrans(retailStoreCreation,targetComponent)}
...
const internalRenderExtraHeader = (retailStoreCreation) =>{
	return null
}
const internalRenderExtraFooter = (retailStoreCreation) =>{
	return null
}
...
class RetailStoreCreationPreference extends Component {
...
}))(Form.create()(RetailStoreCreationPreference))
