...
class EmployeePerformanceTable extends PureComponent {
  state = {
    selectedRowKeys: [],
  }
...
<<<<<<< HEAD
 calcDisplayColumns=()=>{
    ...
    const {displayColumns} = EmployeePerformanceBase
    if(!referenceName){
    ...
    const operationColumn={
      ...
      render: (text, record) => (
        <span>
         ...
         { hasReadPermission(metaInfo)&&<Link to={`/employeePerformance/${record.id}/dashboard`}>{'查看'}</Link>}
    ...
    const operationColumn={
      ...
      render: (text, record) => (
        <span>
          ...
          { hasReadPermission(metaInfo)&&<Link to={`/employeePerformance/${record.id}/dashboard`}>{appLocaleName(userContext,"View")}</Link>}
