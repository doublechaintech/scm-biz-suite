...
import styles from './EmployeePerformance.app.less'
...
class EmployeePerformanceBizApp extends React.PureComponent {
constructor(props) {
    super(props)
     this.state = {
 ...
 getNavMenuItems = (targetObject, style, customTheme) => {
    ...
    return (
	  <Menu
       ...
       <Menu.Item key="workbench">
        <Link to={`/employeePerformance/${this.props.employeePerformance.id}/workbench`}><Icon type="solution" style={{marginRight:"20px"}}/><span>工作台</span></Link>
      </Menu.Item>
        ...
        {filteredMenuItemsGroup(targetObject,this).map((groupedMenuItem,index)=>{
          return(
    <SubMenu id={`submenu-vg${index}`}  key={`vg${index}`} title={<span><Icon type={viewGroupIconNameOf('employee_performance',`${groupedMenuItem.viewGroup}`)} style={{marginRight:"20px"}} /><span>{viewGroupDisplayNameOf('employee_performance',`${groupedMenuItem.viewGroup}`)}</span></span>} >
  ...
  getRequestTypeStepForm = () => {
    ...
    return connect(state => ({
      selectedRows: state._employeePerformance.selectedRows,
      role: "cq",
      currentUpdateIndex: state._employeePerformance.currentUpdateIndex,
      owner: { type: '_employeePerformance', id: state._employeePerformance.id, listName: 'nolist', ref:state._employeePerformance, listDisplayName: appLocaleName(userContext,"List") }, // this is for model namespace and
  ...
  buildRouters = () =>{
    const {EmployeePerformanceWorkbench} = GlobalComponents

    const {EmployeePerformanceDashboard} = GlobalComponents
  	const {EmployeePerformancePermission} = GlobalComponents
  	const {EmployeePerformanceProfile} = GlobalComponents


    const routers=[
    {path:"/employeePerformance/:id/workbench", component: EmployeePerformanceWorkbench},
    
  	{path:"/employeePerformance/:id/profile", component: EmployeePerformanceProfile},
  	{path:"/employeePerformance/:id/permission", component: EmployeePerformancePermission},
  ...
  handleOpenChange = (openKeys) => {
   ...
   render() {
    ...
    const {searchLocalData}=GlobalComponents.EmployeePerformanceBase
    ...
    const renderMenuSwitch=()=>{
     ...
     <Layout>
      ...
      {this.state.showSearch&&(
        ...
        <div style={{backgroundColor:'black'}}  onClick={()=>hideSearchResult()}  >{searchLocalData(this.props.employeePerformance,this.state.searchKeyword)}</div>
       ...
       <Sider
        ...
        >
         ...
         {this.getNavMenuItems(this.props.employeePerformance,"inline","dark")}
