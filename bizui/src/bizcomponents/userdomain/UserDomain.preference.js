...
const internalImageListOf = (userDomain) =>{
<<<<<<< HEAD
...
const internalSettingListOf = (userDomain) =>{
<<<<<<< HEAD
...
const handleTransferSearch =(targetComponent,filterKey,newRequest)=>{
  ...
  const pageNo = 1;
  const candidateReferenceService = UserDomainService[candidateServiceName] 
...
const executeTrans = (userDomain,targetComponent) =>{
...
const buildTransferModal = (userDomain,targetComponent) => {
...
<Modal title={transferModalTitle}
          ...
          onOk={()=>executeTrans(userDomain,targetComponent)}
...
const internalRenderExtraHeader = (userDomain) =>{
	return null
}
const internalRenderExtraFooter = (userDomain) =>{
	return null
}
