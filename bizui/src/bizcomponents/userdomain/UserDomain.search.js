...
class UserDomainSearch extends PureComponent {
  state = {
  ...
  render(){
    ...
    return (
      <PageHeaderLayout title={`${displayName}:${this.props.name}${appLocaleName(userContext,"List")}`}>
        <Card bordered={false}>
          <div className={styles.tableList}>
            <div className={styles.tableListForm}>
              <UserDomainSearchForm {...this.props} />
            </div>
        ...
        {showDeletionDialog(this,UserDomainModalTable,"userDomainIds")}
...
export default Form.create()(UserDomainSearch)
