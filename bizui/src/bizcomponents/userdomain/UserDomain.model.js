...
export default {
  ...
  effects: {
    ...
    *addPublicKeyType({ payload }, { call, put }) {
      const userContext = null
      const {UserDomainService} = GlobalComponents;

      const { id, role, parameters, continueNext } = payload
      console.log('get form parameters', parameters)
      const data = yield call(UserDomainService.addPublicKeyType, id, parameters)
      if (hasError(data)) {
        handleServerError(data)
        return
      }
    ...
    *updatePublicKeyType({ payload }, { call, put }) {
      ...
      const { id, type, parameters, continueNext, selectedRows, currentUpdateIndex } = payload
      console.log('get form parameters', parameters)
      const data = yield call(UserDomainService.updatePublicKeyType, id, parameters)
      if (hasError(data)) {
        handleServerError(data)
        return
      }
    ...
    *removePublicKeyTypeList({ payload }, { call, put }) {
     const userContext = null
      const {UserDomainService} = GlobalComponents; 
      const { id, role, parameters, continueNext } = payload
      console.log('get form parameters', parameters)
      const data = yield call(UserDomainService.removePublicKeyTypeList, id, parameters)
      if (hasError(data)) {
        handleServerError(data)
        return
      }
