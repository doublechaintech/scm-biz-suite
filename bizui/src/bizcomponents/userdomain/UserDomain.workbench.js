...
import styles from './UserDomain.workbench.less'
...
const internalSummaryOf = (cardsData,targetComponent) =>{
	...
	const userDomain = cardsData.cardsSource
	const {UserDomainService} = GlobalComponents
	const userContext = null
	return (
	<div>
	<DescriptionList className={styles.headerList} size="small" col="4">
<Description term="ID" style={{wordBreak: 'break-all'}}>{userDomain.id}</Description> 
...
class UserDomainWorkbench extends Component {
 ...
 state = {
  ...
  render() {
    ...
    const { id,displayName, userAllowListListMetaInfo, secUserListMetaInfo, publicKeyTypeListMetaInfo, userAllowListCount, secUserCount, publicKeyTypeCount } = this.props.userDomain
...
{name: 'publicKeyTypeList', displayName: window.mtrans('public_key_type','user_domain.public_key_type_list',false) ,viewGroup:'__no_group', type:'publicKeyType',count:publicKeyTypeCount,addFunction: true, role: 'publicKeyType', metaInfo: publicKeyTypeListMetaInfo, renderItem: GlobalComponents.PublicKeyTypeBase.renderItemOfList},
   		...
   		subSettingItems: [
{name: 'userAllowListList', displayName: window.mtrans('user_allow_list','user_domain.user_allow_list_list',false),type:'userAllowList',count:userAllowListCount,addFunction: true, role: 'userAllowList', metaInfo: userAllowListListMetaInfo, renderItem: GlobalComponents.UserAllowListBase.renderItemOfList},
...
}))(Form.create()(UserDomainWorkbench))
