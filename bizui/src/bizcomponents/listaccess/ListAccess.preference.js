...
const internalImageListOf = (listAccess) =>{
...
const handleTransferSearch =(targetComponent,filterKey,newRequest)=>{
  ...
  const pageNo = 1;
  const candidateReferenceService = ListAccessService[candidateServiceName] 
...
const executeTrans = (listAccess,targetComponent) =>{
...
const buildTransferModal = (listAccess,targetComponent) => {
...
<Modal title={transferModalTitle}
          ...
          onOk={()=>executeTrans(listAccess,targetComponent)}
...
class ListAccessPreference extends Component {
...
}))(Form.create()(ListAccessPreference))
