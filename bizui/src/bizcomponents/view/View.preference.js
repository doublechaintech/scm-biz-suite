...
import styles from './View.preference.less'
...
const internalImageListOf = (view) =>{
...
const internalSettingListOf = (view) =>{
	const userContext = null
	const optionList = [ 
	]
...
const handleTransferSearch =(targetComponent,filterKey,newRequest)=>{
  ...
  const pageNo = 1;
  const candidateReferenceService = ViewService[candidateServiceName] 
...
const executeTrans = (view,targetComponent) =>{
  ...
  validateFieldsAndScroll((error, values) => {
    ...
    //targetComponent.setState({transferModalVisiable:false})
    dispatch({type:"_view/doJob",payload: payload})
...
const buildTransferModal = (view,targetComponent) => {
...
<Modal title={transferModalTitle}
          ...
          onOk={()=>executeTrans(view,targetComponent)}
...
const internalRenderExtraHeader = (view) =>{
	return null
}
const internalRenderExtraFooter = (view) =>{
	return null
}
...
class ViewPreference extends Component {
