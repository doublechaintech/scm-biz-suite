...
export default {
  ...
  effects: {
    ...
    *analyze({ payload }, { call, put, select }){
      ...
      const data = yield call(ViewService.analyze, payload.id)
    ...
    *view({ payload }, { call, put, select }) { 
      ...
      const cachedData = yield select(state => state._view)
      ...
      const {ViewService} = GlobalComponents;
      const data = yield call(ViewService.view, payload.id)
    ...
    *load({ payload }, { call, put }) { 
      const {ViewService} = GlobalComponents;
      //yield put({ type: 'showLoading', payload })
      const data = yield call(ViewService.load, payload.id, payload.parameters)
    ...
    *gotoCreateForm({ payload }, { put }) {
      ...
      yield put(routerRedux.push(`/view/${id}/list/${role}CreateForm`))
    },
    *gotoUpdateForm({ payload }, { put }) {
      const { id, role, selectedRows, currentUpdateIndex } = payload
      const state = { id, role, selectedRows, currentUpdateIndex }
      const location = { pathname: `/view/${id}/list/${role}UpdateForm`, state }
      yield put(routerRedux.push(location))
    },
    *goback({ payload }, { put }) {
      const { id, type,listName } = payload
      yield put(routerRedux.push(`/view/${id}/list/${type}List/${listName}`))
    },
