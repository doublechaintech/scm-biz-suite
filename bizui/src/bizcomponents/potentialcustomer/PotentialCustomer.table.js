...
import styles from './PotentialCustomer.table.less'
...
class PotentialCustomerTable extends PureComponent {
  state = {
    selectedRowKeys: [],
  }
...
<<<<<<< HEAD
 calcDisplayColumns=()=>{
    ...
    const {displayColumns} = PotentialCustomerBase
    if(!referenceName){
    ...
    const operationColumn={
      ...
      render: (text, record) => (
        <span>
         ...
         { hasReadPermission(metaInfo)&&<Link to={`/potentialCustomer/${record.id}/dashboard`}>{'查看'}</Link>}
    ...
    const operationColumn={
      ...
      render: (text, record) => (
        <span>
          ...
          { hasReadPermission(metaInfo)&&<Link to={`/potentialCustomer/${record.id}/dashboard`}>{appLocaleName(userContext,"View")}</Link>}
