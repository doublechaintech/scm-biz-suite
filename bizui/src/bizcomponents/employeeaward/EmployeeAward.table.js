...
import styles from './EmployeeAward.table.less'
...
class EmployeeAwardTable extends PureComponent {
  state = {
    selectedRowKeys: [],
  }
...
<<<<<<< HEAD
 calcDisplayColumns=()=>{
    ...
    const {displayColumns} = EmployeeAwardBase
    if(!referenceName){
    ...
    const operationColumn={
      ...
      render: (text, record) => (
        <span>
         ...
         { hasReadPermission(metaInfo)&&<Link to={`/employeeAward/${record.id}/dashboard`}>{'查看'}</Link>}
...
export default EmployeeAwardTable
