...
import styles from './EmployeeAward.preference.less'
...
const internalImageListOf = (employeeAward) =>{
<<<<<<< HEAD
...
const internalSettingListOf = (employeeAward) =>{
<<<<<<< HEAD
...
const handleTransferSearch =(targetComponent,filterKey,newRequest)=>{
  ...
  const pageNo = 1;
  const candidateReferenceService = EmployeeAwardService[candidateServiceName] 
...
const executeTrans = (employeeAward,targetComponent) =>{
...
const buildTransferModal = (employeeAward,targetComponent) => {
...
<Modal title={transferModalTitle}
          ...
          onOk={()=>executeTrans(employeeAward,targetComponent)}
...
const internalRenderExtraHeader = (employeeAward) =>{
	return null
}
const internalRenderExtraFooter = (employeeAward) =>{
	return null
}
...
<Description term="员工">{employeeAward.employee==null?"未分配":employeeAward.employee.displayName}
=======
    const userContext = null
	return (
	<DescriptionList className={styles.headerList} size="small" col="4">
<Description term="序号">{employeeAward.id}</Description> 
<Description term="员工">{employeeAward.employee==null?appLocaleName(userContext,"NotAssigned"):employeeAward.employee.displayName}
>>>>>>> 502e8b8dfc403300a992b5083e79c722e85d1854
 <Icon type="swap" onClick={()=>
  showTransferModel(targetComponent,"员工","employee","requestCandidateEmployee",
	      "transferToAnotherEmployee","anotherEmployeeId",employeeAward.employee?employeeAward.employee.id:"")} 
...
class EmployeeAwardPreference extends Component {
...
}))(Form.create()(EmployeeAwardPreference))
