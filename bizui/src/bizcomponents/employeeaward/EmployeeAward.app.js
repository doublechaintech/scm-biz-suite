...
import styles from './EmployeeAward.app.less'
...
class EmployeeAwardBizApp extends React.PureComponent {
constructor(props) {
    super(props)
     this.state = {
 ...
 getNavMenuItems = (targetObject, style, customTheme) => {
    ...
    return (
	  <Menu
       ...
       <Menu.Item key="workbench">
        <Link to={`/employeeAward/${this.props.employeeAward.id}/workbench`}><Icon type="solution" style={{marginRight:"20px"}}/><span>工作台</span></Link>
      </Menu.Item>
        ...
        {filteredMenuItemsGroup(targetObject,this).map((groupedMenuItem,index)=>{
          return(
    <SubMenu id={`submenu-vg${index}`}  key={`vg${index}`} title={<span><Icon type={viewGroupIconNameOf('employee_award',`${groupedMenuItem.viewGroup}`)} style={{marginRight:"20px"}} /><span>{viewGroupDisplayNameOf('employee_award',`${groupedMenuItem.viewGroup}`)}</span></span>} >
  ...
  getRequestTypeStepForm = () => {
    ...
    return connect(state => ({
      selectedRows: state._employeeAward.selectedRows,
      role: "cq",
      currentUpdateIndex: state._employeeAward.currentUpdateIndex,
      owner: { type: '_employeeAward', id: state._employeeAward.id, listName: 'nolist', ref:state._employeeAward, listDisplayName: appLocaleName(userContext,"List") }, // this is for model namespace and
  ...
  buildRouters = () =>{
    const {EmployeeAwardWorkbench} = GlobalComponents

    const {EmployeeAwardDashboard} = GlobalComponents
  	const {EmployeeAwardPermission} = GlobalComponents
  	const {EmployeeAwardProfile} = GlobalComponents


    const routers=[
    {path:"/employeeAward/:id/workbench", component: EmployeeAwardWorkbench},
    
  	{path:"/employeeAward/:id/profile", component: EmployeeAwardProfile},
  	{path:"/employeeAward/:id/permission", component: EmployeeAwardPermission},
  ...
  handleOpenChange = (openKeys) => {
   ...
   render() {
    ...
    const {searchLocalData}=GlobalComponents.EmployeeAwardBase
    ...
    const renderMenuSwitch=()=>{
     ...
     <Layout>
      ...
      {this.state.showSearch&&(
        ...
        <div style={{backgroundColor:'black'}}  onClick={()=>hideSearchResult()}  >{searchLocalData(this.props.employeeAward,this.state.searchKeyword)}</div>
       ...
       <Sider
        ...
        >
         ...
         {this.getNavMenuItems(this.props.employeeAward,"inline","dark")}
