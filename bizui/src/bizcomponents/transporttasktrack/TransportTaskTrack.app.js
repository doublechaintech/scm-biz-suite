...
import styles from './TransportTaskTrack.app.less'
...
class TransportTaskTrackBizApp extends React.PureComponent {
constructor(props) {
    super(props)
     this.state = {
 ...
 getNavMenuItems = (targetObject, style, customTheme) => {
    ...
    return (
	  <Menu
       ...
       <Menu.Item key="workbench">
        <Link to={`/transportTaskTrack/${this.props.transportTaskTrack.id}/workbench`}><Icon type="solution" style={{marginRight:"20px"}}/><span>工作台</span></Link>
      </Menu.Item>
        ...
        {filteredMenuItemsGroup(targetObject,this).map((groupedMenuItem,index)=>{
          return(
    <SubMenu id={`submenu-vg${index}`}  key={`vg${index}`} title={<span><Icon type={viewGroupIconNameOf('transport_task_track',`${groupedMenuItem.viewGroup}`)} style={{marginRight:"20px"}} /><span>{viewGroupDisplayNameOf('transport_task_track',`${groupedMenuItem.viewGroup}`)}</span></span>} >
  ...
  getRequestTypeStepForm = () => {
    ...
    return connect(state => ({
      selectedRows: state._transportTaskTrack.selectedRows,
      role: "cq",
      currentUpdateIndex: state._transportTaskTrack.currentUpdateIndex,
      owner: { type: '_transportTaskTrack', id: state._transportTaskTrack.id, listName: 'nolist', ref:state._transportTaskTrack, listDisplayName: appLocaleName(userContext,"List") }, // this is for model namespace and
  ...
  buildRouters = () =>{
    const {TransportTaskTrackWorkbench} = GlobalComponents

    const {TransportTaskTrackDashboard} = GlobalComponents
  	const {TransportTaskTrackPermission} = GlobalComponents
  	const {TransportTaskTrackProfile} = GlobalComponents


    const routers=[
    {path:"/transportTaskTrack/:id/workbench", component: TransportTaskTrackWorkbench},
    
  	{path:"/transportTaskTrack/:id/profile", component: TransportTaskTrackProfile},
  	{path:"/transportTaskTrack/:id/permission", component: TransportTaskTrackPermission},
  ...
  handleOpenChange = (openKeys) => {
   ...
   render() {
    ...
    const {searchLocalData}=GlobalComponents.TransportTaskTrackBase
    ...
    const renderMenuSwitch=()=>{
     ...
     <Layout>
      ...
      {this.state.showSearch&&(
        ...
        <div style={{backgroundColor:'black'}}  onClick={()=>hideSearchResult()}  >{searchLocalData(this.props.transportTaskTrack,this.state.searchKeyword)}</div>
       ...
       <Sider
        ...
        >
         ...
         {this.getNavMenuItems(this.props.transportTaskTrack,"inline","dark")}
