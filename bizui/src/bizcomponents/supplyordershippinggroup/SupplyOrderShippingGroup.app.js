...
import styles from './SupplyOrderShippingGroup.app.less'
...
class SupplyOrderShippingGroupBizApp extends React.PureComponent {
constructor(props) {
    super(props)
     this.state = {
 ...
 getNavMenuItems = (targetObject, style, customTheme) => {
    ...
    return (
	  <Menu
       ...
       <Menu.Item key="workbench">
        <Link to={`/supplyOrderShippingGroup/${this.props.supplyOrderShippingGroup.id}/workbench`}><Icon type="solution" style={{marginRight:"20px"}}/><span>工作台</span></Link>
      </Menu.Item>
        ...
        {filteredMenuItemsGroup(targetObject,this).map((groupedMenuItem,index)=>{
          return(
    <SubMenu id={`submenu-vg${index}`}  key={`vg${index}`} title={<span><Icon type={viewGroupIconNameOf('supply_order_shipping_group',`${groupedMenuItem.viewGroup}`)} style={{marginRight:"20px"}} /><span>{viewGroupDisplayNameOf('supply_order_shipping_group',`${groupedMenuItem.viewGroup}`)}</span></span>} >
  ...
  getRequestTypeStepForm = () => {
    ...
    return connect(state => ({
      selectedRows: state._supplyOrderShippingGroup.selectedRows,
      role: "cq",
      currentUpdateIndex: state._supplyOrderShippingGroup.currentUpdateIndex,
      owner: { type: '_supplyOrderShippingGroup', id: state._supplyOrderShippingGroup.id, listName: 'nolist', ref:state._supplyOrderShippingGroup, listDisplayName: appLocaleName(userContext,"List") }, // this is for model namespace and
  ...
  buildRouters = () =>{
    const {SupplyOrderShippingGroupWorkbench} = GlobalComponents

    const {SupplyOrderShippingGroupDashboard} = GlobalComponents
  	const {SupplyOrderShippingGroupPermission} = GlobalComponents
  	const {SupplyOrderShippingGroupProfile} = GlobalComponents


    const routers=[
    {path:"/supplyOrderShippingGroup/:id/workbench", component: SupplyOrderShippingGroupWorkbench},
    
  	{path:"/supplyOrderShippingGroup/:id/profile", component: SupplyOrderShippingGroupProfile},
  	{path:"/supplyOrderShippingGroup/:id/permission", component: SupplyOrderShippingGroupPermission},
  ...
  handleOpenChange = (openKeys) => {
   ...
   render() {
    ...
    const {searchLocalData}=GlobalComponents.SupplyOrderShippingGroupBase
    ...
    const renderMenuSwitch=()=>{
     ...
     <Layout>
      ...
      {this.state.showSearch&&(
        ...
        <div style={{backgroundColor:'black'}}  onClick={()=>hideSearchResult()}  >{searchLocalData(this.props.supplyOrderShippingGroup,this.state.searchKeyword)}</div>
       ...
       <Sider
        ...
        >
         ...
         {this.getNavMenuItems(this.props.supplyOrderShippingGroup,"inline","dark")}
