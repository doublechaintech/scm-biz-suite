...
import styles from './SupplyOrderShippingGroup.table.less'
...
class SupplyOrderShippingGroupTable extends PureComponent {
  state = {
    selectedRowKeys: [],
  }
...
<<<<<<< HEAD
 calcDisplayColumns=()=>{
    ...
    const {displayColumns} = SupplyOrderShippingGroupBase
    if(!referenceName){
    ...
    const operationColumn={
      ...
      render: (text, record) => (
        <span>
         ...
         { hasReadPermission(metaInfo)&&<Link to={`/supplyOrderShippingGroup/${record.id}/dashboard`}>{'查看'}</Link>}
    ...
    const operationColumn={
      ...
      render: (text, record) => (
        <span>
          ...
          { hasReadPermission(metaInfo)&&<Link to={`/supplyOrderShippingGroup/${record.id}/dashboard`}>{appLocaleName(userContext,"View")}</Link>}
...
export default SupplyOrderShippingGroupTable
