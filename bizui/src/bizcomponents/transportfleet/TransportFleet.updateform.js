...
import styles from './TransportFleet.updateform.less'
...
class TransportFleetUpdateForm extends Component {
  state = {
  ...
  render() {
    ...
    const submitUpdateForm = () => {
      validateFieldsAndScroll((error, values) => {
        ...
        const transportFleetId = values.id

        const parameters = { ...values, transportFleetId, }
    ...
    const submitUpdateFormAndContinue = () => {
      validateFieldsAndScroll((error, values) => {
        ...
        dispatch({
          type: `${owner.type}/updateTransportFleet`,
          payload: {
            id: owner.id,
            type: 'transportFleet',
            parameters,
    ...
    const skipToNext = () => {
      ...
      const newIndex = currentUpdateIndex + 1
      dispatch({
        type: `${owner.type}/gotoNextTransportFleetUpdateRow`,
        payload: {
          id: owner.id,
          type: 'transportFleet',
          selectedRows,
	...
	const internalRenderTitle = () =>{
      ...
      return (<div>{linkComp}{appLocaleName(userContext,"Update")}运输车队: {(currentUpdateIndex+1)}/{selectedRows.length}</div>)
    }
            ...
            <Row gutter={16}>
              ...
              <Form.Item>
                  {getFieldDecorator('contract', {
                  	initialValue: selectedRow.contract,
...
}))(Form.create()(TransportFleetUpdateForm))
