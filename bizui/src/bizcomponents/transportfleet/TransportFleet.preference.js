...
const internalImageListOf = (transportFleet) =>{
...
const internalSettingListOf = (transportFleet) =>{
...
const handleTransferSearch =(targetComponent,filterKey,newRequest)=>{
  ...
  const pageNo = 1;
  const candidateReferenceService = TransportFleetService[candidateServiceName] 
...
const executeTrans = (transportFleet,targetComponent) =>{
...
const buildTransferModal = (transportFleet,targetComponent) => {
...
<Modal title={transferModalTitle}
          ...
          onOk={()=>executeTrans(transportFleet,targetComponent)}
...
const internalRenderExtraHeader = (transportFleet) =>{
	return null
}
const internalRenderExtraFooter = (transportFleet) =>{
	return null
}
...
class TransportFleetPreference extends Component {
  ...
  render() {
    ...
    const { id,displayName, transportTruckCount, truckDriverCount, transportTaskCount } = this.props.transportFleet
...
}))(Form.create()(TransportFleetPreference))
