...
const showAssociateDialog = (targetComponent) => {
  ...
  <div>
	...
	data={{transportFleetList:selectedRows}} owner={owner}  
...
class TransportFleetSearch extends PureComponent {
  state = {
  ...
  render(){
    ...
    const {TransportFleetTable} = GlobalComponents;
    const {TransportFleetSearchForm} = GlobalComponents;
    ...
    return (
      <PageHeaderLayout title={`${displayName}:${this.props.name}${appLocaleName(userContext,"List")}`}>
        <Card bordered={false}>
          <div className={styles.tableList}>
            <div className={styles.tableListForm}>
              <TransportFleetSearchForm {...this.props} />
            </div>
        ...
        {showDeletionDialog(this,TransportFleetModalTable,"transportFleetIds")}
...
export default Form.create()(TransportFleetSearch)
