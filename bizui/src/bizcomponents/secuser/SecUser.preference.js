...
const internalImageListOf = (secUser) =>{
...
const internalSettingListOf = (secUser) =>{
...
const handleTransferSearch =(targetComponent,filterKey,newRequest)=>{
  ...
  const pageNo = 1;
  const candidateReferenceService = SecUserService[candidateServiceName] 
...
const executeTrans = (secUser,targetComponent) =>{
  ...
  validateFieldsAndScroll((error, values) => {
    ...
    //targetComponent.setState({transferModalVisiable:false})
    dispatch({type:"_secUser/doJob",payload: payload})
...
const buildTransferModal = (secUser,targetComponent) => {
...
<Modal title={transferModalTitle}
          ...
          onOk={()=>executeTrans(secUser,targetComponent)}
...
class SecUserPreference extends Component {
