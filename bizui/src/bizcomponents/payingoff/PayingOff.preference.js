...
const internalImageListOf = (payingOff) =>{
...
const internalSettingListOf = (payingOff) =>{
	const userContext = null
	const optionList = [ 
	]
...
const handleTransferSearch =(targetComponent,filterKey,newRequest)=>{
  ...
  const pageNo = 1;
  const candidateReferenceService = PayingOffService[candidateServiceName] 
...
const executeTrans = (payingOff,targetComponent) =>{
...
const buildTransferModal = (payingOff,targetComponent) => {
...
<Modal title={transferModalTitle}
          ...
          onOk={()=>executeTrans(payingOff,targetComponent)}
...
const internalRenderExtraHeader = (payingOff) =>{
	return null
}
const internalRenderExtraFooter = (payingOff) =>{
	return null
}
...
<Description term="支付">{payingOff.paidFor==null?appLocaleName(userContext,"NotAssigned"):payingOff.paidFor.displayName}
 <Icon type="swap" onClick={()=>
  showTransferModel(targetComponent,"支付","employee","requestCandidatePaidFor",
	      "transferToAnotherPaidFor","anotherPaidForId",payingOff.paidFor?payingOff.paidFor.id:"")} 
...
class PayingOffPreference extends Component {
...
}))(Form.create()(PayingOffPreference))
