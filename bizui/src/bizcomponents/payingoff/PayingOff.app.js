 ...
 getNavMenuItems = (targetObject, style, customTheme) => {
    ...
    return (
	  <Menu
       ...
       <Menu.Item key="workbench">
        <Link to={`/payingOff/${this.props.payingOff.id}/workbench`}><Icon type="solution" style={{marginRight:"20px"}}/><span>工作台</span></Link>
      </Menu.Item>
        ...
        {filteredMenuItemsGroup(targetObject,this).map((groupedMenuItem,index)=>{
          return(
    <SubMenu id={`submenu-vg${index}`}  key={`vg${index}`} title={<span><Icon type={viewGroupIconNameOf('paying_off',`${groupedMenuItem.viewGroup}`)} style={{marginRight:"20px"}} /><span>{viewGroupDisplayNameOf('paying_off',`${groupedMenuItem.viewGroup}`)}</span></span>} >
  ...
  getEmployeeSalarySheetSearch = () => {
    ...
    return connect(state => ({
      rule: state.rule,
      name: window.mtrans('employee_salary_sheet','paying_off.employee_salary_sheet_list',false),
      ...
      returnURL: `/payingOff/${state._payingOff.id}/workbench`,
      ...
      expandForm: state._payingOff.expandForm,
      loading: state._payingOff.loading,
      partialList: state._payingOff.partialList,
      owner: { type: '_payingOff', id: state._payingOff.id,
      referenceName: 'payingOff',
      listName: 'employeeSalarySheetList', ref:state._payingOff,
  ...
  getEmployeeSalarySheetCreateForm = () => {
    ...
    return connect(state => ({
      ...
      metaInfo: state._payingOff.employeeSalarySheetListMetaInfo,
      count: state._payingOff.employeeSalarySheetCount,
      returnURL: `/payingOff/${state._payingOff.id}/list`,
      currentPage: state._payingOff.employeeSalarySheetCurrentPageNumber,
      searchFormParameters: state._payingOff.employeeSalarySheetSearchFormParameters,
      loading: state._payingOff.loading,
      owner: { type: '_payingOff', id: state._payingOff.id || this.getOwnerId(), referenceName: 'payingOff', listName: 'employeeSalarySheetList', ref:state._payingOff, listDisplayName: appLocaleName(userContext,"List")}, // this is for model namespace and
  ...
  getEmployeeSalarySheetUpdateForm = () => {
    ...
    return connect(state => ({
      ...
      role: "employeeSalarySheet",
      currentUpdateIndex: state._payingOff.currentUpdateIndex || 0,
      owner: { type: '_payingOff', id: state._payingOff.id || this.getOwnerId(), listName: 'employeeSalarySheetList', ref:state._payingOff, listDisplayName: appLocaleName(userContext,"List") }, // this is for model namespace and
  ...
  getRequestTypeStepForm = () => {
    ...
    return connect(state => ({
      ...
      owner: { type: '_payingOff', id: state._payingOff.id, listName: 'nolist', ref:state._payingOff, listDisplayName: appLocaleName(userContext,"List") }, // this is for model namespace and
  ...
  buildRouters = () =>{
    const {PayingOffWorkbench} = GlobalComponents

    const {PayingOffDashboard} = GlobalComponents
  	const {PayingOffPermission} = GlobalComponents
  	const {PayingOffProfile} = GlobalComponents


    const routers=[
    {path:"/payingOff/:id/workbench", component: PayingOffWorkbench},
    
  	{path:"/payingOff/:id/profile", component: PayingOffProfile},
  	{path:"/payingOff/:id/permission", component: PayingOffPermission},



  	{path:"/payingOff/:id/list/employeeSalarySheetList", component: this.getEmployeeSalarySheetSearch()},
  	{path:"/payingOff/:id/list/employeeSalarySheetCreateForm", component: this.getEmployeeSalarySheetCreateForm()},
  	{path:"/payingOff/:id/list/employeeSalarySheetUpdateForm", component: this.getEmployeeSalarySheetUpdateForm()},
  ...
  handleOpenChange = (openKeys) => {
   ...
   render() {
    ...
    const {searchLocalData}=GlobalComponents.PayingOffBase
    ...
    const renderMenuSwitch=()=>{
     ...
     <Layout>
      ...
      {this.state.showSearch&&(
        ...
        <div style={{backgroundColor:'black'}}  onClick={()=>hideSearchResult()}  >{searchLocalData(this.props.payingOff,this.state.searchKeyword)}</div>
       ...
       <Sider
        ...
        >
         ...
         {this.getNavMenuItems(this.props.payingOff,"inline","dark")}
