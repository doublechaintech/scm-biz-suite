...
const internalImageListOf = (retailStoreOrderShipment) =>{
<<<<<<< HEAD
...
const internalSettingListOf = (retailStoreOrderShipment) =>{
<<<<<<< HEAD
...
const handleTransferSearch =(targetComponent,filterKey,newRequest)=>{
  ...
  const pageNo = 1;
  const candidateReferenceService = RetailStoreOrderShipmentService[candidateServiceName] 
...
const executeTrans = (retailStoreOrderShipment,targetComponent) =>{
  ...
  validateFieldsAndScroll((error, values) => {
    ...
    const id=retailStoreOrderShipment.id;
    const serviceNameToCall = transferServiceName;

    const payload = {parameters,id,serviceNameToCall}
    
    //targetComponent.setState({transferModalVisiable:false})
    dispatch({type:"_retailStoreOrderShipment/doJob",payload: payload})
...
const buildTransferModal = (retailStoreOrderShipment,targetComponent) => {
...
<Modal title={transferModalTitle}
          ...
          onOk={()=>executeTrans(retailStoreOrderShipment,targetComponent)}
...
const internalRenderExtraHeader = (retailStoreOrderShipment) =>{
	return null
}
const internalRenderExtraFooter = (retailStoreOrderShipment) =>{
	return null
}
...
class RetailStoreOrderShipmentPreference extends Component {
...
>>>>>>> 502e8b8dfc403300a992b5083e79c722e85d1854
  ...
  render() {
    ...
    const { id,displayName, retailStoreOrderCount } = this.props.retailStoreOrderShipment
    const cardsData = {cardsName:"生超订单出货",cardsFor: "retailStoreOrderShipment",cardsSource: this.props.retailStoreOrderShipment,
  		subItems: [
...
}))(Form.create()(RetailStoreOrderShipmentPreference))
