...
const internalImageListOf = (supplyOrderProcessing) =>{
<<<<<<< HEAD
...
const internalSettingListOf = (supplyOrderProcessing) =>{
<<<<<<< HEAD
...
const handleTransferSearch =(targetComponent,filterKey,newRequest)=>{
  ...
  const pageNo = 1;
  const candidateReferenceService = SupplyOrderProcessingService[candidateServiceName] 
...
const executeTrans = (supplyOrderProcessing,targetComponent) =>{
  ...
  validateFieldsAndScroll((error, values) => {
    ...
    const id=supplyOrderProcessing.id;
    const serviceNameToCall = transferServiceName;

    const payload = {parameters,id,serviceNameToCall}
    
    //targetComponent.setState({transferModalVisiable:false})
    dispatch({type:"_supplyOrderProcessing/doJob",payload: payload})
...
const buildTransferModal = (supplyOrderProcessing,targetComponent) => {
...
<Modal title={transferModalTitle}
          ...
          onOk={()=>executeTrans(supplyOrderProcessing,targetComponent)}
...
const internalRenderExtraHeader = (supplyOrderProcessing) =>{
	return null
}
const internalRenderExtraFooter = (supplyOrderProcessing) =>{
	return null
}
...
class SupplyOrderProcessingPreference extends Component {
...
>>>>>>> 502e8b8dfc403300a992b5083e79c722e85d1854
  ...
  render() {
    ...
    const { id,displayName, consumerOrderCount, supplyOrderCount } = this.props.supplyOrderProcessing
    const cardsData = {cardsName:"供应订单处理",cardsFor: "supplyOrderProcessing",cardsSource: this.props.supplyOrderProcessing,
  		subItems: [
...
}))(Form.create()(SupplyOrderProcessingPreference))
