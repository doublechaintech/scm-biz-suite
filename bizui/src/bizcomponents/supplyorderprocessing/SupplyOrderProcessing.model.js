...
export default {
  ...
  effects: {
    ...
    *addConsumerOrder({ payload }, { call, put }) {
      ...
      const location = { pathname: `/supplyOrderProcessing/${id}/list/ConsumerOrderList/消费者订单+${appLocaleName(userContext,'List')}`, state: newState }
      yield put(routerRedux.push(location))
    },
    ...
    *addSupplyOrder({ payload }, { call, put }) {
      ...
      const partialList = true
      const newState = {...data, partialList}
      const location = { pathname: `/supplyOrderProcessing/${id}/list/SupplyOrderList/供应订单+${appLocaleName(userContext,'List')}`, state: newState }
      yield put(routerRedux.push(location))
    },
