...
import styles from './GoodsMovement.workbench.less'
...
const renderSettingMenuItem = (item,cardsData,targetComponent) =>{
  ...
  return (<Menu.Item key={item.name}>
      <Link to={`/goodsMovement/${targetComponent.props.goodsMovement.id}/list/${item.name}/${item.displayName}/`}>
...
const internalSummaryOf = (cardsData,targetComponent) =>{
	...
	const goodsMovement = cardsData.cardsSource
	const {GoodsMovementService} = GlobalComponents
	const userContext = null
	return (
	<div>
	<DescriptionList className={styles.headerList} size="small" col="4">
<Description term="ID" style={{wordBreak: 'break-all'}}>{goodsMovement.id}</Description> 
...
class GoodsMovementWorkbench extends Component {
...
}))(Form.create()(GoodsMovementWorkbench))
