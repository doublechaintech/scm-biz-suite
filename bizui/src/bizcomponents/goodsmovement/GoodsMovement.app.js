...
import styles from './GoodsMovement.app.less'
...
class GoodsMovementBizApp extends React.PureComponent {
constructor(props) {
    super(props)
     this.state = {
 ...
 getNavMenuItems = (targetObject, style, customTheme) => {
    ...
    return (
	  <Menu
       ...
       <Menu.Item key="workbench">
        <Link to={`/goodsMovement/${this.props.goodsMovement.id}/workbench`}><Icon type="solution" style={{marginRight:"20px"}}/><span>工作台</span></Link>
      </Menu.Item>
        ...
        {filteredMenuItemsGroup(targetObject,this).map((groupedMenuItem,index)=>{
          return(
    <SubMenu id={`submenu-vg${index}`}  key={`vg${index}`} title={<span><Icon type={viewGroupIconNameOf('goods_movement',`${groupedMenuItem.viewGroup}`)} style={{marginRight:"20px"}} /><span>{viewGroupDisplayNameOf('goods_movement',`${groupedMenuItem.viewGroup}`)}</span></span>} >
  ...
  getRequestTypeStepForm = () => {
    ...
    return connect(state => ({
      selectedRows: state._goodsMovement.selectedRows,
      role: "cq",
      currentUpdateIndex: state._goodsMovement.currentUpdateIndex,
      owner: { type: '_goodsMovement', id: state._goodsMovement.id, listName: 'nolist', ref:state._goodsMovement, listDisplayName: appLocaleName(userContext,"List") }, // this is for model namespace and
  ...
  buildRouters = () =>{
    const {GoodsMovementWorkbench} = GlobalComponents

    const {GoodsMovementDashboard} = GlobalComponents
  	const {GoodsMovementPermission} = GlobalComponents
  	const {GoodsMovementProfile} = GlobalComponents


    const routers=[
    {path:"/goodsMovement/:id/workbench", component: GoodsMovementWorkbench},
    
  	{path:"/goodsMovement/:id/profile", component: GoodsMovementProfile},
  ...
  handleOpenChange = (openKeys) => {
   ...
   render() {
    ...
    const {searchLocalData}=GlobalComponents.GoodsMovementBase
    ...
    const renderMenuSwitch=()=>{
     ...
     <Layout>
      ...
      {this.state.showSearch&&(
        ...
        <div style={{backgroundColor:'black'}}  onClick={()=>hideSearchResult()}  >{searchLocalData(this.props.goodsMovement,this.state.searchKeyword)}</div>
       ...
       <Sider
        ...
        >
         ...
         {this.getNavMenuItems(this.props.goodsMovement,"inline","dark")}
