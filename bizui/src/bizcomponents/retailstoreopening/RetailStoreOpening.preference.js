...
const internalImageListOf = (retailStoreOpening) =>{
...
const internalSettingListOf = (retailStoreOpening) =>{
	const userContext = null
	const optionList = [ 
	]
...
const handleTransferSearch =(targetComponent,filterKey,newRequest)=>{
  ...
  const pageNo = 1;
  const candidateReferenceService = RetailStoreOpeningService[candidateServiceName] 
...
const executeTrans = (retailStoreOpening,targetComponent) =>{
...
const buildTransferModal = (retailStoreOpening,targetComponent) => {
...
<Modal title={transferModalTitle}
          ...
          onOk={()=>executeTrans(retailStoreOpening,targetComponent)}
...
const internalRenderExtraHeader = (retailStoreOpening) =>{
	return null
}
const internalRenderExtraFooter = (retailStoreOpening) =>{
	return null
}
...
class RetailStoreOpeningPreference extends Component {
  ...
  render() {
    ...
    const { id,displayName, retailStoreCount } = this.props.retailStoreOpening
...
}))(Form.create()(RetailStoreOpeningPreference))
