...
import styles from './RetailStoreOpening.workbench.less'
...
const internalSummaryOf = (cardsData,targetComponent) =>{
	...
	const retailStoreOpening = cardsData.cardsSource
	const {RetailStoreOpeningService} = GlobalComponents
	const userContext = null
	return (
	<div>
	<DescriptionList className={styles.headerList} size="small" col="4">
<Description term="ID" style={{wordBreak: 'break-all'}}>{retailStoreOpening.id}</Description> 
<Description term="评论" style={{wordBreak: 'break-all'}}>{retailStoreOpening.comment}</Description> 
...
class RetailStoreOpeningWorkbench extends Component {
 ...
 state = {
  ...
  render() {
    ...
    const cardsData = {cardsName:window.trans('retail_store_opening'),cardsFor: "retailStoreOpening",
    	cardsSource: this.props.retailStoreOpening,returnURL,displayName,
  		subItems: [
{name: 'retailStoreList', displayName: window.mtrans('retail_store','retail_store_opening.retail_store_list',false) ,viewGroup:'__no_group', type:'retailStore',count:retailStoreCount,addFunction: true, role: 'retailStore', metaInfo: retailStoreListMetaInfo, renderItem: GlobalComponents.RetailStoreBase.renderItemOfList},
...
}))(Form.create()(RetailStoreOpeningWorkbench))
