...
const menuData = {menuName: window.trans('retail_store_opening'), menuFor: "retailStoreOpening",  internalName: "retail_store_opening",
  		subItems: [
...
const settingMenuData = {menuName: window.trans('retail_store_opening'), menuFor: "retailStoreOpening",  internalName: "retail_store_opening",
  		subItems: [
...
const renderActionList=(retailStoreOpening, targetObject, columCount, listName)=>{

	if(!retailStoreOpening){
		return null
	}
	if(!retailStoreOpening.actionList){
		return null
	}
	if(retailStoreOpening.actionList.length === 0){
		return null
	}
	return (
		<div className={styles.overlay}>
			...
			{retailStoreOpening.actionList.map(action=>(<Link key={action.id} to={{pathname: action.actionPath.substring(1), state: {ownerId:targetObject.id,action,selectedRows:[retailStoreOpening]}}} >
...
const renderItemOfList=(retailStoreOpening, targetObject, columCount, listName)=>{
  ...
  const userContext = null
  return (
     <Row key={`${listName}-${retailStoreOpening.id}`} className={styles.itemDesc}>
	...
	<Col span={20}>
      ...
      {renderActionList(retailStoreOpening,targetObject)}
    </Row>
	)
...
const RetailStoreOpeningBase={unpackObjectToFormValues, menuData,settingMenuData,displayColumns,fieldLabels,renderItemOfList, stepOf, searchLocalData}
