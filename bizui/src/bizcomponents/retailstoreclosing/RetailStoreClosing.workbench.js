...
import styles from './RetailStoreClosing.workbench.less'
...
const renderSettingMenuItem = (item,cardsData,targetComponent) =>{
  ...
  return (<Menu.Item key={item.name}>
      <Link to={`/retailStoreClosing/${targetComponent.props.retailStoreClosing.id}/list/${item.name}/${item.displayName}/`}>
...
const internalSummaryOf = (cardsData,targetComponent) =>{
	...
	const retailStoreClosing = cardsData.cardsSource
	const {RetailStoreClosingService} = GlobalComponents
	const userContext = null
	return (
	<div>
	<DescriptionList className={styles.headerList} size="small" col="4">
<Description term="ID" style={{wordBreak: 'break-all'}}>{retailStoreClosing.id}</Description> 
<Description term="评论" style={{wordBreak: 'break-all'}}>{retailStoreClosing.comment}</Description> 
...
class RetailStoreClosingWorkbench extends Component {
 ...
 state = {
  ...
  render() {
    ...
    const cardsData = {cardsName:window.trans('retail_store_closing'),cardsFor: "retailStoreClosing",
    	cardsSource: this.props.retailStoreClosing,returnURL,displayName,
  		subItems: [
{name: 'retailStoreList', displayName: window.mtrans('retail_store','retail_store_closing.retail_store_list',false) ,viewGroup:'__no_group', type:'retailStore',count:retailStoreCount,addFunction: true, role: 'retailStore', metaInfo: retailStoreListMetaInfo, renderItem: GlobalComponents.RetailStoreBase.renderItemOfList},
...
}))(Form.create()(RetailStoreClosingWorkbench))
