...
import styles from './RetailStoreClosing.preference.less'
...
const internalImageListOf = (retailStoreClosing) =>{
...
const internalSettingListOf = (retailStoreClosing) =>{
	const userContext = null
	const optionList = [ 
	]
...
const handleTransferSearch =(targetComponent,filterKey,newRequest)=>{
  ...
  const pageNo = 1;
  const candidateReferenceService = RetailStoreClosingService[candidateServiceName] 
...
const executeTrans = (retailStoreClosing,targetComponent) =>{
...
const buildTransferModal = (retailStoreClosing,targetComponent) => {
...
<Modal title={transferModalTitle}
          ...
          onOk={()=>executeTrans(retailStoreClosing,targetComponent)}
...
const internalRenderExtraHeader = (retailStoreClosing) =>{
	return null
}
const internalRenderExtraFooter = (retailStoreClosing) =>{
	return null
}
...
class RetailStoreClosingPreference extends Component {
  ...
  render() {
    ...
    const cardsData = {cardsName:"商店关闭",cardsFor: "retailStoreClosing",cardsSource: retailStoreClosing,
  		subItems: [
...
}))(Form.create()(RetailStoreClosingPreference))
