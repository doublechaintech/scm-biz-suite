...
import styles from './RetailStoreClosing.updateform.less'
...
class RetailStoreClosingUpdateForm extends Component {
  state = {
  ...
  render() {
    ...
    const submitUpdateForm = () => {
      validateFieldsAndScroll((error, values) => {
        ...
        const retailStoreClosingId = values.id
        const imagesValues = mapBackToImageValues(convertedImagesValues)
        const parameters = { ...values, retailStoreClosingId, ...imagesValues }
    ...
    const submitUpdateFormAndContinue = () => {
      validateFieldsAndScroll((error, values) => {
        ...
        dispatch({
          type: `${owner.type}/updateRetailStoreClosing`,
          payload: {
            id: owner.id,
            type: 'retailStoreClosing',
            parameters,
            selectedRows,
    ...
    const skipToNext = () => {
      ...
      const newIndex = currentUpdateIndex + 1
      dispatch({
        type: `${owner.type}/gotoNextRetailStoreClosingUpdateRow`,
        payload: {
          id: owner.id,
          type: 'retailStoreClosing',
          selectedRows,
...
<<<<<<< HEAD
	...
	const internalRenderTitle = () =>{
      ...
      return (<div>{linkComp}{appLocaleName(userContext,"Update")}商店关闭: {(currentUpdateIndex+1)}/{selectedRows.length}</div>)
    }
...
=======
	...
	const internalRenderTitle = () =>{
      const linkComp=<a onClick={goback}  > <Icon type="double-left" style={{marginRight:"10px"}} /> </a>
      return (<div>{linkComp}{appLocaleName(userContext,"Update")}门店关闭: {(currentUpdateIndex+1)}/{selectedRows.length}</div>)
    }
...
}))(Form.create()(RetailStoreClosingUpdateForm))
