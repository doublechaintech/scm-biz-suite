...
import styles from './WechatWorkappIdentify.app.less'
...
class WechatWorkappIdentifyBizApp extends React.PureComponent {
constructor(props) {
    super(props)
     this.state = {
 ...
 getNavMenuItems = (targetObject, style, customTheme) => {
	  ...
	  <Menu
       ...
       >
             ...
             <Menu.Item key="dashboard">
               <Link to={`/wechatWorkappIdentify/${this.props.wechatWorkappIdentify.id}/dashboard`}><Icon type="dashboard" style={{marginRight:"20px"}}/><span>{appLocaleName(userContext,"Dashboard")}</span></Link>
             </Menu.Item>
  ...
  getRequestTypeStepForm = () => {
    ...
    return connect(state => ({
      selectedRows: state._wechatWorkappIdentify.selectedRows,
      role: "cq",
      currentUpdateIndex: state._wechatWorkappIdentify.currentUpdateIndex,
      owner: { type: '_wechatWorkappIdentify', id: state._wechatWorkappIdentify.id, listName: 'nolist', ref:state._wechatWorkappIdentify, listDisplayName: appLocaleName(userContext,"List") }, // this is for model namespace and
  ...
  buildRouters = () =>{
  	const {WechatWorkappIdentifyDashboard} = GlobalComponents
  	const {WechatWorkappIdentifyPermission} = GlobalComponents
  	const {WechatWorkappIdentifyProfile} = GlobalComponents
  	
  	
  	const routers=[
  	{path:"/wechatWorkappIdentify/:id/dashboard", component: WechatWorkappIdentifyDashboard},
  	{path:"/wechatWorkappIdentify/:id/profile", component: WechatWorkappIdentifyProfile},
  	{path:"/wechatWorkappIdentify/:id/permission", component: WechatWorkappIdentifyPermission},
  ...
  handleOpenChange = (openKeys) => {
   ...
   render() {
    ...
    const {searchLocalData}=GlobalComponents.WechatWorkappIdentifyBase
    ...
    const renderMenuSwitch=()=>{
     ...
     <Layout>
      ...
      {this.state.showSearch&&(
        ...
        <div style={{backgroundColor:'black'}}  onClick={()=>hideSearchResult()}  >{searchLocalData(this.props.wechatWorkappIdentify,this.state.searchKeyword)}</div>
       ...
       <Sider
        ...
        >
         ...
         {this.getNavMenuItems(this.props.wechatWorkappIdentify,"inline","dark")}
