...
import styles from './WechatMiniappIdentify.updateform.less'
...
class WechatMiniappIdentifyUpdateForm extends Component {
  state = {
  ...
  render() {
    ...
    const submitUpdateForm = () => {
      validateFieldsAndScroll((error, values) => {
        ...
        const wechatMiniappIdentifyId = values.id
        const imagesValues = mapBackToImageValues(convertedImagesValues)
        const parameters = { ...values, wechatMiniappIdentifyId, ...imagesValues }
    ...
    const submitUpdateFormAndContinue = () => {
      validateFieldsAndScroll((error, values) => {
        ...
        dispatch({
          type: `${owner.type}/updateWechatMiniappIdentify`,
          payload: {
            id: owner.id,
            type: 'wechatMiniappIdentify',
            parameters,
            selectedRows,
    ...
    const skipToNext = () => {
      ...
      const newIndex = currentUpdateIndex + 1
      dispatch({
        type: `${owner.type}/gotoNextWechatMiniappIdentifyUpdateRow`,
        payload: {
          id: owner.id,
          type: 'wechatMiniappIdentify',
          selectedRows,
	...
	const internalRenderTitle = () =>{
      ...
      return (<div>{linkComp}{appLocaleName(userContext,"Update")}微信Miniapp识别: {(currentUpdateIndex+1)}/{selectedRows.length}</div>)
    }
...
}))(Form.create()(WechatMiniappIdentifyUpdateForm))
