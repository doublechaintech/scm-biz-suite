...
import styles from './WechatMiniappIdentify.app.less'
...
class WechatMiniappIdentifyBizApp extends React.PureComponent {
constructor(props) {
    super(props)
     this.state = {
 ...
 getNavMenuItems = (targetObject, style, customTheme) => {
	  ...
	  <Menu
       ...
       >
             ...
             <Menu.Item key="dashboard">
               <Link to={`/wechatMiniappIdentify/${this.props.wechatMiniappIdentify.id}/dashboard`}><Icon type="dashboard" style={{marginRight:"20px"}}/><span>{appLocaleName(userContext,"Dashboard")}</span></Link>
             </Menu.Item>
  ...
  getRequestTypeStepForm = () => {
    ...
    return connect(state => ({
      selectedRows: state._wechatMiniappIdentify.selectedRows,
      role: "cq",
      currentUpdateIndex: state._wechatMiniappIdentify.currentUpdateIndex,
      owner: { type: '_wechatMiniappIdentify', id: state._wechatMiniappIdentify.id, listName: 'nolist', ref:state._wechatMiniappIdentify, listDisplayName: appLocaleName(userContext,"List") }, // this is for model namespace and
  ...
  buildRouters = () =>{
  	const {WechatMiniappIdentifyDashboard} = GlobalComponents
  	const {WechatMiniappIdentifyPermission} = GlobalComponents
  	const {WechatMiniappIdentifyProfile} = GlobalComponents
  	
  	
  	const routers=[
  	{path:"/wechatMiniappIdentify/:id/dashboard", component: WechatMiniappIdentifyDashboard},
  	{path:"/wechatMiniappIdentify/:id/profile", component: WechatMiniappIdentifyProfile},
  	{path:"/wechatMiniappIdentify/:id/permission", component: WechatMiniappIdentifyPermission},
  ...
  handleOpenChange = (openKeys) => {
   ...
   render() {
    ...
    const {searchLocalData}=GlobalComponents.WechatMiniappIdentifyBase
    ...
    const renderMenuSwitch=()=>{
     ...
     <Layout>
      ...
      {this.state.showSearch&&(
        ...
        <div style={{backgroundColor:'black'}}  onClick={()=>hideSearchResult()}  >{searchLocalData(this.props.wechatMiniappIdentify,this.state.searchKeyword)}</div>
       ...
       <Sider
        ...
        >
         ...
         {this.getNavMenuItems(this.props.wechatMiniappIdentify,"inline","dark")}
