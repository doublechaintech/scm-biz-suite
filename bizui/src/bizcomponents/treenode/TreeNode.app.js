...
import styles from './TreeNode.app.less'
...
class TreeNodeBizApp extends React.PureComponent {
constructor(props) {
    super(props)
     this.state = {
 ...
 getNavMenuItems = (targetObject, style, customTheme) => {
    ...
    return (
	  <Menu
       ...
       <Menu.Item key="workbench">
        <Link to={`/treeNode/${this.props.treeNode.id}/workbench`}><Icon type="solution" style={{marginRight:"20px"}}/><span>工作台</span></Link>
      </Menu.Item>
        ...
        {filteredMenuItemsGroup(targetObject,this).map((groupedMenuItem,index)=>{
          return(
    <SubMenu id={`submenu-vg${index}`}  key={`vg${index}`} title={<span><Icon type={viewGroupIconNameOf('tree_node',`${groupedMenuItem.viewGroup}`)} style={{marginRight:"20px"}} /><span>{viewGroupDisplayNameOf('tree_node',`${groupedMenuItem.viewGroup}`)}</span></span>} >
  ...
  getRequestTypeStepForm = () => {
    ...
    return connect(state => ({
      selectedRows: state._treeNode.selectedRows,
      role: "cq",
      currentUpdateIndex: state._treeNode.currentUpdateIndex,
      owner: { type: '_treeNode', id: state._treeNode.id, listName: 'nolist', ref:state._treeNode, listDisplayName: appLocaleName(userContext,"List") }, // this is for model namespace and
  ...
  buildRouters = () =>{
    const {TreeNodeWorkbench} = GlobalComponents

    const {TreeNodeDashboard} = GlobalComponents
  	const {TreeNodePermission} = GlobalComponents
  	const {TreeNodeProfile} = GlobalComponents


    const routers=[
    {path:"/treeNode/:id/workbench", component: TreeNodeWorkbench},
    
  	{path:"/treeNode/:id/profile", component: TreeNodeProfile},
  	{path:"/treeNode/:id/permission", component: TreeNodePermission},
  ...
  handleOpenChange = (openKeys) => {
   ...
   render() {
    ...
    const {searchLocalData}=GlobalComponents.TreeNodeBase
    ...
    const renderMenuSwitch=()=>{
     ...
     <Layout>
      ...
      {this.state.showSearch&&(
        ...
        <div style={{backgroundColor:'black'}}  onClick={()=>hideSearchResult()}  >{searchLocalData(this.props.treeNode,this.state.searchKeyword)}</div>
       ...
       <Sider
        ...
        >
         ...
         {this.getNavMenuItems(this.props.treeNode,"inline","dark")}
