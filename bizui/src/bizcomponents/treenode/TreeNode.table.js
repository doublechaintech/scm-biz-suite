...
import styles from './TreeNode.table.less'
...
class TreeNodeTable extends PureComponent {
  state = {
    selectedRowKeys: [],
  }
  ...
  enhanceColumnsWithSorter=()=>{
    const {displayColumns} = TreeNodeBase
    ...
    const operationColumn={
      ...
      render: (text, record) => (
        <span>
          ...
          { hasReadPermission(metaInfo)&&<Link to={`/treeNode/${record.id}/dashboard`}>{appLocaleName(userContext,"View")}</Link>}
