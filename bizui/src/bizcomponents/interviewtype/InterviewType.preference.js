...
const internalImageListOf = (interviewType) =>{
...
const internalSettingListOf = (interviewType) =>{
	const userContext = null
	const optionList = [ 
	]
...
const handleTransferSearch =(targetComponent,filterKey,newRequest)=>{
  ...
  const pageNo = 1;
  const candidateReferenceService = InterviewTypeService[candidateServiceName] 
...
const executeTrans = (interviewType,targetComponent) =>{
...
const buildTransferModal = (interviewType,targetComponent) => {
...
<Modal title={transferModalTitle}
          ...
          onOk={()=>executeTrans(interviewType,targetComponent)}
...
const internalRenderExtraHeader = (interviewType) =>{
	return null
}
const internalRenderExtraFooter = (interviewType) =>{
	return null
}
...
class InterviewTypePreference extends Component {
...
}))(Form.create()(InterviewTypePreference))
