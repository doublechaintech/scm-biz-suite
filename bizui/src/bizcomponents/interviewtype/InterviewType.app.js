 ...
 getNavMenuItems = (targetObject, style, customTheme) => {
    ...
    return (
	  <Menu
       ...
       <Menu.Item key="workbench">
        <Link to={`/interviewType/${this.props.interviewType.id}/workbench`}><Icon type="solution" style={{marginRight:"20px"}}/><span>工作台</span></Link>
      </Menu.Item>
        ...
        {filteredMenuItemsGroup(targetObject,this).map((groupedMenuItem,index)=>{
          return(
    <SubMenu id={`submenu-vg${index}`}  key={`vg${index}`} title={<span><Icon type={viewGroupIconNameOf('interview_type',`${groupedMenuItem.viewGroup}`)} style={{marginRight:"20px"}} /><span>{viewGroupDisplayNameOf('interview_type',`${groupedMenuItem.viewGroup}`)}</span></span>} >
  ...
  getEmployeeInterviewSearch = () => {
    ...
    return connect(state => ({
      rule: state.rule,
      name: window.mtrans('employee_interview','interview_type.employee_interview_list',false),
      ...
      returnURL: `/interviewType/${state._interviewType.id}/workbench`,
  ...
  getEmployeeInterviewCreateForm = () => {
    ...
    return connect(state => ({
      ...
      searchFormParameters: state._interviewType.employeeInterviewSearchFormParameters,
      loading: state._interviewType.loading,
      owner: { type: '_interviewType', id: state._interviewType.id || this.getOwnerId(), referenceName: 'interviewType', listName: 'employeeInterviewList', ref:state._interviewType, listDisplayName: appLocaleName(userContext,"List")}, // this is for model namespace and
  ...
  getEmployeeInterviewUpdateForm = () => {
    ...
    return connect(state => ({
      ...
      role: "employeeInterview",
      currentUpdateIndex: state._interviewType.currentUpdateIndex || 0,
      owner: { type: '_interviewType', id: state._interviewType.id || this.getOwnerId(), listName: 'employeeInterviewList', ref:state._interviewType, listDisplayName: appLocaleName(userContext,"List") }, // this is for model namespace and
  ...
  getRequestTypeStepForm = () => {
    ...
    return connect(state => ({
      ...
      owner: { type: '_interviewType', id: state._interviewType.id, listName: 'nolist', ref:state._interviewType, listDisplayName: appLocaleName(userContext,"List") }, // this is for model namespace and
  ...
  buildRouters = () =>{
    const {InterviewTypeWorkbench} = GlobalComponents
    ...
    const routers=[
    {path:"/interviewType/:id/workbench", component: InterviewTypeWorkbench},
  ...
  handleOpenChange = (openKeys) => {
   ...
   render() {
    ...
    const {searchLocalData}=GlobalComponents.InterviewTypeBase
      ...
      {this.state.showSearch&&(
       ...
       <Sider
        ...
        >
         ...
         {this.getNavMenuItems(this.props.interviewType,"inline","dark")}
