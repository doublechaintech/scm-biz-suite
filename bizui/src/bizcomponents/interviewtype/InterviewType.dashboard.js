...
const renderSettingMenuItem = (item,cardsData,targetComponent) =>{
  ...
  return (<Menu.Item key={item.name}>
      <Link to={`/interviewType/${targetComponent.props.interviewType.id}/list/${item.name}/${item.displayName}/`}>
...
const renderSettingMenu = (cardsData,targetComponent) =>{
  ...
  return (<Menu>
    	<Menu.Item key="profile">
  			<Link to={`/interviewType/${targetComponent.props.interviewType.id}/permission`}><Icon type="safety-certificate" theme="twoTone" twoToneColor="#52c41a"/><span>{appLocaleName(userContext,"Permission")}</span></Link>
		</Menu.Item>
		<Menu.Divider />
...
const internalSummaryOf = (cardsData,targetComponent) =>{
	...
	const interviewType = cardsData.cardsSource
	const {InterviewTypeService} = GlobalComponents
	const userContext = null
	return (
	<div>
	<DescriptionList className={styles.headerList} size="small" col="4">
<Description term="ID" style={{wordBreak: 'break-all'}}>{interviewType.id}</Description> 
<Description term="代码" style={{wordBreak: 'break-all'}}>{interviewType.code}</Description> 
<Description term="描述" style={{wordBreak: 'break-all'}}>{interviewType.description}</Description> 
<Description term="详细描述" style={{wordBreak: 'break-all'}}>{interviewType.detailDescription}</Description> 
