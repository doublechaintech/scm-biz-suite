...
const internalImageListOf = (consumerOrderApproval) =>{
...
const internalSettingListOf = (consumerOrderApproval) =>{
...
const handleTransferSearch =(targetComponent,filterKey,newRequest)=>{
  ...
  const pageNo = 1;
  const candidateReferenceService = ConsumerOrderApprovalService[candidateServiceName] 
...
const executeTrans = (consumerOrderApproval,targetComponent) =>{
  ...
  validateFieldsAndScroll((error, values) => {
    ...
    const id=consumerOrderApproval.id;
    const serviceNameToCall = transferServiceName;

    const payload = {parameters,id,serviceNameToCall}
    
    //targetComponent.setState({transferModalVisiable:false})
    dispatch({type:"_consumerOrderApproval/doJob",payload: payload})
...
const buildTransferModal = (consumerOrderApproval,targetComponent) => {
...
<Modal title={transferModalTitle}
          ...
          onOk={()=>executeTrans(consumerOrderApproval,targetComponent)}
...
const internalRenderExtraHeader = (consumerOrderApproval) =>{
	return null
}
const internalRenderExtraFooter = (consumerOrderApproval) =>{
	return null
}
...
class ConsumerOrderApprovalPreference extends Component {
  ...
  render() {
    ...
    const cardsData = {cardsName:"消费者的订单审批",cardsFor: "consumerOrderApproval",cardsSource: this.props.consumerOrderApproval,
  		subItems: [
...
}))(Form.create()(ConsumerOrderApprovalPreference))
