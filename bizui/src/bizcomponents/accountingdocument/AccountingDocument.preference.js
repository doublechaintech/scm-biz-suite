...
const internalImageListOf = (accountingDocument) =>{
...
const internalSettingListOf = (accountingDocument) =>{
	const userContext = null
	const optionList = [ 
	]
...
const handleTransferSearch =(targetComponent,filterKey,newRequest)=>{
  ...
  const pageNo = 1;
  const candidateReferenceService = AccountingDocumentService[candidateServiceName] 
...
const executeTrans = (accountingDocument,targetComponent) =>{
...
const buildTransferModal = (accountingDocument,targetComponent) => {
...
<Modal title={transferModalTitle}
          ...
          onOk={()=>executeTrans(accountingDocument,targetComponent)}
...
const internalRenderExtraHeader = (accountingDocument) =>{
	return null
}
const internalRenderExtraFooter = (accountingDocument) =>{
	return null
}
...
<Description term="会计期间">{accountingDocument.accountingPeriod==null?appLocaleName(userContext,"NotAssigned"):accountingDocument.accountingPeriod.displayName}
 <Icon type="swap" onClick={()=>
  showTransferModel(targetComponent,"会计期间","accountingPeriod","requestCandidateAccountingPeriod",
	      "transferToAnotherAccountingPeriod","anotherAccountingPeriodId",accountingDocument.accountingPeriod?accountingDocument.accountingPeriod.id:"")} 
  style={{fontSize: 20,color:"red"}} />
</Description>
<Description term="文档类型">{accountingDocument.documentType==null?appLocaleName(userContext,"NotAssigned"):accountingDocument.documentType.displayName}
 <Icon type="swap" onClick={()=>
  showTransferModel(targetComponent,"文档类型","accountingDocumentType","requestCandidateDocumentType",
	      "transferToAnotherDocumentType","anotherDocumentTypeId",accountingDocument.documentType?accountingDocument.documentType.id:"")} 
  style={{fontSize: 20,color:"red"}} />
</Description>
...
class AccountingDocumentPreference extends Component {
...
}))(Form.create()(AccountingDocumentPreference))
