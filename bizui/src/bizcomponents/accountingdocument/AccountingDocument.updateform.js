...
import styles from './AccountingDocument.updateform.less'
...
class AccountingDocumentUpdateForm extends Component {
  state = {
  ...
  getSelectedRow() {
    ...
    const convertiedValues = selectedRows.map((item) => {
      return {
        ...item,
        accountingDocumentDate: moment(item.accountingDocumentDate),
...
>>>>>>> 502e8b8dfc403300a992b5083e79c722e85d1854
        ...
        const accountingDocumentId = values.id
        const imagesValues = mapBackToImageValues(convertedImagesValues)
        const parameters = { ...values, accountingDocumentId, ...imagesValues }
    ...
    const submitUpdateFormAndContinue = () => {
      validateFieldsAndScroll((error, values) => {
        ...
        dispatch({
          type: `${owner.type}/updateAccountingDocument`,
          payload: {
            id: owner.id,
    ...
    const skipToNext = () => {
      ...
      const newIndex = currentUpdateIndex + 1
      dispatch({
        type: `${owner.type}/gotoNextAccountingDocumentUpdateRow`,
        payload: {
          id: owner.id,
          type: 'accountingDocument',
          selectedRows,
...
<<<<<<< HEAD
    ...
    return (
      <PageHeaderLayout
        title={"更新会计凭证"+(currentUpdateIndex+1)+"/"+selectedRows.length}
        content="更新会计凭证"
...
=======
	...
	const internalRenderTitle = () =>{
      ...
      return (<div>{linkComp}{appLocaleName(userContext,"Update")}会计凭证: {(currentUpdateIndex+1)}/{selectedRows.length}</div>)
    }
...
}))(Form.create()(AccountingDocumentUpdateForm))
