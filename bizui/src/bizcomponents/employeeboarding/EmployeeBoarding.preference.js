...
import styles from './EmployeeBoarding.preference.less'
...
const internalImageListOf = (employeeBoarding) =>{
...
const internalSettingListOf = (employeeBoarding) =>{
	const userContext = null
	const optionList = [ 
	]
...
const handleTransferSearch =(targetComponent,filterKey,newRequest)=>{
  ...
  const pageNo = 1;
  const candidateReferenceService = EmployeeBoardingService[candidateServiceName] 
...
const executeTrans = (employeeBoarding,targetComponent) =>{
...
const buildTransferModal = (employeeBoarding,targetComponent) => {
...
<Modal title={transferModalTitle}
          ...
          onOk={()=>executeTrans(employeeBoarding,targetComponent)}
...
const internalRenderExtraHeader = (employeeBoarding) =>{
	return null
}
const internalRenderExtraFooter = (employeeBoarding) =>{
	return null
}
...
class EmployeeBoardingPreference extends Component {
...
}))(Form.create()(EmployeeBoardingPreference))
