...
import styles from './ConsumerOrderShipment.table.less'
...
class ConsumerOrderShipmentTable extends PureComponent {
  state = {
    selectedRowKeys: [],
  }
...
<<<<<<< HEAD
 calcDisplayColumns=()=>{
    ...
    const {displayColumns} = ConsumerOrderShipmentBase
    if(!referenceName){
    ...
    const operationColumn={
      ...
      render: (text, record) => (
        <span>
         ...
         { hasReadPermission(metaInfo)&&<Link to={`/consumerOrderShipment/${record.id}/dashboard`}>{'查看'}</Link>}
    ...
    const operationColumn={
      ...
      render: (text, record) => (
        <span>
          ...
          { hasReadPermission(metaInfo)&&<Link to={`/consumerOrderShipment/${record.id}/dashboard`}>{appLocaleName(userContext,"View")}</Link>}
...
export default ConsumerOrderShipmentTable
