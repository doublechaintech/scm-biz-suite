...
const internalImageListOf = (consumerOrderShipment) =>{
...
const internalSettingListOf = (consumerOrderShipment) =>{
...
const handleTransferSearch =(targetComponent,filterKey,newRequest)=>{
  ...
  const pageNo = 1;
  const candidateReferenceService = ConsumerOrderShipmentService[candidateServiceName] 
...
const executeTrans = (consumerOrderShipment,targetComponent) =>{
  ...
  validateFieldsAndScroll((error, values) => {
    ...
    const id=consumerOrderShipment.id;
    const serviceNameToCall = transferServiceName;

    const payload = {parameters,id,serviceNameToCall}
    
    //targetComponent.setState({transferModalVisiable:false})
    dispatch({type:"_consumerOrderShipment/doJob",payload: payload})
...
const buildTransferModal = (consumerOrderShipment,targetComponent) => {
...
<Modal title={transferModalTitle}
          ...
          onOk={()=>executeTrans(consumerOrderShipment,targetComponent)}
...
const internalRenderExtraHeader = (consumerOrderShipment) =>{
	return null
}
const internalRenderExtraFooter = (consumerOrderShipment) =>{
	return null
}
...
class ConsumerOrderShipmentPreference extends Component {
  ...
  render() {
    ...
    const cardsData = {cardsName:"消费者的订单发货",cardsFor: "consumerOrderShipment",cardsSource: this.props.consumerOrderShipment,
  		subItems: [
...
}))(Form.create()(ConsumerOrderShipmentPreference))
