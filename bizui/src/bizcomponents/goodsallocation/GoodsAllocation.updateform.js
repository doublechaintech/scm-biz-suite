...
import styles from './GoodsAllocation.updateform.less'
...
class GoodsAllocationUpdateForm extends Component {
  state = {
...
>>>>>>> 502e8b8dfc403300a992b5083e79c722e85d1854
        ...
        const goodsAllocationId = values.id
        const imagesValues = mapBackToImageValues(convertedImagesValues)
        const parameters = { ...values, goodsAllocationId, ...imagesValues }
    ...
    const submitUpdateFormAndContinue = () => {
      validateFieldsAndScroll((error, values) => {
        ...
        dispatch({
          type: `${owner.type}/updateGoodsAllocation`,
          payload: {
            id: owner.id,
            type: 'goodsAllocation',
            parameters,
            selectedRows,
    ...
    const skipToNext = () => {
      ...
      const newIndex = currentUpdateIndex + 1
      dispatch({
        type: `${owner.type}/gotoNextGoodsAllocationUpdateRow`,
        payload: {
          id: owner.id,
          type: 'goodsAllocation',
          selectedRows,
...
<<<<<<< HEAD
    ...
    return (
      <PageHeaderLayout
        title={"更新货位"+(currentUpdateIndex+1)+"/"+selectedRows.length}
        content="更新货位"
...
=======
	...
	const internalRenderTitle = () =>{
      ...
      return (<div>{linkComp}{appLocaleName(userContext,"Update")}货位: {(currentUpdateIndex+1)}/{selectedRows.length}</div>)
    }
...
}))(Form.create()(GoodsAllocationUpdateForm))
