...
const internalImageListOf = (goodsPackaging) =>{
...
const internalSettingListOf = (goodsPackaging) =>{
	const userContext = null
	const optionList = [ 
	]
...
const handleTransferSearch =(targetComponent,filterKey,newRequest)=>{
  ...
  const pageNo = 1;
  const candidateReferenceService = GoodsPackagingService[candidateServiceName] 
...
const executeTrans = (goodsPackaging,targetComponent) =>{
...
const buildTransferModal = (goodsPackaging,targetComponent) => {
...
<Modal title={transferModalTitle}
          ...
          onOk={()=>executeTrans(goodsPackaging,targetComponent)}
...
const internalRenderExtraHeader = (goodsPackaging) =>{
	return null
}
const internalRenderExtraFooter = (goodsPackaging) =>{
	return null
}
...
class GoodsPackagingPreference extends Component {
  ...
  render() {
    ...
    const { id,displayName, goodsCount } = this.props.goodsPackaging
...
}))(Form.create()(GoodsPackagingPreference))
