 ...
 getNavMenuItems = (targetObject, style, customTheme) => {
    ...
    return (
	  <Menu
       ...
       <Menu.Item key="workbench">
        <Link to={`/terminationType/${this.props.terminationType.id}/workbench`}><Icon type="solution" style={{marginRight:"20px"}}/><span>工作台</span></Link>
      </Menu.Item>
        ...
        {filteredMenuItemsGroup(targetObject,this).map((groupedMenuItem,index)=>{
          return(
    <SubMenu id={`submenu-vg${index}`}  key={`vg${index}`} title={<span><Icon type={viewGroupIconNameOf('termination_type',`${groupedMenuItem.viewGroup}`)} style={{marginRight:"20px"}} /><span>{viewGroupDisplayNameOf('termination_type',`${groupedMenuItem.viewGroup}`)}</span></span>} >
  ...
  getTerminationSearch = () => {
    ...
    return connect(state => ({
      rule: state.rule,
      name: window.mtrans('termination','termination_type.termination_list',false),
      role: "termination",
      ...
      returnURL: `/terminationType/${state._terminationType.id}/workbench`,
  ...
  getTerminationCreateForm = () => {
    ...
    return connect(state => ({
      ...
      metaInfo: state._terminationType.terminationListMetaInfo,
      count: state._terminationType.terminationCount,
      returnURL: `/terminationType/${state._terminationType.id}/list`,
      currentPage: state._terminationType.terminationCurrentPageNumber,
      searchFormParameters: state._terminationType.terminationSearchFormParameters,
      loading: state._terminationType.loading,
      owner: { type: '_terminationType', id: state._terminationType.id || this.getOwnerId(), referenceName: 'type', listName: 'terminationList', ref:state._terminationType, listDisplayName: appLocaleName(userContext,"List")}, // this is for model namespace and
  ...
  getTerminationUpdateForm = () => {
    ...
    return connect(state => ({
      ...
      role: "termination",
      currentUpdateIndex: state._terminationType.currentUpdateIndex || 0,
      owner: { type: '_terminationType', id: state._terminationType.id || this.getOwnerId(), listName: 'terminationList', ref:state._terminationType, listDisplayName: appLocaleName(userContext,"List") }, // this is for model namespace and
  ...
  getRequestTypeStepForm = () => {
    ...
    return connect(state => ({
      ...
      owner: { type: '_terminationType', id: state._terminationType.id, listName: 'nolist', ref:state._terminationType, listDisplayName: appLocaleName(userContext,"List") }, // this is for model namespace and
  ...
  buildRouters = () =>{
    const {TerminationTypeWorkbench} = GlobalComponents

    const {TerminationTypeDashboard} = GlobalComponents
  	const {TerminationTypePermission} = GlobalComponents
  	const {TerminationTypeProfile} = GlobalComponents


    const routers=[
    {path:"/terminationType/:id/workbench", component: TerminationTypeWorkbench},
    
  	{path:"/terminationType/:id/profile", component: TerminationTypeProfile},
  	{path:"/terminationType/:id/permission", component: TerminationTypePermission},
  ...
  handleOpenChange = (openKeys) => {
   ...
   render() {
    ...
    const {searchLocalData}=GlobalComponents.TerminationTypeBase
    ...
    const renderMenuSwitch=()=>{
     ...
     <Layout>
      ...
      {this.state.showSearch&&(
        ...
        <div style={{backgroundColor:'black'}}  onClick={()=>hideSearchResult()}  >{searchLocalData(this.props.terminationType,this.state.searchKeyword)}</div>
       ...
       <Sider
        ...
        >
         ...
         {this.getNavMenuItems(this.props.terminationType,"inline","dark")}
