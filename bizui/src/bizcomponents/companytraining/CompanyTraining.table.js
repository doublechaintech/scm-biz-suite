...
<<<<<<< HEAD
 calcDisplayColumns=()=>{
    ...
    const {displayColumns} = CompanyTrainingBase
    if(!referenceName){
    ...
    const operationColumn={
      ...
      render: (text, record) => (
        <span>
         ...
         { hasReadPermission(metaInfo)&&<Link to={`/companyTraining/${record.id}/dashboard`}>{'查看'}</Link>}
    ...
    const operationColumn={
      ...
      render: (text, record) => (
        <span>
          ...
          { hasReadPermission(metaInfo)&&<Link to={`/companyTraining/${record.id}/dashboard`}>{appLocaleName(userContext,"View")}</Link>}
