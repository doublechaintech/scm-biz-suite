 ...
 getNavMenuItems = (targetObject, style, customTheme) => {
    ...
    return (
	  <Menu
       ...
       <Menu.Item key="workbench">
        <Link to={`/companyTraining/${this.props.companyTraining.id}/workbench`}><Icon type="solution" style={{marginRight:"20px"}}/><span>工作台</span></Link>
      </Menu.Item>
        ...
        {filteredMenuItemsGroup(targetObject,this).map((groupedMenuItem,index)=>{
          return(
    <SubMenu id={`submenu-vg${index}`}  key={`vg${index}`} title={<span><Icon type={viewGroupIconNameOf('company_training',`${groupedMenuItem.viewGroup}`)} style={{marginRight:"20px"}} /><span>{viewGroupDisplayNameOf('company_training',`${groupedMenuItem.viewGroup}`)}</span></span>} >
  ...
  getEmployeeCompanyTrainingSearch = () => {
    ...
    return connect(state => ({
      rule: state.rule,
      name: window.mtrans('employee_company_training','company_training.employee_company_training_list',false),
      ...
      returnURL: `/companyTraining/${state._companyTraining.id}/workbench`,
  ...
  getEmployeeCompanyTrainingCreateForm = () => {
    ...
    return connect(state => ({
      ...
      searchFormParameters: state._companyTraining.employeeCompanyTrainingSearchFormParameters,
      loading: state._companyTraining.loading,
      owner: { type: '_companyTraining', id: state._companyTraining.id || this.getOwnerId(), referenceName: 'training', listName: 'employeeCompanyTrainingList', ref:state._companyTraining, listDisplayName: appLocaleName(userContext,"List")}, // this is for model namespace and
  ...
  getEmployeeCompanyTrainingUpdateForm = () => {
    ...
    return connect(state => ({
      ...
      role: "employeeCompanyTraining",
      currentUpdateIndex: state._companyTraining.currentUpdateIndex || 0,
      owner: { type: '_companyTraining', id: state._companyTraining.id || this.getOwnerId(), listName: 'employeeCompanyTrainingList', ref:state._companyTraining, listDisplayName: appLocaleName(userContext,"List") }, // this is for model namespace and
  ...
  getRequestTypeStepForm = () => {
    ...
    return connect(state => ({
      ...
      owner: { type: '_companyTraining', id: state._companyTraining.id, listName: 'nolist', ref:state._companyTraining, listDisplayName: appLocaleName(userContext,"List") }, // this is for model namespace and
  ...
  buildRouters = () =>{
    const {CompanyTrainingWorkbench} = GlobalComponents
    ...
    const routers=[
    {path:"/companyTraining/:id/workbench", component: CompanyTrainingWorkbench},
  ...
  handleOpenChange = (openKeys) => {
   ...
   render() {
    ...
    const {searchLocalData}=GlobalComponents.CompanyTrainingBase
    ...
    const renderMenuSwitch=()=>{
     ...
     <Layout>
      ...
      {this.state.showSearch&&(
        ...
        <div style={{backgroundColor:'black'}}  onClick={()=>hideSearchResult()}  >{searchLocalData(this.props.companyTraining,this.state.searchKeyword)}</div>
       ...
       <Sider
        ...
        >
         ...
         {this.getNavMenuItems(this.props.companyTraining,"inline","dark")}
