...
import styles from './RetailStoreOrderShippingGroup.app.less'
...
class RetailStoreOrderShippingGroupBizApp extends React.PureComponent {
constructor(props) {
    super(props)
     this.state = {
 ...
 getNavMenuItems = (targetObject, style, customTheme) => {
    ...
    return (
	  <Menu
       ...
       <Menu.Item key="workbench">
        <Link to={`/retailStoreOrderShippingGroup/${this.props.retailStoreOrderShippingGroup.id}/workbench`}><Icon type="solution" style={{marginRight:"20px"}}/><span>工作台</span></Link>
      </Menu.Item>
        ...
        {filteredMenuItemsGroup(targetObject,this).map((groupedMenuItem,index)=>{
          return(
    <SubMenu id={`submenu-vg${index}`}  key={`vg${index}`} title={<span><Icon type={viewGroupIconNameOf('retail_store_order_shipping_group',`${groupedMenuItem.viewGroup}`)} style={{marginRight:"20px"}} /><span>{viewGroupDisplayNameOf('retail_store_order_shipping_group',`${groupedMenuItem.viewGroup}`)}</span></span>} >
  ...
  getRequestTypeStepForm = () => {
    ...
    return connect(state => ({
      selectedRows: state._retailStoreOrderShippingGroup.selectedRows,
      role: "cq",
      currentUpdateIndex: state._retailStoreOrderShippingGroup.currentUpdateIndex,
      owner: { type: '_retailStoreOrderShippingGroup', id: state._retailStoreOrderShippingGroup.id, listName: 'nolist', ref:state._retailStoreOrderShippingGroup, listDisplayName: appLocaleName(userContext,"List") }, // this is for model namespace and
  ...
  buildRouters = () =>{
    const {RetailStoreOrderShippingGroupWorkbench} = GlobalComponents

    const {RetailStoreOrderShippingGroupDashboard} = GlobalComponents
  	const {RetailStoreOrderShippingGroupPermission} = GlobalComponents
  	const {RetailStoreOrderShippingGroupProfile} = GlobalComponents


    const routers=[
    {path:"/retailStoreOrderShippingGroup/:id/workbench", component: RetailStoreOrderShippingGroupWorkbench},
    
  	{path:"/retailStoreOrderShippingGroup/:id/profile", component: RetailStoreOrderShippingGroupProfile},
  	{path:"/retailStoreOrderShippingGroup/:id/permission", component: RetailStoreOrderShippingGroupPermission},
  ...
  handleOpenChange = (openKeys) => {
   ...
   render() {
    ...
    const {searchLocalData}=GlobalComponents.RetailStoreOrderShippingGroupBase
    ...
    const renderMenuSwitch=()=>{
     ...
     <Layout>
      ...
      {this.state.showSearch&&(
        ...
        <div style={{backgroundColor:'black'}}  onClick={()=>hideSearchResult()}  >{searchLocalData(this.props.retailStoreOrderShippingGroup,this.state.searchKeyword)}</div>
       ...
       <Sider
        ...
        >
         ...
         {this.getNavMenuItems(this.props.retailStoreOrderShippingGroup,"inline","dark")}
