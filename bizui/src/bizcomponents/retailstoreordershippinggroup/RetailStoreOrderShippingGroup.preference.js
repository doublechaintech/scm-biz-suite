...
const internalImageListOf = (retailStoreOrderShippingGroup) =>{
...
const internalSettingListOf = (retailStoreOrderShippingGroup) =>{
	const userContext = null
	const optionList = [ 
	]
...
const handleTransferSearch =(targetComponent,filterKey,newRequest)=>{
  ...
  const pageNo = 1;
  const candidateReferenceService = RetailStoreOrderShippingGroupService[candidateServiceName] 
...
const executeTrans = (retailStoreOrderShippingGroup,targetComponent) =>{
  ...
  validateFieldsAndScroll((error, values) => {
    ...
    const id=retailStoreOrderShippingGroup.id;
    const serviceNameToCall = transferServiceName;

    const payload = {parameters,id,serviceNameToCall}
    
    //targetComponent.setState({transferModalVisiable:false})
    dispatch({type:"_retailStoreOrderShippingGroup/doJob",payload: payload})
...
const buildTransferModal = (retailStoreOrderShippingGroup,targetComponent) => {
...
<Modal title={transferModalTitle}
          ...
          onOk={()=>executeTrans(retailStoreOrderShippingGroup,targetComponent)}
...
const internalRenderExtraFooter = (retailStoreOrderShippingGroup) =>{
	return null
}
...
<Description term="订单">{retailStoreOrderShippingGroup.bizOrder==null?appLocaleName(userContext,"NotAssigned"):retailStoreOrderShippingGroup.bizOrder.displayName}
 <Icon type="swap" onClick={()=>
  showTransferModel(targetComponent,"订单","retailStoreOrder","requestCandidateBizOrder",
	      "transferToAnotherBizOrder","anotherBizOrderId",retailStoreOrderShippingGroup.bizOrder?retailStoreOrderShippingGroup.bizOrder.id:"")} 
...
class RetailStoreOrderShippingGroupPreference extends Component {
...
}))(Form.create()(RetailStoreOrderShippingGroupPreference))
