...
import styles from './RetailStoreOrderShippingGroup.workbench.less'
...
const renderSettingMenuItem = (item,cardsData,targetComponent) =>{
  ...
  return (<Menu.Item key={item.name}>
      <Link to={`/retailStoreOrderShippingGroup/${targetComponent.props.retailStoreOrderShippingGroup.id}/list/${item.name}/${item.displayName}/`}>
...
const renderSettingMenu = (cardsData,targetComponent) =>{
  ...
  return (<Menu>
    	<Menu.Item key="profile">
  			<Link to={`/retailStoreOrderShippingGroup/${targetComponent.props.retailStoreOrderShippingGroup.id}/permission`}><Icon type="safety-certificate" theme="twoTone" twoToneColor="#52c41a"/><span>{appLocaleName(userContext,"Permission")}</span></Link>
		</Menu.Item>
		<Menu.Divider />
...
const internalSummaryOf = (cardsData,targetComponent) =>{
	...
	const retailStoreOrderShippingGroup = cardsData.cardsSource
	const {RetailStoreOrderShippingGroupService} = GlobalComponents
	const userContext = null
	return (
	<div>
	<DescriptionList className={styles.headerList} size="small" col="4">
<Description term="ID" style={{wordBreak: 'break-all'}}>{retailStoreOrderShippingGroup.id}</Description> 
...
class RetailStoreOrderShippingGroupWorkbench extends Component {
...
}))(Form.create()(RetailStoreOrderShippingGroupWorkbench))
