...
import styles from './FormMessage.search.less'
import ListViewTool from '../../common/ListView.tool'
import FormMessageBase from './FormMessage.base'
import PermissionSettingService from '../../permission/PermissionSetting.service'
import appLocaleName from '../../common/Locale.tool'
const {fieldLabels} = FormMessageBase
...
const showAssociateDialog = (targetComponent) => {
  ...
  const { GenericFormAssociateForm } = GlobalComponents
  ...
  <div>
	...
	visible={currentAssociateModal==='form'} 
	data={{formMessageList:selectedRows}} owner={owner}  
	onCancel={()=>toggleAssociateModalVisible(targetComponent,'form')} 
	onCreate={()=>toggleAssociateModalVisible(targetComponent,'form')}/> 
...
class FormMessageSearch extends PureComponent {
  state = {
  ...
  render(){
    ...
    const {FormMessageTable} = GlobalComponents;
    const {FormMessageSearchForm} = GlobalComponents;
    const {FormMessageModalTable} = GlobalComponents;
    ...
    return (
      <PageHeaderLayout title={renderTitle()}>
        <Card bordered={false}>
          <div className={styles.tableList}>
            <div className={styles.tableListForm}>
              <FormMessageSearchForm {...this.props} />
            </div>
        ...
        {showDeletionDialog(this,FormMessageModalTable,"formMessageIds")}
...
export default Form.create()(FormMessageSearch)
