...
export default {
  ...
  effects: {
    ...
    *addEmployee({ payload }, { call, put }) {
      ...
      const location = { pathname: `/offerApproval/${id}/list/EmployeeList/Employee+${appLocaleName(userContext,'List')}`, state: newState }
      yield put(routerRedux.push(location))
    },
    *updateEmployee({ payload }, { call, put }) {
      ...
      const location = { pathname: `/offerApproval/${id}/list/EmployeeList/Employee列表`, state: newPlayload }
      yield put(routerRedux.push(location))
    },
