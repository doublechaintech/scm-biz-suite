...
import styles from './MemberWishlist.updateform.less'
...
class MemberWishlistUpdateForm extends Component {
  state = {
...
>>>>>>> 502e8b8dfc403300a992b5083e79c722e85d1854
        ...
        const memberWishlistId = values.id
        const imagesValues = mapBackToImageValues(convertedImagesValues)
        const parameters = { ...values, memberWishlistId, ...imagesValues }
    ...
    const submitUpdateFormAndContinue = () => {
      validateFieldsAndScroll((error, values) => {
        ...
        dispatch({
          type: `${owner.type}/updateMemberWishlist`,
          payload: {
            id: owner.id,
            type: 'memberWishlist',
            parameters,
    ...
    const skipToNext = () => {
      ...
      const newIndex = currentUpdateIndex + 1
      dispatch({
        type: `${owner.type}/gotoNextMemberWishlistUpdateRow`,
        payload: {
          id: owner.id,
          type: 'memberWishlist',
          selectedRows,
...
<<<<<<< HEAD
    ...
    return (
      <PageHeaderLayout
        title={"更新会员收藏"+(currentUpdateIndex+1)+"/"+selectedRows.length}
        content="更新会员收藏"
...
=======
	...
	const internalRenderTitle = () =>{
      ...
      return (<div>{linkComp}{appLocaleName(userContext,"Update")}会员收藏: {(currentUpdateIndex+1)}/{selectedRows.length}</div>)
    }
...
}))(Form.create()(MemberWishlistUpdateForm))
