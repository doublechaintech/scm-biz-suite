...
const menuData = {menuName: window.trans('member_wishlist'), menuFor: "memberWishlist",  internalName: "member_wishlist",
  		subItems: [
...
const settingMenuData = {menuName: window.trans('member_wishlist'), menuFor: "memberWishlist",  internalName: "member_wishlist",
  		subItems: [
...
const renderActionList=(memberWishlist, targetObject, columCount, listName)=>{

	if(!memberWishlist){
		return null
	}
	if(!memberWishlist.actionList){
		return null
	}
	if(memberWishlist.actionList.length === 0){
		return null
	}
	return (
		<div className={styles.overlay}>
			...
			{memberWishlist.actionList.map(action=>(<Link key={action.id} to={{pathname: action.actionPath.substring(1), state: {ownerId:targetObject.id,action,selectedRows:[memberWishlist]}}} >
...
const renderItemOfList=(memberWishlist, targetObject, columCount, listName)=>{
  ...
  const userContext = null
  return (
     <Row key={`${listName}-${memberWishlist.id}`} className={styles.itemDesc}>
	...
	<Col span={20}>
      ...
      {renderActionList(memberWishlist,targetObject)}
    </Row>
	)
...
const MemberWishlistBase={unpackObjectToFormValues, menuData,settingMenuData,displayColumns,fieldLabels,renderItemOfList, stepOf, searchLocalData}
