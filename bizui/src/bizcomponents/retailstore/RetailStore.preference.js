...
const internalImageListOf = (retailStore) =>{
...
const internalSettingListOf = (retailStore) =>{
	const userContext = null
	const optionList = [ 
	]
...
const handleTransferSearch =(targetComponent,filterKey,newRequest)=>{
  ...
  const pageNo = 1;
  const candidateReferenceService = RetailStoreService[candidateServiceName] 
...
const executeTrans = (retailStore,targetComponent) =>{
...
const buildTransferModal = (retailStore,targetComponent) => {
...
<Modal title={transferModalTitle}
          ...
          onOk={()=>executeTrans(retailStore,targetComponent)}
...
const internalRenderExtraHeader = (retailStore) =>{
	return null
}
const internalRenderExtraFooter = (retailStore) =>{
	return null
}
...
<Description term="城市服务中心">{retailStore.cityServiceCenter==null?appLocaleName(userContext,"NotAssigned"):retailStore.cityServiceCenter.displayName}
 <Icon type="swap" onClick={()=>
  showTransferModel(targetComponent,"城市服务中心","retailStoreCityServiceCenter","requestCandidateCityServiceCenter",
	      "transferToAnotherCityServiceCenter","anotherCityServiceCenterId",retailStore.cityServiceCenter?retailStore.cityServiceCenter.id:"")} 
...
class RetailStorePreference extends Component {
  ...
  render() {
    ...
    const { id,displayName, consumerOrderCount, retailStoreOrderCount, goodsCount, transportTaskCount, accountSetCount } = this.props.retailStore
...
}))(Form.create()(RetailStorePreference))
