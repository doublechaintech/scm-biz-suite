...
const internalImageListOf = (retailStoreOrderDelivery) =>{
<<<<<<< HEAD
...
const internalSettingListOf = (retailStoreOrderDelivery) =>{
<<<<<<< HEAD
...
const handleTransferSearch =(targetComponent,filterKey,newRequest)=>{
  ...
  const pageNo = 1;
  const candidateReferenceService = RetailStoreOrderDeliveryService[candidateServiceName] 
...
const executeTrans = (retailStoreOrderDelivery,targetComponent) =>{
  ...
  validateFieldsAndScroll((error, values) => {
    ...
    const id=retailStoreOrderDelivery.id;
    const serviceNameToCall = transferServiceName;

    const payload = {parameters,id,serviceNameToCall}
    
    //targetComponent.setState({transferModalVisiable:false})
    dispatch({type:"_retailStoreOrderDelivery/doJob",payload: payload})
...
const buildTransferModal = (retailStoreOrderDelivery,targetComponent) => {
...
<Modal title={transferModalTitle}
          ...
          onOk={()=>executeTrans(retailStoreOrderDelivery,targetComponent)}
...
const internalRenderExtraHeader = (retailStoreOrderDelivery) =>{
	return null
}
const internalRenderExtraFooter = (retailStoreOrderDelivery) =>{
	return null
}
...
class RetailStoreOrderDeliveryPreference extends Component {
...
>>>>>>> 502e8b8dfc403300a992b5083e79c722e85d1854
  ...
  render() {
    ...
    const { id,displayName, retailStoreOrderCount } = this.props.retailStoreOrderDelivery
    const cardsData = {cardsName:"生超订单交付",cardsFor: "retailStoreOrderDelivery",cardsSource: this.props.retailStoreOrderDelivery,
  		subItems: [
...
}))(Form.create()(RetailStoreOrderDeliveryPreference))
