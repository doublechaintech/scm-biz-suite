...
const internalImageListOf = (goodsShelfStockCount) =>{
...
const internalSettingListOf = (goodsShelfStockCount) =>{
	const userContext = null
	const optionList = [ 
	]
...
const handleTransferSearch =(targetComponent,filterKey,newRequest)=>{
  ...
  const pageNo = 1;
  const candidateReferenceService = GoodsShelfStockCountService[candidateServiceName] 
...
const executeTrans = (goodsShelfStockCount,targetComponent) =>{
  ...
  validateFieldsAndScroll((error, values) => {
    ...
    const id=goodsShelfStockCount.id;
    const serviceNameToCall = transferServiceName;

    const payload = {parameters,id,serviceNameToCall}
    
    //targetComponent.setState({transferModalVisiable:false})
    dispatch({type:"_goodsShelfStockCount/doJob",payload: payload})
...
const buildTransferModal = (goodsShelfStockCount,targetComponent) => {
...
<Modal title={transferModalTitle}
          ...
          onOk={()=>executeTrans(goodsShelfStockCount,targetComponent)}
...
const internalRenderExtraFooter = (goodsShelfStockCount) =>{
	return null
}
...
<Description term="架">{goodsShelfStockCount.shelf==null?appLocaleName(userContext,"NotAssigned"):goodsShelfStockCount.shelf.displayName}
 <Icon type="swap" onClick={()=>
  showTransferModel(targetComponent,"架","goodsShelf","requestCandidateShelf",
	      "transferToAnotherShelf","anotherShelfId",goodsShelfStockCount.shelf?goodsShelfStockCount.shelf.id:"")} 
...
class GoodsShelfStockCountPreference extends Component {
...
}))(Form.create()(GoodsShelfStockCountPreference))
