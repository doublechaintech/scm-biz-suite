 ...
 getNavMenuItems = (targetObject, style, customTheme) => {
    ...
    return (
	  <Menu
       ...
       <Menu.Item key="workbench">
        <Link to={`/shippingSpace/${this.props.shippingSpace.id}/workbench`}><Icon type="solution" style={{marginRight:"20px"}}/><span>工作台</span></Link>
      </Menu.Item>
        ...
        {filteredMenuItemsGroup(targetObject,this).map((groupedMenuItem,index)=>{
          return(
    <SubMenu id={`submenu-vg${index}`}  key={`vg${index}`} title={<span><Icon type={viewGroupIconNameOf('shipping_space',`${groupedMenuItem.viewGroup}`)} style={{marginRight:"20px"}} /><span>{viewGroupDisplayNameOf('shipping_space',`${groupedMenuItem.viewGroup}`)}</span></span>} >
  ...
  getGoodsCreateForm = () => {
    ...
    return connect(state => ({
      ...
      metaInfo: state._shippingSpace.goodsListMetaInfo,
      count: state._shippingSpace.goodsCount,
      returnURL: `/shippingSpace/${state._shippingSpace.id}/list`,
      currentPage: state._shippingSpace.goodsCurrentPageNumber,
      searchFormParameters: state._shippingSpace.goodsSearchFormParameters,
      loading: state._shippingSpace.loading,
      owner: { type: '_shippingSpace', id: state._shippingSpace.id || this.getOwnerId(), referenceName: 'shippingSpace', listName: 'goodsList', ref:state._shippingSpace, listDisplayName: appLocaleName(userContext,"List")}, // this is for model namespace and
    }))(GoodsCreateForm)
  }
  ...
  getGoodsUpdateForm = () => {
    ...
    return connect(state => ({
      ...
      role: "goods",
      currentUpdateIndex: state._shippingSpace.currentUpdateIndex || 0,
      owner: { type: '_shippingSpace', id: state._shippingSpace.id || this.getOwnerId(), listName: 'goodsList', ref:state._shippingSpace, listDisplayName: appLocaleName(userContext,"List") }, // this is for model namespace and
    }))(GoodsUpdateForm)
  }
  ...
  getRequestTypeStepForm = () => {
    ...
    return connect(state => ({
      ...
      owner: { type: '_shippingSpace', id: state._shippingSpace.id, listName: 'nolist', ref:state._shippingSpace, listDisplayName: appLocaleName(userContext,"List") }, // this is for model namespace and
  ...
  buildRouters = () =>{
    const {ShippingSpaceWorkbench} = GlobalComponents
    ...
    const routers=[
    {path:"/shippingSpace/:id/workbench", component: ShippingSpaceWorkbench},
  ...
  handleOpenChange = (openKeys) => {
   ...
   render() {
    ...
    const {searchLocalData}=GlobalComponents.ShippingSpaceBase
    ...
    const renderMenuSwitch=()=>{
     ...
     <Layout>
      ...
      {this.state.showSearch&&(
        ...
        <div style={{backgroundColor:'black'}}  onClick={()=>hideSearchResult()}  >{searchLocalData(this.props.shippingSpace,this.state.searchKeyword)}</div>
       ...
       <Sider
        ...
        >
         ...
         {this.getNavMenuItems(this.props.shippingSpace,"inline","dark")}
