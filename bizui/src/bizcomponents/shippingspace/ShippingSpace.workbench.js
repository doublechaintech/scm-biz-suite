...
import styles from './ShippingSpace.workbench.less'
...
const internalSummaryOf = (cardsData,targetComponent) =>{
	...
	const shippingSpace = cardsData.cardsSource
	const {ShippingSpaceService} = GlobalComponents
	const userContext = null
	return (
	<div>
	<DescriptionList className={styles.headerList} size="small" col="4">
<Description term="ID" style={{wordBreak: 'break-all'}}>{shippingSpace.id}</Description> 
...
<Description term="更新于">{ moment(shippingSpace.lastUpdateTime).format('YYYY-MM-DD HH:mm')}</Description> 
...
class ShippingSpaceWorkbench extends Component {
 ...
 state = {
  ...
  render() {
    ...
    const cardsData = {cardsName:window.trans('shipping_space'),cardsFor: "shippingSpace",
    	cardsSource: this.props.shippingSpace,returnURL,displayName,
  		subItems: [
{name: 'goodsList', displayName: window.mtrans('goods','shipping_space.goods_list',false) ,viewGroup:'__no_group', type:'goods',count:goodsCount,addFunction: true, role: 'goods', metaInfo: goodsListMetaInfo, renderItem: GlobalComponents.GoodsBase.renderItemOfList},
...
}))(Form.create()(ShippingSpaceWorkbench))
