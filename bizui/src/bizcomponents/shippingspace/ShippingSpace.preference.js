...
const internalImageListOf = (shippingSpace) =>{
...
const internalSettingListOf = (shippingSpace) =>{
	const userContext = null
	const optionList = [ 
	]
...
const handleTransferSearch =(targetComponent,filterKey,newRequest)=>{
  ...
  const pageNo = 1;
  const candidateReferenceService = ShippingSpaceService[candidateServiceName] 
...
const executeTrans = (shippingSpace,targetComponent) =>{
...
const buildTransferModal = (shippingSpace,targetComponent) => {
...
<Modal title={transferModalTitle}
          ...
          onOk={()=>executeTrans(shippingSpace,targetComponent)}
...
const internalRenderExtraHeader = (shippingSpace) =>{
	return null
}
const internalRenderExtraFooter = (shippingSpace) =>{
	return null
}
...
<Description term="仓库">{shippingSpace.warehouse==null?appLocaleName(userContext,"NotAssigned"):shippingSpace.warehouse.displayName}
 <Icon type="swap" onClick={()=>
  showTransferModel(targetComponent,"仓库","warehouse","requestCandidateWarehouse",
	      "transferToAnotherWarehouse","anotherWarehouseId",shippingSpace.warehouse?shippingSpace.warehouse.id:"")} 
...
class ShippingSpacePreference extends Component {
