...
import styles from './CityPartner.workbench.less'
...
const internalSummaryOf = (cardsData,targetComponent) =>{
	...
	const cityPartner = cardsData.cardsSource
	const {CityPartnerService} = GlobalComponents
	const userContext = null
	return (
	<div>
	<DescriptionList className={styles.headerList} size="small" col="4">
<Description term="ID" style={{wordBreak: 'break-all'}}>{cityPartner.id}</Description> 
<Description term="名称" style={{wordBreak: 'break-all'}}>{cityPartner.name}</Description> 
<Description term="手机" style={{wordBreak: 'break-all'}}>{cityPartner.mobile}</Description> 
...
<Description term="描述" style={{wordBreak: 'break-all'}}>{cityPartner.description}</Description> 
<Description term="更新于">{ moment(cityPartner.lastUpdateTime).format('YYYY-MM-DD HH:mm')}</Description> 
...
class CityPartnerWorkbench extends Component {
...
}))(Form.create()(CityPartnerWorkbench))
