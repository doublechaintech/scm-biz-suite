 ...
 getNavMenuItems = (targetObject, style, customTheme) => {
    ...
    return (
	  <Menu
       ...
       <Menu.Item key="workbench">
        <Link to={`/sku/${this.props.sku.id}/workbench`}><Icon type="solution" style={{marginRight:"20px"}}/><span>工作台</span></Link>
      </Menu.Item>
        ...
        {filteredMenuItemsGroup(targetObject,this).map((groupedMenuItem,index)=>{
          return(
    <SubMenu id={`submenu-vg${index}`}  key={`vg${index}`} title={<span><Icon type={viewGroupIconNameOf('sku',`${groupedMenuItem.viewGroup}`)} style={{marginRight:"20px"}} /><span>{viewGroupDisplayNameOf('sku',`${groupedMenuItem.viewGroup}`)}</span></span>} >
  ...
  getGoodsSearch = () => {
    ...
    return connect(state => ({
      ...
      expandForm: state._sku.expandForm,
      loading: state._sku.loading,
      partialList: state._sku.partialList,
      owner: { type: '_sku', id: state._sku.id,
      referenceName: 'sku',
      listName: 'goodsList', ref:state._sku,
  ...
  getGoodsCreateForm = () => {
    ...
    return connect(state => ({
      ...
      metaInfo: state._sku.goodsListMetaInfo,
      count: state._sku.goodsCount,
      returnURL: `/sku/${state._sku.id}/list`,
      currentPage: state._sku.goodsCurrentPageNumber,
      searchFormParameters: state._sku.goodsSearchFormParameters,
      loading: state._sku.loading,
      owner: { type: '_sku', id: state._sku.id || this.getOwnerId(), referenceName: 'sku', listName: 'goodsList', ref:state._sku, listDisplayName: appLocaleName(userContext,"List")}, // this is for model namespace and
    }))(GoodsCreateForm)
  }
  ...
  getGoodsUpdateForm = () => {
    ...
    return connect(state => ({
      selectedRows: state._sku.selectedRows,
      role: "goods",
      currentUpdateIndex: state._sku.currentUpdateIndex || 0,
      owner: { type: '_sku', id: state._sku.id || this.getOwnerId(), listName: 'goodsList', ref:state._sku, listDisplayName: appLocaleName(userContext,"List") }, // this is for model namespace and
    }))(GoodsUpdateForm)
  }
  ...
  getRequestTypeStepForm = () => {
    ...
    return connect(state => ({
      selectedRows: state._sku.selectedRows,
      role: "cq",
      currentUpdateIndex: state._sku.currentUpdateIndex,
      owner: { type: '_sku', id: state._sku.id, listName: 'nolist', ref:state._sku, listDisplayName: appLocaleName(userContext,"List") }, // this is for model namespace and
  ...
  buildRouters = () =>{
    const {SkuWorkbench} = GlobalComponents

    const {SkuDashboard} = GlobalComponents
  	const {SkuPermission} = GlobalComponents
  	const {SkuProfile} = GlobalComponents


    const routers=[
    {path:"/sku/:id/workbench", component: SkuWorkbench},
    
  	{path:"/sku/:id/profile", component: SkuProfile},
  	{path:"/sku/:id/permission", component: SkuPermission},



  	{path:"/sku/:id/list/goodsList", component: this.getGoodsSearch()},
  	{path:"/sku/:id/list/goodsCreateForm", component: this.getGoodsCreateForm()},
  	{path:"/sku/:id/list/goodsUpdateForm", component: this.getGoodsUpdateForm()},
  ...
  handleOpenChange = (openKeys) => {
   ...
   render() {
    ...
    const {searchLocalData}=GlobalComponents.SkuBase
    ...
    const renderMenuSwitch=()=>{
     ...
     <Layout>
      ...
      {this.state.showSearch&&(
        ...
        <div style={{backgroundColor:'black'}}  onClick={()=>hideSearchResult()}  >{searchLocalData(this.props.sku,this.state.searchKeyword)}</div>
       ...
       <Sider
        ...
        >
         ...
         {this.getNavMenuItems(this.props.sku,"inline","dark")}
