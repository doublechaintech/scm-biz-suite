 ...
 getNavMenuItems = (targetObject, style, customTheme) => {
	  ...
	  <Menu
       ...
       >
             ...
             <Menu.Item key="dashboard">
               <Link to={`/sku/${this.props.sku.id}/dashboard`}><Icon type="dashboard" style={{marginRight:"20px"}}/><span>{appLocaleName(userContext,"Dashboard")}</span></Link>
             </Menu.Item>
  ...
  getGoodsCreateForm = () => {
    ...
    return connect(state => ({
      ...
      searchFormParameters: state._sku.goodsSearchFormParameters,
      loading: state._sku.loading,
      owner: { type: '_sku', id: state._sku.id, referenceName: 'sku', listName: 'goodsList', ref:state._sku, listDisplayName: appLocaleName(userContext,"List")}, // this is for model namespace and
    }))(GoodsCreateForm)
  }
  ...
  getGoodsUpdateForm = () => {
    ...
    return connect(state => ({
      ...
      owner: { type: '_sku', id: state._sku.id, listName: 'goodsList', ref:state._sku, listDisplayName: appLocaleName(userContext,"List") }, // this is for model namespace and
    }))(GoodsUpdateForm)
  }
