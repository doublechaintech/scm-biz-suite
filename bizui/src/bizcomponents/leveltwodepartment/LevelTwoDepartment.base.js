...
const menuData = {menuName: window.trans('level_two_department'), menuFor: "levelTwoDepartment",  internalName: "level_two_department",
  		subItems: [
...
const settingMenuData = {menuName: window.trans('level_two_department'), menuFor: "levelTwoDepartment",  internalName: "level_two_department",
  		subItems: [
...
const displayColumns = [
  { title: fieldLabels.id, debugtype: 'string', dataIndex: 'id', width: '6', render: (text, record)=>renderTextCell(text,record,'levelTwoDepartment') , sorter: true },
...
const renderActionList=(levelTwoDepartment, targetObject, columCount, listName)=>{

	if(!levelTwoDepartment){
		return null
	}
	if(!levelTwoDepartment.actionList){
		return null
	}
	if(levelTwoDepartment.actionList.length === 0){
		return null
	}
	return (
		<div className={styles.overlay}>
			...
			{levelTwoDepartment.actionList.map(action=>(<Link key={action.id} to={{pathname: action.actionPath.substring(1), state: {ownerId:targetObject.id,action,selectedRows:[levelTwoDepartment]}}} >
...
const renderItemOfList=(levelTwoDepartment, targetObject, columCount, listName)=>{
  ...
  const userContext = null
  return (
     <Row key={`${listName}-${levelTwoDepartment.id}`} className={styles.itemDesc}>
	...
	<Col span={20}>
      ...
      {renderActionList(levelTwoDepartment,targetObject)}
    </Row>
	)
...
const LevelTwoDepartmentBase={unpackObjectToFormValues, menuData,settingMenuData,displayColumns,fieldLabels,renderItemOfList, stepOf, searchLocalData}
