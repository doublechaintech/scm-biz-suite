...
import styles from './LevelTwoDepartment.updateform.less'
...
class LevelTwoDepartmentUpdateForm extends Component {
  state = {
...
>>>>>>> 502e8b8dfc403300a992b5083e79c722e85d1854
        ...
        const levelTwoDepartmentId = values.id
        const imagesValues = mapBackToImageValues(convertedImagesValues)
        const parameters = { ...values, levelTwoDepartmentId, ...imagesValues }
    ...
    const submitUpdateFormAndContinue = () => {
      validateFieldsAndScroll((error, values) => {
        ...
        dispatch({
          type: `${owner.type}/updateLevelTwoDepartment`,
          payload: {
            id: owner.id,
            type: 'levelTwoDepartment',
            parameters,
            selectedRows,
    ...
    const skipToNext = () => {
      ...
      const newIndex = currentUpdateIndex + 1
      dispatch({
        type: `${owner.type}/gotoNextLevelTwoDepartmentUpdateRow`,
        payload: {
          id: owner.id,
          type: 'levelTwoDepartment',
          selectedRows,
...
<<<<<<< HEAD
    ...
    return (
      <PageHeaderLayout
        title={"更新二级部门"+(currentUpdateIndex+1)+"/"+selectedRows.length}
        content="更新二级部门"
...
=======
	...
	const internalRenderTitle = () =>{
      ...
      return (<div>{linkComp}{appLocaleName(userContext,"Update")}二级部门: {(currentUpdateIndex+1)}/{selectedRows.length}</div>)
    }
...
}))(Form.create()(LevelTwoDepartmentUpdateForm))
