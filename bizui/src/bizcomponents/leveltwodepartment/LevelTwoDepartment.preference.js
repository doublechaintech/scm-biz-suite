...
const internalImageListOf = (levelTwoDepartment) =>{
...
const internalSettingListOf = (levelTwoDepartment) =>{
	const userContext = null
	const optionList = [ 
	]
...
const handleTransferSearch =(targetComponent,filterKey,newRequest)=>{
  ...
  const pageNo = 1;
  const candidateReferenceService = LevelTwoDepartmentService[candidateServiceName] 
...
const executeTrans = (levelTwoDepartment,targetComponent) =>{
...
const buildTransferModal = (levelTwoDepartment,targetComponent) => {
...
<Modal title={transferModalTitle}
          ...
          onOk={()=>executeTrans(levelTwoDepartment,targetComponent)}
...
const internalRenderExtraHeader = (levelTwoDepartment) =>{
	return null
}
const internalRenderExtraFooter = (levelTwoDepartment) =>{
	return null
}
...
<Description term="属于">{levelTwoDepartment.belongsTo==null?appLocaleName(userContext,"NotAssigned"):levelTwoDepartment.belongsTo.displayName}
 <Icon type="swap" onClick={()=>
  showTransferModel(targetComponent,"属于","levelOneDepartment","requestCandidateBelongsTo",
	      "transferToAnotherBelongsTo","anotherBelongsToId",levelTwoDepartment.belongsTo?levelTwoDepartment.belongsTo.id:"")} 
...
class LevelTwoDepartmentPreference extends Component {
  ...
  render() {
    ...
    const cardsData = {cardsName:"二级部门",cardsFor: "levelTwoDepartment",cardsSource: this.props.levelTwoDepartment,
  		subItems: [
...
}))(Form.create()(LevelTwoDepartmentPreference))
