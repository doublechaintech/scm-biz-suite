...
import styles from './SupplyOrderLineItem.workbench.less'
...
const internalSummaryOf = (cardsData,targetComponent) =>{
	...
	const supplyOrderLineItem = cardsData.cardsSource
	const {SupplyOrderLineItemService} = GlobalComponents
	const userContext = null
	return (
	<div>
	<DescriptionList className={styles.headerList} size="small" col="4">
<Description term="ID" style={{wordBreak: 'break-all'}}>{supplyOrderLineItem.id}</Description> 
...
class SupplyOrderLineItemWorkbench extends Component {
...
}))(Form.create()(SupplyOrderLineItemWorkbench))
