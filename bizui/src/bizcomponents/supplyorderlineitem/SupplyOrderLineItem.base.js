...
const renderItemOfList=(supplyOrderLineItem, targetComponent, columCount)=>{
  const displayColumnsCount = columCount || 4
  const userContext = null
  return (
    <Card key={supplyOrderLineItem.id} style={{marginTop:"10px"}}>
	...
	<Col span={20}>
      ...
      <DescriptionList  key={supplyOrderLineItem.id} size="small" col={displayColumnsCount} >
        ...
        <Description term={fieldLabels.bizOrder}><Tag color='blue' title={`${supplyOrderLineItem.bizOrder.id}-${supplyOrderLineItem.bizOrder.displayName}`}>{supplyOrderLineItem.bizOrder==null?appLocaleName(userContext,"NotAssigned"):`${leftChars(supplyOrderLineItem.bizOrder.displayName,15)}`}
        </Tag></Description>
