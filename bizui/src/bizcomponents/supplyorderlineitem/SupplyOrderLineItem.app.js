...
import styles from './SupplyOrderLineItem.app.less'
...
class SupplyOrderLineItemBizApp extends React.PureComponent {
constructor(props) {
    super(props)
     this.state = {
 ...
 getNavMenuItems = (targetObject, style, customTheme) => {
    ...
    return (
	  <Menu
       ...
       <Menu.Item key="workbench">
        <Link to={`/supplyOrderLineItem/${this.props.supplyOrderLineItem.id}/workbench`}><Icon type="solution" style={{marginRight:"20px"}}/><span>工作台</span></Link>
      </Menu.Item>
        ...
        {filteredMenuItemsGroup(targetObject,this).map((groupedMenuItem,index)=>{
          return(
    <SubMenu id={`submenu-vg${index}`}  key={`vg${index}`} title={<span><Icon type={viewGroupIconNameOf('supply_order_line_item',`${groupedMenuItem.viewGroup}`)} style={{marginRight:"20px"}} /><span>{viewGroupDisplayNameOf('supply_order_line_item',`${groupedMenuItem.viewGroup}`)}</span></span>} >
  ...
  getRequestTypeStepForm = () => {
    ...
    return connect(state => ({
      selectedRows: state._supplyOrderLineItem.selectedRows,
      role: "cq",
      currentUpdateIndex: state._supplyOrderLineItem.currentUpdateIndex,
      owner: { type: '_supplyOrderLineItem', id: state._supplyOrderLineItem.id, listName: 'nolist', ref:state._supplyOrderLineItem, listDisplayName: appLocaleName(userContext,"List") }, // this is for model namespace and
  ...
  buildRouters = () =>{
    const {SupplyOrderLineItemWorkbench} = GlobalComponents

    const {SupplyOrderLineItemDashboard} = GlobalComponents
  	const {SupplyOrderLineItemPermission} = GlobalComponents
  	const {SupplyOrderLineItemProfile} = GlobalComponents


    const routers=[
    {path:"/supplyOrderLineItem/:id/workbench", component: SupplyOrderLineItemWorkbench},
    
  	{path:"/supplyOrderLineItem/:id/profile", component: SupplyOrderLineItemProfile},
  	{path:"/supplyOrderLineItem/:id/permission", component: SupplyOrderLineItemPermission},
  ...
  handleOpenChange = (openKeys) => {
   ...
   render() {
    ...
    const {searchLocalData}=GlobalComponents.SupplyOrderLineItemBase
    ...
    const renderMenuSwitch=()=>{
     ...
     <Layout>
      ...
      {this.state.showSearch&&(
        ...
        <div style={{backgroundColor:'black'}}  onClick={()=>hideSearchResult()}  >{searchLocalData(this.props.supplyOrderLineItem,this.state.searchKeyword)}</div>
       ...
       <Sider
        ...
        >
         ...
         {this.getNavMenuItems(this.props.supplyOrderLineItem,"inline","dark")}
