...
import styles from './SupplyOrderPaymentGroup.app.less'
...
class SupplyOrderPaymentGroupBizApp extends React.PureComponent {
constructor(props) {
    super(props)
     this.state = {
 ...
 getNavMenuItems = (targetObject, style, customTheme) => {
    ...
    return (
	  <Menu
       ...
       <Menu.Item key="workbench">
        <Link to={`/supplyOrderPaymentGroup/${this.props.supplyOrderPaymentGroup.id}/workbench`}><Icon type="solution" style={{marginRight:"20px"}}/><span>工作台</span></Link>
      </Menu.Item>
        ...
        {filteredMenuItemsGroup(targetObject,this).map((groupedMenuItem,index)=>{
          return(
    <SubMenu id={`submenu-vg${index}`}  key={`vg${index}`} title={<span><Icon type={viewGroupIconNameOf('supply_order_payment_group',`${groupedMenuItem.viewGroup}`)} style={{marginRight:"20px"}} /><span>{viewGroupDisplayNameOf('supply_order_payment_group',`${groupedMenuItem.viewGroup}`)}</span></span>} >
  ...
  getRequestTypeStepForm = () => {
    ...
    return connect(state => ({
      selectedRows: state._supplyOrderPaymentGroup.selectedRows,
      role: "cq",
      currentUpdateIndex: state._supplyOrderPaymentGroup.currentUpdateIndex,
      owner: { type: '_supplyOrderPaymentGroup', id: state._supplyOrderPaymentGroup.id, listName: 'nolist', ref:state._supplyOrderPaymentGroup, listDisplayName: appLocaleName(userContext,"List") }, // this is for model namespace and
  ...
  buildRouters = () =>{
    const {SupplyOrderPaymentGroupWorkbench} = GlobalComponents

    const {SupplyOrderPaymentGroupDashboard} = GlobalComponents
  	const {SupplyOrderPaymentGroupPermission} = GlobalComponents
  	const {SupplyOrderPaymentGroupProfile} = GlobalComponents


    const routers=[
    {path:"/supplyOrderPaymentGroup/:id/workbench", component: SupplyOrderPaymentGroupWorkbench},
    
  	{path:"/supplyOrderPaymentGroup/:id/profile", component: SupplyOrderPaymentGroupProfile},
  	{path:"/supplyOrderPaymentGroup/:id/permission", component: SupplyOrderPaymentGroupPermission},
  ...
  handleOpenChange = (openKeys) => {
   ...
   render() {
    ...
    const {searchLocalData}=GlobalComponents.SupplyOrderPaymentGroupBase
    ...
    const renderMenuSwitch=()=>{
     ...
     <Layout>
      ...
      {this.state.showSearch&&(
        ...
        <div style={{backgroundColor:'black'}}  onClick={()=>hideSearchResult()}  >{searchLocalData(this.props.supplyOrderPaymentGroup,this.state.searchKeyword)}</div>
       ...
       <Sider
        ...
        >
         ...
         {this.getNavMenuItems(this.props.supplyOrderPaymentGroup,"inline","dark")}
