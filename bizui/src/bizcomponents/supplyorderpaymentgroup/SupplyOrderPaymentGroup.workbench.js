...
import styles from './SupplyOrderPaymentGroup.workbench.less'
...
const renderSettingMenuItem = (item,cardsData,targetComponent) =>{
  ...
  return (<Menu.Item key={item.name}>
      <Link to={`/supplyOrderPaymentGroup/${targetComponent.props.supplyOrderPaymentGroup.id}/list/${item.name}/${item.displayName}/`}>
...
const internalSummaryOf = (cardsData,targetComponent) =>{
	...
	const supplyOrderPaymentGroup = cardsData.cardsSource
	const {SupplyOrderPaymentGroupService} = GlobalComponents
	const userContext = null
	return (
	<div>
	<DescriptionList className={styles.headerList} size="small" col="4">
<Description term="ID" style={{wordBreak: 'break-all'}}>{supplyOrderPaymentGroup.id}</Description> 
...
class SupplyOrderPaymentGroupWorkbench extends Component {
 ...
 state = {
  ...
  render() {
    ...
    const cardsData = {cardsName:window.trans('supply_order_payment_group'),cardsFor: "supplyOrderPaymentGroup",
...
}))(Form.create()(SupplyOrderPaymentGroupWorkbench))
