...
import styles from './SupplyOrderPaymentGroup.preference.less'
...
const internalImageListOf = (supplyOrderPaymentGroup) =>{
...
const internalSettingListOf = (supplyOrderPaymentGroup) =>{
	const userContext = null
	const optionList = [ 
	]
...
const handleTransferSearch =(targetComponent,filterKey,newRequest)=>{
  ...
  const pageNo = 1;
  const candidateReferenceService = SupplyOrderPaymentGroupService[candidateServiceName] 
...
const executeTrans = (supplyOrderPaymentGroup,targetComponent) =>{
  ...
  validateFieldsAndScroll((error, values) => {
    ...
    const id=supplyOrderPaymentGroup.id;
    const serviceNameToCall = transferServiceName;

    const payload = {parameters,id,serviceNameToCall}
    
    //targetComponent.setState({transferModalVisiable:false})
    dispatch({type:"_supplyOrderPaymentGroup/doJob",payload: payload})
...
const buildTransferModal = (supplyOrderPaymentGroup,targetComponent) => {
...
<Modal title={transferModalTitle}
          ...
          onOk={()=>executeTrans(supplyOrderPaymentGroup,targetComponent)}
...
const internalRenderExtraHeader = (supplyOrderPaymentGroup) =>{
	return null
}
const internalRenderExtraFooter = (supplyOrderPaymentGroup) =>{
	return null
}
...
<Description term="订单">{supplyOrderPaymentGroup.bizOrder==null?appLocaleName(userContext,"NotAssigned"):supplyOrderPaymentGroup.bizOrder.displayName}
 <Icon type="swap" onClick={()=>
  showTransferModel(targetComponent,"订单","supplyOrder","requestCandidateBizOrder",
	      "transferToAnotherBizOrder","anotherBizOrderId",supplyOrderPaymentGroup.bizOrder?supplyOrderPaymentGroup.bizOrder.id:"")} 
...
class SupplyOrderPaymentGroupPreference extends Component {
  ...
  render() {
    ...
    const cardsData = {cardsName:"供应订单付款组",cardsFor: "supplyOrderPaymentGroup",cardsSource: supplyOrderPaymentGroup,
  		subItems: [
...
}))(Form.create()(SupplyOrderPaymentGroupPreference))
