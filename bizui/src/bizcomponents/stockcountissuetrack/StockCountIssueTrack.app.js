...
import styles from './StockCountIssueTrack.app.less'
...
class StockCountIssueTrackBizApp extends React.PureComponent {
constructor(props) {
    super(props)
     this.state = {
 ...
 getNavMenuItems = (targetObject, style, customTheme) => {
    ...
    return (
	  <Menu
       ...
       <Menu.Item key="workbench">
        <Link to={`/stockCountIssueTrack/${this.props.stockCountIssueTrack.id}/workbench`}><Icon type="solution" style={{marginRight:"20px"}}/><span>工作台</span></Link>
      </Menu.Item>
        ...
        {filteredMenuItemsGroup(targetObject,this).map((groupedMenuItem,index)=>{
          return(
    <SubMenu id={`submenu-vg${index}`}  key={`vg${index}`} title={<span><Icon type={viewGroupIconNameOf('stock_count_issue_track',`${groupedMenuItem.viewGroup}`)} style={{marginRight:"20px"}} /><span>{viewGroupDisplayNameOf('stock_count_issue_track',`${groupedMenuItem.viewGroup}`)}</span></span>} >
  ...
  getRequestTypeStepForm = () => {
    ...
    return connect(state => ({
      selectedRows: state._stockCountIssueTrack.selectedRows,
      role: "cq",
      currentUpdateIndex: state._stockCountIssueTrack.currentUpdateIndex,
      owner: { type: '_stockCountIssueTrack', id: state._stockCountIssueTrack.id, listName: 'nolist', ref:state._stockCountIssueTrack, listDisplayName: appLocaleName(userContext,"List") }, // this is for model namespace and
  ...
  buildRouters = () =>{
    const {StockCountIssueTrackWorkbench} = GlobalComponents

    const {StockCountIssueTrackDashboard} = GlobalComponents
  	const {StockCountIssueTrackPermission} = GlobalComponents
  	const {StockCountIssueTrackProfile} = GlobalComponents


    const routers=[
    {path:"/stockCountIssueTrack/:id/workbench", component: StockCountIssueTrackWorkbench},
    
  	{path:"/stockCountIssueTrack/:id/profile", component: StockCountIssueTrackProfile},
  	{path:"/stockCountIssueTrack/:id/permission", component: StockCountIssueTrackPermission},
  ...
  handleOpenChange = (openKeys) => {
   ...
   render() {
    ...
    const {searchLocalData}=GlobalComponents.StockCountIssueTrackBase
    ...
    const renderMenuSwitch=()=>{
     ...
     <Layout>
      ...
      {this.state.showSearch&&(
        ...
        <div style={{backgroundColor:'black'}}  onClick={()=>hideSearchResult()}  >{searchLocalData(this.props.stockCountIssueTrack,this.state.searchKeyword)}</div>
       ...
       <Sider
        ...
        >
         ...
         {this.getNavMenuItems(this.props.stockCountIssueTrack,"inline","dark")}
