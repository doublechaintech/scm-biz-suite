...
import styles from './StockCountIssueTrack.updateform.less'
...
class StockCountIssueTrackUpdateForm extends Component {
  state = {
  ...
  getSelectedRow() {
    ...
    const convertiedValues = selectedRows.map((item) => {
      return {
        ...item,
        countTime: moment(item.countTime),
...
>>>>>>> 502e8b8dfc403300a992b5083e79c722e85d1854
        ...
        const stockCountIssueTrackId = values.id
        const imagesValues = mapBackToImageValues(convertedImagesValues)
        const parameters = { ...values, stockCountIssueTrackId, ...imagesValues }
    ...
    const submitUpdateFormAndContinue = () => {
      validateFieldsAndScroll((error, values) => {
        ...
        dispatch({
          type: `${owner.type}/updateStockCountIssueTrack`,
          payload: {
            id: owner.id,
            type: 'stockCountIssueTrack',
            parameters,
            selectedRows,
    ...
    const skipToNext = () => {
      ...
      const newIndex = currentUpdateIndex + 1
      dispatch({
        type: `${owner.type}/gotoNextStockCountIssueTrackUpdateRow`,
        payload: {
          id: owner.id,
          type: 'stockCountIssueTrack',
          selectedRows,
...
<<<<<<< HEAD
    ...
    return (
      <PageHeaderLayout
        title={"更新库存计数问题跟踪"+(currentUpdateIndex+1)+"/"+selectedRows.length}
        content="更新库存计数问题跟踪"
...
=======
	...
	const internalRenderTitle = () =>{
      ...
      return (<div>{linkComp}{appLocaleName(userContext,"Update")}库存计数问题跟踪: {(currentUpdateIndex+1)}/{selectedRows.length}</div>)
    }
...
}))(Form.create()(StockCountIssueTrackUpdateForm))
