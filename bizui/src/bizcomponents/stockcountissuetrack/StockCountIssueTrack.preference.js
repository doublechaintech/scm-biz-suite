...
const internalImageListOf = (stockCountIssueTrack) =>{
...
const internalSettingListOf = (stockCountIssueTrack) =>{
	const userContext = null
	const optionList = [ 
	]
...
const handleTransferSearch =(targetComponent,filterKey,newRequest)=>{
  ...
  const pageNo = 1;
  const candidateReferenceService = StockCountIssueTrackService[candidateServiceName] 
...
const executeTrans = (stockCountIssueTrack,targetComponent) =>{
  ...
  validateFieldsAndScroll((error, values) => {
    ...
    const id=stockCountIssueTrack.id;
    const serviceNameToCall = transferServiceName;

    const payload = {parameters,id,serviceNameToCall}
    
    //targetComponent.setState({transferModalVisiable:false})
    dispatch({type:"_stockCountIssueTrack/doJob",payload: payload})
...
const buildTransferModal = (stockCountIssueTrack,targetComponent) => {
...
<Modal title={transferModalTitle}
          ...
          onOk={()=>executeTrans(stockCountIssueTrack,targetComponent)}
...
const internalRenderExtraHeader = (stockCountIssueTrack) =>{
	return null
}
const internalRenderExtraFooter = (stockCountIssueTrack) =>{
	return null
}
...
<Description term="盘点">{stockCountIssueTrack.stockCount==null?appLocaleName(userContext,"NotAssigned"):stockCountIssueTrack.stockCount.displayName}
 <Icon type="swap" onClick={()=>
  showTransferModel(targetComponent,"盘点","goodsShelfStockCount","requestCandidateStockCount",
	      "transferToAnotherStockCount","anotherStockCountId",stockCountIssueTrack.stockCount?stockCountIssueTrack.stockCount.id:"")} 
...
class StockCountIssueTrackPreference extends Component {
...
}))(Form.create()(StockCountIssueTrackPreference))
