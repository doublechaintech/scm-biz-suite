...
const internalImageListOf = (publicHoliday) =>{
...
const internalSettingListOf = (publicHoliday) =>{
	const userContext = null
	const optionList = [ 
	]
...
const handleTransferSearch =(targetComponent,filterKey,newRequest)=>{
  ...
  const pageNo = 1;
  const candidateReferenceService = PublicHolidayService[candidateServiceName] 
...
const executeTrans = (publicHoliday,targetComponent) =>{
...
const buildTransferModal = (publicHoliday,targetComponent) => {
...
<Modal title={transferModalTitle}
          ...
          onOk={()=>executeTrans(publicHoliday,targetComponent)}
...
const internalRenderExtraHeader = (publicHoliday) =>{
	return null
}
const internalRenderExtraFooter = (publicHoliday) =>{
	return null
}
...
class PublicHolidayPreference extends Component {
...
}))(Form.create()(PublicHolidayPreference))
