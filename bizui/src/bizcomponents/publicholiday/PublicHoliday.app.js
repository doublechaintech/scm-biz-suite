...
import styles from './PublicHoliday.app.less'
...
class PublicHolidayBizApp extends React.PureComponent {
constructor(props) {
    super(props)
     this.state = {
 ...
 getNavMenuItems = (targetObject, style, customTheme) => {
    ...
    return (
	  <Menu
       ...
       <Menu.Item key="workbench">
        <Link to={`/publicHoliday/${this.props.publicHoliday.id}/workbench`}><Icon type="solution" style={{marginRight:"20px"}}/><span>工作台</span></Link>
      </Menu.Item>
        ...
        {filteredMenuItemsGroup(targetObject,this).map((groupedMenuItem,index)=>{
          return(
    <SubMenu id={`submenu-vg${index}`}  key={`vg${index}`} title={<span><Icon type={viewGroupIconNameOf('public_holiday',`${groupedMenuItem.viewGroup}`)} style={{marginRight:"20px"}} /><span>{viewGroupDisplayNameOf('public_holiday',`${groupedMenuItem.viewGroup}`)}</span></span>} >
  ...
  getRequestTypeStepForm = () => {
    ...
    return connect(state => ({
      selectedRows: state._publicHoliday.selectedRows,
      role: "cq",
      currentUpdateIndex: state._publicHoliday.currentUpdateIndex,
      owner: { type: '_publicHoliday', id: state._publicHoliday.id, listName: 'nolist', ref:state._publicHoliday, listDisplayName: appLocaleName(userContext,"List") }, // this is for model namespace and
  ...
  buildRouters = () =>{
    const {PublicHolidayWorkbench} = GlobalComponents

    const {PublicHolidayDashboard} = GlobalComponents
  	const {PublicHolidayPermission} = GlobalComponents
  	const {PublicHolidayProfile} = GlobalComponents


    const routers=[
    {path:"/publicHoliday/:id/workbench", component: PublicHolidayWorkbench},
    
  	{path:"/publicHoliday/:id/profile", component: PublicHolidayProfile},
  	{path:"/publicHoliday/:id/permission", component: PublicHolidayPermission},
  ...
  handleOpenChange = (openKeys) => {
   ...
   render() {
    ...
    const {searchLocalData}=GlobalComponents.PublicHolidayBase
    ...
    const renderMenuSwitch=()=>{
     ...
     <Layout>
      ...
      {this.state.showSearch&&(
        ...
        <div style={{backgroundColor:'black'}}  onClick={()=>hideSearchResult()}  >{searchLocalData(this.props.publicHoliday,this.state.searchKeyword)}</div>
       ...
       <Sider
        ...
        >
         ...
         {this.getNavMenuItems(this.props.publicHoliday,"inline","dark")}
