...
const renderItemOfList=(retailStoreOrder, targetComponent, columCount)=>{
  ...
  return (
    <Card key={retailStoreOrder.id} style={{marginTop:"10px"}}>
	...
	<Col span={20}>
      ...
      <DescriptionList  key={retailStoreOrder.id} size="small" col={displayColumnsCount} >
        ...
        <Description term={fieldLabels.buyer}><Tag color='blue' title={`${retailStoreOrder.buyer.id}-${retailStoreOrder.buyer.displayName}`}>{retailStoreOrder.buyer==null?appLocaleName(userContext,"NotAssigned"):`${leftChars(retailStoreOrder.buyer.displayName,15)}`}
        </Tag></Description>
