...
const internalImageListOf = (retailStoreOrder) =>{
...
const internalSettingListOf = (retailStoreOrder) =>{
	const userContext = null
	const optionList = [ 
	]
...
const handleTransferSearch =(targetComponent,filterKey,newRequest)=>{
  ...
  const pageNo = 1;
  const candidateReferenceService = RetailStoreOrderService[candidateServiceName] 
...
const executeTrans = (retailStoreOrder,targetComponent) =>{
...
const buildTransferModal = (retailStoreOrder,targetComponent) => {
...
<Modal title={transferModalTitle}
          ...
          onOk={()=>executeTrans(retailStoreOrder,targetComponent)}
...
const internalRenderExtraHeader = (retailStoreOrder) =>{
	return null
}
const internalRenderExtraFooter = (retailStoreOrder) =>{
	return null
}
...
<Description term="买方">{retailStoreOrder.buyer==null?appLocaleName(userContext,"NotAssigned"):retailStoreOrder.buyer.displayName}
 <Icon type="swap" onClick={()=>
  showTransferModel(targetComponent,"买方","retailStore","requestCandidateBuyer",
	      "transferToAnotherBuyer","anotherBuyerId",retailStoreOrder.buyer?retailStoreOrder.buyer.id:"")} 
...
class RetailStoreOrderPreference extends Component {
...
}))(Form.create()(RetailStoreOrderPreference))
