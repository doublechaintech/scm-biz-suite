...
const internalImageListOf = (warehouse) =>{
<<<<<<< HEAD
...
const internalSettingListOf = (warehouse) =>{
<<<<<<< HEAD
...
const handleTransferSearch =(targetComponent,filterKey,newRequest)=>{
  ...
  const pageNo = 1;
  const candidateReferenceService = WarehouseService[candidateServiceName] 
...
const executeTrans = (warehouse,targetComponent) =>{
  ...
  validateFieldsAndScroll((error, values) => {
    ...
    //targetComponent.setState({transferModalVisiable:false})
    dispatch({type:"_warehouse/doJob",payload: payload})
...
const buildTransferModal = (warehouse,targetComponent) => {
...
<Modal title={transferModalTitle}
          ...
          onOk={()=>executeTrans(warehouse,targetComponent)}
...
const internalRenderExtraHeader = (warehouse) =>{
	return null
}
const internalRenderExtraFooter = (warehouse) =>{
	return null
}
...
class WarehousePreference extends Component {
...
>>>>>>> 502e8b8dfc403300a992b5083e79c722e85d1854
  ...
  render() {
    ...
    const { id,displayName, storageSpaceCount, smartPalletCount, supplierSpaceCount, receivingSpaceCount, shippingSpaceCount, damageSpaceCount, warehouseAssetCount } = this.props.warehouse
    const cardsData = {cardsName:"仓库",cardsFor: "warehouse",cardsSource: this.props.warehouse,
  		subItems: [
...
}))(Form.create()(WarehousePreference))
