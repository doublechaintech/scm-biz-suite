...
import styles from './WechatWorkappIdentity.app.less'
...
class WechatWorkappIdentityBizApp extends React.PureComponent {
constructor(props) {
    super(props)
     this.state = {
 ...
 getNavMenuItems = (targetObject, style, customTheme) => {
    ...
    return (
	  <Menu
       ...
       <Menu.Item key="workbench">
        <Link to={`/wechatWorkappIdentity/${this.props.wechatWorkappIdentity.id}/workbench`}><Icon type="solution" style={{marginRight:"20px"}}/><span>工作台</span></Link>
      </Menu.Item>
        ...
        {filteredMenuItemsGroup(targetObject,this).map((groupedMenuItem,index)=>{
          return(
    <SubMenu id={`submenu-vg${index}`}  key={`vg${index}`} title={<span><Icon type={viewGroupIconNameOf('wechat_workapp_identity',`${groupedMenuItem.viewGroup}`)} style={{marginRight:"20px"}} /><span>{viewGroupDisplayNameOf('wechat_workapp_identity',`${groupedMenuItem.viewGroup}`)}</span></span>} >
  ...
  getRequestTypeStepForm = () => {
    ...
    return connect(state => ({
      selectedRows: state._wechatWorkappIdentity.selectedRows,
      role: "cq",
      currentUpdateIndex: state._wechatWorkappIdentity.currentUpdateIndex,
      owner: { type: '_wechatWorkappIdentity', id: state._wechatWorkappIdentity.id, listName: 'nolist', ref:state._wechatWorkappIdentity, listDisplayName: appLocaleName(userContext,"List") }, // this is for model namespace and
  ...
  buildRouters = () =>{
    const {WechatWorkappIdentityWorkbench} = GlobalComponents

    const {WechatWorkappIdentityDashboard} = GlobalComponents
  	const {WechatWorkappIdentityPermission} = GlobalComponents
  	const {WechatWorkappIdentityProfile} = GlobalComponents


    const routers=[
    {path:"/wechatWorkappIdentity/:id/workbench", component: WechatWorkappIdentityWorkbench},
    
  	{path:"/wechatWorkappIdentity/:id/profile", component: WechatWorkappIdentityProfile},
  	{path:"/wechatWorkappIdentity/:id/permission", component: WechatWorkappIdentityPermission},
  ...
  handleOpenChange = (openKeys) => {
   ...
   render() {
    ...
    const {searchLocalData}=GlobalComponents.WechatWorkappIdentityBase
    ...
    const renderMenuSwitch=()=>{
     ...
     <Layout>
      ...
      {this.state.showSearch&&(
        ...
        <div style={{backgroundColor:'black'}}  onClick={()=>hideSearchResult()}  >{searchLocalData(this.props.wechatWorkappIdentity,this.state.searchKeyword)}</div>
       ...
       <Sider
        ...
        >
         ...
         {this.getNavMenuItems(this.props.wechatWorkappIdentity,"inline","dark")}
