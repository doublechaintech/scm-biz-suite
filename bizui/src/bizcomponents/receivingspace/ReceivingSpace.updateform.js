...
import styles from './ReceivingSpace.updateform.less'
...
class ReceivingSpaceUpdateForm extends Component {
  state = {
  ...
  render() {
    ...
    const submitUpdateForm = () => {
      validateFieldsAndScroll((error, values) => {
        ...
        const receivingSpaceId = values.id

        const parameters = { ...values, receivingSpaceId, }
    ...
    const submitUpdateFormAndContinue = () => {
      validateFieldsAndScroll((error, values) => {
        ...
        dispatch({
          type: `${owner.type}/updateReceivingSpace`,
          payload: {
            id: owner.id,
            type: 'receivingSpace',
            parameters,
    ...
    const skipToNext = () => {
      ...
      const newIndex = currentUpdateIndex + 1
      dispatch({
        type: `${owner.type}/gotoNextReceivingSpaceUpdateRow`,
        payload: {
          id: owner.id,
          type: 'receivingSpace',
          selectedRows,
	...
	const internalRenderTitle = () =>{
      ...
      return (<div>{linkComp}{appLocaleName(userContext,"Update")}收货区: {(currentUpdateIndex+1)}/{selectedRows.length}</div>)
    }
              ...
              <Col lg={24} md={24} sm={24}>
                <Form.Item label={fieldLabels.totalArea} {...formItemLayout}>
                  {getFieldDecorator('totalArea', {
                    ...
                    <SmallTextInput size="large"  placeholder={fieldLabels.totalArea} />
...
}))(Form.create()(ReceivingSpaceUpdateForm))
