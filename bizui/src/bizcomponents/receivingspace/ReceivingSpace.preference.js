...
const internalImageListOf = (receivingSpace) =>{
...
const internalSettingListOf = (receivingSpace) =>{
	const userContext = null
	const optionList = [ 
	]
...
const handleTransferSearch =(targetComponent,filterKey,newRequest)=>{
  ...
  const pageNo = 1;
  const candidateReferenceService = ReceivingSpaceService[candidateServiceName] 
...
const executeTrans = (receivingSpace,targetComponent) =>{
...
const buildTransferModal = (receivingSpace,targetComponent) => {
...
<Modal title={transferModalTitle}
          ...
          onOk={()=>executeTrans(receivingSpace,targetComponent)}
...
const internalRenderExtraHeader = (receivingSpace) =>{
	return null
}
const internalRenderExtraFooter = (receivingSpace) =>{
	return null
}
...
<Description term="仓库">{receivingSpace.warehouse==null?appLocaleName(userContext,"NotAssigned"):receivingSpace.warehouse.displayName}
 <Icon type="swap" onClick={()=>
  showTransferModel(targetComponent,"仓库","warehouse","requestCandidateWarehouse",
	      "transferToAnotherWarehouse","anotherWarehouseId",receivingSpace.warehouse?receivingSpace.warehouse.id:"")} 
...
class ReceivingSpacePreference extends Component {
