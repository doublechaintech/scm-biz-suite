 ...
 getNavMenuItems = (targetObject, style, customTheme) => {
    ...
    return (
	  <Menu
       ...
       <Menu.Item key="workbench">
        <Link to={`/receivingSpace/${this.props.receivingSpace.id}/workbench`}><Icon type="solution" style={{marginRight:"20px"}}/><span>工作台</span></Link>
      </Menu.Item>
        ...
        {filteredMenuItemsGroup(targetObject,this).map((groupedMenuItem,index)=>{
          return(
    <SubMenu id={`submenu-vg${index}`}  key={`vg${index}`} title={<span><Icon type={viewGroupIconNameOf('receiving_space',`${groupedMenuItem.viewGroup}`)} style={{marginRight:"20px"}} /><span>{viewGroupDisplayNameOf('receiving_space',`${groupedMenuItem.viewGroup}`)}</span></span>} >
  ...
  getGoodsSearch = () => {
    ...
    return connect(state => ({
      rule: state.rule,
      name: window.mtrans('goods','receiving_space.goods_list',false),
      role: "goods",
      ...
      returnURL: `/receivingSpace/${state._receivingSpace.id}/workbench`,
  ...
  getGoodsCreateForm = () => {
    ...
    return connect(state => ({
      ...
      metaInfo: state._receivingSpace.goodsListMetaInfo,
      count: state._receivingSpace.goodsCount,
      returnURL: `/receivingSpace/${state._receivingSpace.id}/list`,
      currentPage: state._receivingSpace.goodsCurrentPageNumber,
      searchFormParameters: state._receivingSpace.goodsSearchFormParameters,
      loading: state._receivingSpace.loading,
      owner: { type: '_receivingSpace', id: state._receivingSpace.id || this.getOwnerId(), referenceName: 'receivingSpace', listName: 'goodsList', ref:state._receivingSpace, listDisplayName: appLocaleName(userContext,"List")}, // this is for model namespace and
    }))(GoodsCreateForm)
  }
  ...
  getGoodsUpdateForm = () => {
    ...
    return connect(state => ({
      ...
      role: "goods",
      currentUpdateIndex: state._receivingSpace.currentUpdateIndex || 0,
      owner: { type: '_receivingSpace', id: state._receivingSpace.id || this.getOwnerId(), listName: 'goodsList', ref:state._receivingSpace, listDisplayName: appLocaleName(userContext,"List") }, // this is for model namespace and
    }))(GoodsUpdateForm)
  }
  ...
  getRequestTypeStepForm = () => {
    ...
    return connect(state => ({
      ...
      owner: { type: '_receivingSpace', id: state._receivingSpace.id, listName: 'nolist', ref:state._receivingSpace, listDisplayName: appLocaleName(userContext,"List") }, // this is for model namespace and
  ...
  buildRouters = () =>{
    const {ReceivingSpaceWorkbench} = GlobalComponents
    ...
    const routers=[
    {path:"/receivingSpace/:id/workbench", component: ReceivingSpaceWorkbench},
  ...
  handleOpenChange = (openKeys) => {
   ...
   render() {
    ...
    const {searchLocalData}=GlobalComponents.ReceivingSpaceBase
    ...
    const renderMenuSwitch=()=>{
     ...
     <Layout>
      ...
      {this.state.showSearch&&(
        ...
        <div style={{backgroundColor:'black'}}  onClick={()=>hideSearchResult()}  >{searchLocalData(this.props.receivingSpace,this.state.searchKeyword)}</div>
       ...
       <Sider
        ...
        >
         ...
         {this.getNavMenuItems(this.props.receivingSpace,"inline","dark")}
