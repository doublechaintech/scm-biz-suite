...
import styles from './ReceivingSpace.workbench.less'
...
const renderSettingMenuItem = (item,cardsData,targetComponent) =>{
  ...
  return (<Menu.Item key={item.name}>
      <Link to={`/receivingSpace/${targetComponent.props.receivingSpace.id}/list/${item.name}/${item.displayName}/`}>
...
const internalSummaryOf = (cardsData,targetComponent) =>{
	...
	const receivingSpace = cardsData.cardsSource
	const {ReceivingSpaceService} = GlobalComponents
	const userContext = null
	return (
	<div>
	<DescriptionList className={styles.headerList} size="small" col="4">
<Description term="ID" style={{wordBreak: 'break-all'}}>{receivingSpace.id}</Description> 
...
<Description term="更新于">{ moment(receivingSpace.lastUpdateTime).format('YYYY-MM-DD HH:mm')}</Description> 
...
class ReceivingSpaceWorkbench extends Component {
...
}))(Form.create()(ReceivingSpaceWorkbench))
