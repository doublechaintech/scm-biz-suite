...
import styles from './ConsumerOrderLineItem.workbench.less'
...
const internalSummaryOf = (cardsData,targetComponent) =>{
	...
	const consumerOrderLineItem = cardsData.cardsSource
	const {ConsumerOrderLineItemService} = GlobalComponents
	const userContext = null
	return (
	<div>
	<DescriptionList className={styles.headerList} size="small" col="4">
<Description term="ID" style={{wordBreak: 'break-all'}}>{consumerOrderLineItem.id}</Description> 
...
<Description term="更新于">{ moment(consumerOrderLineItem.lastUpdateTime).format('YYYY-MM-DD HH:mm')}</Description> 
...
class ConsumerOrderLineItemWorkbench extends Component {
...
}))(Form.create()(ConsumerOrderLineItemWorkbench))
