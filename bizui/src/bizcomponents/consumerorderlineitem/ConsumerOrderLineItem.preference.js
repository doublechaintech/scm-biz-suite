...
const internalImageListOf = (consumerOrderLineItem) =>{
<<<<<<< HEAD
...
const internalSettingListOf = (consumerOrderLineItem) =>{
<<<<<<< HEAD
...
const handleTransferSearch =(targetComponent,filterKey,newRequest)=>{
  ...
  const pageNo = 1;
  const candidateReferenceService = ConsumerOrderLineItemService[candidateServiceName] 
...
const executeTrans = (consumerOrderLineItem,targetComponent) =>{
  ...
  validateFieldsAndScroll((error, values) => {
    ...
    const id=consumerOrderLineItem.id;
    const serviceNameToCall = transferServiceName;

    const payload = {parameters,id,serviceNameToCall}
    
    //targetComponent.setState({transferModalVisiable:false})
    dispatch({type:"_consumerOrderLineItem/doJob",payload: payload})
...
const buildTransferModal = (consumerOrderLineItem,targetComponent) => {
...
<Modal title={transferModalTitle}
          ...
          onOk={()=>executeTrans(consumerOrderLineItem,targetComponent)}
...
const internalRenderExtraHeader = (consumerOrderLineItem) =>{
	return null
}
const internalRenderExtraFooter = (consumerOrderLineItem) =>{
	return null
}
...
=======
	...
	<DescriptionList className={styles.headerList} size="small" col="4">
<Description term="序号">{consumerOrderLineItem.id}</Description> 
<Description term="订单">{consumerOrderLineItem.bizOrder==null?appLocaleName(userContext,"NotAssigned"):consumerOrderLineItem.bizOrder.displayName}
>>>>>>> 502e8b8dfc403300a992b5083e79c722e85d1854
 <Icon type="swap" onClick={()=>
  showTransferModel(targetComponent,"订单","consumerOrder","requestCandidateBizOrder",
	      "transferToAnotherBizOrder","anotherBizOrderId",consumerOrderLineItem.bizOrder?consumerOrderLineItem.bizOrder.id:"")} 
...
class ConsumerOrderLineItemPreference extends Component {
...
}))(Form.create()(ConsumerOrderLineItemPreference))
