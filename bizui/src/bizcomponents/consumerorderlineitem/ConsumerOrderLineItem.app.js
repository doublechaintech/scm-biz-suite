...
import styles from './ConsumerOrderLineItem.app.less'
...
class ConsumerOrderLineItemBizApp extends React.PureComponent {
constructor(props) {
    super(props)
     this.state = {
 ...
 getNavMenuItems = (targetObject, style, customTheme) => {
    ...
    return (
	  <Menu
       ...
       <Menu.Item key="workbench">
        <Link to={`/consumerOrderLineItem/${this.props.consumerOrderLineItem.id}/workbench`}><Icon type="solution" style={{marginRight:"20px"}}/><span>工作台</span></Link>
      </Menu.Item>
        ...
        {filteredMenuItemsGroup(targetObject,this).map((groupedMenuItem,index)=>{
          return(
    <SubMenu id={`submenu-vg${index}`}  key={`vg${index}`} title={<span><Icon type={viewGroupIconNameOf('consumer_order_line_item',`${groupedMenuItem.viewGroup}`)} style={{marginRight:"20px"}} /><span>{viewGroupDisplayNameOf('consumer_order_line_item',`${groupedMenuItem.viewGroup}`)}</span></span>} >
  ...
  getRequestTypeStepForm = () => {
    ...
    return connect(state => ({
      selectedRows: state._consumerOrderLineItem.selectedRows,
      role: "cq",
      currentUpdateIndex: state._consumerOrderLineItem.currentUpdateIndex,
      owner: { type: '_consumerOrderLineItem', id: state._consumerOrderLineItem.id, listName: 'nolist', ref:state._consumerOrderLineItem, listDisplayName: appLocaleName(userContext,"List") }, // this is for model namespace and
  ...
  buildRouters = () =>{
    const {ConsumerOrderLineItemWorkbench} = GlobalComponents

    const {ConsumerOrderLineItemDashboard} = GlobalComponents
  	const {ConsumerOrderLineItemPermission} = GlobalComponents
  	const {ConsumerOrderLineItemProfile} = GlobalComponents


    const routers=[
    {path:"/consumerOrderLineItem/:id/workbench", component: ConsumerOrderLineItemWorkbench},
    
  	{path:"/consumerOrderLineItem/:id/profile", component: ConsumerOrderLineItemProfile},
  	{path:"/consumerOrderLineItem/:id/permission", component: ConsumerOrderLineItemPermission},
  ...
  handleOpenChange = (openKeys) => {
   ...
   render() {
    ...
    const {searchLocalData}=GlobalComponents.ConsumerOrderLineItemBase
    ...
    const renderMenuSwitch=()=>{
     ...
     <Layout>
      ...
      {this.state.showSearch&&(
        ...
        <div style={{backgroundColor:'black'}}  onClick={()=>hideSearchResult()}  >{searchLocalData(this.props.consumerOrderLineItem,this.state.searchKeyword)}</div>
       ...
       <Sider
        ...
        >
         ...
         {this.getNavMenuItems(this.props.consumerOrderLineItem,"inline","dark")}
