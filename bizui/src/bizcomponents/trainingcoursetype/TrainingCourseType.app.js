 ...
 getNavMenuItems = (targetObject, style, customTheme) => {
    ...
    return (
	  <Menu
       ...
       <Menu.Item key="workbench">
        <Link to={`/trainingCourseType/${this.props.trainingCourseType.id}/workbench`}><Icon type="solution" style={{marginRight:"20px"}}/><span>工作台</span></Link>
      </Menu.Item>
        ...
        {filteredMenuItemsGroup(targetObject,this).map((groupedMenuItem,index)=>{
          return(
    <SubMenu id={`submenu-vg${index}`}  key={`vg${index}`} title={<span><Icon type={viewGroupIconNameOf('training_course_type',`${groupedMenuItem.viewGroup}`)} style={{marginRight:"20px"}} /><span>{viewGroupDisplayNameOf('training_course_type',`${groupedMenuItem.viewGroup}`)}</span></span>} >
  ...
  getCompanyTrainingSearch = () => {
    ...
    return connect(state => ({
      ...
      returnURL: `/trainingCourseType/${state._trainingCourseType.id}/workbench`,
  ...
  getCompanyTrainingCreateForm = () => {
    ...
    return connect(state => ({
      ...
      searchFormParameters: state._trainingCourseType.companyTrainingSearchFormParameters,
      loading: state._trainingCourseType.loading,
      owner: { type: '_trainingCourseType', id: state._trainingCourseType.id || this.getOwnerId(), referenceName: 'trainingCourseType', listName: 'companyTrainingList', ref:state._trainingCourseType, listDisplayName: appLocaleName(userContext,"List")}, // this is for model namespace and
  ...
  getCompanyTrainingUpdateForm = () => {
    ...
    return connect(state => ({
      ...
      role: "companyTraining",
      currentUpdateIndex: state._trainingCourseType.currentUpdateIndex || 0,
      owner: { type: '_trainingCourseType', id: state._trainingCourseType.id || this.getOwnerId(), listName: 'companyTrainingList', ref:state._trainingCourseType, listDisplayName: appLocaleName(userContext,"List") }, // this is for model namespace and
  ...
  getRequestTypeStepForm = () => {
    ...
    return connect(state => ({
      ...
      owner: { type: '_trainingCourseType', id: state._trainingCourseType.id, listName: 'nolist', ref:state._trainingCourseType, listDisplayName: appLocaleName(userContext,"List") }, // this is for model namespace and
  ...
  buildRouters = () =>{
    const {TrainingCourseTypeWorkbench} = GlobalComponents
    ...
    const routers=[
    {path:"/trainingCourseType/:id/workbench", component: TrainingCourseTypeWorkbench},
