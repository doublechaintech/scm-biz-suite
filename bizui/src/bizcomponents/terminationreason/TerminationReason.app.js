 ...
 getNavMenuItems = (targetObject, style, customTheme) => {
    ...
    return (
	  <Menu
       ...
       <Menu.Item key="workbench">
        <Link to={`/terminationReason/${this.props.terminationReason.id}/workbench`}><Icon type="solution" style={{marginRight:"20px"}}/><span>工作台</span></Link>
      </Menu.Item>
        ...
        {filteredMenuItemsGroup(targetObject,this).map((groupedMenuItem,index)=>{
          return(
    <SubMenu id={`submenu-vg${index}`}  key={`vg${index}`} title={<span><Icon type={viewGroupIconNameOf('termination_reason',`${groupedMenuItem.viewGroup}`)} style={{marginRight:"20px"}} /><span>{viewGroupDisplayNameOf('termination_reason',`${groupedMenuItem.viewGroup}`)}</span></span>} >
  ...
  getTerminationSearch = () => {
    ...
    return connect(state => ({
      rule: state.rule,
      name: window.mtrans('termination','termination_reason.termination_list',false),
      role: "termination",
      ...
      returnURL: `/terminationReason/${state._terminationReason.id}/workbench`,
  ...
  getTerminationCreateForm = () => {
    ...
    return connect(state => ({
      ...
      metaInfo: state._terminationReason.terminationListMetaInfo,
      count: state._terminationReason.terminationCount,
      returnURL: `/terminationReason/${state._terminationReason.id}/list`,
      currentPage: state._terminationReason.terminationCurrentPageNumber,
      searchFormParameters: state._terminationReason.terminationSearchFormParameters,
      loading: state._terminationReason.loading,
      owner: { type: '_terminationReason', id: state._terminationReason.id || this.getOwnerId(), referenceName: 'reason', listName: 'terminationList', ref:state._terminationReason, listDisplayName: appLocaleName(userContext,"List")}, // this is for model namespace and
  ...
  getTerminationUpdateForm = () => {
    ...
    return connect(state => ({
      ...
      role: "termination",
      currentUpdateIndex: state._terminationReason.currentUpdateIndex || 0,
      owner: { type: '_terminationReason', id: state._terminationReason.id || this.getOwnerId(), listName: 'terminationList', ref:state._terminationReason, listDisplayName: appLocaleName(userContext,"List") }, // this is for model namespace and
  ...
  getRequestTypeStepForm = () => {
    ...
    return connect(state => ({
      ...
      owner: { type: '_terminationReason', id: state._terminationReason.id, listName: 'nolist', ref:state._terminationReason, listDisplayName: appLocaleName(userContext,"List") }, // this is for model namespace and
  ...
  buildRouters = () =>{
    const {TerminationReasonWorkbench} = GlobalComponents

    const {TerminationReasonDashboard} = GlobalComponents
  	const {TerminationReasonPermission} = GlobalComponents
  	const {TerminationReasonProfile} = GlobalComponents


    const routers=[
    {path:"/terminationReason/:id/workbench", component: TerminationReasonWorkbench},
    
  	{path:"/terminationReason/:id/profile", component: TerminationReasonProfile},
  	{path:"/terminationReason/:id/permission", component: TerminationReasonPermission},
  ...
  handleOpenChange = (openKeys) => {
   ...
   render() {
    ...
    const {searchLocalData}=GlobalComponents.TerminationReasonBase
    ...
    const renderMenuSwitch=()=>{
     ...
     <Layout>
      ...
      {this.state.showSearch&&(
        ...
        <div style={{backgroundColor:'black'}}  onClick={()=>hideSearchResult()}  >{searchLocalData(this.props.terminationReason,this.state.searchKeyword)}</div>
       ...
       <Sider
        ...
        >
         ...
         {this.getNavMenuItems(this.props.terminationReason,"inline","dark")}
