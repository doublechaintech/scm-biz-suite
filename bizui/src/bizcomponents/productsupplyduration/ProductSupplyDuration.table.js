...
import styles from './ProductSupplyDuration.table.less'
...
class ProductSupplyDurationTable extends PureComponent {
  state = {
    selectedRowKeys: [],
  }
...
<<<<<<< HEAD
 calcDisplayColumns=()=>{
    ...
    const {displayColumns} = ProductSupplyDurationBase
    if(!referenceName){
    ...
    const operationColumn={
      ...
      render: (text, record) => (
        <span>
         ...
         { hasReadPermission(metaInfo)&&<Link to={`/productSupplyDuration/${record.id}/dashboard`}>{'查看'}</Link>}
    ...
    const operationColumn={
      ...
      render: (text, record) => (
        <span>
          ...
          { hasReadPermission(metaInfo)&&<Link to={`/productSupplyDuration/${record.id}/dashboard`}>{appLocaleName(userContext,"View")}</Link>}
...
export default ProductSupplyDurationTable
