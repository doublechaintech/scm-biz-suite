...
import styles from './ProductSupplyDuration.app.less'
...
class ProductSupplyDurationBizApp extends React.PureComponent {
constructor(props) {
    super(props)
     this.state = {
 ...
 getNavMenuItems = (targetObject, style, customTheme) => {
    ...
    return (
	  <Menu
       ...
       <Menu.Item key="workbench">
        <Link to={`/productSupplyDuration/${this.props.productSupplyDuration.id}/workbench`}><Icon type="solution" style={{marginRight:"20px"}}/><span>工作台</span></Link>
      </Menu.Item>
        ...
        {filteredMenuItemsGroup(targetObject,this).map((groupedMenuItem,index)=>{
          return(
    <SubMenu id={`submenu-vg${index}`}  key={`vg${index}`} title={<span><Icon type={viewGroupIconNameOf('product_supply_duration',`${groupedMenuItem.viewGroup}`)} style={{marginRight:"20px"}} /><span>{viewGroupDisplayNameOf('product_supply_duration',`${groupedMenuItem.viewGroup}`)}</span></span>} >
  ...
  getRequestTypeStepForm = () => {
    ...
    return connect(state => ({
      selectedRows: state._productSupplyDuration.selectedRows,
      role: "cq",
      currentUpdateIndex: state._productSupplyDuration.currentUpdateIndex,
      owner: { type: '_productSupplyDuration', id: state._productSupplyDuration.id, listName: 'nolist', ref:state._productSupplyDuration, listDisplayName: appLocaleName(userContext,"List") }, // this is for model namespace and
  ...
  buildRouters = () =>{
    const {ProductSupplyDurationWorkbench} = GlobalComponents

    const {ProductSupplyDurationDashboard} = GlobalComponents
  	const {ProductSupplyDurationPermission} = GlobalComponents
  	const {ProductSupplyDurationProfile} = GlobalComponents


    const routers=[
    {path:"/productSupplyDuration/:id/workbench", component: ProductSupplyDurationWorkbench},
    
  	{path:"/productSupplyDuration/:id/profile", component: ProductSupplyDurationProfile},
  	{path:"/productSupplyDuration/:id/permission", component: ProductSupplyDurationPermission},
  ...
  handleOpenChange = (openKeys) => {
   ...
   render() {
    ...
    const {searchLocalData}=GlobalComponents.ProductSupplyDurationBase
    ...
    const renderMenuSwitch=()=>{
     ...
     <Layout>
      ...
      {this.state.showSearch&&(
        ...
        <div style={{backgroundColor:'black'}}  onClick={()=>hideSearchResult()}  >{searchLocalData(this.props.productSupplyDuration,this.state.searchKeyword)}</div>
       ...
       <Sider
        ...
        >
         ...
         {this.getNavMenuItems(this.props.productSupplyDuration,"inline","dark")}
