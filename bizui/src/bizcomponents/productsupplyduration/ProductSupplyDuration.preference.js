...
const internalImageListOf = (productSupplyDuration) =>{
...
const internalSettingListOf = (productSupplyDuration) =>{
	const userContext = null
	const optionList = [ 
	]
...
const handleTransferSearch =(targetComponent,filterKey,newRequest)=>{
  ...
  const pageNo = 1;
  const candidateReferenceService = ProductSupplyDurationService[candidateServiceName] 
...
const executeTrans = (productSupplyDuration,targetComponent) =>{
  ...
  validateFieldsAndScroll((error, values) => {
    ...
    const id=productSupplyDuration.id;
    const serviceNameToCall = transferServiceName;

    const payload = {parameters,id,serviceNameToCall}
    
    //targetComponent.setState({transferModalVisiable:false})
    dispatch({type:"_productSupplyDuration/doJob",payload: payload})
...
const buildTransferModal = (productSupplyDuration,targetComponent) => {
...
<Modal title={transferModalTitle}
          ...
          onOk={()=>executeTrans(productSupplyDuration,targetComponent)}
...
const internalRenderExtraHeader = (productSupplyDuration) =>{
	return null
}
const internalRenderExtraFooter = (productSupplyDuration) =>{
	return null
}
...
<Description term="产品">{productSupplyDuration.product==null?appLocaleName(userContext,"NotAssigned"):productSupplyDuration.product.displayName}
 <Icon type="swap" onClick={()=>
  showTransferModel(targetComponent,"产品","supplierProduct","requestCandidateProduct",
	      "transferToAnotherProduct","anotherProductId",productSupplyDuration.product?productSupplyDuration.product.id:"")} 
...
class ProductSupplyDurationPreference extends Component {
...
}))(Form.create()(ProductSupplyDurationPreference))
