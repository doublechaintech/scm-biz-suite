...
import styles from './ProductSupplyDuration.updateform.less'
...
class ProductSupplyDurationUpdateForm extends Component {
  state = {
  ...
  render() {
    ...
    const submitUpdateForm = () => {
      validateFieldsAndScroll((error, values) => {
        ...
        const productSupplyDurationId = values.id

        const parameters = { ...values, productSupplyDurationId, }
    ...
    const submitUpdateFormAndContinue = () => {
      validateFieldsAndScroll((error, values) => {
        ...
        dispatch({
          type: `${owner.type}/updateProductSupplyDuration`,
          payload: {
            id: owner.id,
            type: 'productSupplyDuration',
            parameters,
            selectedRows,
    ...
    const skipToNext = () => {
      ...
      const newIndex = currentUpdateIndex + 1
      dispatch({
        type: `${owner.type}/gotoNextProductSupplyDurationUpdateRow`,
        payload: {
          id: owner.id,
          type: 'productSupplyDuration',
          selectedRows,
	...
	const internalRenderTitle = () =>{
      ...
      return (<div>{linkComp}{appLocaleName(userContext,"Update")}产品供应时间: {(currentUpdateIndex+1)}/{selectedRows.length}</div>)
    }
              ...
              <Col lg={24} md={24} sm={24}>
                <Form.Item label={fieldLabels.duration} {...formItemLayout}>
                  {getFieldDecorator('duration', {
                    ...
                    <SmallTextInput size="large"  placeholder={fieldLabels.duration} />
...
}))(Form.create()(ProductSupplyDurationUpdateForm))
