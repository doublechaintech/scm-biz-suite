...
class RetailStoreMemberGiftCardConsumeRecordUpdateForm extends Component {
  state = {
  ...
  getSelectedRow() {
    ...
    const convertiedValues = selectedRows.map((item) => {
      return {
        ...item,
        occureTime: moment(item.occureTime),
  ...
  render() {
    ...
    const submitUpdateFormAndContinue = () => {
      validateFieldsAndScroll((error, values) => {
        ...
        dispatch({
          type: `${owner.type}/updateRetailStoreMemberGiftCardConsumeRecord`,
          payload: {
            id: owner.id,
    ...
    return (
      <PageHeaderLayout
        title={"更新零售商店会员卡消费记录"+(currentUpdateIndex+1)+"/"+selectedRows.length}
        content="更新零售商店会员卡消费记录"
...
}))(Form.create()(RetailStoreMemberGiftCardConsumeRecordUpdateForm))
