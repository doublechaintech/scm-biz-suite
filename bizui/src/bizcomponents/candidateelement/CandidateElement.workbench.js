...
import styles from './CandidateElement.workbench.less'
...
const imageList =(candidateElement)=>{return [
	   {"title":'图像',"imageLocation":candidateElement.image},
]}
...
const renderSettingMenuItem = (item,cardsData,targetComponent) =>{
  ...
  return (<Menu.Item key={item.name}>
      <Link to={`/candidateElement/${targetComponent.props.candidateElement.id}/list/${item.name}/${item.displayName}/`}>
...
const internalSummaryOf = (cardsData,targetComponent) =>{
	...
	const candidateElement = cardsData.cardsSource
	const {CandidateElementService} = GlobalComponents
	const userContext = null
	return (
	<div>
	<DescriptionList className={styles.headerList} size="small" col="4">
<Description term="ID" style={{wordBreak: 'break-all'}}>{candidateElement.id}</Description> 
<Description term="标题" style={{wordBreak: 'break-all'}}>{candidateElement.name}</Description> 
...
class CandidateElementWorkbench extends Component {
...
}))(Form.create()(CandidateElementWorkbench))
