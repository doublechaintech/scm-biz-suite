...
const menuData = {menuName: window.trans('candidate_element'), menuFor: "candidateElement",  internalName: "candidate_element",
  		subItems: [
...
const settingMenuData = {menuName: window.trans('candidate_element'), menuFor: "candidateElement",  internalName: "candidate_element",
  		subItems: [
...
const renderActionList=(candidateElement, targetObject, columCount, listName)=>{

	if(!candidateElement){
		return null
	}
	if(!candidateElement.actionList){
		return null
	}
	if(candidateElement.actionList.length === 0){
		return null
	}
	return (
		<div className={styles.overlay}>
			...
			{candidateElement.actionList.map(action=>(<Link key={action.id} to={{pathname: action.actionPath.substring(1), state: {ownerId:targetObject.id,action,selectedRows:[candidateElement]}}} >
...
const renderItemOfList=(candidateElement, targetObject, columCount, listName)=>{
  ...
  const userContext = null
  return (
     <Row key={`${listName}-${candidateElement.id}`} className={styles.itemDesc}>
	...
	<Col span={20}>
      ...
      {renderActionList(candidateElement,targetObject)}
    </Row>
	)
...
const packFormValuesToObject = ( formValuesToPack )=>{
	const {name, type, image, containerId} = formValuesToPack
	const container = {id: containerId, version: 2^31}
	const data = {name, type, image, container}
	return data
}
const unpackObjectToFormValues = ( objectToUnpack )=>{
	const {name, type, image, container} = objectToUnpack
	const containerId = container ? container.id : null
	const data = {name, type, image, containerId}
	return data
}
...
const CandidateElementBase={unpackObjectToFormValues, menuData,settingMenuData,displayColumns,fieldLabels,renderItemOfList, stepOf, searchLocalData}
