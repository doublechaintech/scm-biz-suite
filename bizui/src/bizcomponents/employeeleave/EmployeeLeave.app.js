...
import styles from './EmployeeLeave.app.less'
...
class EmployeeLeaveBizApp extends React.PureComponent {
constructor(props) {
    super(props)
     this.state = {
 ...
 getNavMenuItems = (targetObject, style, customTheme) => {
    ...
    return (
	  <Menu
       ...
       <Menu.Item key="workbench">
        <Link to={`/employeeLeave/${this.props.employeeLeave.id}/workbench`}><Icon type="solution" style={{marginRight:"20px"}}/><span>工作台</span></Link>
      </Menu.Item>
        ...
        {filteredMenuItemsGroup(targetObject,this).map((groupedMenuItem,index)=>{
          return(
    <SubMenu id={`submenu-vg${index}`}  key={`vg${index}`} title={<span><Icon type={viewGroupIconNameOf('employee_leave',`${groupedMenuItem.viewGroup}`)} style={{marginRight:"20px"}} /><span>{viewGroupDisplayNameOf('employee_leave',`${groupedMenuItem.viewGroup}`)}</span></span>} >
  ...
  getRequestTypeStepForm = () => {
    ...
    return connect(state => ({
      selectedRows: state._employeeLeave.selectedRows,
      role: "cq",
      currentUpdateIndex: state._employeeLeave.currentUpdateIndex,
      owner: { type: '_employeeLeave', id: state._employeeLeave.id, listName: 'nolist', ref:state._employeeLeave, listDisplayName: appLocaleName(userContext,"List") }, // this is for model namespace and
  ...
  buildRouters = () =>{
    const {EmployeeLeaveWorkbench} = GlobalComponents

    const {EmployeeLeaveDashboard} = GlobalComponents
  	const {EmployeeLeavePermission} = GlobalComponents
  	const {EmployeeLeaveProfile} = GlobalComponents


    const routers=[
    {path:"/employeeLeave/:id/workbench", component: EmployeeLeaveWorkbench},
    
  	{path:"/employeeLeave/:id/profile", component: EmployeeLeaveProfile},
  	{path:"/employeeLeave/:id/permission", component: EmployeeLeavePermission},
  ...
  handleOpenChange = (openKeys) => {
   ...
   render() {
    ...
    const {searchLocalData}=GlobalComponents.EmployeeLeaveBase
    ...
    const renderMenuSwitch=()=>{
     ...
     <Layout>
      ...
      {this.state.showSearch&&(
        ...
        <div style={{backgroundColor:'black'}}  onClick={()=>hideSearchResult()}  >{searchLocalData(this.props.employeeLeave,this.state.searchKeyword)}</div>
       ...
       <Sider
        ...
        >
         ...
         {this.getNavMenuItems(this.props.employeeLeave,"inline","dark")}
