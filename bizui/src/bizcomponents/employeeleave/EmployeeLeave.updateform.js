...
import styles from './EmployeeLeave.updateform.less'
...
class EmployeeLeaveUpdateForm extends Component {
  state = {
  ...
  render() {
    ...
    const submitUpdateForm = () => {
      validateFieldsAndScroll((error, values) => {
        ...
        const employeeLeaveId = values.id

        const parameters = { ...values, employeeLeaveId, }
    ...
    const submitUpdateFormAndContinue = () => {
      validateFieldsAndScroll((error, values) => {
        ...
        dispatch({
          type: `${owner.type}/updateEmployeeLeave`,
          payload: {
            id: owner.id,
            type: 'employeeLeave',
            parameters,
    ...
    const skipToNext = () => {
      ...
      const newIndex = currentUpdateIndex + 1
      dispatch({
        type: `${owner.type}/gotoNextEmployeeLeaveUpdateRow`,
        payload: {
          id: owner.id,
          type: 'employeeLeave',
          selectedRows,
	...
	const internalRenderTitle = () =>{
      ...
      return (<div>{linkComp}{appLocaleName(userContext,"Update")}请假记录: {(currentUpdateIndex+1)}/{selectedRows.length}</div>)
    }
              ...
              <Col lg={24} md={24} sm={24}>
                <Form.Item label={fieldLabels.leaveDurationHour} {...formItemLayout}>
                  {getFieldDecorator('leaveDurationHour', {
                    ...
                    <SmallTextInput size="large"  placeholder={fieldLabels.leaveDurationHour} />
...
}))(Form.create()(EmployeeLeaveUpdateForm))
