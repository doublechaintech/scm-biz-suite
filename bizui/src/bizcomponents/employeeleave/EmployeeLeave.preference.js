...
const internalImageListOf = (employeeLeave) =>{
...
const internalSettingListOf = (employeeLeave) =>{
	const userContext = null
	const optionList = [ 
	]
...
const handleTransferSearch =(targetComponent,filterKey,newRequest)=>{
  ...
  const pageNo = 1;
  const candidateReferenceService = EmployeeLeaveService[candidateServiceName] 
...
const executeTrans = (employeeLeave,targetComponent) =>{
...
const buildTransferModal = (employeeLeave,targetComponent) => {
...
<Modal title={transferModalTitle}
          ...
          onOk={()=>executeTrans(employeeLeave,targetComponent)}
...
const internalRenderExtraHeader = (employeeLeave) =>{
	return null
}
const internalRenderExtraFooter = (employeeLeave) =>{
	return null
}
...
<Description term="谁">{employeeLeave.who==null?appLocaleName(userContext,"NotAssigned"):employeeLeave.who.displayName}
 <Icon type="swap" onClick={()=>
  showTransferModel(targetComponent,"谁","employee","requestCandidateWho",
	      "transferToAnotherWho","anotherWhoId",employeeLeave.who?employeeLeave.who.id:"")} 
  style={{fontSize: 20,color:"red"}} />
</Description>
<Description term="类型">{employeeLeave.type==null?appLocaleName(userContext,"NotAssigned"):employeeLeave.type.displayName}
 <Icon type="swap" onClick={()=>
  showTransferModel(targetComponent,"类型","leaveType","requestCandidateType",
	      "transferToAnotherType","anotherTypeId",employeeLeave.type?employeeLeave.type.id:"")} 
  style={{fontSize: 20,color:"red"}} />
</Description>
...
class EmployeeLeavePreference extends Component {
  ...
  render() {
    ...
    const cardsData = {cardsName:"请假记录",cardsFor: "employeeLeave",cardsSource: this.props.employeeLeave,
  		subItems: [
