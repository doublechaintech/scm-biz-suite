...
const renderItemOfList=(consumerOrderShippingGroup, targetComponent, columCount)=>{
  const displayColumnsCount = columCount || 4
  const userContext = null
  return (
    <Card key={consumerOrderShippingGroup.id} style={{marginTop:"10px"}}>
	...
	<Col span={20}>
      ...
      <DescriptionList  key={consumerOrderShippingGroup.id} size="small" col={displayColumnsCount} >
        ...
        <Description term={fieldLabels.bizOrder}><Tag color='blue' title={`${consumerOrderShippingGroup.bizOrder.id}-${consumerOrderShippingGroup.bizOrder.displayName}`}>{consumerOrderShippingGroup.bizOrder==null?appLocaleName(userContext,"NotAssigned"):`${leftChars(consumerOrderShippingGroup.bizOrder.displayName,15)}`}
        </Tag></Description>
...
const ConsumerOrderShippingGroupBase={menuData,settingMenuData,displayColumns,fieldLabels,renderItemOfList, stepOf, searchLocalData}
