...
import styles from './ConsumerOrderShippingGroup.app.less'
...
class ConsumerOrderShippingGroupBizApp extends React.PureComponent {
constructor(props) {
    super(props)
     this.state = {
 ...
 getNavMenuItems = (targetObject, style, customTheme) => {
    ...
    return (
	  <Menu
       ...
       <Menu.Item key="workbench">
        <Link to={`/consumerOrderShippingGroup/${this.props.consumerOrderShippingGroup.id}/workbench`}><Icon type="solution" style={{marginRight:"20px"}}/><span>工作台</span></Link>
      </Menu.Item>
        ...
        {filteredMenuItemsGroup(targetObject,this).map((groupedMenuItem,index)=>{
          return(
    <SubMenu id={`submenu-vg${index}`}  key={`vg${index}`} title={<span><Icon type={viewGroupIconNameOf('consumer_order_shipping_group',`${groupedMenuItem.viewGroup}`)} style={{marginRight:"20px"}} /><span>{viewGroupDisplayNameOf('consumer_order_shipping_group',`${groupedMenuItem.viewGroup}`)}</span></span>} >
  ...
  getRequestTypeStepForm = () => {
    ...
    return connect(state => ({
      selectedRows: state._consumerOrderShippingGroup.selectedRows,
      role: "cq",
      currentUpdateIndex: state._consumerOrderShippingGroup.currentUpdateIndex,
      owner: { type: '_consumerOrderShippingGroup', id: state._consumerOrderShippingGroup.id, listName: 'nolist', ref:state._consumerOrderShippingGroup, listDisplayName: appLocaleName(userContext,"List") }, // this is for model namespace and
  ...
  buildRouters = () =>{
    const {ConsumerOrderShippingGroupWorkbench} = GlobalComponents

    const {ConsumerOrderShippingGroupDashboard} = GlobalComponents
  	const {ConsumerOrderShippingGroupPermission} = GlobalComponents
  	const {ConsumerOrderShippingGroupProfile} = GlobalComponents


    const routers=[
    {path:"/consumerOrderShippingGroup/:id/workbench", component: ConsumerOrderShippingGroupWorkbench},
    
  	{path:"/consumerOrderShippingGroup/:id/profile", component: ConsumerOrderShippingGroupProfile},
  	{path:"/consumerOrderShippingGroup/:id/permission", component: ConsumerOrderShippingGroupPermission},
  ...
  handleOpenChange = (openKeys) => {
   ...
   render() {
    ...
    const {searchLocalData}=GlobalComponents.ConsumerOrderShippingGroupBase
    ...
    const renderMenuSwitch=()=>{
     ...
     <Layout>
      ...
      {this.state.showSearch&&(
        ...
        <div style={{backgroundColor:'black'}}  onClick={()=>hideSearchResult()}  >{searchLocalData(this.props.consumerOrderShippingGroup,this.state.searchKeyword)}</div>
       ...
       <Sider
        ...
        >
         ...
         {this.getNavMenuItems(this.props.consumerOrderShippingGroup,"inline","dark")}
