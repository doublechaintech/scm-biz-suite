...
import styles from './ConsumerOrderShippingGroup.workbench.less'
...
const renderSettingMenuItem = (item,cardsData,targetComponent) =>{
  ...
  return (<Menu.Item key={item.name}>
      <Link to={`/consumerOrderShippingGroup/${targetComponent.props.consumerOrderShippingGroup.id}/list/${item.name}/${item.displayName}/`}>
...
const renderSettingMenu = (cardsData,targetComponent) =>{
  ...
  return (<Menu>
    	<Menu.Item key="profile">
  			<Link to={`/consumerOrderShippingGroup/${targetComponent.props.consumerOrderShippingGroup.id}/permission`}><Icon type="safety-certificate" theme="twoTone" twoToneColor="#52c41a"/><span>{appLocaleName(userContext,"Permission")}</span></Link>
		</Menu.Item>
		<Menu.Divider />
...
const internalSummaryOf = (cardsData,targetComponent) =>{
	...
	const consumerOrderShippingGroup = cardsData.cardsSource
	const {ConsumerOrderShippingGroupService} = GlobalComponents
	const userContext = null
	return (
	<div>
	<DescriptionList className={styles.headerList} size="small" col="4">
<Description term="ID" style={{wordBreak: 'break-all'}}>{consumerOrderShippingGroup.id}</Description> 
...
<Description term="金额" style={{wordBreak: 'break-all'}}>{consumerOrderShippingGroup.amount}</Description> 
...
class ConsumerOrderShippingGroupWorkbench extends Component {
 ...
 state = {
  ...
  render() {
    ...
    const cardsData = {cardsName:window.trans('consumer_order_shipping_group'),cardsFor: "consumerOrderShippingGroup",
...
}))(Form.create()(ConsumerOrderShippingGroupWorkbench))
