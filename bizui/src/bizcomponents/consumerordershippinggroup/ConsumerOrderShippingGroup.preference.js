...
const internalImageListOf = (consumerOrderShippingGroup) =>{
...
const internalSettingListOf = (consumerOrderShippingGroup) =>{
	const userContext = null
	const optionList = [ 
	]
...
const handleTransferSearch =(targetComponent,filterKey,newRequest)=>{
  ...
  const pageNo = 1;
  const candidateReferenceService = ConsumerOrderShippingGroupService[candidateServiceName] 
...
const executeTrans = (consumerOrderShippingGroup,targetComponent) =>{
  ...
  validateFieldsAndScroll((error, values) => {
    ...
    const id=consumerOrderShippingGroup.id;
    const serviceNameToCall = transferServiceName;

    const payload = {parameters,id,serviceNameToCall}
    
    //targetComponent.setState({transferModalVisiable:false})
    dispatch({type:"_consumerOrderShippingGroup/doJob",payload: payload})
...
const buildTransferModal = (consumerOrderShippingGroup,targetComponent) => {
...
<Modal title={transferModalTitle}
          ...
          onOk={()=>executeTrans(consumerOrderShippingGroup,targetComponent)}
...
const internalRenderExtraFooter = (consumerOrderShippingGroup) =>{
	return null
}
...
<Description term="订单">{consumerOrderShippingGroup.bizOrder==null?appLocaleName(userContext,"NotAssigned"):consumerOrderShippingGroup.bizOrder.displayName}
 <Icon type="swap" onClick={()=>
  showTransferModel(targetComponent,"订单","consumerOrder","requestCandidateBizOrder",
	      "transferToAnotherBizOrder","anotherBizOrderId",consumerOrderShippingGroup.bizOrder?consumerOrderShippingGroup.bizOrder.id:"")} 
...
class ConsumerOrderShippingGroupPreference extends Component {
...
}))(Form.create()(ConsumerOrderShippingGroupPreference))
