...
import styles from './LevelThreeDepartment.updateform.less'
...
class LevelThreeDepartmentUpdateForm extends Component {
  state = {
...
>>>>>>> 502e8b8dfc403300a992b5083e79c722e85d1854
        ...
        const levelThreeDepartmentId = values.id
        const imagesValues = mapBackToImageValues(convertedImagesValues)
        const parameters = { ...values, levelThreeDepartmentId, ...imagesValues }
    ...
    const submitUpdateFormAndContinue = () => {
      validateFieldsAndScroll((error, values) => {
        ...
        dispatch({
          type: `${owner.type}/updateLevelThreeDepartment`,
          payload: {
            id: owner.id,
            type: 'levelThreeDepartment',
            parameters,
            selectedRows,
    ...
    const skipToNext = () => {
      ...
      const newIndex = currentUpdateIndex + 1
      dispatch({
        type: `${owner.type}/gotoNextLevelThreeDepartmentUpdateRow`,
        payload: {
          id: owner.id,
          type: 'levelThreeDepartment',
          selectedRows,
...
<<<<<<< HEAD
    ...
    return (
      <PageHeaderLayout
        title={"更新三级部门"+(currentUpdateIndex+1)+"/"+selectedRows.length}
        content="更新三级部门"
...
=======
	...
	const internalRenderTitle = () =>{
      ...
      return (<div>{linkComp}{appLocaleName(userContext,"Update")}三级部门: {(currentUpdateIndex+1)}/{selectedRows.length}</div>)
    }
...
}))(Form.create()(LevelThreeDepartmentUpdateForm))
