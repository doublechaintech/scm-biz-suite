...
import styles from './LevelThreeDepartment.app.less'
 ...
 getNavMenuItems = (targetObject, style, customTheme) => {
    ...
    return (
	  <Menu
       ...
       <Menu.Item key="workbench">
        <Link to={`/levelThreeDepartment/${this.props.levelThreeDepartment.id}/workbench`}><Icon type="solution" style={{marginRight:"20px"}}/><span>工作台</span></Link>
      </Menu.Item>
        ...
        {filteredMenuItemsGroup(targetObject,this).map((groupedMenuItem,index)=>{
          return(
    <SubMenu id={`submenu-vg${index}`}  key={`vg${index}`} title={<span><Icon type={viewGroupIconNameOf('level_three_department',`${groupedMenuItem.viewGroup}`)} style={{marginRight:"20px"}} /><span>{viewGroupDisplayNameOf('level_three_department',`${groupedMenuItem.viewGroup}`)}</span></span>} >
  ...
  getEmployeeSearch = () => {
    ...
    return connect(state => ({
      rule: state.rule,
      name: window.mtrans('employee','level_three_department.employee_list',false),
      role: "employee",
      ...
      returnURL: `/levelThreeDepartment/${state._levelThreeDepartment.id}/workbench`,
      ...
      expandForm: state._levelThreeDepartment.expandForm,
      loading: state._levelThreeDepartment.loading,
      partialList: state._levelThreeDepartment.partialList,
  ...
  getEmployeeCreateForm = () => {
    ...
    return connect(state => ({
      ...
      searchFormParameters: state._levelThreeDepartment.employeeSearchFormParameters,
      loading: state._levelThreeDepartment.loading,
      owner: { type: '_levelThreeDepartment', id: state._levelThreeDepartment.id || this.getOwnerId(), referenceName: 'department', listName: 'employeeList', ref:state._levelThreeDepartment, listDisplayName: appLocaleName(userContext,"List")}, // this is for model namespace and
    }))(EmployeeCreateForm)
  }
  ...
  getEmployeeUpdateForm = () => {
    ...
    return connect(state => ({
      ...
      role: "employee",
      currentUpdateIndex: state._levelThreeDepartment.currentUpdateIndex || 0,
      owner: { type: '_levelThreeDepartment', id: state._levelThreeDepartment.id || this.getOwnerId(), listName: 'employeeList', ref:state._levelThreeDepartment, listDisplayName: appLocaleName(userContext,"List") }, // this is for model namespace and
    }))(EmployeeUpdateForm)
  }
  ...
  getRequestTypeStepForm = () => {
    ...
    return connect(state => ({
      ...
      owner: { type: '_levelThreeDepartment', id: state._levelThreeDepartment.id, listName: 'nolist', ref:state._levelThreeDepartment, listDisplayName: appLocaleName(userContext,"List") }, // this is for model namespace and
  ...
  buildRouters = () =>{
    const {LevelThreeDepartmentWorkbench} = GlobalComponents
    ...
    const routers=[
    {path:"/levelThreeDepartment/:id/workbench", component: LevelThreeDepartmentWorkbench},
  	...
  	{path:"/levelThreeDepartment/:id/list/employeeList", component: this.getEmployeeSearch()},
  	{path:"/levelThreeDepartment/:id/list/employeeCreateForm", component: this.getEmployeeCreateForm()},
  	{path:"/levelThreeDepartment/:id/list/employeeUpdateForm", component: this.getEmployeeUpdateForm()},
  ...
  handleOpenChange = (openKeys) => {
   ...
   render() {
    ...
    const {searchLocalData}=GlobalComponents.LevelThreeDepartmentBase
    ...
    const renderMenuSwitch=()=>{
     ...
     <Layout>
      ...
      {this.state.showSearch&&(
        ...
        <div style={{backgroundColor:'black'}}  onClick={()=>hideSearchResult()}  >{searchLocalData(this.props.levelThreeDepartment,this.state.searchKeyword)}</div>
       ...
       <Sider
        ...
        >
         ...
         {this.getNavMenuItems(this.props.levelThreeDepartment,"inline","dark")}
