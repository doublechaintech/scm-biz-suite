...
const internalImageListOf = (retailStoreFranchising) =>{
...
const internalSettingListOf = (retailStoreFranchising) =>{
	const userContext = null
	const optionList = [ 
	]
...
const handleTransferSearch =(targetComponent,filterKey,newRequest)=>{
  ...
  const pageNo = 1;
  const candidateReferenceService = RetailStoreFranchisingService[candidateServiceName] 
...
const executeTrans = (retailStoreFranchising,targetComponent) =>{
  ...
  validateFieldsAndScroll((error, values) => {
    ...
    const id=retailStoreFranchising.id;
    const serviceNameToCall = transferServiceName;

    const payload = {parameters,id,serviceNameToCall}
    
    //targetComponent.setState({transferModalVisiable:false})
    dispatch({type:"_retailStoreFranchising/doJob",payload: payload})
...
const buildTransferModal = (retailStoreFranchising,targetComponent) => {
...
<Modal title={transferModalTitle}
          ...
          onOk={()=>executeTrans(retailStoreFranchising,targetComponent)}
...
const internalRenderExtraHeader = (retailStoreFranchising) =>{
	return null
}
const internalRenderExtraFooter = (retailStoreFranchising) =>{
	return null
}
...
class RetailStoreFranchisingPreference extends Component {
...
}))(Form.create()(RetailStoreFranchisingPreference))
