 ...
 getNavMenuItems = (targetObject, style, customTheme) => {
    ...
    return (
	  <Menu
       ...
       <Menu.Item key="workbench">
        <Link to={`/retailStoreFranchising/${this.props.retailStoreFranchising.id}/workbench`}><Icon type="solution" style={{marginRight:"20px"}}/><span>工作台</span></Link>
      </Menu.Item>
        ...
        {filteredMenuItemsGroup(targetObject,this).map((groupedMenuItem,index)=>{
          return(
    <SubMenu id={`submenu-vg${index}`}  key={`vg${index}`} title={<span><Icon type={viewGroupIconNameOf('retail_store_franchising',`${groupedMenuItem.viewGroup}`)} style={{marginRight:"20px"}} /><span>{viewGroupDisplayNameOf('retail_store_franchising',`${groupedMenuItem.viewGroup}`)}</span></span>} >
  ...
  getRetailStoreSearch = () => {
    ...
    return connect(state => ({
      rule: state.rule,
      name: window.mtrans('retail_store','retail_store_franchising.retail_store_list',false),
      role: "retailStore",
      ...
      returnURL: `/retailStoreFranchising/${state._retailStoreFranchising.id}/workbench`,
  ...
  getRetailStoreCreateForm = () => {
    ...
    return connect(state => ({
      ...
      metaInfo: state._retailStoreFranchising.retailStoreListMetaInfo,
      count: state._retailStoreFranchising.retailStoreCount,
      returnURL: `/retailStoreFranchising/${state._retailStoreFranchising.id}/list`,
      currentPage: state._retailStoreFranchising.retailStoreCurrentPageNumber,
      searchFormParameters: state._retailStoreFranchising.retailStoreSearchFormParameters,
      loading: state._retailStoreFranchising.loading,
      owner: { type: '_retailStoreFranchising', id: state._retailStoreFranchising.id || this.getOwnerId(), referenceName: 'franchising', listName: 'retailStoreList', ref:state._retailStoreFranchising, listDisplayName: appLocaleName(userContext,"List")}, // this is for model namespace and
  ...
  getRetailStoreUpdateForm = () => {
    ...
    return connect(state => ({
      ...
      role: "retailStore",
      currentUpdateIndex: state._retailStoreFranchising.currentUpdateIndex || 0,
      owner: { type: '_retailStoreFranchising', id: state._retailStoreFranchising.id || this.getOwnerId(), listName: 'retailStoreList', ref:state._retailStoreFranchising, listDisplayName: appLocaleName(userContext,"List") }, // this is for model namespace and
  ...
  getRequestTypeStepForm = () => {
    ...
    return connect(state => ({
      ...
      owner: { type: '_retailStoreFranchising', id: state._retailStoreFranchising.id, listName: 'nolist', ref:state._retailStoreFranchising, listDisplayName: appLocaleName(userContext,"List") }, // this is for model namespace and
  ...
  buildRouters = () =>{
    const {RetailStoreFranchisingWorkbench} = GlobalComponents
    ...
    const routers=[
    {path:"/retailStoreFranchising/:id/workbench", component: RetailStoreFranchisingWorkbench},
  ...
  handleOpenChange = (openKeys) => {
   ...
   render() {
    ...
    const {searchLocalData}=GlobalComponents.RetailStoreFranchisingBase
    ...
    const renderMenuSwitch=()=>{
     ...
     <Layout>
      ...
      {this.state.showSearch&&(
        ...
        <div style={{backgroundColor:'black'}}  onClick={()=>hideSearchResult()}  >{searchLocalData(this.props.retailStoreFranchising,this.state.searchKeyword)}</div>
       ...
       <Sider
        ...
        >
         ...
         {this.getNavMenuItems(this.props.retailStoreFranchising,"inline","dark")}
