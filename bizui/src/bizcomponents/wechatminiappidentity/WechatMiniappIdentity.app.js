...
import styles from './WechatMiniappIdentity.app.less'
...
class WechatMiniappIdentityBizApp extends React.PureComponent {
constructor(props) {
    super(props)
     this.state = {
 ...
 getNavMenuItems = (targetObject, style, customTheme) => {
    ...
    return (
	  <Menu
       ...
       <Menu.Item key="workbench">
        <Link to={`/wechatMiniappIdentity/${this.props.wechatMiniappIdentity.id}/workbench`}><Icon type="solution" style={{marginRight:"20px"}}/><span>工作台</span></Link>
      </Menu.Item>
        ...
        {filteredMenuItemsGroup(targetObject,this).map((groupedMenuItem,index)=>{
          return(
    <SubMenu id={`submenu-vg${index}`}  key={`vg${index}`} title={<span><Icon type={viewGroupIconNameOf('wechat_miniapp_identity',`${groupedMenuItem.viewGroup}`)} style={{marginRight:"20px"}} /><span>{viewGroupDisplayNameOf('wechat_miniapp_identity',`${groupedMenuItem.viewGroup}`)}</span></span>} >
  ...
  getRequestTypeStepForm = () => {
    ...
    return connect(state => ({
      selectedRows: state._wechatMiniappIdentity.selectedRows,
      role: "cq",
      currentUpdateIndex: state._wechatMiniappIdentity.currentUpdateIndex,
      owner: { type: '_wechatMiniappIdentity', id: state._wechatMiniappIdentity.id, listName: 'nolist', ref:state._wechatMiniappIdentity, listDisplayName: appLocaleName(userContext,"List") }, // this is for model namespace and
  ...
  buildRouters = () =>{
    const {WechatMiniappIdentityWorkbench} = GlobalComponents

    const {WechatMiniappIdentityDashboard} = GlobalComponents
  	const {WechatMiniappIdentityPermission} = GlobalComponents
  	const {WechatMiniappIdentityProfile} = GlobalComponents


    const routers=[
    {path:"/wechatMiniappIdentity/:id/workbench", component: WechatMiniappIdentityWorkbench},
    
  	{path:"/wechatMiniappIdentity/:id/profile", component: WechatMiniappIdentityProfile},
  	{path:"/wechatMiniappIdentity/:id/permission", component: WechatMiniappIdentityPermission},
  ...
  handleOpenChange = (openKeys) => {
   ...
   render() {
    ...
    const {searchLocalData}=GlobalComponents.WechatMiniappIdentityBase
    ...
    const renderMenuSwitch=()=>{
     ...
     <Layout>
      ...
      {this.state.showSearch&&(
        ...
        <div style={{backgroundColor:'black'}}  onClick={()=>hideSearchResult()}  >{searchLocalData(this.props.wechatMiniappIdentity,this.state.searchKeyword)}</div>
       ...
       <Sider
        ...
        >
         ...
         {this.getNavMenuItems(this.props.wechatMiniappIdentity,"inline","dark")}
