...
import styles from './EmployeeQualifier.app.less'
...
class EmployeeQualifierBizApp extends React.PureComponent {
constructor(props) {
    super(props)
     this.state = {
 ...
 getNavMenuItems = (targetObject, style, customTheme) => {
    ...
    return (
	  <Menu
       ...
       <Menu.Item key="workbench">
        <Link to={`/employeeQualifier/${this.props.employeeQualifier.id}/workbench`}><Icon type="solution" style={{marginRight:"20px"}}/><span>工作台</span></Link>
      </Menu.Item>
        ...
        {filteredMenuItemsGroup(targetObject,this).map((groupedMenuItem,index)=>{
          return(
    <SubMenu id={`submenu-vg${index}`}  key={`vg${index}`} title={<span><Icon type={viewGroupIconNameOf('employee_qualifier',`${groupedMenuItem.viewGroup}`)} style={{marginRight:"20px"}} /><span>{viewGroupDisplayNameOf('employee_qualifier',`${groupedMenuItem.viewGroup}`)}</span></span>} >
  ...
  getRequestTypeStepForm = () => {
    ...
    return connect(state => ({
      selectedRows: state._employeeQualifier.selectedRows,
      role: "cq",
      currentUpdateIndex: state._employeeQualifier.currentUpdateIndex,
      owner: { type: '_employeeQualifier', id: state._employeeQualifier.id, listName: 'nolist', ref:state._employeeQualifier, listDisplayName: appLocaleName(userContext,"List") }, // this is for model namespace and
  ...
  buildRouters = () =>{
    const {EmployeeQualifierWorkbench} = GlobalComponents

    const {EmployeeQualifierDashboard} = GlobalComponents
  	const {EmployeeQualifierPermission} = GlobalComponents
  	const {EmployeeQualifierProfile} = GlobalComponents


    const routers=[
    {path:"/employeeQualifier/:id/workbench", component: EmployeeQualifierWorkbench},
    
  	{path:"/employeeQualifier/:id/profile", component: EmployeeQualifierProfile},
  	{path:"/employeeQualifier/:id/permission", component: EmployeeQualifierPermission},
  ...
  handleOpenChange = (openKeys) => {
   ...
   render() {
    ...
    const {searchLocalData}=GlobalComponents.EmployeeQualifierBase
    ...
    const renderMenuSwitch=()=>{
     ...
     <Layout>
      ...
      {this.state.showSearch&&(
        ...
        <div style={{backgroundColor:'black'}}  onClick={()=>hideSearchResult()}  >{searchLocalData(this.props.employeeQualifier,this.state.searchKeyword)}</div>
       ...
       <Sider
        ...
        >
         ...
         {this.getNavMenuItems(this.props.employeeQualifier,"inline","dark")}
