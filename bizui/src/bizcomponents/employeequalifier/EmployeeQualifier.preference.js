...
const internalImageListOf = (employeeQualifier) =>{
<<<<<<< HEAD
...
const internalSettingListOf = (employeeQualifier) =>{
<<<<<<< HEAD
...
const handleTransferSearch =(targetComponent,filterKey,newRequest)=>{
  ...
  const pageNo = 1;
  const candidateReferenceService = EmployeeQualifierService[candidateServiceName] 
...
const executeTrans = (employeeQualifier,targetComponent) =>{
...
const buildTransferModal = (employeeQualifier,targetComponent) => {
...
<Modal title={transferModalTitle}
          ...
          onOk={()=>executeTrans(employeeQualifier,targetComponent)}
...
const internalRenderExtraFooter = (employeeQualifier) =>{
	return null
}
...
=======
	...
	<DescriptionList className={styles.headerList} size="small" col="4">
<Description term="序号">{employeeQualifier.id}</Description> 
<Description term="员工">{employeeQualifier.employee==null?appLocaleName(userContext,"NotAssigned"):employeeQualifier.employee.displayName}
>>>>>>> 502e8b8dfc403300a992b5083e79c722e85d1854
 <Icon type="swap" onClick={()=>
  showTransferModel(targetComponent,"员工","employee","requestCandidateEmployee",
	      "transferToAnotherEmployee","anotherEmployeeId",employeeQualifier.employee?employeeQualifier.employee.id:"")} 
...
class EmployeeQualifierPreference extends Component {
...
}))(Form.create()(EmployeeQualifierPreference))
