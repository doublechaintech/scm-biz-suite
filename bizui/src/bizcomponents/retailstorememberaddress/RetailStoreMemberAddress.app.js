...
import styles from './RetailStoreMemberAddress.app.less'
...
class RetailStoreMemberAddressBizApp extends React.PureComponent {
constructor(props) {
    super(props)
     this.state = {
 ...
 getNavMenuItems = (targetObject, style, customTheme) => {
    ...
    return (
	  <Menu
       ...
       <Menu.Item key="workbench">
        <Link to={`/retailStoreMemberAddress/${this.props.retailStoreMemberAddress.id}/workbench`}><Icon type="solution" style={{marginRight:"20px"}}/><span>工作台</span></Link>
      </Menu.Item>
        ...
        {filteredMenuItemsGroup(targetObject,this).map((groupedMenuItem,index)=>{
          return(
    <SubMenu id={`submenu-vg${index}`}  key={`vg${index}`} title={<span><Icon type={viewGroupIconNameOf('retail_store_member_address',`${groupedMenuItem.viewGroup}`)} style={{marginRight:"20px"}} /><span>{viewGroupDisplayNameOf('retail_store_member_address',`${groupedMenuItem.viewGroup}`)}</span></span>} >
  ...
  getRequestTypeStepForm = () => {
    ...
    return connect(state => ({
      selectedRows: state._retailStoreMemberAddress.selectedRows,
      role: "cq",
      currentUpdateIndex: state._retailStoreMemberAddress.currentUpdateIndex,
      owner: { type: '_retailStoreMemberAddress', id: state._retailStoreMemberAddress.id, listName: 'nolist', ref:state._retailStoreMemberAddress, listDisplayName: appLocaleName(userContext,"List") }, // this is for model namespace and
  ...
  buildRouters = () =>{
    const {RetailStoreMemberAddressWorkbench} = GlobalComponents

    const {RetailStoreMemberAddressDashboard} = GlobalComponents
  	const {RetailStoreMemberAddressPermission} = GlobalComponents
  	const {RetailStoreMemberAddressProfile} = GlobalComponents


    const routers=[
    {path:"/retailStoreMemberAddress/:id/workbench", component: RetailStoreMemberAddressWorkbench},
    
  	{path:"/retailStoreMemberAddress/:id/profile", component: RetailStoreMemberAddressProfile},
  	{path:"/retailStoreMemberAddress/:id/permission", component: RetailStoreMemberAddressPermission},
  ...
  handleOpenChange = (openKeys) => {
   ...
   render() {
    ...
    const {searchLocalData}=GlobalComponents.RetailStoreMemberAddressBase
    ...
    const renderMenuSwitch=()=>{
     ...
     <Layout>
      ...
      {this.state.showSearch&&(
        ...
        <div style={{backgroundColor:'black'}}  onClick={()=>hideSearchResult()}  >{searchLocalData(this.props.retailStoreMemberAddress,this.state.searchKeyword)}</div>
       ...
       <Sider
        ...
        >
         ...
         {this.getNavMenuItems(this.props.retailStoreMemberAddress,"inline","dark")}
