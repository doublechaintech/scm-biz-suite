...
const internalImageListOf = (retailStoreMemberAddress) =>{
<<<<<<< HEAD
...
const internalSettingListOf = (retailStoreMemberAddress) =>{
<<<<<<< HEAD
...
const handleTransferSearch =(targetComponent,filterKey,newRequest)=>{
  ...
  const pageNo = 1;
  const candidateReferenceService = RetailStoreMemberAddressService[candidateServiceName] 
...
const executeTrans = (retailStoreMemberAddress,targetComponent) =>{
  ...
  validateFieldsAndScroll((error, values) => {
    ...
    const id=retailStoreMemberAddress.id;
    const serviceNameToCall = transferServiceName;

    const payload = {parameters,id,serviceNameToCall}
    
    //targetComponent.setState({transferModalVisiable:false})
    dispatch({type:"_retailStoreMemberAddress/doJob",payload: payload})
...
const buildTransferModal = (retailStoreMemberAddress,targetComponent) => {
...
<Modal title={transferModalTitle}
          ...
          onOk={()=>executeTrans(retailStoreMemberAddress,targetComponent)}
...
const internalRenderExtraHeader = (retailStoreMemberAddress) =>{
	return null
}
const internalRenderExtraFooter = (retailStoreMemberAddress) =>{
	return null
}
...
=======
<Description term="业主">{retailStoreMemberAddress.owner==null?appLocaleName(userContext,"NotAssigned"):retailStoreMemberAddress.owner.displayName}
>>>>>>> 502e8b8dfc403300a992b5083e79c722e85d1854
 <Icon type="swap" onClick={()=>
  showTransferModel(targetComponent,"业主","retailStoreMember","requestCandidateOwner",
	      "transferToAnotherOwner","anotherOwnerId",retailStoreMemberAddress.owner?retailStoreMemberAddress.owner.id:"")} 
...
class RetailStoreMemberAddressPreference extends Component {
...
}))(Form.create()(RetailStoreMemberAddressPreference))
