 ...
 getNavMenuItems = (targetObject, style, customTheme) => {
	  ...
	  <Menu
       ...
       <Menu.Item key="workbench">
        ...
        {filteredMenuItemsGroup(targetObject,this).map((groupedMenuItem,index)=>{
          return(
    <SubMenu id={`submenu-vg${index}`}  key={`vg${index}`} title={<span><Icon type={viewGroupIconNameOf('transport_truck',`${groupedMenuItem.viewGroup}`)} style={{marginRight:"20px"}} /><span>{viewGroupDisplayNameOf('transport_truck',`${groupedMenuItem.viewGroup}`)}</span></span>} >
  ...
  getTransportTaskSearch = () => {
    ...
    return connect(state => ({
      rule: state.rule,
      name: window.mtrans('transport_task','transport_truck.transport_task_list',false),
      role: "transportTask",
