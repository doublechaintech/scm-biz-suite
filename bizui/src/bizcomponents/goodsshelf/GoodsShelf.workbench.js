...
import styles from './GoodsShelf.workbench.less'
...
const internalSummaryOf = (cardsData,targetComponent) =>{
	...
	const goodsShelf = cardsData.cardsSource
	const {GoodsShelfService} = GlobalComponents
	const userContext = null
	return (
	<div>
	<DescriptionList className={styles.headerList} size="small" col="4">
<Description term="ID" style={{wordBreak: 'break-all'}}>{goodsShelf.id}</Description> 
...
<Description term="更新于">{ moment(goodsShelf.lastUpdateTime).format('YYYY-MM-DD HH:mm')}</Description> 
...
class GoodsShelfWorkbench extends Component {
...
}))(Form.create()(GoodsShelfWorkbench))
