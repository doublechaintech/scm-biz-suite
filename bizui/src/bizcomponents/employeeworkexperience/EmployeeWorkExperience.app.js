...
import styles from './EmployeeWorkExperience.app.less'
...
class EmployeeWorkExperienceBizApp extends React.PureComponent {
constructor(props) {
    super(props)
     this.state = {
 ...
 getNavMenuItems = (targetObject, style, customTheme) => {
    ...
    return (
	  <Menu
       ...
       <Menu.Item key="workbench">
        <Link to={`/employeeWorkExperience/${this.props.employeeWorkExperience.id}/workbench`}><Icon type="solution" style={{marginRight:"20px"}}/><span>工作台</span></Link>
      </Menu.Item>
        ...
        {filteredMenuItemsGroup(targetObject,this).map((groupedMenuItem,index)=>{
          return(
    <SubMenu id={`submenu-vg${index}`}  key={`vg${index}`} title={<span><Icon type={viewGroupIconNameOf('employee_work_experience',`${groupedMenuItem.viewGroup}`)} style={{marginRight:"20px"}} /><span>{viewGroupDisplayNameOf('employee_work_experience',`${groupedMenuItem.viewGroup}`)}</span></span>} >
  ...
  getRequestTypeStepForm = () => {
    ...
    return connect(state => ({
      selectedRows: state._employeeWorkExperience.selectedRows,
      role: "cq",
      currentUpdateIndex: state._employeeWorkExperience.currentUpdateIndex,
      owner: { type: '_employeeWorkExperience', id: state._employeeWorkExperience.id, listName: 'nolist', ref:state._employeeWorkExperience, listDisplayName: appLocaleName(userContext,"List") }, // this is for model namespace and
  ...
  buildRouters = () =>{
    const {EmployeeWorkExperienceWorkbench} = GlobalComponents

    const {EmployeeWorkExperienceDashboard} = GlobalComponents
  	const {EmployeeWorkExperiencePermission} = GlobalComponents
  	const {EmployeeWorkExperienceProfile} = GlobalComponents


    const routers=[
    {path:"/employeeWorkExperience/:id/workbench", component: EmployeeWorkExperienceWorkbench},
    
  	{path:"/employeeWorkExperience/:id/profile", component: EmployeeWorkExperienceProfile},
  	{path:"/employeeWorkExperience/:id/permission", component: EmployeeWorkExperiencePermission},
  ...
  handleOpenChange = (openKeys) => {
   ...
   render() {
    ...
    const {searchLocalData}=GlobalComponents.EmployeeWorkExperienceBase
    ...
    const renderMenuSwitch=()=>{
     ...
     <Layout>
      ...
      {this.state.showSearch&&(
        ...
        <div style={{backgroundColor:'black'}}  onClick={()=>hideSearchResult()}  >{searchLocalData(this.props.employeeWorkExperience,this.state.searchKeyword)}</div>
       ...
       <Sider
        ...
        >
         ...
         {this.getNavMenuItems(this.props.employeeWorkExperience,"inline","dark")}
