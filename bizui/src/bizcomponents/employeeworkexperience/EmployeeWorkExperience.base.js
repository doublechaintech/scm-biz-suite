...
const menuData = {menuName: window.trans('employee_work_experience'), menuFor: "employeeWorkExperience",  internalName: "employee_work_experience",
  		subItems: [
...
const settingMenuData = {menuName: window.trans('employee_work_experience'), menuFor: "employeeWorkExperience",  internalName: "employee_work_experience",
  		subItems: [
...
const displayColumns = [
  { title: fieldLabels.id, debugtype: 'string', dataIndex: 'id', width: '6', render: (text, record)=>renderTextCell(text,record,'employeeWorkExperience') , sorter: true },
...
const renderActionList=(employeeWorkExperience, targetObject, columCount, listName)=>{

	if(!employeeWorkExperience){
		return null
	}
	if(!employeeWorkExperience.actionList){
		return null
	}
	if(employeeWorkExperience.actionList.length === 0){
		return null
	}
	return (
		<div className={styles.overlay}>
			...
			{employeeWorkExperience.actionList.map(action=>(<Link key={action.id} to={{pathname: action.actionPath.substring(1), state: {ownerId:targetObject.id,action,selectedRows:[employeeWorkExperience]}}} >
...
const renderItemOfList=(employeeWorkExperience, targetObject, columCount, listName)=>{
  ...
  const userContext = null
  return (
     <Row key={`${listName}-${employeeWorkExperience.id}`} className={styles.itemDesc}>
	...
	<Col span={20}>
      ...
      {renderActionList(employeeWorkExperience,targetObject)}
    </Row>
	)
...
const packFormValuesToObject = ( formValuesToPack )=>{
	const {employeeId, start, end, company, description} = formValuesToPack
	const employee = {id: employeeId, version: 2^31}
	const data = {employee, start:moment(start).valueOf(), end:moment(end).valueOf(), company, description}
	return data
}
const unpackObjectToFormValues = ( objectToUnpack )=>{
	const {employee, start, end, company, description} = objectToUnpack
	const employeeId = employee ? employee.id : null
	const data = {employeeId, start:moment(start), end:moment(end), company, description}
	return data
}
...
const EmployeeWorkExperienceBase={unpackObjectToFormValues, menuData,settingMenuData,displayColumns,fieldLabels,renderItemOfList, stepOf, searchLocalData}
