...
const internalImageListOf = (employeeWorkExperience) =>{
<<<<<<< HEAD
...
const internalSettingListOf = (employeeWorkExperience) =>{
<<<<<<< HEAD
...
const handleTransferSearch =(targetComponent,filterKey,newRequest)=>{
  ...
  const pageNo = 1;
  const candidateReferenceService = EmployeeWorkExperienceService[candidateServiceName] 
...
const executeTrans = (employeeWorkExperience,targetComponent) =>{
  ...
  validateFieldsAndScroll((error, values) => {
    ...
    const id=employeeWorkExperience.id;
    const serviceNameToCall = transferServiceName;

    const payload = {parameters,id,serviceNameToCall}
    
    //targetComponent.setState({transferModalVisiable:false})
    dispatch({type:"_employeeWorkExperience/doJob",payload: payload})
...
const buildTransferModal = (employeeWorkExperience,targetComponent) => {
...
<Modal title={transferModalTitle}
          ...
          onOk={()=>executeTrans(employeeWorkExperience,targetComponent)}
...
const internalRenderExtraFooter = (employeeWorkExperience) =>{
	return null
}
...
<Description term="员工">{employeeWorkExperience.employee==null?"未分配":employeeWorkExperience.employee.displayName}
=======
    const userContext = null
	return (
	<DescriptionList className={styles.headerList} size="small" col="4">
<Description term="序号">{employeeWorkExperience.id}</Description> 
<Description term="员工">{employeeWorkExperience.employee==null?appLocaleName(userContext,"NotAssigned"):employeeWorkExperience.employee.displayName}
>>>>>>> 502e8b8dfc403300a992b5083e79c722e85d1854
 <Icon type="swap" onClick={()=>
  showTransferModel(targetComponent,"员工","employee","requestCandidateEmployee",
	      "transferToAnotherEmployee","anotherEmployeeId",employeeWorkExperience.employee?employeeWorkExperience.employee.id:"")} 
...
class EmployeeWorkExperiencePreference extends Component {
...
}))(Form.create()(EmployeeWorkExperiencePreference))
