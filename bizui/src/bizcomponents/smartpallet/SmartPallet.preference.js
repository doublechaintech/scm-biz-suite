...
const internalImageListOf = (smartPallet) =>{
...
const internalSettingListOf = (smartPallet) =>{
	const userContext = null
	const optionList = [ 
	]
...
const handleTransferSearch =(targetComponent,filterKey,newRequest)=>{
  ...
  const pageNo = 1;
  const candidateReferenceService = SmartPalletService[candidateServiceName] 
...
const executeTrans = (smartPallet,targetComponent) =>{
...
const buildTransferModal = (smartPallet,targetComponent) => {
...
<Modal title={transferModalTitle}
          ...
          onOk={()=>executeTrans(smartPallet,targetComponent)}
...
const internalRenderExtraHeader = (smartPallet) =>{
	return null
}
const internalRenderExtraFooter = (smartPallet) =>{
	return null
}
...
<Description term="仓库">{smartPallet.warehouse==null?appLocaleName(userContext,"NotAssigned"):smartPallet.warehouse.displayName}
 <Icon type="swap" onClick={()=>
  showTransferModel(targetComponent,"仓库","warehouse","requestCandidateWarehouse",
	      "transferToAnotherWarehouse","anotherWarehouseId",smartPallet.warehouse?smartPallet.warehouse.id:"")} 
...
class SmartPalletPreference extends Component {
...
}))(Form.create()(SmartPalletPreference))
