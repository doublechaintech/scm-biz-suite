...
const internalImageListOf = (accountingPeriod) =>{
...
const internalSettingListOf = (accountingPeriod) =>{
	const userContext = null
	const optionList = [ 
	]
...
const handleTransferSearch =(targetComponent,filterKey,newRequest)=>{
  ...
  const pageNo = 1;
  const candidateReferenceService = AccountingPeriodService[candidateServiceName] 
...
const executeTrans = (accountingPeriod,targetComponent) =>{
...
const buildTransferModal = (accountingPeriod,targetComponent) => {
...
<Modal title={transferModalTitle}
          ...
          onOk={()=>executeTrans(accountingPeriod,targetComponent)}
...
const internalRenderExtraHeader = (accountingPeriod) =>{
	return null
}
const internalRenderExtraFooter = (accountingPeriod) =>{
	return null
}
...
<Description term="账套">{accountingPeriod.accountSet==null?appLocaleName(userContext,"NotAssigned"):accountingPeriod.accountSet.displayName}
 <Icon type="swap" onClick={()=>
  showTransferModel(targetComponent,"账套","accountSet","requestCandidateAccountSet",
	      "transferToAnotherAccountSet","anotherAccountSetId",accountingPeriod.accountSet?accountingPeriod.accountSet.id:"")} 
...
class AccountingPeriodPreference extends Component {
...
}))(Form.create()(AccountingPeriodPreference))
