...
import styles from './Report.preference.less'
...
const internalImageListOf = (report) =>{
...
const internalSettingListOf = (report) =>{
...
const handleTransferSearch =(targetComponent,filterKey,newRequest)=>{
  ...
  const pageNo = 1;
  const candidateReferenceService = ReportService[candidateServiceName] 
...
const executeTrans = (report,targetComponent) =>{
  ...
  validateFieldsAndScroll((error, values) => {
    ...
    //targetComponent.setState({transferModalVisiable:false})
    dispatch({type:"_report/doJob",payload: payload})
...
const buildTransferModal = (report,targetComponent) => {
...
<Modal title={transferModalTitle}
          ...
          onOk={()=>executeTrans(report,targetComponent)}
...
const internalRenderExtraHeader = (report) =>{
	return null
}
const internalRenderExtraFooter = (report) =>{
	return null
}
...
const renderPermissionSetting = report => {
  const {ReportBase} = GlobalComponents
  return <PermissionSetting targetObject={report}  targetObjectMeta={ReportBase}/>


}


class ReportPreference extends Component {
  ...
  render() {
    ...
    const { id,displayName, reportLineCount } = this.props.report
