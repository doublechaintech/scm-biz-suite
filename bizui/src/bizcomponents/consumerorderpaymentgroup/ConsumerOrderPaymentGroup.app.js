...
import styles from './ConsumerOrderPaymentGroup.app.less'
...
class ConsumerOrderPaymentGroupBizApp extends React.PureComponent {
constructor(props) {
    super(props)
     this.state = {
 ...
 getNavMenuItems = (targetObject, style, customTheme) => {
    ...
    return (
	  <Menu
       ...
       <Menu.Item key="workbench">
        <Link to={`/consumerOrderPaymentGroup/${this.props.consumerOrderPaymentGroup.id}/workbench`}><Icon type="solution" style={{marginRight:"20px"}}/><span>工作台</span></Link>
      </Menu.Item>
        ...
        {filteredMenuItemsGroup(targetObject,this).map((groupedMenuItem,index)=>{
          return(
    <SubMenu id={`submenu-vg${index}`}  key={`vg${index}`} title={<span><Icon type={viewGroupIconNameOf('consumer_order_payment_group',`${groupedMenuItem.viewGroup}`)} style={{marginRight:"20px"}} /><span>{viewGroupDisplayNameOf('consumer_order_payment_group',`${groupedMenuItem.viewGroup}`)}</span></span>} >
  ...
  getRequestTypeStepForm = () => {
    ...
    return connect(state => ({
      selectedRows: state._consumerOrderPaymentGroup.selectedRows,
      role: "cq",
      currentUpdateIndex: state._consumerOrderPaymentGroup.currentUpdateIndex,
      owner: { type: '_consumerOrderPaymentGroup', id: state._consumerOrderPaymentGroup.id, listName: 'nolist', ref:state._consumerOrderPaymentGroup, listDisplayName: appLocaleName(userContext,"List") }, // this is for model namespace and
  ...
  buildRouters = () =>{
    const {ConsumerOrderPaymentGroupWorkbench} = GlobalComponents

    const {ConsumerOrderPaymentGroupDashboard} = GlobalComponents
  	const {ConsumerOrderPaymentGroupPermission} = GlobalComponents
  	const {ConsumerOrderPaymentGroupProfile} = GlobalComponents


    const routers=[
    {path:"/consumerOrderPaymentGroup/:id/workbench", component: ConsumerOrderPaymentGroupWorkbench},
    
  	{path:"/consumerOrderPaymentGroup/:id/profile", component: ConsumerOrderPaymentGroupProfile},
  	{path:"/consumerOrderPaymentGroup/:id/permission", component: ConsumerOrderPaymentGroupPermission},
  ...
  handleOpenChange = (openKeys) => {
   ...
   render() {
    ...
    const {searchLocalData}=GlobalComponents.ConsumerOrderPaymentGroupBase
    ...
    const renderMenuSwitch=()=>{
     ...
     <Layout>
      ...
      {this.state.showSearch&&(
        ...
        <div style={{backgroundColor:'black'}}  onClick={()=>hideSearchResult()}  >{searchLocalData(this.props.consumerOrderPaymentGroup,this.state.searchKeyword)}</div>
       ...
       <Sider
        ...
        >
         ...
         {this.getNavMenuItems(this.props.consumerOrderPaymentGroup,"inline","dark")}
