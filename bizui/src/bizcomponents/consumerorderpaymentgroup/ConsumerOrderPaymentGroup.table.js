...
import styles from './ConsumerOrderPaymentGroup.table.less'
...
class ConsumerOrderPaymentGroupTable extends PureComponent {
  state = {
    selectedRowKeys: [],
  }
...
<<<<<<< HEAD
 calcDisplayColumns=()=>{
    ...
    const {displayColumns} = ConsumerOrderPaymentGroupBase
    if(!referenceName){
    ...
    const operationColumn={
      ...
      render: (text, record) => (
        <span>
         ...
         { hasReadPermission(metaInfo)&&<Link to={`/consumerOrderPaymentGroup/${record.id}/dashboard`}>{'查看'}</Link>}
    ...
    const operationColumn={
      ...
      render: (text, record) => (
        <span>
          ...
          { hasReadPermission(metaInfo)&&<Link to={`/consumerOrderPaymentGroup/${record.id}/dashboard`}>{appLocaleName(userContext,"View")}</Link>}
...
export default ConsumerOrderPaymentGroupTable
