...
import styles from './KeypairIdentify.updateform.less'
...
class KeypairIdentifyUpdateForm extends Component {
  state = {
  ...
  render() {
    ...
    const submitUpdateForm = () => {
      validateFieldsAndScroll((error, values) => {
        ...
        const keypairIdentifyId = values.id
        const imagesValues = mapBackToImageValues(convertedImagesValues)
        const parameters = { ...values, keypairIdentifyId, ...imagesValues }
    ...
    const submitUpdateFormAndContinue = () => {
      validateFieldsAndScroll((error, values) => {
        ...
        dispatch({
          type: `${owner.type}/updateKeypairIdentify`,
          payload: {
            id: owner.id,
            type: 'keypairIdentify',
            parameters,
            selectedRows,
    ...
    const skipToNext = () => {
      ...
      const newIndex = currentUpdateIndex + 1
      dispatch({
        type: `${owner.type}/gotoNextKeypairIdentifyUpdateRow`,
        payload: {
          id: owner.id,
          type: 'keypairIdentify',
          selectedRows,
	...
	const internalRenderTitle = () =>{
      ...
      return (<div>{linkComp}{appLocaleName(userContext,"Update")}密钥对识别: {(currentUpdateIndex+1)}/{selectedRows.length}</div>)
    }
	...
	return (
      ...
      >
       ...
       <Card title={window.trans('keypair_identify')} className={styles.card} bordered={false}>
          <Form >
          	<Row gutter={16}> 
              <Col lg={24} md={24} sm={24}>
                <Form.Item>
...
}))(Form.create()(KeypairIdentifyUpdateForm))
