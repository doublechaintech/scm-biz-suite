...
import styles from './KeypairIdentify.app.less'
...
class KeypairIdentifyBizApp extends React.PureComponent {
constructor(props) {
    super(props)
     this.state = {
 ...
 getNavMenuItems = (targetObject, style, customTheme) => {
	  ...
	  <Menu
       ...
       >
             ...
             <Menu.Item key="dashboard">
               <Link to={`/keypairIdentify/${this.props.keypairIdentify.id}/dashboard`}><Icon type="dashboard" style={{marginRight:"20px"}}/><span>{appLocaleName(userContext,"Dashboard")}</span></Link>
             </Menu.Item>
  ...
  getRequestTypeStepForm = () => {
    ...
    return connect(state => ({
      selectedRows: state._keypairIdentify.selectedRows,
      role: "cq",
      currentUpdateIndex: state._keypairIdentify.currentUpdateIndex,
      owner: { type: '_keypairIdentify', id: state._keypairIdentify.id, listName: 'nolist', ref:state._keypairIdentify, listDisplayName: appLocaleName(userContext,"List") }, // this is for model namespace and
  ...
  buildRouters = () =>{
  	const {KeypairIdentifyDashboard} = GlobalComponents
  	const {KeypairIdentifyPermission} = GlobalComponents
  	const {KeypairIdentifyProfile} = GlobalComponents
  	
  	
  	const routers=[
  	{path:"/keypairIdentify/:id/dashboard", component: KeypairIdentifyDashboard},
  	{path:"/keypairIdentify/:id/profile", component: KeypairIdentifyProfile},
  	{path:"/keypairIdentify/:id/permission", component: KeypairIdentifyPermission},
  ...
  handleOpenChange = (openKeys) => {
   ...
   render() {
    ...
    const {searchLocalData}=GlobalComponents.KeypairIdentifyBase
    ...
    const renderMenuSwitch=()=>{
     ...
     <Layout>
      ...
      {this.state.showSearch&&(
        ...
        <div style={{backgroundColor:'black'}}  onClick={()=>hideSearchResult()}  >{searchLocalData(this.props.keypairIdentify,this.state.searchKeyword)}</div>
       ...
       <Sider
        ...
        >
         ...
         {this.getNavMenuItems(this.props.keypairIdentify,"inline","dark")}
