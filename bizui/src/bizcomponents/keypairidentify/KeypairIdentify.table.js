...
import styles from './KeypairIdentify.table.less'
...
class KeypairIdentifyTable extends PureComponent {
  state = {
    selectedRowKeys: [],
  }
  ...
  enhanceColumnsWithSorter=()=>{
    const {displayColumns} = KeypairIdentifyBase
    ...
    const operationColumn={
      ...
      render: (text, record) => (
        <span>
          ...
          { hasReadPermission(metaInfo)&&<Link to={`/keypairIdentify/${record.id}/dashboard`}>{appLocaleName(userContext,"View")}</Link>}
...
export default KeypairIdentifyTable
