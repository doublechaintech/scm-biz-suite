 ...
 getNavMenuItems = (targetObject, style, customTheme) => {
    ...
    return (
	  <Menu
       ...
       <Menu.Item key="workbench">
        <Link to={`/catalog/${this.props.catalog.id}/workbench`}><Icon type="solution" style={{marginRight:"20px"}}/><span>工作台</span></Link>
      </Menu.Item>
        ...
        {filteredMenuItemsGroup(targetObject,this).map((groupedMenuItem,index)=>{
          return(
    <SubMenu id={`submenu-vg${index}`}  key={`vg${index}`} title={<span><Icon type={viewGroupIconNameOf('catalog',`${groupedMenuItem.viewGroup}`)} style={{marginRight:"20px"}} /><span>{viewGroupDisplayNameOf('catalog',`${groupedMenuItem.viewGroup}`)}</span></span>} >
  ...
  getLevelOneCategorySearch = () => {
    ...
    return connect(state => ({
      rule: state.rule,
      name: window.mtrans('level_one_category','catalog.level_one_category_list',false),
      ...
      returnURL: `/catalog/${state._catalog.id}/workbench`,
  ...
  getLevelOneCategoryCreateForm = () => {
    ...
    return connect(state => ({
      ...
      metaInfo: state._catalog.levelOneCategoryListMetaInfo,
      count: state._catalog.levelOneCategoryCount,
      returnURL: `/catalog/${state._catalog.id}/list`,
      currentPage: state._catalog.levelOneCategoryCurrentPageNumber,
      searchFormParameters: state._catalog.levelOneCategorySearchFormParameters,
      loading: state._catalog.loading,
      owner: { type: '_catalog', id: state._catalog.id || this.getOwnerId(), referenceName: 'catalog', listName: 'levelOneCategoryList', ref:state._catalog, listDisplayName: appLocaleName(userContext,"List")}, // this is for model namespace and
  ...
  getLevelOneCategoryUpdateForm = () => {
    ...
    return connect(state => ({
      ...
      role: "levelOneCategory",
      currentUpdateIndex: state._catalog.currentUpdateIndex || 0,
      owner: { type: '_catalog', id: state._catalog.id || this.getOwnerId(), listName: 'levelOneCategoryList', ref:state._catalog, listDisplayName: appLocaleName(userContext,"List") }, // this is for model namespace and
  ...
  buildRouters = () =>{
    const {CatalogWorkbench} = GlobalComponents

    const {CatalogDashboard} = GlobalComponents
  	const {CatalogPermission} = GlobalComponents
  	const {CatalogProfile} = GlobalComponents


    const routers=[
    {path:"/catalog/:id/workbench", component: CatalogWorkbench},
    
  	{path:"/catalog/:id/profile", component: CatalogProfile},
  	{path:"/catalog/:id/permission", component: CatalogPermission},
    ...
    const renderMenuSwitch=()=>{
     ...
     <Layout>
      ...
      {this.state.showSearch&&(
        ...
        <div style={{backgroundColor:'black'}}  onClick={()=>hideSearchResult()}  >{searchLocalData(this.props.catalog,this.state.searchKeyword)}</div>
       ...
       <Sider
        ...
        >
         ...
         {this.getNavMenuItems(this.props.catalog,"inline","dark")}
