...
import styles from './Catalog.workbench.less'
...
const renderSettingMenuItem = (item,cardsData,targetComponent) =>{
  ...
  return (<Menu.Item key={item.name}>
      <Link to={`/catalog/${targetComponent.props.catalog.id}/list/${item.name}/${item.displayName}/`}>
...
const internalSummaryOf = (cardsData,targetComponent) =>{
	...
	const catalog = cardsData.cardsSource
	const {CatalogService} = GlobalComponents
	const userContext = null
	return (
	<div>
	<DescriptionList className={styles.headerList} size="small" col="4">
<Description term="ID" style={{wordBreak: 'break-all'}}>{catalog.id}</Description> 
...
class CatalogWorkbench extends Component {
 ...
 state = {
  ...
  render() {
    ...
    const { id,displayName, levelOneCategoryListMetaInfo, levelOneCategoryCount } = this.props.catalog
    if(!this.props.catalog.class){
      return null
    }
    const returnURL = this.props.returnURL

    const cardsData = {cardsName:window.trans('catalog'),cardsFor: "catalog",
    	cardsSource: this.props.catalog,returnURL,displayName,
  		subItems: [
{name: 'levelOneCategoryList', displayName: window.mtrans('level_one_category','catalog.level_one_category_list',false) ,viewGroup:'__no_group', type:'levelOneCategory',count:levelOneCategoryCount,addFunction: true, role: 'levelOneCategory', metaInfo: levelOneCategoryListMetaInfo, renderItem: GlobalComponents.LevelOneCategoryBase.renderItemOfList},
