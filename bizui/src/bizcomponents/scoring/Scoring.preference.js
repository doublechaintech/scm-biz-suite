...
const internalImageListOf = (scoring) =>{
...
const internalSettingListOf = (scoring) =>{
...
const handleTransferSearch =(targetComponent,filterKey,newRequest)=>{
  ...
  const pageNo = 1;
  const candidateReferenceService = ScoringService[candidateServiceName] 
...
const executeTrans = (scoring,targetComponent) =>{
  ...
  validateFieldsAndScroll((error, values) => {
    ...
    //targetComponent.setState({transferModalVisiable:false})
    dispatch({type:"_scoring/doJob",payload: payload})
...
const buildTransferModal = (scoring,targetComponent) => {
...
<Modal title={transferModalTitle}
          ...
          onOk={()=>executeTrans(scoring,targetComponent)}
...
const internalRenderExtraHeader = (scoring) =>{
	return null
}
const internalRenderExtraFooter = (scoring) =>{
	return null
}
...
class ScoringPreference extends Component {
