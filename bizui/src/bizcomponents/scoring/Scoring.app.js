 ...
 getNavMenuItems = (targetObject, style, customTheme) => {
    ...
    return (
	  <Menu
       ...
       <Menu.Item key="workbench">
        <Link to={`/scoring/${this.props.scoring.id}/workbench`}><Icon type="solution" style={{marginRight:"20px"}}/><span>工作台</span></Link>
      </Menu.Item>
        ...
        {filteredMenuItemsGroup(targetObject,this).map((groupedMenuItem,index)=>{
          return(
    <SubMenu id={`submenu-vg${index}`}  key={`vg${index}`} title={<span><Icon type={viewGroupIconNameOf('scoring',`${groupedMenuItem.viewGroup}`)} style={{marginRight:"20px"}} /><span>{viewGroupDisplayNameOf('scoring',`${groupedMenuItem.viewGroup}`)}</span></span>} >
  ...
  getEmployeeCompanyTrainingSearch = () => {
    ...
    return connect(state => ({
      rule: state.rule,
      name: window.mtrans('employee_company_training','scoring.employee_company_training_list',false),
      ...
      returnURL: `/scoring/${state._scoring.id}/workbench`,
  ...
  getEmployeeCompanyTrainingCreateForm = () => {
    ...
    return connect(state => ({
      ...
      searchFormParameters: state._scoring.employeeCompanyTrainingSearchFormParameters,
      loading: state._scoring.loading,
      owner: { type: '_scoring', id: state._scoring.id || this.getOwnerId(), referenceName: 'scoring', listName: 'employeeCompanyTrainingList', ref:state._scoring, listDisplayName: appLocaleName(userContext,"List")}, // this is for model namespace and
  ...
  getEmployeeCompanyTrainingUpdateForm = () => {
    ...
    return connect(state => ({
      ...
      role: "employeeCompanyTraining",
      currentUpdateIndex: state._scoring.currentUpdateIndex || 0,
      owner: { type: '_scoring', id: state._scoring.id || this.getOwnerId(), listName: 'employeeCompanyTrainingList', ref:state._scoring, listDisplayName: appLocaleName(userContext,"List") }, // this is for model namespace and
  ...
  getRequestTypeStepForm = () => {
    ...
    return connect(state => ({
      ...
      owner: { type: '_scoring', id: state._scoring.id, listName: 'nolist', ref:state._scoring, listDisplayName: appLocaleName(userContext,"List") }, // this is for model namespace and
  ...
  buildRouters = () =>{
    const {ScoringWorkbench} = GlobalComponents
    ...
    const routers=[
    {path:"/scoring/:id/workbench", component: ScoringWorkbench},
      ...
      {this.state.showSearch&&(
       ...
       <Sider
        ...
        >
         ...
         {this.getNavMenuItems(this.props.scoring,"inline","dark")}
