...
import styles from './Scoring.workbench.less'
...
const renderSettingMenuItem = (item,cardsData,targetComponent) =>{
  ...
  return (<Menu.Item key={item.name}>
      <Link to={`/scoring/${targetComponent.props.scoring.id}/list/${item.name}/${item.displayName}/`}>
...
const renderSettingMenu = (cardsData,targetComponent) =>{
  ...
  return (<Menu>
    	<Menu.Item key="profile">
  			<Link to={`/scoring/${targetComponent.props.scoring.id}/permission`}><Icon type="safety-certificate" theme="twoTone" twoToneColor="#52c41a"/><span>{appLocaleName(userContext,"Permission")}</span></Link>
		</Menu.Item>
		<Menu.Divider />
...
const internalSummaryOf = (cardsData,targetComponent) =>{
	...
	const scoring = cardsData.cardsSource
	const {ScoringService} = GlobalComponents
	const userContext = null
	return (
	<div>
	<DescriptionList className={styles.headerList} size="small" col="4">
<Description term="ID" style={{wordBreak: 'break-all'}}>{scoring.id}</Description> 
<Description term="由谁打分" style={{wordBreak: 'break-all'}}>{scoring.scoredBy}</Description> 
<Description term="分数" style={{wordBreak: 'break-all'}}>{scoring.score}</Description> 
<Description term="评论" style={{wordBreak: 'break-all'}}>{scoring.comment}</Description> 
...
class ScoringWorkbench extends Component {
