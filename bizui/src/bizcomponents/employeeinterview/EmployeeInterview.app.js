...
import styles from './EmployeeInterview.app.less'
...
class EmployeeInterviewBizApp extends React.PureComponent {
constructor(props) {
    super(props)
     this.state = {
 ...
 getNavMenuItems = (targetObject, style, customTheme) => {
    ...
    return (
	  <Menu
       ...
       <Menu.Item key="workbench">
        <Link to={`/employeeInterview/${this.props.employeeInterview.id}/workbench`}><Icon type="solution" style={{marginRight:"20px"}}/><span>工作台</span></Link>
      </Menu.Item>
        ...
        {filteredMenuItemsGroup(targetObject,this).map((groupedMenuItem,index)=>{
          return(
    <SubMenu id={`submenu-vg${index}`}  key={`vg${index}`} title={<span><Icon type={viewGroupIconNameOf('employee_interview',`${groupedMenuItem.viewGroup}`)} style={{marginRight:"20px"}} /><span>{viewGroupDisplayNameOf('employee_interview',`${groupedMenuItem.viewGroup}`)}</span></span>} >
  ...
  getRequestTypeStepForm = () => {
    ...
    return connect(state => ({
      selectedRows: state._employeeInterview.selectedRows,
      role: "cq",
      currentUpdateIndex: state._employeeInterview.currentUpdateIndex,
      owner: { type: '_employeeInterview', id: state._employeeInterview.id, listName: 'nolist', ref:state._employeeInterview, listDisplayName: appLocaleName(userContext,"List") }, // this is for model namespace and
  ...
  buildRouters = () =>{
    const {EmployeeInterviewWorkbench} = GlobalComponents

    const {EmployeeInterviewDashboard} = GlobalComponents
  	const {EmployeeInterviewPermission} = GlobalComponents
  	const {EmployeeInterviewProfile} = GlobalComponents


    const routers=[
    {path:"/employeeInterview/:id/workbench", component: EmployeeInterviewWorkbench},
    
  	{path:"/employeeInterview/:id/profile", component: EmployeeInterviewProfile},
  	{path:"/employeeInterview/:id/permission", component: EmployeeInterviewPermission},
  ...
  handleOpenChange = (openKeys) => {
   ...
   render() {
    ...
    const {searchLocalData}=GlobalComponents.EmployeeInterviewBase
    ...
    const renderMenuSwitch=()=>{
     ...
     <Layout>
      ...
      {this.state.showSearch&&(
        ...
        <div style={{backgroundColor:'black'}}  onClick={()=>hideSearchResult()}  >{searchLocalData(this.props.employeeInterview,this.state.searchKeyword)}</div>
       ...
       <Sider
        ...
        >
         ...
         {this.getNavMenuItems(this.props.employeeInterview,"inline","dark")}
