...
import styles from './SupplierSpace.updateform.less'
...
class SupplierSpaceUpdateForm extends Component {
  state = {
  ...
  render() {
    ...
    const submitUpdateForm = () => {
      validateFieldsAndScroll((error, values) => {
        ...
        const supplierSpaceId = values.id

        const parameters = { ...values, supplierSpaceId, }
    ...
    const submitUpdateFormAndContinue = () => {
      validateFieldsAndScroll((error, values) => {
        ...
        dispatch({
          type: `${owner.type}/updateSupplierSpace`,
          payload: {
            id: owner.id,
            type: 'supplierSpace',
            parameters,
    ...
    const skipToNext = () => {
      ...
      const newIndex = currentUpdateIndex + 1
      dispatch({
        type: `${owner.type}/gotoNextSupplierSpaceUpdateRow`,
        payload: {
          id: owner.id,
          type: 'supplierSpace',
          selectedRows,
	...
	const internalRenderTitle = () =>{
      ...
      return (<div>{linkComp}{appLocaleName(userContext,"Update")}供应商的空间: {(currentUpdateIndex+1)}/{selectedRows.length}</div>)
    }
...
}))(Form.create()(SupplierSpaceUpdateForm))
