 ...
 getNavMenuItems = (targetObject, style, customTheme) => {
    ...
    return (
	  <Menu
       ...
       <Menu.Item key="workbench">
        <Link to={`/supplierSpace/${this.props.supplierSpace.id}/workbench`}><Icon type="solution" style={{marginRight:"20px"}}/><span>工作台</span></Link>
      </Menu.Item>
        ...
        {filteredMenuItemsGroup(targetObject,this).map((groupedMenuItem,index)=>{
          return(
    <SubMenu id={`submenu-vg${index}`}  key={`vg${index}`} title={<span><Icon type={viewGroupIconNameOf('supplier_space',`${groupedMenuItem.viewGroup}`)} style={{marginRight:"20px"}} /><span>{viewGroupDisplayNameOf('supplier_space',`${groupedMenuItem.viewGroup}`)}</span></span>} >
  ...
  getGoodsShelfSearch = () => {
    ...
    return connect(state => ({
      rule: state.rule,
      name: window.mtrans('goods_shelf','supplier_space.goods_shelf_list',false),
      role: "goodsShelf",
      ...
      returnURL: `/supplierSpace/${state._supplierSpace.id}/workbench`,
  ...
  getGoodsShelfCreateForm = () => {
    ...
    return connect(state => ({
      ...
      searchFormParameters: state._supplierSpace.goodsShelfSearchFormParameters,
      loading: state._supplierSpace.loading,
      owner: { type: '_supplierSpace', id: state._supplierSpace.id || this.getOwnerId(), referenceName: 'supplierSpace', listName: 'goodsShelfList', ref:state._supplierSpace, listDisplayName: appLocaleName(userContext,"List")}, // this is for model namespace and
  ...
  getGoodsShelfUpdateForm = () => {
    ...
    return connect(state => ({
      ...
      role: "goodsShelf",
      currentUpdateIndex: state._supplierSpace.currentUpdateIndex || 0,
      owner: { type: '_supplierSpace', id: state._supplierSpace.id || this.getOwnerId(), listName: 'goodsShelfList', ref:state._supplierSpace, listDisplayName: appLocaleName(userContext,"List") }, // this is for model namespace and
  ...
  getRequestTypeStepForm = () => {
    ...
    return connect(state => ({
      ...
      owner: { type: '_supplierSpace', id: state._supplierSpace.id, listName: 'nolist', ref:state._supplierSpace, listDisplayName: appLocaleName(userContext,"List") }, // this is for model namespace and
  ...
  buildRouters = () =>{
    const {SupplierSpaceWorkbench} = GlobalComponents
    ...
    const routers=[
    {path:"/supplierSpace/:id/workbench", component: SupplierSpaceWorkbench},
  ...
  handleOpenChange = (openKeys) => {
   ...
   render() {
    ...
    const {searchLocalData}=GlobalComponents.SupplierSpaceBase
    ...
    const renderMenuSwitch=()=>{
     ...
     <Layout>
      ...
      {this.state.showSearch&&(
        ...
        <div style={{backgroundColor:'black'}}  onClick={()=>hideSearchResult()}  >{searchLocalData(this.props.supplierSpace,this.state.searchKeyword)}</div>
       ...
       <Sider
        ...
        >
         ...
         {this.getNavMenuItems(this.props.supplierSpace,"inline","dark")}
