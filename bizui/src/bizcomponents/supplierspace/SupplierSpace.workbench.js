...
import styles from './SupplierSpace.workbench.less'
...
const internalSummaryOf = (cardsData,targetComponent) =>{
	...
	const supplierSpace = cardsData.cardsSource
	const {SupplierSpaceService} = GlobalComponents
	const userContext = null
	return (
	<div>
	<DescriptionList className={styles.headerList} size="small" col="4">
<Description term="ID" style={{wordBreak: 'break-all'}}>{supplierSpace.id}</Description> 
...
<Description term="更新于">{ moment(supplierSpace.lastUpdateTime).format('YYYY-MM-DD HH:mm')}</Description> 
...
class SupplierSpaceWorkbench extends Component {
...
}))(Form.create()(SupplierSpaceWorkbench))
