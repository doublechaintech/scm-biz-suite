...
const handleTransferSearch =(targetComponent,filterKey,newRequest)=>{
  ...
  const pageNo = 1;
  const candidateReferenceService = SupplierSpaceService[candidateServiceName] 
...
const executeTrans = (supplierSpace,targetComponent) =>{
...
const buildTransferModal = (supplierSpace,targetComponent) => {
...
<Modal title={transferModalTitle}
          ...
          onOk={()=>executeTrans(supplierSpace,targetComponent)}
...
const internalRenderExtraHeader = (supplierSpace) =>{
	return null
}
const internalRenderExtraFooter = (supplierSpace) =>{
	return null
}
...
class SupplierSpacePreference extends Component {
...
}))(Form.create()(SupplierSpacePreference))
