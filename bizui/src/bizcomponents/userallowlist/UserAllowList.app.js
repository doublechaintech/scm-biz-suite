...
import styles from './UserAllowList.app.less'
...
class UserAllowListBizApp extends React.PureComponent {
constructor(props) {
    super(props)
     this.state = {
 ...
 getNavMenuItems = (targetObject, style, customTheme) => {
    ...
    return (
	  <Menu
       ...
       <Menu.Item key="workbench">
        <Link to={`/userAllowList/${this.props.userAllowList.id}/workbench`}><Icon type="solution" style={{marginRight:"20px"}}/><span>工作台</span></Link>
      </Menu.Item>
        ...
        {filteredMenuItemsGroup(targetObject,this).map((groupedMenuItem,index)=>{
          return(
    <SubMenu id={`submenu-vg${index}`}  key={`vg${index}`} title={<span><Icon type={viewGroupIconNameOf('user_allow_list',`${groupedMenuItem.viewGroup}`)} style={{marginRight:"20px"}} /><span>{viewGroupDisplayNameOf('user_allow_list',`${groupedMenuItem.viewGroup}`)}</span></span>} >
  ...
  getRequestTypeStepForm = () => {
    ...
    return connect(state => ({
      selectedRows: state._userAllowList.selectedRows,
      role: "cq",
      currentUpdateIndex: state._userAllowList.currentUpdateIndex,
      owner: { type: '_userAllowList', id: state._userAllowList.id, listName: 'nolist', ref:state._userAllowList, listDisplayName: appLocaleName(userContext,"List") }, // this is for model namespace and
  ...
  buildRouters = () =>{
    const {UserAllowListWorkbench} = GlobalComponents

    const {UserAllowListDashboard} = GlobalComponents
  	const {UserAllowListPermission} = GlobalComponents
  	const {UserAllowListProfile} = GlobalComponents


    const routers=[
    {path:"/userAllowList/:id/workbench", component: UserAllowListWorkbench},
    
  	{path:"/userAllowList/:id/profile", component: UserAllowListProfile},
  	{path:"/userAllowList/:id/permission", component: UserAllowListPermission},
  ...
  handleOpenChange = (openKeys) => {
   ...
   render() {
    ...
    const {searchLocalData}=GlobalComponents.UserAllowListBase
    ...
    const renderMenuSwitch=()=>{
     ...
     <Layout>
      ...
      {this.state.showSearch&&(
        ...
        <div style={{backgroundColor:'black'}}  onClick={()=>hideSearchResult()}  >{searchLocalData(this.props.userAllowList,this.state.searchKeyword)}</div>
       ...
       <Sider
        ...
        >
         ...
         {this.getNavMenuItems(this.props.userAllowList,"inline","dark")}
