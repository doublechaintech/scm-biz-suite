...
const imageList =(formField)=>{return [
	 ]}

const internalImageListOf = (formField) =>defaultImageListOf(formField,imageList)

const optionList =(formField)=>{return [ 
...
const internalSettingListOf = (formField) =>defaultSettingListOf(formField, optionList)
...
const renderSettingMenuItem = (item,cardsData,targetComponent) =>{
  ...
  return (<Menu.Item key={item.name}>
      <Link to={`/formField/${targetComponent.props.formField.id}/list/${item.name}/${item.displayName}/`}>
...
const renderSettingMenu = (cardsData,targetComponent) =>{
  ...
  return (<Menu>
    	<Menu.Item key="profile">
  			<Link to={`/formField/${targetComponent.props.formField.id}/permission`}><Icon type="safety-certificate" theme="twoTone" twoToneColor="#52c41a"/><span>{appLocaleName(userContext,"Permission")}</span></Link>
		</Menu.Item>
		<Menu.Divider />
...
}))(Form.create()(FormFieldDashboard))
