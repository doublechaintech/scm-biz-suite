...
import styles from './EmployeeSalarySheet.app.less'
...
class EmployeeSalarySheetBizApp extends React.PureComponent {
constructor(props) {
    super(props)
     this.state = {
 ...
 getNavMenuItems = (targetObject, style, customTheme) => {
    ...
    return (
	  <Menu
       ...
       <Menu.Item key="workbench">
        <Link to={`/employeeSalarySheet/${this.props.employeeSalarySheet.id}/workbench`}><Icon type="solution" style={{marginRight:"20px"}}/><span>工作台</span></Link>
      </Menu.Item>
        ...
        {filteredMenuItemsGroup(targetObject,this).map((groupedMenuItem,index)=>{
          return(
    <SubMenu id={`submenu-vg${index}`}  key={`vg${index}`} title={<span><Icon type={viewGroupIconNameOf('employee_salary_sheet',`${groupedMenuItem.viewGroup}`)} style={{marginRight:"20px"}} /><span>{viewGroupDisplayNameOf('employee_salary_sheet',`${groupedMenuItem.viewGroup}`)}</span></span>} >
  ...
  getRequestTypeStepForm = () => {
    ...
    return connect(state => ({
      selectedRows: state._employeeSalarySheet.selectedRows,
      role: "cq",
      currentUpdateIndex: state._employeeSalarySheet.currentUpdateIndex,
      owner: { type: '_employeeSalarySheet', id: state._employeeSalarySheet.id, listName: 'nolist', ref:state._employeeSalarySheet, listDisplayName: appLocaleName(userContext,"List") }, // this is for model namespace and
  ...
  buildRouters = () =>{
    const {EmployeeSalarySheetWorkbench} = GlobalComponents

    const {EmployeeSalarySheetDashboard} = GlobalComponents
  	const {EmployeeSalarySheetPermission} = GlobalComponents
  	const {EmployeeSalarySheetProfile} = GlobalComponents


    const routers=[
    {path:"/employeeSalarySheet/:id/workbench", component: EmployeeSalarySheetWorkbench},
    
  	{path:"/employeeSalarySheet/:id/profile", component: EmployeeSalarySheetProfile},
  	{path:"/employeeSalarySheet/:id/permission", component: EmployeeSalarySheetPermission},
  ...
  handleOpenChange = (openKeys) => {
   ...
   render() {
    ...
    const {searchLocalData}=GlobalComponents.EmployeeSalarySheetBase
    ...
    const renderMenuSwitch=()=>{
     ...
     <Layout>
      ...
      {this.state.showSearch&&(
        ...
        <div style={{backgroundColor:'black'}}  onClick={()=>hideSearchResult()}  >{searchLocalData(this.props.employeeSalarySheet,this.state.searchKeyword)}</div>
       ...
       <Sider
        ...
        >
         ...
         {this.getNavMenuItems(this.props.employeeSalarySheet,"inline","dark")}
