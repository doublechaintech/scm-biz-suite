...
const handleTransferSearch =(targetComponent,filterKey,newRequest)=>{
  ...
  const pageNo = 1;
  const candidateReferenceService = EmployeeSalarySheetService[candidateServiceName] 
...
const executeTrans = (employeeSalarySheet,targetComponent) =>{
...
const buildTransferModal = (employeeSalarySheet,targetComponent) => {
...
<Modal title={transferModalTitle}
          ...
          onOk={()=>executeTrans(employeeSalarySheet,targetComponent)}
...
const internalRenderExtraFooter = (employeeSalarySheet) =>{
	return null
}
...
class EmployeeSalarySheetPreference extends Component {
...
}))(Form.create()(EmployeeSalarySheetPreference))
