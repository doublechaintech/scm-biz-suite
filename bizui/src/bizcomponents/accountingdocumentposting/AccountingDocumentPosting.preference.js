...
import styles from './AccountingDocumentPosting.preference.less'
...
const internalImageListOf = (accountingDocumentPosting) =>{
...
const internalSettingListOf = (accountingDocumentPosting) =>{
...
const handleTransferSearch =(targetComponent,filterKey,newRequest)=>{
  ...
  const pageNo = 1;
  const candidateReferenceService = AccountingDocumentPostingService[candidateServiceName] 
...
const executeTrans = (accountingDocumentPosting,targetComponent) =>{
  ...
  validateFieldsAndScroll((error, values) => {
    ...
    const id=accountingDocumentPosting.id;
    const serviceNameToCall = transferServiceName;

    const payload = {parameters,id,serviceNameToCall}
    
    //targetComponent.setState({transferModalVisiable:false})
    dispatch({type:"_accountingDocumentPosting/doJob",payload: payload})
...
const buildTransferModal = (accountingDocumentPosting,targetComponent) => {
...
<Modal title={transferModalTitle}
          ...
          onOk={()=>executeTrans(accountingDocumentPosting,targetComponent)}
...
const internalRenderExtraHeader = (accountingDocumentPosting) =>{
	return null
}
const internalRenderExtraFooter = (accountingDocumentPosting) =>{
	return null
}
...
class AccountingDocumentPostingPreference extends Component {
  ...
  render() {
    ...
    const { id,displayName, accountingDocumentCount } = this.props.accountingDocumentPosting
...
}))(Form.create()(AccountingDocumentPostingPreference))
