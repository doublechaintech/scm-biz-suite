...
<<<<<<< HEAD
  { title: '序号', debugtype: 'string', dataIndex: 'id', width: '20', render: (text, record)=>renderTextCell(text,record,'accountingDocumentPosting') },
...
const AccountingDocumentPostingBase={menuData,displayColumns,fieldLabels,displayColumns}
=======
  ...
  return (
    <div key={accountingDocumentPosting.id}>
      ...
      <DescriptionList  key={accountingDocumentPosting.id} size="small" col="4">
        ...
        <Description term="制造日期"><div>{ moment(accountingDocumentPosting.makeDate).format('YYYY-MM-DD')}</div></Description> 
