...
import styles from './AccountingDocumentPosting.table.less'
...
class AccountingDocumentPostingTable extends PureComponent {
  state = {
    selectedRowKeys: [],
  }
...
<<<<<<< HEAD
 calcDisplayColumns=()=>{
    ...
    const {displayColumns} = AccountingDocumentPostingBase
    if(!referenceName){
    ...
    const operationColumn={
      ...
      render: (text, record) => (
        <span>
         ...
         { hasReadPermission(metaInfo)&&<Link to={`/accountingDocumentPosting/${record.id}/dashboard`}>{'查看'}</Link>}
    ...
    const operationColumn={
      ...
      render: (text, record) => (
        <span>
          ...
          { hasReadPermission(metaInfo)&&<Link to={`/accountingDocumentPosting/${record.id}/dashboard`}>{appLocaleName(userContext,"View")}</Link>}
...
export default AccountingDocumentPostingTable
