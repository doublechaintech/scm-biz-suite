...
import styles from './SkillType.app.less'
 ...
 getNavMenuItems = (targetObject, style, customTheme) => {
    ...
    return (
	  <Menu
       ...
       <Menu.Item key="workbench">
        <Link to={`/skillType/${this.props.skillType.id}/workbench`}><Icon type="solution" style={{marginRight:"20px"}}/><span>工作台</span></Link>
      </Menu.Item>
        ...
        {filteredMenuItemsGroup(targetObject,this).map((groupedMenuItem,index)=>{
          return(
    <SubMenu id={`submenu-vg${index}`}  key={`vg${index}`} title={<span><Icon type={viewGroupIconNameOf('skill_type',`${groupedMenuItem.viewGroup}`)} style={{marginRight:"20px"}} /><span>{viewGroupDisplayNameOf('skill_type',`${groupedMenuItem.viewGroup}`)}</span></span>} >
  ...
  getEmployeeSkillSearch = () => {
    ...
    return connect(state => ({
      rule: state.rule,
      name: window.mtrans('employee_skill','skill_type.employee_skill_list',false),
      role: "employeeSkill",
      ...
      returnURL: `/skillType/${state._skillType.id}/workbench`,
      ...
      expandForm: state._skillType.expandForm,
      loading: state._skillType.loading,
      partialList: state._skillType.partialList,
      owner: { type: '_skillType', id: state._skillType.id,
      referenceName: 'skillType',
      listName: 'employeeSkillList', ref:state._skillType,
  ...
  getEmployeeSkillCreateForm = () => {
    ...
    return connect(state => ({
      ...
      metaInfo: state._skillType.employeeSkillListMetaInfo,
      count: state._skillType.employeeSkillCount,
      returnURL: `/skillType/${state._skillType.id}/list`,
      currentPage: state._skillType.employeeSkillCurrentPageNumber,
      searchFormParameters: state._skillType.employeeSkillSearchFormParameters,
      loading: state._skillType.loading,
      owner: { type: '_skillType', id: state._skillType.id || this.getOwnerId(), referenceName: 'skillType', listName: 'employeeSkillList', ref:state._skillType, listDisplayName: appLocaleName(userContext,"List")}, // this is for model namespace and
  ...
  getEmployeeSkillUpdateForm = () => {
    ...
    return connect(state => ({
      selectedRows: state._skillType.selectedRows,
      role: "employeeSkill",
      currentUpdateIndex: state._skillType.currentUpdateIndex || 0,
      owner: { type: '_skillType', id: state._skillType.id || this.getOwnerId(), listName: 'employeeSkillList', ref:state._skillType, listDisplayName: appLocaleName(userContext,"List") }, // this is for model namespace and
  ...
  getRequestTypeStepForm = () => {
    ...
    return connect(state => ({
      selectedRows: state._skillType.selectedRows,
      role: "cq",
      currentUpdateIndex: state._skillType.currentUpdateIndex,
      owner: { type: '_skillType', id: state._skillType.id, listName: 'nolist', ref:state._skillType, listDisplayName: appLocaleName(userContext,"List") }, // this is for model namespace and
  ...
  buildRouters = () =>{
    const {SkillTypeWorkbench} = GlobalComponents

    const {SkillTypeDashboard} = GlobalComponents
  	const {SkillTypePermission} = GlobalComponents
  	const {SkillTypeProfile} = GlobalComponents


    const routers=[
    {path:"/skillType/:id/workbench", component: SkillTypeWorkbench},
    
  	{path:"/skillType/:id/profile", component: SkillTypeProfile},
  	{path:"/skillType/:id/permission", component: SkillTypePermission},



  	{path:"/skillType/:id/list/employeeSkillList", component: this.getEmployeeSkillSearch()},
      ...
      {this.state.showSearch&&(
       ...
       <Sider
        ...
        >
         ...
         {this.getNavMenuItems(this.props.skillType,"inline","dark")}
