...
import styles from './SkillType.preference.less'
...
const internalImageListOf = (skillType) =>{
...
const internalSettingListOf = (skillType) =>{
...
const handleTransferSearch =(targetComponent,filterKey,newRequest)=>{
  ...
  const pageNo = 1;
  const candidateReferenceService = SkillTypeService[candidateServiceName] 
...
const executeTrans = (skillType,targetComponent) =>{
...
const buildTransferModal = (skillType,targetComponent) => {
...
<Modal title={transferModalTitle}
          ...
          onOk={()=>executeTrans(skillType,targetComponent)}
...
const internalRenderExtraHeader = (skillType) =>{
	return null
}
const internalRenderExtraFooter = (skillType) =>{
	return null
}
...
class SkillTypePreference extends Component {
...
}))(Form.create()(SkillTypePreference))
