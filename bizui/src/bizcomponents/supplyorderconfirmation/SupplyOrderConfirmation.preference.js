...
import styles from './SupplyOrderConfirmation.preference.less'
...
const internalImageListOf = (supplyOrderConfirmation) =>{
...
const internalSettingListOf = (supplyOrderConfirmation) =>{
...
const handleTransferSearch =(targetComponent,filterKey,newRequest)=>{
  ...
  const pageNo = 1;
  const candidateReferenceService = SupplyOrderConfirmationService[candidateServiceName] 
...
const executeTrans = (supplyOrderConfirmation,targetComponent) =>{
  ...
  validateFieldsAndScroll((error, values) => {
    ...
    const id=supplyOrderConfirmation.id;
    const serviceNameToCall = transferServiceName;

    const payload = {parameters,id,serviceNameToCall}
    
    //targetComponent.setState({transferModalVisiable:false})
    dispatch({type:"_supplyOrderConfirmation/doJob",payload: payload})
...
const buildTransferModal = (supplyOrderConfirmation,targetComponent) => {
...
<Modal title={transferModalTitle}
          ...
          onOk={()=>executeTrans(supplyOrderConfirmation,targetComponent)}
...
const internalRenderExtraHeader = (supplyOrderConfirmation) =>{
	return null
}
const internalRenderExtraFooter = (supplyOrderConfirmation) =>{
	return null
}
...
class SupplyOrderConfirmationPreference extends Component {
  ...
  render() {
    ...
    const { id,displayName, consumerOrderCount, supplyOrderCount } = this.props.supplyOrderConfirmation
    const cardsData = {cardsName:"供应订单确认",cardsFor: "supplyOrderConfirmation",cardsSource: this.props.supplyOrderConfirmation,
  		subItems: [
...
}))(Form.create()(SupplyOrderConfirmationPreference))
