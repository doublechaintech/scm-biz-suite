...
const internalImageListOf = (accountingDocumentCreation) =>{
...
const internalSettingListOf = (accountingDocumentCreation) =>{
	const userContext = null
	const optionList = [ 
	]
...
const handleTransferSearch =(targetComponent,filterKey,newRequest)=>{
  ...
  const pageNo = 1;
  const candidateReferenceService = AccountingDocumentCreationService[candidateServiceName] 
...
const executeTrans = (accountingDocumentCreation,targetComponent) =>{
  ...
  validateFieldsAndScroll((error, values) => {
    ...
    const id=accountingDocumentCreation.id;
    const serviceNameToCall = transferServiceName;

    const payload = {parameters,id,serviceNameToCall}
    
    //targetComponent.setState({transferModalVisiable:false})
    dispatch({type:"_accountingDocumentCreation/doJob",payload: payload})
...
const buildTransferModal = (accountingDocumentCreation,targetComponent) => {
...
<Modal title={transferModalTitle}
          ...
          onOk={()=>executeTrans(accountingDocumentCreation,targetComponent)}
...
const internalRenderExtraFooter = (accountingDocumentCreation) =>{
	return null
}
...
class AccountingDocumentCreationPreference extends Component {
