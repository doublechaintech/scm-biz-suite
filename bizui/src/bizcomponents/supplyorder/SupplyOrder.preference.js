...
const internalImageListOf = (supplyOrder) =>{
...
const internalSettingListOf = (supplyOrder) =>{
	const userContext = null
	const optionList = [ 
	]
...
const handleTransferSearch =(targetComponent,filterKey,newRequest)=>{
  ...
  const pageNo = 1;
  const candidateReferenceService = SupplyOrderService[candidateServiceName] 
...
const executeTrans = (supplyOrder,targetComponent) =>{
...
const buildTransferModal = (supplyOrder,targetComponent) => {
...
<Modal title={transferModalTitle}
          ...
          onOk={()=>executeTrans(supplyOrder,targetComponent)}
...
const internalRenderExtraHeader = (supplyOrder) =>{
	return null
}
const internalRenderExtraFooter = (supplyOrder) =>{
	return null
}
...
<Description term="卖方">{supplyOrder.seller==null?appLocaleName(userContext,"NotAssigned"):supplyOrder.seller.displayName}
 <Icon type="swap" onClick={()=>
  showTransferModel(targetComponent,"卖方","goodsSupplier","requestCandidateSeller",
	      "transferToAnotherSeller","anotherSellerId",supplyOrder.seller?supplyOrder.seller.id:"")} 
...
class SupplyOrderPreference extends Component {
...
}))(Form.create()(SupplyOrderPreference))
