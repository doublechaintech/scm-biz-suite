...
import styles from './SupplyOrder.updateform.less'
...
class SupplyOrderUpdateForm extends Component {
  state = {
...
>>>>>>> 502e8b8dfc403300a992b5083e79c722e85d1854
        ...
        const supplyOrderId = values.id
        const imagesValues = mapBackToImageValues(convertedImagesValues)
        const parameters = { ...values, supplyOrderId, ...imagesValues }
    ...
    const submitUpdateFormAndContinue = () => {
      validateFieldsAndScroll((error, values) => {
        ...
        dispatch({
          type: `${owner.type}/updateSupplyOrder`,
          payload: {
            id: owner.id,
            type: 'supplyOrder',
            parameters,
    ...
    const skipToNext = () => {
      ...
      const newIndex = currentUpdateIndex + 1
      dispatch({
        type: `${owner.type}/gotoNextSupplyOrderUpdateRow`,
        payload: {
          id: owner.id,
          type: 'supplyOrder',
          selectedRows,
...
<<<<<<< HEAD
    ...
    return (
      <PageHeaderLayout
        title={"更新供应订单"+(currentUpdateIndex+1)+"/"+selectedRows.length}
        content="更新供应订单"
...
=======
	...
	const internalRenderTitle = () =>{
      ...
      return (<div>{linkComp}{appLocaleName(userContext,"Update")}供应订单: {(currentUpdateIndex+1)}/{selectedRows.length}</div>)
    }
...
}))(Form.create()(SupplyOrderUpdateForm))
