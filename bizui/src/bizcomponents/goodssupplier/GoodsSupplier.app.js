 ...
 getNavMenuItems = (targetObject, style, customTheme) => {
	  ...
	  <Menu
       ...
       <Menu.Item key="workbench">
        ...
        {filteredMenuItemsGroup(targetObject,this).map((groupedMenuItem,index)=>{
          return(
    <SubMenu id={`submenu-vg${index}`}  key={`vg${index}`} title={<span><Icon type={viewGroupIconNameOf('goods_supplier',`${groupedMenuItem.viewGroup}`)} style={{marginRight:"20px"}} /><span>{viewGroupDisplayNameOf('goods_supplier',`${groupedMenuItem.viewGroup}`)}</span></span>} >
  ...
  getSupplierProductSearch = () => {
    ...
    return connect(state => ({
      rule: state.rule,
      name: window.mtrans('supplier_product','goods_supplier.supplier_product_list',false),
  ...
  getSupplyOrderSearch = () => {
    ...
    return connect(state => ({
      rule: state.rule,
      name: window.mtrans('supply_order','goods_supplier.supply_order_list',false),
      role: "supplyOrder",
  ...
  getAccountSetSearch = () => {
    ...
    return connect(state => ({
      rule: state.rule,
      name: window.mtrans('account_set','goods_supplier.account_set_list',false),
      role: "accountSet",
