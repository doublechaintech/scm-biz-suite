...
import styles from './UiAction.app.less'
...
class UiActionBizApp extends React.PureComponent {
constructor(props) {
    super(props)
     this.state = {
 ...
 getNavMenuItems = (targetObject, style, customTheme) => {
    ...
    return (
	  <Menu
       ...
       <Menu.Item key="workbench">
        <Link to={`/uiAction/${this.props.uiAction.id}/workbench`}><Icon type="solution" style={{marginRight:"20px"}}/><span>工作台</span></Link>
      </Menu.Item>
        ...
        {filteredMenuItemsGroup(targetObject,this).map((groupedMenuItem,index)=>{
          return(
    <SubMenu id={`submenu-vg${index}`}  key={`vg${index}`} title={<span><Icon type={viewGroupIconNameOf('ui_action',`${groupedMenuItem.viewGroup}`)} style={{marginRight:"20px"}} /><span>{viewGroupDisplayNameOf('ui_action',`${groupedMenuItem.viewGroup}`)}</span></span>} >
  ...
  getRequestTypeStepForm = () => {
    ...
    return connect(state => ({
      selectedRows: state._uiAction.selectedRows,
      role: "cq",
      currentUpdateIndex: state._uiAction.currentUpdateIndex,
      owner: { type: '_uiAction', id: state._uiAction.id, listName: 'nolist', ref:state._uiAction, listDisplayName: appLocaleName(userContext,"List") }, // this is for model namespace and
  ...
  buildRouters = () =>{
    const {UiActionWorkbench} = GlobalComponents

    const {UiActionDashboard} = GlobalComponents
  	const {UiActionPermission} = GlobalComponents
  	const {UiActionProfile} = GlobalComponents


    const routers=[
    {path:"/uiAction/:id/workbench", component: UiActionWorkbench},
    
  	{path:"/uiAction/:id/profile", component: UiActionProfile},
  	{path:"/uiAction/:id/permission", component: UiActionPermission},
  ...
  handleOpenChange = (openKeys) => {
   ...
   render() {
    ...
    const {searchLocalData}=GlobalComponents.UiActionBase
    ...
    const renderMenuSwitch=()=>{
     ...
     <Layout>
      ...
      {this.state.showSearch&&(
        ...
        <div style={{backgroundColor:'black'}}  onClick={()=>hideSearchResult()}  >{searchLocalData(this.props.uiAction,this.state.searchKeyword)}</div>
       ...
       <Sider
        ...
        >
         ...
         {this.getNavMenuItems(this.props.uiAction,"inline","dark")}
