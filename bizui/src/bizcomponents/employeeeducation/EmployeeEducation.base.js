...
const menuData = {menuName: window.trans('employee_education'), menuFor: "employeeEducation",  internalName: "employee_education",
  		subItems: [
...
const settingMenuData = {menuName: window.trans('employee_education'), menuFor: "employeeEducation",  internalName: "employee_education",
  		subItems: [
...
const displayColumns = [
  { title: fieldLabels.id, debugtype: 'string', dataIndex: 'id', width: '6', render: (text, record)=>renderTextCell(text,record,'employeeEducation') , sorter: true },
...
const renderActionList=(employeeEducation, targetObject, columCount, listName)=>{

	if(!employeeEducation){
		return null
	}
	if(!employeeEducation.actionList){
		return null
	}
	if(employeeEducation.actionList.length === 0){
		return null
	}
	return (
		<div className={styles.overlay}>
			...
			{employeeEducation.actionList.map(action=>(<Link key={action.id} to={{pathname: action.actionPath.substring(1), state: {ownerId:targetObject.id,action,selectedRows:[employeeEducation]}}} >
...
const renderItemOfList=(employeeEducation, targetObject, columCount, listName)=>{
  ...
  const userContext = null
  return (
     <Row key={`${listName}-${employeeEducation.id}`} className={styles.itemDesc}>
	...
	<Col span={20}>
      ...
      {renderActionList(employeeEducation,targetObject)}
    </Row>
	)
...
const EmployeeEducationBase={unpackObjectToFormValues, menuData,settingMenuData,displayColumns,fieldLabels,renderItemOfList, stepOf, searchLocalData}
