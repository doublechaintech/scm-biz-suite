 ...
 getNavMenuItems = (targetObject, style, customTheme) => {
    ...
    return (
	  <Menu
       ...
       <Menu.Item key="workbench">
        <Link to={`/employeeEducation/${this.props.employeeEducation.id}/workbench`}><Icon type="solution" style={{marginRight:"20px"}}/><span>工作台</span></Link>
      </Menu.Item>
        ...
        {filteredMenuItemsGroup(targetObject,this).map((groupedMenuItem,index)=>{
          return(
    <SubMenu id={`submenu-vg${index}`}  key={`vg${index}`} title={<span><Icon type={viewGroupIconNameOf('employee_education',`${groupedMenuItem.viewGroup}`)} style={{marginRight:"20px"}} /><span>{viewGroupDisplayNameOf('employee_education',`${groupedMenuItem.viewGroup}`)}</span></span>} >
  ...
  getRequestTypeStepForm = () => {
    ...
    return connect(state => ({
      selectedRows: state._employeeEducation.selectedRows,
      role: "cq",
      currentUpdateIndex: state._employeeEducation.currentUpdateIndex,
      owner: { type: '_employeeEducation', id: state._employeeEducation.id, listName: 'nolist', ref:state._employeeEducation, listDisplayName: appLocaleName(userContext,"List") }, // this is for model namespace and
  ...
  buildRouters = () =>{
    const {EmployeeEducationWorkbench} = GlobalComponents

    const {EmployeeEducationDashboard} = GlobalComponents
  	const {EmployeeEducationPermission} = GlobalComponents
  	const {EmployeeEducationProfile} = GlobalComponents


    const routers=[
    {path:"/employeeEducation/:id/workbench", component: EmployeeEducationWorkbench},
    
  	{path:"/employeeEducation/:id/profile", component: EmployeeEducationProfile},
  	{path:"/employeeEducation/:id/permission", component: EmployeeEducationPermission},
  ...
  handleOpenChange = (openKeys) => {
   ...
   render() {
    ...
    const {searchLocalData}=GlobalComponents.EmployeeEducationBase
    ...
    const renderMenuSwitch=()=>{
     ...
     <Layout>
      ...
      {this.state.showSearch&&(
        ...
        <div style={{backgroundColor:'black'}}  onClick={()=>hideSearchResult()}  >{searchLocalData(this.props.employeeEducation,this.state.searchKeyword)}</div>
       ...
       <Sider
        ...
        >
         ...
         {this.getNavMenuItems(this.props.employeeEducation,"inline","dark")}
