...
import styles from './EmployeeEducation.workbench.less'
...
const renderSettingMenuItem = (item,cardsData,targetComponent) =>{
  ...
  return (<Menu.Item key={item.name}>
      <Link to={`/employeeEducation/${targetComponent.props.employeeEducation.id}/list/${item.name}/${item.displayName}/`}>
...
const renderSettingMenu = (cardsData,targetComponent) =>{
  ...
  return (<Menu>
    	<Menu.Item key="profile">
  			<Link to={`/employeeEducation/${targetComponent.props.employeeEducation.id}/permission`}><Icon type="safety-certificate" theme="twoTone" twoToneColor="#52c41a"/><span>{appLocaleName(userContext,"Permission")}</span></Link>
		</Menu.Item>
		<Menu.Divider />
...
const internalSummaryOf = (cardsData,targetComponent) =>{
	...
	const employeeEducation = cardsData.cardsSource
	const {EmployeeEducationService} = GlobalComponents
	const userContext = null
	return (
	<div>
	<DescriptionList className={styles.headerList} size="small" col="4">
<Description term="ID" style={{wordBreak: 'break-all'}}>{employeeEducation.id}</Description> 
...
<Description term="类型" style={{wordBreak: 'break-all'}}>{employeeEducation.type}</Description> 
<Description term="备注" style={{wordBreak: 'break-all'}}>{employeeEducation.remark}</Description> 
...
class EmployeeEducationWorkbench extends Component {
 ...
 state = {
  ...
  render() {
    ...
    const cardsData = {cardsName:window.trans('employee_education'),cardsFor: "employeeEducation",
...
}))(Form.create()(EmployeeEducationWorkbench))
