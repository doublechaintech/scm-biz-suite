...
const internalImageListOf = (memberWishlistProduct) =>{
...
const internalSettingListOf = (memberWishlistProduct) =>{
	const userContext = null
	const optionList = [ 
	]
...
const handleTransferSearch =(targetComponent,filterKey,newRequest)=>{
  ...
  const pageNo = 1;
  const candidateReferenceService = MemberWishlistProductService[candidateServiceName] 
...
const executeTrans = (memberWishlistProduct,targetComponent) =>{
  ...
  validateFieldsAndScroll((error, values) => {
    ...
    const id=memberWishlistProduct.id;
    const serviceNameToCall = transferServiceName;

    const payload = {parameters,id,serviceNameToCall}
    
    //targetComponent.setState({transferModalVisiable:false})
    dispatch({type:"_memberWishlistProduct/doJob",payload: payload})
...
const buildTransferModal = (memberWishlistProduct,targetComponent) => {
...
<Modal title={transferModalTitle}
          ...
          onOk={()=>executeTrans(memberWishlistProduct,targetComponent)}
...
const internalRenderExtraHeader = (memberWishlistProduct) =>{
	return null
}
const internalRenderExtraFooter = (memberWishlistProduct) =>{
	return null
}
...
<Description term="业主">{memberWishlistProduct.owner==null?appLocaleName(userContext,"NotAssigned"):memberWishlistProduct.owner.displayName}
 <Icon type="swap" onClick={()=>
  showTransferModel(targetComponent,"业主","memberWishlist","requestCandidateOwner",
	      "transferToAnotherOwner","anotherOwnerId",memberWishlistProduct.owner?memberWishlistProduct.owner.id:"")} 
...
class MemberWishlistProductPreference extends Component {
...
}))(Form.create()(MemberWishlistProductPreference))
