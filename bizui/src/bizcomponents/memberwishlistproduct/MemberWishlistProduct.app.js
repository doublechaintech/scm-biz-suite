...
import styles from './MemberWishlistProduct.app.less'
...
class MemberWishlistProductBizApp extends React.PureComponent {
constructor(props) {
    super(props)
     this.state = {
 ...
 getNavMenuItems = (targetObject, style, customTheme) => {
    ...
    return (
	  <Menu
       ...
       <Menu.Item key="workbench">
        <Link to={`/memberWishlistProduct/${this.props.memberWishlistProduct.id}/workbench`}><Icon type="solution" style={{marginRight:"20px"}}/><span>工作台</span></Link>
      </Menu.Item>
        ...
        {filteredMenuItemsGroup(targetObject,this).map((groupedMenuItem,index)=>{
          return(
    <SubMenu id={`submenu-vg${index}`}  key={`vg${index}`} title={<span><Icon type={viewGroupIconNameOf('member_wishlist_product',`${groupedMenuItem.viewGroup}`)} style={{marginRight:"20px"}} /><span>{viewGroupDisplayNameOf('member_wishlist_product',`${groupedMenuItem.viewGroup}`)}</span></span>} >
  ...
  getRequestTypeStepForm = () => {
    ...
    return connect(state => ({
      selectedRows: state._memberWishlistProduct.selectedRows,
      role: "cq",
      currentUpdateIndex: state._memberWishlistProduct.currentUpdateIndex,
      owner: { type: '_memberWishlistProduct', id: state._memberWishlistProduct.id, listName: 'nolist', ref:state._memberWishlistProduct, listDisplayName: appLocaleName(userContext,"List") }, // this is for model namespace and
  ...
  buildRouters = () =>{
    const {MemberWishlistProductWorkbench} = GlobalComponents

    const {MemberWishlistProductDashboard} = GlobalComponents
  	const {MemberWishlistProductPermission} = GlobalComponents
  	const {MemberWishlistProductProfile} = GlobalComponents


    const routers=[
    {path:"/memberWishlistProduct/:id/workbench", component: MemberWishlistProductWorkbench},
    
  	{path:"/memberWishlistProduct/:id/profile", component: MemberWishlistProductProfile},
  	{path:"/memberWishlistProduct/:id/permission", component: MemberWishlistProductPermission},
  ...
  handleOpenChange = (openKeys) => {
   ...
   render() {
    ...
    const {searchLocalData}=GlobalComponents.MemberWishlistProductBase
    ...
    const renderMenuSwitch=()=>{
     ...
     <Layout>
      ...
      {this.state.showSearch&&(
        ...
        <div style={{backgroundColor:'black'}}  onClick={()=>hideSearchResult()}  >{searchLocalData(this.props.memberWishlistProduct,this.state.searchKeyword)}</div>
       ...
       <Sider
        ...
        >
         ...
         {this.getNavMenuItems(this.props.memberWishlistProduct,"inline","dark")}
