...
import styles from './MemberWishlistProduct.workbench.less'
...
const renderSettingMenuItem = (item,cardsData,targetComponent) =>{
  ...
  return (<Menu.Item key={item.name}>
      <Link to={`/memberWishlistProduct/${targetComponent.props.memberWishlistProduct.id}/list/${item.name}/${item.displayName}/`}>
...
const renderSettingMenu = (cardsData,targetComponent) =>{
  ...
  return (<Menu>
    	<Menu.Item key="profile">
  			<Link to={`/memberWishlistProduct/${targetComponent.props.memberWishlistProduct.id}/permission`}><Icon type="safety-certificate" theme="twoTone" twoToneColor="#52c41a"/><span>{appLocaleName(userContext,"Permission")}</span></Link>
		</Menu.Item>
		<Menu.Divider />
...
const internalSummaryOf = (cardsData,targetComponent) =>{
	...
	const memberWishlistProduct = cardsData.cardsSource
	const {MemberWishlistProductService} = GlobalComponents
	const userContext = null
	return (
	<div>
	<DescriptionList className={styles.headerList} size="small" col="4">
<Description term="ID" style={{wordBreak: 'break-all'}}>{memberWishlistProduct.id}</Description> 
...
class MemberWishlistProductWorkbench extends Component {
...
}))(Form.create()(MemberWishlistProductWorkbench))
