...
import styles from './LevelThreeCategory.workbench.less'
...
const renderSettingMenuItem = (item,cardsData,targetComponent) =>{
  ...
  return (<Menu.Item key={item.name}>
      <Link to={`/levelThreeCategory/${targetComponent.props.levelThreeCategory.id}/list/${item.name}/${item.displayName}/`}>
...
const internalSummaryOf = (cardsData,targetComponent) =>{
	...
	const levelThreeCategory = cardsData.cardsSource
	const {LevelThreeCategoryService} = GlobalComponents
	const userContext = null
	return (
	<div>
	<DescriptionList className={styles.headerList} size="small" col="4">
<Description term="ID" style={{wordBreak: 'break-all'}}>{levelThreeCategory.id}</Description> 
...
class LevelThreeCategoryWorkbench extends Component {
...
}))(Form.create()(LevelThreeCategoryWorkbench))
