...
import styles from './LevelThreeCategory.base.less'
const {
...
const renderItemOfList=(levelThreeCategory, targetComponent, columCount)=>{
...
>>>>>>> b1266426b024c6919f91c6b5be4635d10d614fe9
  ...
  return (
    <Card key={levelThreeCategory.id} style={{marginTop:"10px"}}>
		
	<Col span={4}>
		<Avatar size={90} style={{ backgroundColor: genColor(), verticalAlign: 'middle' }}>
			{leftChars(levelThreeCategory.displayName)}
		</Avatar>
	</Col>
	<Col span={20}>
      ...
      <DescriptionList  key={levelThreeCategory.id} size="small" col={displayColumnsCount} >
        <Description term={fieldLabels.id} style={{wordBreak: 'break-all'}}>{levelThreeCategory.id}</Description> 
        <Description term={fieldLabels.parentCategory}><Tag color='blue' title={`${levelThreeCategory.parentCategory.id}-${levelThreeCategory.parentCategory.displayName}`}>{levelThreeCategory.parentCategory==null?appLocaleName(userContext,"NotAssigned"):`${leftChars(levelThreeCategory.parentCategory.displayName,15)}`}
        </Tag></Description>
...
const LevelThreeCategoryBase={menuData,settingMenuData,displayColumns,fieldLabels,renderItemOfList, stepOf, searchLocalData}
