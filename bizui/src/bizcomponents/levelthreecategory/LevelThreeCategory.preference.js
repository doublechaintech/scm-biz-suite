...
const internalImageListOf = (levelThreeCategory) =>{
...
const internalSettingListOf = (levelThreeCategory) =>{
	const userContext = null
	const optionList = [ 
	]
...
const handleTransferSearch =(targetComponent,filterKey,newRequest)=>{
  ...
  const pageNo = 1;
  const candidateReferenceService = LevelThreeCategoryService[candidateServiceName] 
...
const executeTrans = (levelThreeCategory,targetComponent) =>{
...
const buildTransferModal = (levelThreeCategory,targetComponent) => {
...
<Modal title={transferModalTitle}
          ...
          onOk={()=>executeTrans(levelThreeCategory,targetComponent)}
...
const internalRenderExtraHeader = (levelThreeCategory) =>{
	return null
}
const internalRenderExtraFooter = (levelThreeCategory) =>{
	return null
}
...
<Description term="父类">{levelThreeCategory.parentCategory==null?appLocaleName(userContext,"NotAssigned"):levelThreeCategory.parentCategory.displayName}
 <Icon type="swap" onClick={()=>
  showTransferModel(targetComponent,"父类","levelTwoCategory","requestCandidateParentCategory",
	      "transferToAnotherParentCategory","anotherParentCategoryId",levelThreeCategory.parentCategory?levelThreeCategory.parentCategory.id:"")} 
...
class LevelThreeCategoryPreference extends Component {
  ...
  render() {
    ...
    const { id,displayName, productCount } = this.props.levelThreeCategory
...
}))(Form.create()(LevelThreeCategoryPreference))
