...
import styles from './LevelThreeCategory.app.less'
...
class LevelThreeCategoryBizApp extends React.PureComponent {
constructor(props) {
    super(props)
     this.state = {
 ...
 getNavMenuItems = (targetObject, style, customTheme) => {
    ...
    return (
	  <Menu
       ...
       <Menu.Item key="workbench">
        <Link to={`/levelThreeCategory/${this.props.levelThreeCategory.id}/workbench`}><Icon type="solution" style={{marginRight:"20px"}}/><span>工作台</span></Link>
      </Menu.Item>
        ...
        {filteredMenuItemsGroup(targetObject,this).map((groupedMenuItem,index)=>{
          return(
    <SubMenu id={`submenu-vg${index}`}  key={`vg${index}`} title={<span><Icon type={viewGroupIconNameOf('level_three_category',`${groupedMenuItem.viewGroup}`)} style={{marginRight:"20px"}} /><span>{viewGroupDisplayNameOf('level_three_category',`${groupedMenuItem.viewGroup}`)}</span></span>} >
  ...
  getProductSearch = () => {
    ...
    return connect(state => ({
      rule: state.rule,
      name: window.mtrans('product','level_three_category.product_list',false),
      role: "product",
      ...
      returnURL: `/levelThreeCategory/${state._levelThreeCategory.id}/workbench`,
      ...
      expandForm: state._levelThreeCategory.expandForm,
      loading: state._levelThreeCategory.loading,
      partialList: state._levelThreeCategory.partialList,
      owner: { type: '_levelThreeCategory', id: state._levelThreeCategory.id,
      referenceName: 'parentCategory',
      listName: 'productList', ref:state._levelThreeCategory,
  ...
  getProductCreateForm = () => {
    ...
    return connect(state => ({
      ...
      metaInfo: state._levelThreeCategory.productListMetaInfo,
      count: state._levelThreeCategory.productCount,
      returnURL: `/levelThreeCategory/${state._levelThreeCategory.id}/list`,
      currentPage: state._levelThreeCategory.productCurrentPageNumber,
      searchFormParameters: state._levelThreeCategory.productSearchFormParameters,
      loading: state._levelThreeCategory.loading,
      owner: { type: '_levelThreeCategory', id: state._levelThreeCategory.id || this.getOwnerId(), referenceName: 'parentCategory', listName: 'productList', ref:state._levelThreeCategory, listDisplayName: appLocaleName(userContext,"List")}, // this is for model namespace and
    }))(ProductCreateForm)
  }
  ...
  getProductUpdateForm = () => {
    ...
    return connect(state => ({
      selectedRows: state._levelThreeCategory.selectedRows,
      role: "product",
      currentUpdateIndex: state._levelThreeCategory.currentUpdateIndex || 0,
      owner: { type: '_levelThreeCategory', id: state._levelThreeCategory.id || this.getOwnerId(), listName: 'productList', ref:state._levelThreeCategory, listDisplayName: appLocaleName(userContext,"List") }, // this is for model namespace and
    }))(ProductUpdateForm)
  }
  ...
  getRequestTypeStepForm = () => {
    ...
    return connect(state => ({
      selectedRows: state._levelThreeCategory.selectedRows,
      role: "cq",
      currentUpdateIndex: state._levelThreeCategory.currentUpdateIndex,
      owner: { type: '_levelThreeCategory', id: state._levelThreeCategory.id, listName: 'nolist', ref:state._levelThreeCategory, listDisplayName: appLocaleName(userContext,"List") }, // this is for model namespace and
  ...
  buildRouters = () =>{
    const {LevelThreeCategoryWorkbench} = GlobalComponents
    ...
    const routers=[
    {path:"/levelThreeCategory/:id/workbench", component: LevelThreeCategoryWorkbench},
  	...
  	{path:"/levelThreeCategory/:id/list/productList", component: this.getProductSearch()},
  	{path:"/levelThreeCategory/:id/list/productCreateForm", component: this.getProductCreateForm()},
  	{path:"/levelThreeCategory/:id/list/productUpdateForm", component: this.getProductUpdateForm()},
  ...
  handleOpenChange = (openKeys) => {
   ...
   render() {
    ...
    const {searchLocalData}=GlobalComponents.LevelThreeCategoryBase
    ...
    const renderMenuSwitch=()=>{
     ...
     <Layout>
      ...
      {this.state.showSearch&&(
        ...
        <div style={{backgroundColor:'black'}}  onClick={()=>hideSearchResult()}  >{searchLocalData(this.props.levelThreeCategory,this.state.searchKeyword)}</div>
       ...
       <Sider
        ...
        >
         ...
         {this.getNavMenuItems(this.props.levelThreeCategory,"inline","dark")}
