...
import styles from './EmployeeAttendance.app.less'
...
class EmployeeAttendanceBizApp extends React.PureComponent {
constructor(props) {
    super(props)
     this.state = {
 ...
 getNavMenuItems = (targetObject, style, customTheme) => {
    ...
    return (
	  <Menu
       ...
       <Menu.Item key="workbench">
        <Link to={`/employeeAttendance/${this.props.employeeAttendance.id}/workbench`}><Icon type="solution" style={{marginRight:"20px"}}/><span>工作台</span></Link>
      </Menu.Item>
        ...
        {filteredMenuItemsGroup(targetObject,this).map((groupedMenuItem,index)=>{
          return(
    <SubMenu id={`submenu-vg${index}`}  key={`vg${index}`} title={<span><Icon type={viewGroupIconNameOf('employee_attendance',`${groupedMenuItem.viewGroup}`)} style={{marginRight:"20px"}} /><span>{viewGroupDisplayNameOf('employee_attendance',`${groupedMenuItem.viewGroup}`)}</span></span>} >
  ...
  getRequestTypeStepForm = () => {
    ...
    return connect(state => ({
      selectedRows: state._employeeAttendance.selectedRows,
      role: "cq",
      currentUpdateIndex: state._employeeAttendance.currentUpdateIndex,
      owner: { type: '_employeeAttendance', id: state._employeeAttendance.id, listName: 'nolist', ref:state._employeeAttendance, listDisplayName: appLocaleName(userContext,"List") }, // this is for model namespace and
  ...
  buildRouters = () =>{
    const {EmployeeAttendanceWorkbench} = GlobalComponents

    const {EmployeeAttendanceDashboard} = GlobalComponents
  	const {EmployeeAttendancePermission} = GlobalComponents
  	const {EmployeeAttendanceProfile} = GlobalComponents


    const routers=[
    {path:"/employeeAttendance/:id/workbench", component: EmployeeAttendanceWorkbench},
    
  	{path:"/employeeAttendance/:id/profile", component: EmployeeAttendanceProfile},
  	{path:"/employeeAttendance/:id/permission", component: EmployeeAttendancePermission},
  ...
  handleOpenChange = (openKeys) => {
   ...
   render() {
    ...
    const {searchLocalData}=GlobalComponents.EmployeeAttendanceBase
    ...
    const renderMenuSwitch=()=>{
     ...
     <Layout>
      ...
      {this.state.showSearch&&(
        ...
        <div style={{backgroundColor:'black'}}  onClick={()=>hideSearchResult()}  >{searchLocalData(this.props.employeeAttendance,this.state.searchKeyword)}</div>
       ...
       <Sider
        ...
        >
         ...
         {this.getNavMenuItems(this.props.employeeAttendance,"inline","dark")}
