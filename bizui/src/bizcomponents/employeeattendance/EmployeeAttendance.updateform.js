...
class EmployeeAttendanceUpdateForm extends Component {
  state = {
  ...
  getSelectedRow() {
    ...
    const convertiedValues = selectedRows.map((item) => {
      return {
        ...item,
        enterTime: moment(item.enterTime),
        leaveTime: moment(item.leaveTime),
  ...
  render() {
    ...
    const submitUpdateFormAndContinue = () => {
      validateFieldsAndScroll((error, values) => {
        ...
        dispatch({
          type: `${owner.type}/updateEmployeeAttendance`,
          payload: {
            id: owner.id,
...
}))(Form.create()(EmployeeAttendanceUpdateForm))
