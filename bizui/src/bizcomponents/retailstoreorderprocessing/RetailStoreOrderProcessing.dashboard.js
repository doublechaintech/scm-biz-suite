...
const renderSettingMenuItem = (item,cardsData,targetComponent) =>{
  ...
  return (<Menu.Item key={item.name}>
      <Link to={`/retailStoreOrderProcessing/${targetComponent.props.retailStoreOrderProcessing.id}/list/${item.name}/${item.displayName}/`}>
...
const renderSettingMenu = (cardsData,targetComponent) =>{
  ...
  return (<Menu>
    	<Menu.Item key="profile">
  			<Link to={`/retailStoreOrderProcessing/${targetComponent.props.retailStoreOrderProcessing.id}/permission`}><Icon type="safety-certificate" theme="twoTone" twoToneColor="#52c41a"/><span>{appLocaleName(userContext,"Permission")}</span></Link>
		</Menu.Item>
		<Menu.Divider />
...
<Description term="序号" style={{wordBreak: 'break-all'}}>{retailStoreOrderProcessing.id}</Description> 
<Description term="谁" style={{wordBreak: 'break-all'}}>{retailStoreOrderProcessing.who}</Description> 
...
class RetailStoreOrderProcessingDashboard extends Component {
 ...
 state = {
  ...
  render() {
    ...
    const cardsData = {cardsName:"生超订单处理",cardsFor: "retailStoreOrderProcessing",
    	cardsSource: this.props.retailStoreOrderProcessing,returnURL,displayName,
  		subItems: [
<<<<<<< HEAD
