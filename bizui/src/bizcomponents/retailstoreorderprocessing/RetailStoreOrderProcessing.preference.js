...
const internalImageListOf = (retailStoreOrderProcessing) =>{
...
const internalSettingListOf = (retailStoreOrderProcessing) =>{
...
const handleTransferSearch =(targetComponent,filterKey,newRequest)=>{
  ...
  const pageNo = 1;
  const candidateReferenceService = RetailStoreOrderProcessingService[candidateServiceName] 
...
const executeTrans = (retailStoreOrderProcessing,targetComponent) =>{
  ...
  validateFieldsAndScroll((error, values) => {
    ...
    const id=retailStoreOrderProcessing.id;
    const serviceNameToCall = transferServiceName;

    const payload = {parameters,id,serviceNameToCall}
    
    //targetComponent.setState({transferModalVisiable:false})
    dispatch({type:"_retailStoreOrderProcessing/doJob",payload: payload})
...
const buildTransferModal = (retailStoreOrderProcessing,targetComponent) => {
...
<Modal title={transferModalTitle}
          ...
          onOk={()=>executeTrans(retailStoreOrderProcessing,targetComponent)}
...
const internalRenderExtraHeader = (retailStoreOrderProcessing) =>{
	return null
}
const internalRenderExtraFooter = (retailStoreOrderProcessing) =>{
	return null
}
...
class RetailStoreOrderProcessingPreference extends Component {
  ...
  render() {
    ...
    const { id,displayName, retailStoreOrderCount } = this.props.retailStoreOrderProcessing
...
}))(Form.create()(RetailStoreOrderProcessingPreference))
