...
import styles from './RetailStoreOrderProcessing.table.less'
...
class RetailStoreOrderProcessingTable extends PureComponent {
  state = {
    selectedRowKeys: [],
  }
...
<<<<<<< HEAD
 calcDisplayColumns=()=>{
    ...
    const {displayColumns} = RetailStoreOrderProcessingBase
    if(!referenceName){
    ...
    const operationColumn={
      ...
      render: (text, record) => (
        <span>
         ...
         { hasReadPermission(metaInfo)&&<Link to={`/retailStoreOrderProcessing/${record.id}/dashboard`}>{'查看'}</Link>}
    ...
    const operationColumn={
      ...
      render: (text, record) => (
        <span>
          ...
          { hasReadPermission(metaInfo)&&<Link to={`/retailStoreOrderProcessing/${record.id}/dashboard`}>{appLocaleName(userContext,"View")}</Link>}
...
export default RetailStoreOrderProcessingTable
