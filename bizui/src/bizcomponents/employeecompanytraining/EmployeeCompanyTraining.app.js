...
import styles from './EmployeeCompanyTraining.app.less'
...
class EmployeeCompanyTrainingBizApp extends React.PureComponent {
constructor(props) {
    super(props)
     this.state = {
 ...
 getNavMenuItems = (targetObject, style, customTheme) => {
    ...
    return (
	  <Menu
       ...
       <Menu.Item key="workbench">
        <Link to={`/employeeCompanyTraining/${this.props.employeeCompanyTraining.id}/workbench`}><Icon type="solution" style={{marginRight:"20px"}}/><span>工作台</span></Link>
      </Menu.Item>
        ...
        {filteredMenuItemsGroup(targetObject,this).map((groupedMenuItem,index)=>{
          return(
    <SubMenu id={`submenu-vg${index}`}  key={`vg${index}`} title={<span><Icon type={viewGroupIconNameOf('employee_company_training',`${groupedMenuItem.viewGroup}`)} style={{marginRight:"20px"}} /><span>{viewGroupDisplayNameOf('employee_company_training',`${groupedMenuItem.viewGroup}`)}</span></span>} >
  ...
  getRequestTypeStepForm = () => {
    ...
    return connect(state => ({
      selectedRows: state._employeeCompanyTraining.selectedRows,
      role: "cq",
      currentUpdateIndex: state._employeeCompanyTraining.currentUpdateIndex,
      owner: { type: '_employeeCompanyTraining', id: state._employeeCompanyTraining.id, listName: 'nolist', ref:state._employeeCompanyTraining, listDisplayName: appLocaleName(userContext,"List") }, // this is for model namespace and
  ...
  buildRouters = () =>{
    const {EmployeeCompanyTrainingWorkbench} = GlobalComponents

    const {EmployeeCompanyTrainingDashboard} = GlobalComponents
  	const {EmployeeCompanyTrainingPermission} = GlobalComponents
  	const {EmployeeCompanyTrainingProfile} = GlobalComponents


    const routers=[
    {path:"/employeeCompanyTraining/:id/workbench", component: EmployeeCompanyTrainingWorkbench},
    
  	{path:"/employeeCompanyTraining/:id/profile", component: EmployeeCompanyTrainingProfile},
  	{path:"/employeeCompanyTraining/:id/permission", component: EmployeeCompanyTrainingPermission},
  ...
  handleOpenChange = (openKeys) => {
   ...
   render() {
    ...
    const {searchLocalData}=GlobalComponents.EmployeeCompanyTrainingBase
    ...
    const renderMenuSwitch=()=>{
     ...
     <Layout>
      ...
      {this.state.showSearch&&(
        ...
        <div style={{backgroundColor:'black'}}  onClick={()=>hideSearchResult()}  >{searchLocalData(this.props.employeeCompanyTraining,this.state.searchKeyword)}</div>
       ...
       <Sider
        ...
        >
         ...
         {this.getNavMenuItems(this.props.employeeCompanyTraining,"inline","dark")}
