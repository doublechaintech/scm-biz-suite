...
const renderItemOfList=(retailStoreOrderPaymentGroup, targetComponent, columCount)=>{
  const displayColumnsCount = columCount || 4
  const userContext = null
  return (
    <Card key={retailStoreOrderPaymentGroup.id} style={{marginTop:"10px"}}>
	...
	<Col span={20}>
      ...
      <DescriptionList  key={retailStoreOrderPaymentGroup.id} size="small" col={displayColumnsCount} >
        ...
        <Description term={fieldLabels.bizOrder}><Tag color='blue' title={`${retailStoreOrderPaymentGroup.bizOrder.id}-${retailStoreOrderPaymentGroup.bizOrder.displayName}`}>{retailStoreOrderPaymentGroup.bizOrder==null?appLocaleName(userContext,"NotAssigned"):`${leftChars(retailStoreOrderPaymentGroup.bizOrder.displayName,15)}`}
        </Tag></Description>
