...
import styles from './RetailStoreOrderPaymentGroup.app.less'
...
class RetailStoreOrderPaymentGroupBizApp extends React.PureComponent {
constructor(props) {
    super(props)
     this.state = {
 ...
 getNavMenuItems = (targetObject, style, customTheme) => {
    ...
    return (
	  <Menu
       ...
       <Menu.Item key="workbench">
        <Link to={`/retailStoreOrderPaymentGroup/${this.props.retailStoreOrderPaymentGroup.id}/workbench`}><Icon type="solution" style={{marginRight:"20px"}}/><span>工作台</span></Link>
      </Menu.Item>
        ...
        {filteredMenuItemsGroup(targetObject,this).map((groupedMenuItem,index)=>{
          return(
    <SubMenu id={`submenu-vg${index}`}  key={`vg${index}`} title={<span><Icon type={viewGroupIconNameOf('retail_store_order_payment_group',`${groupedMenuItem.viewGroup}`)} style={{marginRight:"20px"}} /><span>{viewGroupDisplayNameOf('retail_store_order_payment_group',`${groupedMenuItem.viewGroup}`)}</span></span>} >
  ...
  getRequestTypeStepForm = () => {
    ...
    return connect(state => ({
      selectedRows: state._retailStoreOrderPaymentGroup.selectedRows,
      role: "cq",
      currentUpdateIndex: state._retailStoreOrderPaymentGroup.currentUpdateIndex,
      owner: { type: '_retailStoreOrderPaymentGroup', id: state._retailStoreOrderPaymentGroup.id, listName: 'nolist', ref:state._retailStoreOrderPaymentGroup, listDisplayName: appLocaleName(userContext,"List") }, // this is for model namespace and
  ...
  buildRouters = () =>{
    const {RetailStoreOrderPaymentGroupWorkbench} = GlobalComponents

    const {RetailStoreOrderPaymentGroupDashboard} = GlobalComponents
  	const {RetailStoreOrderPaymentGroupPermission} = GlobalComponents
  	const {RetailStoreOrderPaymentGroupProfile} = GlobalComponents


    const routers=[
    {path:"/retailStoreOrderPaymentGroup/:id/workbench", component: RetailStoreOrderPaymentGroupWorkbench},
    
  	{path:"/retailStoreOrderPaymentGroup/:id/profile", component: RetailStoreOrderPaymentGroupProfile},
  ...
  handleOpenChange = (openKeys) => {
   ...
   render() {
    ...
    const {searchLocalData}=GlobalComponents.RetailStoreOrderPaymentGroupBase
    ...
    const renderMenuSwitch=()=>{
     ...
     <Layout>
      ...
      {this.state.showSearch&&(
        ...
        <div style={{backgroundColor:'black'}}  onClick={()=>hideSearchResult()}  >{searchLocalData(this.props.retailStoreOrderPaymentGroup,this.state.searchKeyword)}</div>
       ...
       <Sider
        ...
        >
         ...
         {this.getNavMenuItems(this.props.retailStoreOrderPaymentGroup,"inline","dark")}
