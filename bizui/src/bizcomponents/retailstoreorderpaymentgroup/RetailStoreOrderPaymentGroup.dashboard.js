...
const renderSettingMenuItem = (item,cardsData,targetComponent) =>{
  ...
  return (<Menu.Item key={item.name}>
      <Link to={`/retailStoreOrderPaymentGroup/${targetComponent.props.retailStoreOrderPaymentGroup.id}/list/${item.name}/${item.displayName}/`}>
...
const renderSettingMenu = (cardsData,targetComponent) =>{
  ...
  return (<Menu>
    	<Menu.Item key="profile">
  			<Link to={`/retailStoreOrderPaymentGroup/${targetComponent.props.retailStoreOrderPaymentGroup.id}/permission`}><Icon type="safety-certificate" theme="twoTone" twoToneColor="#52c41a"/><span>{appLocaleName(userContext,"Permission")}</span></Link>
		</Menu.Item>
		<Menu.Divider />
...
const internalSummaryOf = (cardsData,targetComponent) =>{
	...
	const retailStoreOrderPaymentGroup = cardsData.cardsSource
	const {RetailStoreOrderPaymentGroupService} = GlobalComponents
	const userContext = null
	return (
	<div>
	<DescriptionList className={styles.headerList} size="small" col="4">
<Description term="ID" style={{wordBreak: 'break-all'}}>{retailStoreOrderPaymentGroup.id}</Description> 
<Description term="名称" style={{wordBreak: 'break-all'}}>{retailStoreOrderPaymentGroup.name}</Description> 
...
<Description term="卡号码" style={{wordBreak: 'break-all'}}>{retailStoreOrderPaymentGroup.cardNumber}</Description> 
