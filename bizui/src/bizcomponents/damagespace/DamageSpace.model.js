...
export default {
  ...
  effects: {
    ...
    *addGoodsShelf({ payload }, { call, put }) {
      ...
      const location = { pathname: `/damageSpace/${id}/list/GoodsShelfList/Goods Shelf+${appLocaleName(userContext,'List')}`, state: newState }
      yield put(routerRedux.push(location))
    },
    *updateGoodsShelf({ payload }, { call, put }) {
      ...
      const location = { pathname: `/damageSpace/${id}/list/GoodsShelfList/Goods Shelf列表`, state: newPlayload }
      yield put(routerRedux.push(location))
    },
