...
import styles from './DamageSpace.updateform.less'
...
class DamageSpaceUpdateForm extends Component {
  state = {
  ...
  render() {
    ...
    const submitUpdateForm = () => {
      validateFieldsAndScroll((error, values) => {
        ...
        const damageSpaceId = values.id

        const parameters = { ...values, damageSpaceId, }
    ...
    const submitUpdateFormAndContinue = () => {
      validateFieldsAndScroll((error, values) => {
        ...
        dispatch({
          type: `${owner.type}/updateDamageSpace`,
          payload: {
            id: owner.id,
            type: 'damageSpace',
            parameters,
    ...
    const skipToNext = () => {
      ...
      const newIndex = currentUpdateIndex + 1
      dispatch({
        type: `${owner.type}/gotoNextDamageSpaceUpdateRow`,
        payload: {
          id: owner.id,
          type: 'damageSpace',
          selectedRows,
	...
	const internalRenderTitle = () =>{
      ...
      return (<div>{linkComp}{appLocaleName(userContext,"Update")}残次货物存放区: {(currentUpdateIndex+1)}/{selectedRows.length}</div>)
    }
...
}))(Form.create()(DamageSpaceUpdateForm))
