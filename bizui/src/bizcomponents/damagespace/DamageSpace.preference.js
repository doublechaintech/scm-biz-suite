...
const internalImageListOf = (damageSpace) =>{
...
const internalSettingListOf = (damageSpace) =>{
	const userContext = null
	const optionList = [ 
	]
...
const handleTransferSearch =(targetComponent,filterKey,newRequest)=>{
  ...
  const pageNo = 1;
  const candidateReferenceService = DamageSpaceService[candidateServiceName] 
...
const executeTrans = (damageSpace,targetComponent) =>{
...
const buildTransferModal = (damageSpace,targetComponent) => {
...
<Modal title={transferModalTitle}
          ...
          onOk={()=>executeTrans(damageSpace,targetComponent)}
...
const internalRenderExtraHeader = (damageSpace) =>{
	return null
}
const internalRenderExtraFooter = (damageSpace) =>{
	return null
}
...
<Description term="仓库">{damageSpace.warehouse==null?appLocaleName(userContext,"NotAssigned"):damageSpace.warehouse.displayName}
 <Icon type="swap" onClick={()=>
  showTransferModel(targetComponent,"仓库","warehouse","requestCandidateWarehouse",
	      "transferToAnotherWarehouse","anotherWarehouseId",damageSpace.warehouse?damageSpace.warehouse.id:"")} 
...
class DamageSpacePreference extends Component {
...
}))(Form.create()(DamageSpacePreference))
