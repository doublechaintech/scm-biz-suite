...
const internalImageListOf = (retailStoreOrderApproval) =>{
...
const internalSettingListOf = (retailStoreOrderApproval) =>{
...
const handleTransferSearch =(targetComponent,filterKey,newRequest)=>{
  ...
  const pageNo = 1;
  const candidateReferenceService = RetailStoreOrderApprovalService[candidateServiceName] 
...
const executeTrans = (retailStoreOrderApproval,targetComponent) =>{
  ...
  validateFieldsAndScroll((error, values) => {
    ...
    const id=retailStoreOrderApproval.id;
    const serviceNameToCall = transferServiceName;

    const payload = {parameters,id,serviceNameToCall}
    
    //targetComponent.setState({transferModalVisiable:false})
    dispatch({type:"_retailStoreOrderApproval/doJob",payload: payload})
...
const buildTransferModal = (retailStoreOrderApproval,targetComponent) => {
...
<Modal title={transferModalTitle}
          ...
          onOk={()=>executeTrans(retailStoreOrderApproval,targetComponent)}
...
const internalRenderExtraHeader = (retailStoreOrderApproval) =>{
	return null
}
const internalRenderExtraFooter = (retailStoreOrderApproval) =>{
	return null
}
...
class RetailStoreOrderApprovalPreference extends Component {
  ...
  render() {
    ...
    const { id,displayName, retailStoreOrderCount } = this.props.retailStoreOrderApproval
    const cardsData = {cardsName:"生超订单批准",cardsFor: "retailStoreOrderApproval",cardsSource: this.props.retailStoreOrderApproval,
  		subItems: [
...
}))(Form.create()(RetailStoreOrderApprovalPreference))
