...
import styles from './ReportLine.preference.less'
...
const internalImageListOf = (reportLine) =>{
...
const internalSettingListOf = (reportLine) =>{
...
const handleTransferSearch =(targetComponent,filterKey,newRequest)=>{
  ...
  const pageNo = 1;
  const candidateReferenceService = ReportLineService[candidateServiceName] 
...
const executeTrans = (reportLine,targetComponent) =>{
...
const buildTransferModal = (reportLine,targetComponent) => {
...
<Modal title={transferModalTitle}
          ...
          onOk={()=>executeTrans(reportLine,targetComponent)}
...
const internalRenderExtraHeader = (reportLine) =>{
	return null
}
const internalRenderExtraFooter = (reportLine) =>{
	return null
}
...
<Description term="业主">{reportLine.owner==null?"未分配":reportLine.owner.displayName}
 <Icon type="swap" onClick={()=>
  showTransferModel(targetComponent,"业主","report","requestCandidateOwner",
	      "transferToAnotherOwner","anotherOwnerId",reportLine.owner?reportLine.owner.id:"")} 
...
const renderPermissionSetting = reportLine => {
  const {ReportLineBase} = GlobalComponents
  return <PermissionSetting targetObject={reportLine}  targetObjectMeta={ReportLineBase}/>


}


class ReportLinePreference extends Component {
...
}))(Form.create()(ReportLinePreference))
