...
import styles from './ReportLine.table.less'
...
class ReportLineTable extends PureComponent {
  state = {
    selectedRowKeys: [],
  }
 ...
 calcDisplayColumns=()=>{
    ...
    const {displayColumns} = ReportLineBase
    if(!referenceName){
    ...
    const operationColumn={
      ...
      render: (text, record) => (
        <span>
         ...
         { hasReadPermission(metaInfo)&&<Link to={`/reportLine/${record.id}/dashboard`}>{'查看'}</Link>}
...
export default ReportLineTable
