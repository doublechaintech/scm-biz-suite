...
const showAssociateDialog = (targetComponent) => {
  ...
  const { ReportAssociateForm } = GlobalComponents
  ...
  <div>
	...
	data={{reportLineList:selectedRows}} owner={owner}  
...
class ReportLineSearch extends PureComponent {
  state = {
  ...
  render(){
    ...
    const {ReportLineTable} = GlobalComponents;
    const {ReportLineSearchForm} = GlobalComponents;
    const {ReportLineModalTable} = GlobalComponents;
    ...
    return (
      <PageHeaderLayout title={`${displayName}:${this.props.name}列表`}>
        <Card bordered={false}>
          <div className={styles.tableList}>
            <div className={styles.tableListForm}>
              <ReportLineSearchForm {...this.props} />
            </div>
        ...
        {showDeletionDialog(this,ReportLineModalTable,"reportLineIds")}
...
export default Form.create()(ReportLineSearch)
