...
import styles from './CityEvent.workbench.less'
...
const internalSummaryOf = (cardsData,targetComponent) =>{
	...
	const cityEvent = cardsData.cardsSource
	const {CityEventService} = GlobalComponents
	const userContext = null
	return (
	<div>
	<DescriptionList className={styles.headerList} size="small" col="4">
<Description term="ID" style={{wordBreak: 'break-all'}}>{cityEvent.id}</Description> 
...
<Description term="更新于">{ moment(cityEvent.lastUpdateTime).format('YYYY-MM-DD HH:mm')}</Description> 
...
class CityEventWorkbench extends Component {
...
}))(Form.create()(CityEventWorkbench))
