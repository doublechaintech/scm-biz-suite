 ...
 getNavMenuItems = (targetObject, style, customTheme) => {
    ...
    return (
	  <Menu
       ...
       <Menu.Item key="workbench">
        <Link to={`/cityEvent/${this.props.cityEvent.id}/workbench`}><Icon type="solution" style={{marginRight:"20px"}}/><span>工作台</span></Link>
      </Menu.Item>
        ...
        {filteredMenuItemsGroup(targetObject,this).map((groupedMenuItem,index)=>{
          return(
    <SubMenu id={`submenu-vg${index}`}  key={`vg${index}`} title={<span><Icon type={viewGroupIconNameOf('city_event',`${groupedMenuItem.viewGroup}`)} style={{marginRight:"20px"}} /><span>{viewGroupDisplayNameOf('city_event',`${groupedMenuItem.viewGroup}`)}</span></span>} >
  ...
  getEventAttendanceSearch = () => {
    ...
    return connect(state => ({
      rule: state.rule,
      name: window.mtrans('event_attendance','city_event.event_attendance_list',false),
      ...
      returnURL: `/cityEvent/${state._cityEvent.id}/workbench`,
  ...
  getEventAttendanceCreateForm = () => {
    ...
    return connect(state => ({
      ...
      searchFormParameters: state._cityEvent.eventAttendanceSearchFormParameters,
      loading: state._cityEvent.loading,
      owner: { type: '_cityEvent', id: state._cityEvent.id || this.getOwnerId(), referenceName: 'cityEvent', listName: 'eventAttendanceList', ref:state._cityEvent, listDisplayName: appLocaleName(userContext,"List")}, // this is for model namespace and
  ...
  getEventAttendanceUpdateForm = () => {
    ...
    return connect(state => ({
      ...
      role: "eventAttendance",
      currentUpdateIndex: state._cityEvent.currentUpdateIndex || 0,
      owner: { type: '_cityEvent', id: state._cityEvent.id || this.getOwnerId(), listName: 'eventAttendanceList', ref:state._cityEvent, listDisplayName: appLocaleName(userContext,"List") }, // this is for model namespace and
  ...
  getRequestTypeStepForm = () => {
    ...
    return connect(state => ({
      ...
      owner: { type: '_cityEvent', id: state._cityEvent.id, listName: 'nolist', ref:state._cityEvent, listDisplayName: appLocaleName(userContext,"List") }, // this is for model namespace and
  ...
  buildRouters = () =>{
    const {CityEventWorkbench} = GlobalComponents
    ...
    const routers=[
    {path:"/cityEvent/:id/workbench", component: CityEventWorkbench},
  ...
  handleOpenChange = (openKeys) => {
   ...
   render() {
    ...
    const {searchLocalData}=GlobalComponents.CityEventBase
    ...
    const renderMenuSwitch=()=>{
     ...
     <Layout>
      ...
      {this.state.showSearch&&(
        ...
        <div style={{backgroundColor:'black'}}  onClick={()=>hideSearchResult()}  >{searchLocalData(this.props.cityEvent,this.state.searchKeyword)}</div>
       ...
       <Sider
        ...
        >
         ...
         {this.getNavMenuItems(this.props.cityEvent,"inline","dark")}
