...
const internalImageListOf = (cityEvent) =>{
...
const internalSettingListOf = (cityEvent) =>{
	const userContext = null
	const optionList = [ 
	]
...
const handleTransferSearch =(targetComponent,filterKey,newRequest)=>{
  ...
  const pageNo = 1;
  const candidateReferenceService = CityEventService[candidateServiceName] 
...
const executeTrans = (cityEvent,targetComponent) =>{
...
const buildTransferModal = (cityEvent,targetComponent) => {
...
<Modal title={transferModalTitle}
          ...
          onOk={()=>executeTrans(cityEvent,targetComponent)}
...
const internalRenderExtraHeader = (cityEvent) =>{
	return null
}
const internalRenderExtraFooter = (cityEvent) =>{
	return null
}
...
<Description term="城市服务中心">{cityEvent.cityServiceCenter==null?appLocaleName(userContext,"NotAssigned"):cityEvent.cityServiceCenter.displayName}
 <Icon type="swap" onClick={()=>
  showTransferModel(targetComponent,"城市服务中心","retailStoreCityServiceCenter","requestCandidateCityServiceCenter",
	      "transferToAnotherCityServiceCenter","anotherCityServiceCenterId",cityEvent.cityServiceCenter?cityEvent.cityServiceCenter.id:"")} 
...
class CityEventPreference extends Component {
  ...
  render() {
    ...
    const { id,displayName, eventAttendanceCount } = this.props.cityEvent
...
}))(Form.create()(CityEventPreference))
