...
const internalImageListOf = (accountingDocumentConfirmation) =>{
...
const internalSettingListOf = (accountingDocumentConfirmation) =>{
...
const handleTransferSearch =(targetComponent,filterKey,newRequest)=>{
  ...
  const pageNo = 1;
  const candidateReferenceService = AccountingDocumentConfirmationService[candidateServiceName] 
...
const executeTrans = (accountingDocumentConfirmation,targetComponent) =>{
  ...
  validateFieldsAndScroll((error, values) => {
    ...
    const id=accountingDocumentConfirmation.id;
    const serviceNameToCall = transferServiceName;

    const payload = {parameters,id,serviceNameToCall}
    
    //targetComponent.setState({transferModalVisiable:false})
    dispatch({type:"_accountingDocumentConfirmation/doJob",payload: payload})
...
const buildTransferModal = (accountingDocumentConfirmation,targetComponent) => {
...
<Modal title={transferModalTitle}
          ...
          onOk={()=>executeTrans(accountingDocumentConfirmation,targetComponent)}
...
const internalRenderExtraFooter = (accountingDocumentConfirmation) =>{
	return null
}
...
class AccountingDocumentConfirmationPreference extends Component {
...
}))(Form.create()(AccountingDocumentConfirmationPreference))
