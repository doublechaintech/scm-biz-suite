...
import styles from './EmployeeSkill.app.less'
...
class EmployeeSkillBizApp extends React.PureComponent {
constructor(props) {
    super(props)
     this.state = {
 ...
 getNavMenuItems = (targetObject, style, customTheme) => {
    ...
    return (
	  <Menu
       ...
       <Menu.Item key="workbench">
        <Link to={`/employeeSkill/${this.props.employeeSkill.id}/workbench`}><Icon type="solution" style={{marginRight:"20px"}}/><span>工作台</span></Link>
      </Menu.Item>
        ...
        {filteredMenuItemsGroup(targetObject,this).map((groupedMenuItem,index)=>{
          return(
    <SubMenu id={`submenu-vg${index}`}  key={`vg${index}`} title={<span><Icon type={viewGroupIconNameOf('employee_skill',`${groupedMenuItem.viewGroup}`)} style={{marginRight:"20px"}} /><span>{viewGroupDisplayNameOf('employee_skill',`${groupedMenuItem.viewGroup}`)}</span></span>} >
  ...
  getRequestTypeStepForm = () => {
    ...
    return connect(state => ({
      selectedRows: state._employeeSkill.selectedRows,
      role: "cq",
      currentUpdateIndex: state._employeeSkill.currentUpdateIndex,
      owner: { type: '_employeeSkill', id: state._employeeSkill.id, listName: 'nolist', ref:state._employeeSkill, listDisplayName: appLocaleName(userContext,"List") }, // this is for model namespace and
  ...
  buildRouters = () =>{
    const {EmployeeSkillWorkbench} = GlobalComponents

    const {EmployeeSkillDashboard} = GlobalComponents
  	const {EmployeeSkillPermission} = GlobalComponents
  	const {EmployeeSkillProfile} = GlobalComponents


    const routers=[
    {path:"/employeeSkill/:id/workbench", component: EmployeeSkillWorkbench},
    
  	{path:"/employeeSkill/:id/profile", component: EmployeeSkillProfile},
  	{path:"/employeeSkill/:id/permission", component: EmployeeSkillPermission},
  ...
  handleOpenChange = (openKeys) => {
   ...
   render() {
    ...
    const {searchLocalData}=GlobalComponents.EmployeeSkillBase
    ...
    const renderMenuSwitch=()=>{
     ...
     <Layout>
      ...
      {this.state.showSearch&&(
        ...
        <div style={{backgroundColor:'black'}}  onClick={()=>hideSearchResult()}  >{searchLocalData(this.props.employeeSkill,this.state.searchKeyword)}</div>
       ...
       <Sider
        ...
        >
         ...
         {this.getNavMenuItems(this.props.employeeSkill,"inline","dark")}
