...
import styles from './OfferAcceptance.updateform.less'
...
class OfferAcceptanceUpdateForm extends Component {
  state = {
  ...
  getSelectedRow() {
    ...
    const convertiedValues = selectedRows.map((item) => {
      return {
        ...item,
        acceptTime: moment(item.acceptTime),
...
>>>>>>> 502e8b8dfc403300a992b5083e79c722e85d1854
        ...
        const offerAcceptanceId = values.id
        const imagesValues = mapBackToImageValues(convertedImagesValues)
        const parameters = { ...values, offerAcceptanceId, ...imagesValues }
    ...
    const submitUpdateFormAndContinue = () => {
      validateFieldsAndScroll((error, values) => {
        ...
        dispatch({
          type: `${owner.type}/updateOfferAcceptance`,
          payload: {
            id: owner.id,
    ...
    const skipToNext = () => {
      ...
      const newIndex = currentUpdateIndex + 1
      dispatch({
        type: `${owner.type}/gotoNextOfferAcceptanceUpdateRow`,
        payload: {
          id: owner.id,
          type: 'offerAcceptance',
          selectedRows,
...
<<<<<<< HEAD
    ...
    return (
      <PageHeaderLayout
        title={"更新接受工作要约"+(currentUpdateIndex+1)+"/"+selectedRows.length}
        content="更新接受工作要约"
...
=======
	...
	const internalRenderTitle = () =>{
      ...
      return (<div>{linkComp}{appLocaleName(userContext,"Update")}接受工作要约: {(currentUpdateIndex+1)}/{selectedRows.length}</div>)
    }
              ...
              <Col lg={12} md={12} sm={24}>
                <Form.Item label={fieldLabels.acceptTime} {...formItemLayout}>
                  {getFieldDecorator('acceptTime', {
                    initialValue: selectedRow.acceptTime,
...
}))(Form.create()(OfferAcceptanceUpdateForm))
