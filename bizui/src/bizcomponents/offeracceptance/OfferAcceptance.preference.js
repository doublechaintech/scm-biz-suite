...
const internalImageListOf = (offerAcceptance) =>{
...
const internalSettingListOf = (offerAcceptance) =>{
	const userContext = null
	const optionList = [ 
	]
...
const handleTransferSearch =(targetComponent,filterKey,newRequest)=>{
  ...
  const pageNo = 1;
  const candidateReferenceService = OfferAcceptanceService[candidateServiceName] 
...
const executeTrans = (offerAcceptance,targetComponent) =>{
...
const buildTransferModal = (offerAcceptance,targetComponent) => {
...
<Modal title={transferModalTitle}
          ...
          onOk={()=>executeTrans(offerAcceptance,targetComponent)}
...
const internalRenderExtraHeader = (offerAcceptance) =>{
	return null
}
const internalRenderExtraFooter = (offerAcceptance) =>{
	return null
}
...
class OfferAcceptancePreference extends Component {
  ...
  render() {
    ...
    const cardsData = {cardsName:"接受工作要约",cardsFor: "offerAcceptance",cardsSource: this.props.offerAcceptance,
  		subItems: [
...
}))(Form.create()(OfferAcceptancePreference))
