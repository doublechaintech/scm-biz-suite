...
class OfferAcceptanceSearch extends PureComponent {
  state = {
  ...
  render(){
    ...
    const {OfferAcceptanceTable} = GlobalComponents;
    const {OfferAcceptanceSearchForm} = GlobalComponents;
    const {OfferAcceptanceModalTable} = GlobalComponents;
    ...
    return (
      <PageHeaderLayout title={`${displayName}:${this.props.name}列表`}>
        <Card bordered={false}>
          <div className={styles.tableList}>
            <div className={styles.tableListForm}>
              <OfferAcceptanceSearchForm {...this.props} />
            </div>
        ...
        {showDeletionDialog(this,OfferAcceptanceModalTable,"offerAcceptanceIds")}
...
export default Form.create()(OfferAcceptanceSearch)
