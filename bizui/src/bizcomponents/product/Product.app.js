 ...
 getNavMenuItems = (targetObject, style, customTheme) => {
    ...
    return (
	  <Menu
       ...
       <Menu.Item key="workbench">
        <Link to={`/product/${this.props.product.id}/workbench`}><Icon type="solution" style={{marginRight:"20px"}}/><span>工作台</span></Link>
      </Menu.Item>
        ...
        {filteredMenuItemsGroup(targetObject,this).map((groupedMenuItem,index)=>{
          return(
    <SubMenu id={`submenu-vg${index}`}  key={`vg${index}`} title={<span><Icon type={viewGroupIconNameOf('product',`${groupedMenuItem.viewGroup}`)} style={{marginRight:"20px"}} /><span>{viewGroupDisplayNameOf('product',`${groupedMenuItem.viewGroup}`)}</span></span>} >
  ...
  getSkuCreateForm = () => {
    ...
    return connect(state => ({
      ...
      metaInfo: state._product.skuListMetaInfo,
      count: state._product.skuCount,
      returnURL: `/product/${state._product.id}/list`,
      currentPage: state._product.skuCurrentPageNumber,
      searchFormParameters: state._product.skuSearchFormParameters,
      loading: state._product.loading,
      owner: { type: '_product', id: state._product.id || this.getOwnerId(), referenceName: 'product', listName: 'skuList', ref:state._product, listDisplayName: appLocaleName(userContext,"List")}, // this is for model namespace and
    }))(SkuCreateForm)
  }
  ...
  getSkuUpdateForm = () => {
    ...
    return connect(state => ({
      ...
      role: "sku",
      currentUpdateIndex: state._product.currentUpdateIndex || 0,
      owner: { type: '_product', id: state._product.id || this.getOwnerId(), listName: 'skuList', ref:state._product, listDisplayName: appLocaleName(userContext,"List") }, // this is for model namespace and
    }))(SkuUpdateForm)
  }
  ...
  getRequestTypeStepForm = () => {
    ...
    return connect(state => ({
      ...
      owner: { type: '_product', id: state._product.id, listName: 'nolist', ref:state._product, listDisplayName: appLocaleName(userContext,"List") }, // this is for model namespace and
  ...
  buildRouters = () =>{
    const {ProductWorkbench} = GlobalComponents

    const {ProductDashboard} = GlobalComponents
  	const {ProductPermission} = GlobalComponents
  	const {ProductProfile} = GlobalComponents


    const routers=[
    {path:"/product/:id/workbench", component: ProductWorkbench},
    
  	{path:"/product/:id/profile", component: ProductProfile},
  	{path:"/product/:id/permission", component: ProductPermission},
  ...
  handleOpenChange = (openKeys) => {
   ...
   render() {
    ...
    const {searchLocalData}=GlobalComponents.ProductBase
    ...
    const renderMenuSwitch=()=>{
     ...
     <Layout>
      ...
      {this.state.showSearch&&(
        ...
        <div style={{backgroundColor:'black'}}  onClick={()=>hideSearchResult()}  >{searchLocalData(this.props.product,this.state.searchKeyword)}</div>
       ...
       <Sider
        ...
        >
         ...
         {this.getNavMenuItems(this.props.product,"inline","dark")}
