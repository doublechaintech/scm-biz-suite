...
import styles from './JobApplication.updateform.less'
...
class JobApplicationUpdateForm extends Component {
  state = {
  ...
  getSelectedRow() {
    ...
    const convertiedValues = selectedRows.map((item) => {
      return {
        ...item,
        applicationTime: moment(item.applicationTime),
  ...
  render() {
    ...
    const submitUpdateForm = () => {
      validateFieldsAndScroll((error, values) => {
        ...
        const jobApplicationId = values.id
        const imagesValues = mapBackToImageValues(convertedImagesValues)
        const parameters = { ...values, jobApplicationId, ...imagesValues }
    ...
    const submitUpdateFormAndContinue = () => {
      validateFieldsAndScroll((error, values) => {
        ...
        dispatch({
          type: `${owner.type}/updateJobApplication`,
          payload: {
            id: owner.id,
            type: 'jobApplication',
            parameters,
    ...
    const skipToNext = () => {
      ...
      const newIndex = currentUpdateIndex + 1
      dispatch({
        type: `${owner.type}/gotoNextJobApplicationUpdateRow`,
        payload: {
          id: owner.id,
          type: 'jobApplication',
          selectedRows,
	...
	const internalRenderTitle = () =>{
      ...
      return (<div>{linkComp}{appLocaleName(userContext,"Update")}工作申请: {(currentUpdateIndex+1)}/{selectedRows.length}</div>)
    }
              ...
              <Col lg={24} md={24} sm={24}>
                <Form.Item label={fieldLabels.applicationTime} {...formItemLayout}>
                  {getFieldDecorator('applicationTime', {
                    initialValue: selectedRow.applicationTime,
                    rules: [{ required: true, message: appLocaleName(userContext,"PleaseInput") }],
                  })(
                    <DatePicker size="large" format="YYYY-MM-DD"  placeHolder={fieldLabels.applicationTime}/>
...
}))(Form.create()(JobApplicationUpdateForm))
