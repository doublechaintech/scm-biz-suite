...
const internalImageListOf = (jobApplication) =>{
...
const internalSettingListOf = (jobApplication) =>{
	const userContext = null
	const optionList = [ 
	]
...
const handleTransferSearch =(targetComponent,filterKey,newRequest)=>{
  ...
  const pageNo = 1;
  const candidateReferenceService = JobApplicationService[candidateServiceName] 
...
const executeTrans = (jobApplication,targetComponent) =>{
...
const buildTransferModal = (jobApplication,targetComponent) => {
...
<Modal title={transferModalTitle}
          ...
          onOk={()=>executeTrans(jobApplication,targetComponent)}
...
const internalRenderExtraHeader = (jobApplication) =>{
	return null
}
const internalRenderExtraFooter = (jobApplication) =>{
	return null
}
...
class JobApplicationPreference extends Component {
...
}))(Form.create()(JobApplicationPreference))
