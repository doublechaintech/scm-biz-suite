...
const internalImageListOf = (retailStoreInvestmentInvitation) =>{
<<<<<<< HEAD
...
const internalSettingListOf = (retailStoreInvestmentInvitation) =>{
<<<<<<< HEAD
...
const handleTransferSearch =(targetComponent,filterKey,newRequest)=>{
  ...
  const pageNo = 1;
  const candidateReferenceService = RetailStoreInvestmentInvitationService[candidateServiceName] 
...
const executeTrans = (retailStoreInvestmentInvitation,targetComponent) =>{
  ...
  validateFieldsAndScroll((error, values) => {
    ...
    const id=retailStoreInvestmentInvitation.id;
    const serviceNameToCall = transferServiceName;

    const payload = {parameters,id,serviceNameToCall}
    
    //targetComponent.setState({transferModalVisiable:false})
    dispatch({type:"_retailStoreInvestmentInvitation/doJob",payload: payload})
...
const buildTransferModal = (retailStoreInvestmentInvitation,targetComponent) => {
...
<Modal title={transferModalTitle}
          ...
          onOk={()=>executeTrans(retailStoreInvestmentInvitation,targetComponent)}
...
const internalRenderExtraHeader = (retailStoreInvestmentInvitation) =>{
	return null
}
const internalRenderExtraFooter = (retailStoreInvestmentInvitation) =>{
	return null
}
...
class RetailStoreInvestmentInvitationPreference extends Component {
