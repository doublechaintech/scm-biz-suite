 ...
 getNavMenuItems = (targetObject, style, customTheme) => {
    ...
    return (
	  <Menu
       ...
       <Menu.Item key="workbench">
        <Link to={`/retailStoreInvestmentInvitation/${this.props.retailStoreInvestmentInvitation.id}/workbench`}><Icon type="solution" style={{marginRight:"20px"}}/><span>工作台</span></Link>
      </Menu.Item>
        ...
        {filteredMenuItemsGroup(targetObject,this).map((groupedMenuItem,index)=>{
          return(
    <SubMenu id={`submenu-vg${index}`}  key={`vg${index}`} title={<span><Icon type={viewGroupIconNameOf('retail_store_investment_invitation',`${groupedMenuItem.viewGroup}`)} style={{marginRight:"20px"}} /><span>{viewGroupDisplayNameOf('retail_store_investment_invitation',`${groupedMenuItem.viewGroup}`)}</span></span>} >
  ...
  getRetailStoreSearch = () => {
    ...
    return connect(state => ({
      rule: state.rule,
      name: window.mtrans('retail_store','retail_store_investment_invitation.retail_store_list',false),
      role: "retailStore",
      ...
      returnURL: `/retailStoreInvestmentInvitation/${state._retailStoreInvestmentInvitation.id}/workbench`,
  ...
  getRetailStoreCreateForm = () => {
    ...
    return connect(state => ({
      ...
      metaInfo: state._retailStoreInvestmentInvitation.retailStoreListMetaInfo,
      count: state._retailStoreInvestmentInvitation.retailStoreCount,
      returnURL: `/retailStoreInvestmentInvitation/${state._retailStoreInvestmentInvitation.id}/list`,
      currentPage: state._retailStoreInvestmentInvitation.retailStoreCurrentPageNumber,
      searchFormParameters: state._retailStoreInvestmentInvitation.retailStoreSearchFormParameters,
      loading: state._retailStoreInvestmentInvitation.loading,
      owner: { type: '_retailStoreInvestmentInvitation', id: state._retailStoreInvestmentInvitation.id || this.getOwnerId(), referenceName: 'investmentInvitation', listName: 'retailStoreList', ref:state._retailStoreInvestmentInvitation, listDisplayName: appLocaleName(userContext,"List")}, // this is for model namespace and
  ...
  getRetailStoreUpdateForm = () => {
    ...
    return connect(state => ({
      ...
      role: "retailStore",
      currentUpdateIndex: state._retailStoreInvestmentInvitation.currentUpdateIndex || 0,
      owner: { type: '_retailStoreInvestmentInvitation', id: state._retailStoreInvestmentInvitation.id || this.getOwnerId(), listName: 'retailStoreList', ref:state._retailStoreInvestmentInvitation, listDisplayName: appLocaleName(userContext,"List") }, // this is for model namespace and
  ...
  getRequestTypeStepForm = () => {
    ...
    return connect(state => ({
      ...
      owner: { type: '_retailStoreInvestmentInvitation', id: state._retailStoreInvestmentInvitation.id, listName: 'nolist', ref:state._retailStoreInvestmentInvitation, listDisplayName: appLocaleName(userContext,"List") }, // this is for model namespace and
  ...
  buildRouters = () =>{
    const {RetailStoreInvestmentInvitationWorkbench} = GlobalComponents
    ...
    const routers=[
    {path:"/retailStoreInvestmentInvitation/:id/workbench", component: RetailStoreInvestmentInvitationWorkbench},
  ...
  handleOpenChange = (openKeys) => {
   ...
   render() {
    ...
    const {searchLocalData}=GlobalComponents.RetailStoreInvestmentInvitationBase
    ...
    const renderMenuSwitch=()=>{
     ...
     <Layout>
      ...
      {this.state.showSearch&&(
        ...
        <div style={{backgroundColor:'black'}}  onClick={()=>hideSearchResult()}  >{searchLocalData(this.props.retailStoreInvestmentInvitation,this.state.searchKeyword)}</div>
       ...
       <Sider
        ...
        >
         ...
         {this.getNavMenuItems(this.props.retailStoreInvestmentInvitation,"inline","dark")}
