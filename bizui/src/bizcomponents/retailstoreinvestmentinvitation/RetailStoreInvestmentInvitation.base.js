...
import styles from './RetailStoreInvestmentInvitation.base.less'
const {
...
const displayColumns = [
  { title: fieldLabels.id, debugtype: 'string', dataIndex: 'id', width: '6', render: (text, record)=>renderTextCell(text,record,'retailStoreInvestmentInvitation') , sorter: true },
...
const renderItemOfList=(retailStoreInvestmentInvitation, targetComponent, columCount, listName)=>{
  
  if(!retailStoreInvestmentInvitation){
  	return null
  }
  if(!retailStoreInvestmentInvitation.id){
  	return null
  }
  ...
  const userContext = null
  return (
    <Card key={`${listName}-${retailStoreInvestmentInvitation.id}`} style={{marginTop:"10px"}}>
		
	<Col span={4}>
		<Avatar size={90} style={{ backgroundColor: genColor(), verticalAlign: 'middle' }}>
			{leftChars(retailStoreInvestmentInvitation.displayName)}
		</Avatar>
	</Col>
	<Col span={20}>
      ...
      <DescriptionList  key={retailStoreInvestmentInvitation.id} size="small" col={displayColumnsCount} >
...
const RetailStoreInvestmentInvitationBase={menuData,settingMenuData,displayColumns,fieldLabels,renderItemOfList, stepOf, searchLocalData}
