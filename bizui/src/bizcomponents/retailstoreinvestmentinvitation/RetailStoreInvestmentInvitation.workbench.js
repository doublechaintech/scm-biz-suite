...
import styles from './RetailStoreInvestmentInvitation.workbench.less'
...
const renderSettingMenuItem = (item,cardsData,targetComponent) =>{
  ...
  return (<Menu.Item key={item.name}>
      <Link to={`/retailStoreInvestmentInvitation/${targetComponent.props.retailStoreInvestmentInvitation.id}/list/${item.name}/${item.displayName}/`}>
...
const renderSettingMenu = (cardsData,targetComponent) =>{
  ...
  return (<Menu>
    	<Menu.Item key="profile">
  			<Link to={`/retailStoreInvestmentInvitation/${targetComponent.props.retailStoreInvestmentInvitation.id}/permission`}><Icon type="safety-certificate" theme="twoTone" twoToneColor="#52c41a"/><span>{appLocaleName(userContext,"Permission")}</span></Link>
		</Menu.Item>
		<Menu.Divider />
...
const internalSummaryOf = (cardsData,targetComponent) =>{
	...
	const retailStoreInvestmentInvitation = cardsData.cardsSource
	const {RetailStoreInvestmentInvitationService} = GlobalComponents
	const userContext = null
	return (
	<div>
	<DescriptionList className={styles.headerList} size="small" col="4">
<Description term="ID" style={{wordBreak: 'break-all'}}>{retailStoreInvestmentInvitation.id}</Description> 
<Description term="评论" style={{wordBreak: 'break-all'}}>{retailStoreInvestmentInvitation.comment}</Description> 
...
class RetailStoreInvestmentInvitationWorkbench extends Component {
 ...
 state = {
  ...
  render() {
    ...
    const cardsData = {cardsName:window.trans('retail_store_investment_invitation'),cardsFor: "retailStoreInvestmentInvitation",
    	cardsSource: this.props.retailStoreInvestmentInvitation,returnURL,displayName,
  		subItems: [
{name: 'retailStoreList', displayName: window.mtrans('retail_store','retail_store_investment_invitation.retail_store_list',false) ,viewGroup:'__no_group', type:'retailStore',count:retailStoreCount,addFunction: true, role: 'retailStore', metaInfo: retailStoreListMetaInfo, renderItem: GlobalComponents.RetailStoreBase.renderItemOfList},
...
}))(Form.create()(RetailStoreInvestmentInvitationWorkbench))
