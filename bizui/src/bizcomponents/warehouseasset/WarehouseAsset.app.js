...
import styles from './WarehouseAsset.app.less'
...
class WarehouseAssetBizApp extends React.PureComponent {
constructor(props) {
    super(props)
     this.state = {
 ...
 getNavMenuItems = (targetObject, style, customTheme) => {
    ...
    return (
	  <Menu
       ...
       <Menu.Item key="workbench">
        <Link to={`/warehouseAsset/${this.props.warehouseAsset.id}/workbench`}><Icon type="solution" style={{marginRight:"20px"}}/><span>工作台</span></Link>
      </Menu.Item>
        ...
        {filteredMenuItemsGroup(targetObject,this).map((groupedMenuItem,index)=>{
          return(
    <SubMenu id={`submenu-vg${index}`}  key={`vg${index}`} title={<span><Icon type={viewGroupIconNameOf('warehouse_asset',`${groupedMenuItem.viewGroup}`)} style={{marginRight:"20px"}} /><span>{viewGroupDisplayNameOf('warehouse_asset',`${groupedMenuItem.viewGroup}`)}</span></span>} >
  ...
  getRequestTypeStepForm = () => {
    ...
    return connect(state => ({
      selectedRows: state._warehouseAsset.selectedRows,
      role: "cq",
      currentUpdateIndex: state._warehouseAsset.currentUpdateIndex,
      owner: { type: '_warehouseAsset', id: state._warehouseAsset.id, listName: 'nolist', ref:state._warehouseAsset, listDisplayName: appLocaleName(userContext,"List") }, // this is for model namespace and
  ...
  buildRouters = () =>{
    const {WarehouseAssetWorkbench} = GlobalComponents

    const {WarehouseAssetDashboard} = GlobalComponents
  	const {WarehouseAssetPermission} = GlobalComponents
  	const {WarehouseAssetProfile} = GlobalComponents


    const routers=[
    {path:"/warehouseAsset/:id/workbench", component: WarehouseAssetWorkbench},
    
  	{path:"/warehouseAsset/:id/profile", component: WarehouseAssetProfile},
  	{path:"/warehouseAsset/:id/permission", component: WarehouseAssetPermission},
  ...
  handleOpenChange = (openKeys) => {
   ...
   render() {
    ...
    const {searchLocalData}=GlobalComponents.WarehouseAssetBase
    ...
    const renderMenuSwitch=()=>{
     ...
     <Layout>
      ...
      {this.state.showSearch&&(
        ...
        <div style={{backgroundColor:'black'}}  onClick={()=>hideSearchResult()}  >{searchLocalData(this.props.warehouseAsset,this.state.searchKeyword)}</div>
       ...
       <Sider
        ...
        >
         ...
         {this.getNavMenuItems(this.props.warehouseAsset,"inline","dark")}
