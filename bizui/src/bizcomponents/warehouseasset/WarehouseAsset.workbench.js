...
import styles from './WarehouseAsset.workbench.less'
...
const internalSummaryOf = (cardsData,targetComponent) =>{
	...
	const warehouseAsset = cardsData.cardsSource
	const {WarehouseAssetService} = GlobalComponents
	const userContext = null
	return (
	<div>
	<DescriptionList className={styles.headerList} size="small" col="4">
<Description term="ID" style={{wordBreak: 'break-all'}}>{warehouseAsset.id}</Description> 
...
<Description term="更新于">{ moment(warehouseAsset.lastUpdateTime).format('YYYY-MM-DD HH:mm')}</Description> 
...
class WarehouseAssetWorkbench extends Component {
...
}))(Form.create()(WarehouseAssetWorkbench))
