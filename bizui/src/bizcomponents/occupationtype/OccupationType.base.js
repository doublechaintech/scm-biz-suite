...
import styles from './OccupationType.base.less'
const {
...
const displayColumns = [
  { title: fieldLabels.id, debugtype: 'string', dataIndex: 'id', width: '6', render: (text, record)=>renderTextCell(text,record,'occupationType') , sorter: true },
...
const renderItemOfList=(occupationType, targetComponent, columCount, listName)=>{
  
  if(!occupationType){
  	return null
  }
  if(!occupationType.id){
  	return null
  }
  ...
  const userContext = null
  return (
    <Card key={`${listName}-${occupationType.id}`} style={{marginTop:"10px"}}>
		
	<Col span={4}>
		<Avatar size={90} style={{ backgroundColor: genColor(), verticalAlign: 'middle' }}>
			{leftChars(occupationType.displayName)}
		</Avatar>
	</Col>
	<Col span={20}>
      ...
      <DescriptionList  key={occupationType.id} size="small" col={displayColumnsCount} >
...
const OccupationTypeBase={menuData,settingMenuData,displayColumns,fieldLabels,renderItemOfList, stepOf, searchLocalData}
