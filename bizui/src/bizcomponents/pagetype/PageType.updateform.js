...
import styles from './PageType.updateform.less'
...
class PageTypeUpdateForm extends Component {
  state = {
  ...
  render() {
    ...
    const submitUpdateForm = () => {
      validateFieldsAndScroll((error, values) => {
        ...
        const pageTypeId = values.id

        const parameters = { ...values, pageTypeId, }
    ...
    const submitUpdateFormAndContinue = () => {
      validateFieldsAndScroll((error, values) => {
        ...
        dispatch({
          type: `${owner.type}/updatePageType`,
          payload: {
            id: owner.id,
            type: 'pageType',
            parameters,
    ...
    const skipToNext = () => {
      ...
      const newIndex = currentUpdateIndex + 1
      dispatch({
        type: `${owner.type}/gotoNextPageTypeUpdateRow`,
        payload: {
          id: owner.id,
          type: 'pageType',
          selectedRows,
	...
	const internalRenderTitle = () =>{
      ...
      return (<div>{linkComp}{appLocaleName(userContext,"Update")}页面类型: {(currentUpdateIndex+1)}/{selectedRows.length}</div>)
    }
              ...
              <Col lg={24} md={24} sm={24}>
                <Form.Item label={fieldLabels.code} {...formItemLayout}>
                  {getFieldDecorator('code', {
                    ...
                    <SmallTextInput size="large"  placeholder={fieldLabels.code} disabled/>
...
}))(Form.create()(PageTypeUpdateForm))
