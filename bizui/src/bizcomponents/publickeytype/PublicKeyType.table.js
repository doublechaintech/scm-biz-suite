...
import styles from './PublicKeyType.table.less'
...
class PublicKeyTypeTable extends PureComponent {
  state = {
    selectedRowKeys: [],
  }
  ...
  enhanceColumnsWithSorter=()=>{
    const {displayColumns} = PublicKeyTypeBase
    ...
    const operationColumn={
      ...
      render: (text, record) => (
        <span>
          ...
          { hasReadPermission(metaInfo)&&<Link to={`/publicKeyType/${record.id}/dashboard`}>{appLocaleName(userContext,"View")}</Link>}
...
export default PublicKeyTypeTable
