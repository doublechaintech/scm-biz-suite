...
export default {
  ...
  effects: {
    ...
    *view({ payload }, { call, put, select }) { 
      ...
      const {PublicKeyTypeService} = GlobalComponents;
      const data = yield call(PublicKeyTypeService.view, payload.id)
    ...
    *load({ payload }, { call, put }) { 
      const {PublicKeyTypeService} = GlobalComponents;
      //yield put({ type: 'showLoading', payload })
      const data = yield call(PublicKeyTypeService.load, payload.id, payload.parameters)
    ...
    *gotoCreateForm({ payload }, { put }) {
      ...
      yield put(routerRedux.push(`/publicKeyType/${id}/list/${role}CreateForm`))
    },
    *gotoUpdateForm({ payload }, { put }) {
      const { id, role, selectedRows, currentUpdateIndex } = payload
      const state = { id, role, selectedRows, currentUpdateIndex }
      const location = { pathname: `/publicKeyType/${id}/list/${role}UpdateForm`, state }
      yield put(routerRedux.push(location))
    },
    *goback({ payload }, { put }) {
      const { id, type,listName } = payload
      yield put(routerRedux.push(`/publicKeyType/${id}/list/${type}List/${listName}`))
    },
