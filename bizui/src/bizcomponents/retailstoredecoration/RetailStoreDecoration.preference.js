...
import styles from './RetailStoreDecoration.preference.less'
...
const internalImageListOf = (retailStoreDecoration) =>{
<<<<<<< HEAD
...
const internalSettingListOf = (retailStoreDecoration) =>{
<<<<<<< HEAD
...
const handleTransferSearch =(targetComponent,filterKey,newRequest)=>{
  ...
  const pageNo = 1;
  const candidateReferenceService = RetailStoreDecorationService[candidateServiceName] 
...
const executeTrans = (retailStoreDecoration,targetComponent) =>{
  ...
  validateFieldsAndScroll((error, values) => {
    ...
    const id=retailStoreDecoration.id;
    const serviceNameToCall = transferServiceName;

    const payload = {parameters,id,serviceNameToCall}
    
    //targetComponent.setState({transferModalVisiable:false})
    dispatch({type:"_retailStoreDecoration/doJob",payload: payload})
...
const buildTransferModal = (retailStoreDecoration,targetComponent) => {
...
<Modal title={transferModalTitle}
          ...
          onOk={()=>executeTrans(retailStoreDecoration,targetComponent)}
...
const internalRenderExtraHeader = (retailStoreDecoration) =>{
	return null
}
const internalRenderExtraFooter = (retailStoreDecoration) =>{
	return null
}
...
class RetailStoreDecorationPreference extends Component {
...
}))(Form.create()(RetailStoreDecorationPreference))
