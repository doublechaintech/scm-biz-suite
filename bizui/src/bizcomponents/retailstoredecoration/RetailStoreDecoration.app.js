 ...
 getNavMenuItems = (targetObject, style, customTheme) => {
    ...
    return (
	  <Menu
       ...
       <Menu.Item key="workbench">
        <Link to={`/retailStoreDecoration/${this.props.retailStoreDecoration.id}/workbench`}><Icon type="solution" style={{marginRight:"20px"}}/><span>工作台</span></Link>
      </Menu.Item>
        ...
        {filteredMenuItemsGroup(targetObject,this).map((groupedMenuItem,index)=>{
          return(
    <SubMenu id={`submenu-vg${index}`}  key={`vg${index}`} title={<span><Icon type={viewGroupIconNameOf('retail_store_decoration',`${groupedMenuItem.viewGroup}`)} style={{marginRight:"20px"}} /><span>{viewGroupDisplayNameOf('retail_store_decoration',`${groupedMenuItem.viewGroup}`)}</span></span>} >
  ...
  getRetailStoreSearch = () => {
    ...
    return connect(state => ({
      rule: state.rule,
      name: window.mtrans('retail_store','retail_store_decoration.retail_store_list',false),
      role: "retailStore",
      ...
      returnURL: `/retailStoreDecoration/${state._retailStoreDecoration.id}/workbench`,
  ...
  getRetailStoreCreateForm = () => {
    ...
    return connect(state => ({
      ...
      metaInfo: state._retailStoreDecoration.retailStoreListMetaInfo,
      count: state._retailStoreDecoration.retailStoreCount,
      returnURL: `/retailStoreDecoration/${state._retailStoreDecoration.id}/list`,
      currentPage: state._retailStoreDecoration.retailStoreCurrentPageNumber,
      searchFormParameters: state._retailStoreDecoration.retailStoreSearchFormParameters,
      loading: state._retailStoreDecoration.loading,
      owner: { type: '_retailStoreDecoration', id: state._retailStoreDecoration.id || this.getOwnerId(), referenceName: 'decoration', listName: 'retailStoreList', ref:state._retailStoreDecoration, listDisplayName: appLocaleName(userContext,"List")}, // this is for model namespace and
  ...
  getRetailStoreUpdateForm = () => {
    ...
    return connect(state => ({
      ...
      role: "retailStore",
      currentUpdateIndex: state._retailStoreDecoration.currentUpdateIndex || 0,
      owner: { type: '_retailStoreDecoration', id: state._retailStoreDecoration.id || this.getOwnerId(), listName: 'retailStoreList', ref:state._retailStoreDecoration, listDisplayName: appLocaleName(userContext,"List") }, // this is for model namespace and
  ...
  getRequestTypeStepForm = () => {
    ...
    return connect(state => ({
      ...
      owner: { type: '_retailStoreDecoration', id: state._retailStoreDecoration.id, listName: 'nolist', ref:state._retailStoreDecoration, listDisplayName: appLocaleName(userContext,"List") }, // this is for model namespace and
  ...
  buildRouters = () =>{
    const {RetailStoreDecorationWorkbench} = GlobalComponents

    const {RetailStoreDecorationDashboard} = GlobalComponents
  	const {RetailStoreDecorationPermission} = GlobalComponents
  	const {RetailStoreDecorationProfile} = GlobalComponents


    const routers=[
    {path:"/retailStoreDecoration/:id/workbench", component: RetailStoreDecorationWorkbench},
    
  	{path:"/retailStoreDecoration/:id/profile", component: RetailStoreDecorationProfile},
  	{path:"/retailStoreDecoration/:id/permission", component: RetailStoreDecorationPermission},
  ...
  handleOpenChange = (openKeys) => {
   ...
   render() {
    ...
    const {searchLocalData}=GlobalComponents.RetailStoreDecorationBase
    ...
    const renderMenuSwitch=()=>{
     ...
     <Layout>
      ...
      {this.state.showSearch&&(
        ...
        <div style={{backgroundColor:'black'}}  onClick={()=>hideSearchResult()}  >{searchLocalData(this.props.retailStoreDecoration,this.state.searchKeyword)}</div>
       ...
       <Sider
        ...
        >
         ...
         {this.getNavMenuItems(this.props.retailStoreDecoration,"inline","dark")}
