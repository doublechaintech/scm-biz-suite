...
import styles from './RetailStoreDecoration.workbench.less'
...
const renderSettingMenuItem = (item,cardsData,targetComponent) =>{
  ...
  return (<Menu.Item key={item.name}>
      <Link to={`/retailStoreDecoration/${targetComponent.props.retailStoreDecoration.id}/list/${item.name}/${item.displayName}/`}>
...
const renderSettingMenu = (cardsData,targetComponent) =>{
  ...
  return (<Menu>
    	<Menu.Item key="profile">
  			<Link to={`/retailStoreDecoration/${targetComponent.props.retailStoreDecoration.id}/permission`}><Icon type="safety-certificate" theme="twoTone" twoToneColor="#52c41a"/><span>{appLocaleName(userContext,"Permission")}</span></Link>
		</Menu.Item>
		<Menu.Divider />
...
const internalSummaryOf = (cardsData,targetComponent) =>{
	...
	const retailStoreDecoration = cardsData.cardsSource
	const {RetailStoreDecorationService} = GlobalComponents
	const userContext = null
	return (
	<div>
	<DescriptionList className={styles.headerList} size="small" col="4">
<Description term="ID" style={{wordBreak: 'break-all'}}>{retailStoreDecoration.id}</Description> 
<Description term="评论" style={{wordBreak: 'break-all'}}>{retailStoreDecoration.comment}</Description> 
...
class RetailStoreDecorationWorkbench extends Component {
 ...
 state = {
  ...
  render() {
    ...
    const cardsData = {cardsName:window.trans('retail_store_decoration'),cardsFor: "retailStoreDecoration",
    	cardsSource: this.props.retailStoreDecoration,returnURL,displayName,
  		subItems: [
{name: 'retailStoreList', displayName: window.mtrans('retail_store','retail_store_decoration.retail_store_list',false) ,viewGroup:'__no_group', type:'retailStore',count:retailStoreCount,addFunction: true, role: 'retailStore', metaInfo: retailStoreListMetaInfo, renderItem: GlobalComponents.RetailStoreBase.renderItemOfList},
...
}))(Form.create()(RetailStoreDecorationWorkbench))
