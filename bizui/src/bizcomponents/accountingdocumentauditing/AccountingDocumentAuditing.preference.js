...
const internalImageListOf = (accountingDocumentAuditing) =>{
...
const internalSettingListOf = (accountingDocumentAuditing) =>{
	const userContext = null
	const optionList = [ 
	]
...
const handleTransferSearch =(targetComponent,filterKey,newRequest)=>{
  ...
  const pageNo = 1;
  const candidateReferenceService = AccountingDocumentAuditingService[candidateServiceName] 
...
const executeTrans = (accountingDocumentAuditing,targetComponent) =>{
  ...
  validateFieldsAndScroll((error, values) => {
    ...
    const id=accountingDocumentAuditing.id;
    const serviceNameToCall = transferServiceName;

    const payload = {parameters,id,serviceNameToCall}
    
    //targetComponent.setState({transferModalVisiable:false})
    dispatch({type:"_accountingDocumentAuditing/doJob",payload: payload})
...
const buildTransferModal = (accountingDocumentAuditing,targetComponent) => {
...
<Modal title={transferModalTitle}
          ...
          onOk={()=>executeTrans(accountingDocumentAuditing,targetComponent)}
...
const internalRenderExtraHeader = (accountingDocumentAuditing) =>{
	return null
}
const internalRenderExtraFooter = (accountingDocumentAuditing) =>{
	return null
}
...
class AccountingDocumentAuditingPreference extends Component {
...
}))(Form.create()(AccountingDocumentAuditingPreference))
