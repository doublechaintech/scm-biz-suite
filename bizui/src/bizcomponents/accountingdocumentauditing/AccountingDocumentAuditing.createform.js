...
import styles from './AccountingDocumentAuditing.createform.less'
...
class AccountingDocumentAuditingCreateForm extends Component {
  state = {
  ...
  render() {
    ...
    const submitCreateFormAndContinue = () => {
      validateFieldsAndScroll((error, values) => {
        ...
        const parameters = { ...values, ...imagesValues }
        dispatch({
          type: `${owner.type}/addAccountingDocumentAuditing`,
          payload: { id: owner.id, type: 'accountingDocumentAuditing', parameters, continueNext: true },
        })
      })
    }
	...
	return (
      <PageHeaderLayout
        title={`${appLocaleName(userContext,"CreateNew")}Accounting Document Auditing`}
        content={`${appLocaleName(userContext,"CreateNew")}Accounting Document Auditing`}
...
}))(Form.create()(AccountingDocumentAuditingCreateForm))
