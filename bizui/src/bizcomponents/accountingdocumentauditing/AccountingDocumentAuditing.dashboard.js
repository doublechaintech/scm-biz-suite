...
const renderSettingMenuItem = (item,cardsData,targetComponent) =>{
  ...
  return (<Menu.Item key={item.name}>
      <Link to={`/accountingDocumentAuditing/${targetComponent.props.accountingDocumentAuditing.id}/list/${item.name}/${item.displayName}/`}>
...
const renderSettingMenu = (cardsData,targetComponent) =>{
  ...
  return (<Menu>
    	<Menu.Item key="profile">
  			<Link to={`/accountingDocumentAuditing/${targetComponent.props.accountingDocumentAuditing.id}/permission`}><Icon type="safety-certificate" theme="twoTone" twoToneColor="#52c41a"/><span>{appLocaleName(userContext,"Permission")}</span></Link>
		</Menu.Item>
		<Menu.Divider />
...
<Description term="序号" style={{wordBreak: 'break-all'}}>{accountingDocumentAuditing.id}</Description> 
<Description term="谁" style={{wordBreak: 'break-all'}}>{accountingDocumentAuditing.who}</Description> 
<Description term="评论" style={{wordBreak: 'break-all'}}>{accountingDocumentAuditing.comments}</Description> 
