...
import styles from './AccountingDocumentAuditing.table.less'
...
class AccountingDocumentAuditingTable extends PureComponent {
  state = {
    selectedRowKeys: [],
  }
...
<<<<<<< HEAD
 calcDisplayColumns=()=>{
    ...
    const {displayColumns} = AccountingDocumentAuditingBase
    if(!referenceName){
    ...
    const operationColumn={
      ...
      render: (text, record) => (
        <span>
         ...
         { hasReadPermission(metaInfo)&&<Link to={`/accountingDocumentAuditing/${record.id}/dashboard`}>{'查看'}</Link>}
    ...
    const operationColumn={
      ...
      render: (text, record) => (
        <span>
          ...
          { hasReadPermission(metaInfo)&&<Link to={`/accountingDocumentAuditing/${record.id}/dashboard`}>{appLocaleName(userContext,"View")}</Link>}
...
export default AccountingDocumentAuditingTable
