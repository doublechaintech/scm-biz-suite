...
import styles from './ConsumerOrderPriceAdjustment.app.less'
...
class ConsumerOrderPriceAdjustmentBizApp extends React.PureComponent {
constructor(props) {
    super(props)
     this.state = {
 ...
 getNavMenuItems = (targetObject, style, customTheme) => {
    ...
    return (
	  <Menu
       ...
       <Menu.Item key="workbench">
        <Link to={`/consumerOrderPriceAdjustment/${this.props.consumerOrderPriceAdjustment.id}/workbench`}><Icon type="solution" style={{marginRight:"20px"}}/><span>工作台</span></Link>
      </Menu.Item>
        ...
        {filteredMenuItemsGroup(targetObject,this).map((groupedMenuItem,index)=>{
          return(
    <SubMenu id={`submenu-vg${index}`}  key={`vg${index}`} title={<span><Icon type={viewGroupIconNameOf('consumer_order_price_adjustment',`${groupedMenuItem.viewGroup}`)} style={{marginRight:"20px"}} /><span>{viewGroupDisplayNameOf('consumer_order_price_adjustment',`${groupedMenuItem.viewGroup}`)}</span></span>} >
  ...
  getRequestTypeStepForm = () => {
    ...
    return connect(state => ({
      selectedRows: state._consumerOrderPriceAdjustment.selectedRows,
      role: "cq",
      currentUpdateIndex: state._consumerOrderPriceAdjustment.currentUpdateIndex,
      owner: { type: '_consumerOrderPriceAdjustment', id: state._consumerOrderPriceAdjustment.id, listName: 'nolist', ref:state._consumerOrderPriceAdjustment, listDisplayName: appLocaleName(userContext,"List") }, // this is for model namespace and
  ...
  buildRouters = () =>{
    const {ConsumerOrderPriceAdjustmentWorkbench} = GlobalComponents

    const {ConsumerOrderPriceAdjustmentDashboard} = GlobalComponents
  	const {ConsumerOrderPriceAdjustmentPermission} = GlobalComponents
  	const {ConsumerOrderPriceAdjustmentProfile} = GlobalComponents


    const routers=[
    {path:"/consumerOrderPriceAdjustment/:id/workbench", component: ConsumerOrderPriceAdjustmentWorkbench},
    
  	{path:"/consumerOrderPriceAdjustment/:id/profile", component: ConsumerOrderPriceAdjustmentProfile},
  	{path:"/consumerOrderPriceAdjustment/:id/permission", component: ConsumerOrderPriceAdjustmentPermission},
  ...
  handleOpenChange = (openKeys) => {
   ...
   render() {
    ...
    const {searchLocalData}=GlobalComponents.ConsumerOrderPriceAdjustmentBase
    ...
    const renderMenuSwitch=()=>{
     ...
     <Layout>
      ...
      {this.state.showSearch&&(
        ...
        <div style={{backgroundColor:'black'}}  onClick={()=>hideSearchResult()}  >{searchLocalData(this.props.consumerOrderPriceAdjustment,this.state.searchKeyword)}</div>
       ...
       <Sider
        ...
        >
         ...
         {this.getNavMenuItems(this.props.consumerOrderPriceAdjustment,"inline","dark")}
