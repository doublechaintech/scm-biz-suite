...
const internalImageListOf = (consumerOrderPriceAdjustment) =>{
...
const internalSettingListOf = (consumerOrderPriceAdjustment) =>{
	const userContext = null
	const optionList = [ 
	]
...
const handleTransferSearch =(targetComponent,filterKey,newRequest)=>{
  ...
  const pageNo = 1;
  const candidateReferenceService = ConsumerOrderPriceAdjustmentService[candidateServiceName] 
...
const executeTrans = (consumerOrderPriceAdjustment,targetComponent) =>{
  ...
  validateFieldsAndScroll((error, values) => {
    ...
    const id=consumerOrderPriceAdjustment.id;
    const serviceNameToCall = transferServiceName;

    const payload = {parameters,id,serviceNameToCall}
    
    //targetComponent.setState({transferModalVisiable:false})
    dispatch({type:"_consumerOrderPriceAdjustment/doJob",payload: payload})
...
const buildTransferModal = (consumerOrderPriceAdjustment,targetComponent) => {
...
<Modal title={transferModalTitle}
          ...
          onOk={()=>executeTrans(consumerOrderPriceAdjustment,targetComponent)}
...
const internalRenderExtraHeader = (consumerOrderPriceAdjustment) =>{
	return null
}
const internalRenderExtraFooter = (consumerOrderPriceAdjustment) =>{
	return null
}
...
<Description term="订单">{consumerOrderPriceAdjustment.bizOrder==null?appLocaleName(userContext,"NotAssigned"):consumerOrderPriceAdjustment.bizOrder.displayName}
 <Icon type="swap" onClick={()=>
  showTransferModel(targetComponent,"订单","consumerOrder","requestCandidateBizOrder",
	      "transferToAnotherBizOrder","anotherBizOrderId",consumerOrderPriceAdjustment.bizOrder?consumerOrderPriceAdjustment.bizOrder.id:"")} 
...
class ConsumerOrderPriceAdjustmentPreference extends Component {
...
}))(Form.create()(ConsumerOrderPriceAdjustmentPreference))
