...
const handleTransferSearch =(targetComponent,filterKey,newRequest)=>{
  ...
  const pageNo = 1;
  const candidateReferenceService = ProvinceCenterEmployeeService[candidateServiceName] 
...
const executeTrans = (provinceCenterEmployee,targetComponent) =>{
  ...
  validateFieldsAndScroll((error, values) => {
    ...
    const id=provinceCenterEmployee.id;
    const serviceNameToCall = transferServiceName;

    const payload = {parameters,id,serviceNameToCall}
    
    //targetComponent.setState({transferModalVisiable:false})
    dispatch({type:"_provinceCenterEmployee/doJob",payload: payload})
...
const buildTransferModal = (provinceCenterEmployee,targetComponent) => {
...
<Modal title={transferModalTitle}
          ...
          onOk={()=>executeTrans(provinceCenterEmployee,targetComponent)}
...
const internalRenderExtraHeader = (provinceCenterEmployee) =>{
	return null
}
const internalRenderExtraFooter = (provinceCenterEmployee) =>{
	return null
}
...
class ProvinceCenterEmployeePreference extends Component {
...
}))(Form.create()(ProvinceCenterEmployeePreference))
