...
import styles from './ProvinceCenterEmployee.app.less'
...
class ProvinceCenterEmployeeBizApp extends React.PureComponent {
constructor(props) {
    super(props)
     this.state = {
 ...
 getNavMenuItems = (targetObject, style, customTheme) => {
    ...
    return (
	  <Menu
       ...
       <Menu.Item key="workbench">
        <Link to={`/provinceCenterEmployee/${this.props.provinceCenterEmployee.id}/workbench`}><Icon type="solution" style={{marginRight:"20px"}}/><span>工作台</span></Link>
      </Menu.Item>
        ...
        {filteredMenuItemsGroup(targetObject,this).map((groupedMenuItem,index)=>{
          return(
    <SubMenu id={`submenu-vg${index}`}  key={`vg${index}`} title={<span><Icon type={viewGroupIconNameOf('province_center_employee',`${groupedMenuItem.viewGroup}`)} style={{marginRight:"20px"}} /><span>{viewGroupDisplayNameOf('province_center_employee',`${groupedMenuItem.viewGroup}`)}</span></span>} >
  ...
  getRequestTypeStepForm = () => {
    ...
    return connect(state => ({
      selectedRows: state._provinceCenterEmployee.selectedRows,
      role: "cq",
      currentUpdateIndex: state._provinceCenterEmployee.currentUpdateIndex,
      owner: { type: '_provinceCenterEmployee', id: state._provinceCenterEmployee.id, listName: 'nolist', ref:state._provinceCenterEmployee, listDisplayName: appLocaleName(userContext,"List") }, // this is for model namespace and
  ...
  buildRouters = () =>{
    const {ProvinceCenterEmployeeWorkbench} = GlobalComponents

    const {ProvinceCenterEmployeeDashboard} = GlobalComponents
  	const {ProvinceCenterEmployeePermission} = GlobalComponents
  	const {ProvinceCenterEmployeeProfile} = GlobalComponents


    const routers=[
    {path:"/provinceCenterEmployee/:id/workbench", component: ProvinceCenterEmployeeWorkbench},
    
  	{path:"/provinceCenterEmployee/:id/profile", component: ProvinceCenterEmployeeProfile},
  	{path:"/provinceCenterEmployee/:id/permission", component: ProvinceCenterEmployeePermission},
  ...
  handleOpenChange = (openKeys) => {
   ...
   render() {
    ...
    const {searchLocalData}=GlobalComponents.ProvinceCenterEmployeeBase
    ...
    const renderMenuSwitch=()=>{
     ...
     <Layout>
      ...
      {this.state.showSearch&&(
        ...
        <div style={{backgroundColor:'black'}}  onClick={()=>hideSearchResult()}  >{searchLocalData(this.props.provinceCenterEmployee,this.state.searchKeyword)}</div>
       ...
       <Sider
        ...
        >
         ...
         {this.getNavMenuItems(this.props.provinceCenterEmployee,"inline","dark")}
