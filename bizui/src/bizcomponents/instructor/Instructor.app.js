 ...
 getNavMenuItems = (targetObject, style, customTheme) => {
    ...
    return (
	  <Menu
       ...
       <Menu.Item key="workbench">
        <Link to={`/instructor/${this.props.instructor.id}/workbench`}><Icon type="solution" style={{marginRight:"20px"}}/><span>工作台</span></Link>
      </Menu.Item>
        ...
        {filteredMenuItemsGroup(targetObject,this).map((groupedMenuItem,index)=>{
          return(
    <SubMenu id={`submenu-vg${index}`}  key={`vg${index}`} title={<span><Icon type={viewGroupIconNameOf('instructor',`${groupedMenuItem.viewGroup}`)} style={{marginRight:"20px"}} /><span>{viewGroupDisplayNameOf('instructor',`${groupedMenuItem.viewGroup}`)}</span></span>} >
  ...
  getCompanyTrainingSearch = () => {
    ...
    return connect(state => ({
      rule: state.rule,
      name: window.mtrans('company_training','instructor.company_training_list',false),
      ...
      returnURL: `/instructor/${state._instructor.id}/workbench`,
      ...
      expandForm: state._instructor.expandForm,
      loading: state._instructor.loading,
      partialList: state._instructor.partialList,
  ...
  getCompanyTrainingCreateForm = () => {
    ...
    return connect(state => ({
      ...
      searchFormParameters: state._instructor.companyTrainingSearchFormParameters,
      loading: state._instructor.loading,
      owner: { type: '_instructor', id: state._instructor.id || this.getOwnerId(), referenceName: 'instructor', listName: 'companyTrainingList', ref:state._instructor, listDisplayName: appLocaleName(userContext,"List")}, // this is for model namespace and
  ...
  getCompanyTrainingUpdateForm = () => {
    ...
    return connect(state => ({
      ...
      role: "companyTraining",
      currentUpdateIndex: state._instructor.currentUpdateIndex || 0,
      owner: { type: '_instructor', id: state._instructor.id || this.getOwnerId(), listName: 'companyTrainingList', ref:state._instructor, listDisplayName: appLocaleName(userContext,"List") }, // this is for model namespace and
  ...
  getRequestTypeStepForm = () => {
    ...
    return connect(state => ({
      ...
      owner: { type: '_instructor', id: state._instructor.id, listName: 'nolist', ref:state._instructor, listDisplayName: appLocaleName(userContext,"List") }, // this is for model namespace and
  ...
  buildRouters = () =>{
    const {InstructorWorkbench} = GlobalComponents
    ...
    const routers=[
    {path:"/instructor/:id/workbench", component: InstructorWorkbench},
  	...
  	{path:"/instructor/:id/list/companyTrainingList", component: this.getCompanyTrainingSearch()},
  	{path:"/instructor/:id/list/companyTrainingCreateForm", component: this.getCompanyTrainingCreateForm()},
  	{path:"/instructor/:id/list/companyTrainingUpdateForm", component: this.getCompanyTrainingUpdateForm()},
  ...
  handleOpenChange = (openKeys) => {
   ...
   render() {
    ...
    const {searchLocalData}=GlobalComponents.InstructorBase
    ...
    const renderMenuSwitch=()=>{
     ...
     <Layout>
      ...
      {this.state.showSearch&&(
        ...
        <div style={{backgroundColor:'black'}}  onClick={()=>hideSearchResult()}  >{searchLocalData(this.props.instructor,this.state.searchKeyword)}</div>
       ...
       <Sider
        ...
        >
         ...
         {this.getNavMenuItems(this.props.instructor,"inline","dark")}
