...
const internalImageListOf = (instructor) =>{
<<<<<<< HEAD
...
const internalSettingListOf = (instructor) =>{
<<<<<<< HEAD
...
const handleTransferSearch =(targetComponent,filterKey,newRequest)=>{
  ...
  const pageNo = 1;
  const candidateReferenceService = InstructorService[candidateServiceName] 
...
const executeTrans = (instructor,targetComponent) =>{
  ...
  validateFieldsAndScroll((error, values) => {
    ...
    //targetComponent.setState({transferModalVisiable:false})
    dispatch({type:"_instructor/doJob",payload: payload})
...
const buildTransferModal = (instructor,targetComponent) => {
...
<Modal title={transferModalTitle}
          ...
          onOk={()=>executeTrans(instructor,targetComponent)}
...
const internalRenderExtraHeader = (instructor) =>{
	return null
}
const internalRenderExtraFooter = (instructor) =>{
	return null
}
...
class InstructorPreference extends Component {
...
}))(Form.create()(InstructorPreference))
