 ...
 getNavMenuItems = (targetObject, style, customTheme) => {
    ...
    return (
	  <Menu
       ...
       <Menu.Item key="workbench">
        <Link to={`/levelTwoCategory/${this.props.levelTwoCategory.id}/workbench`}><Icon type="solution" style={{marginRight:"20px"}}/><span>工作台</span></Link>
      </Menu.Item>
        ...
        {filteredMenuItemsGroup(targetObject,this).map((groupedMenuItem,index)=>{
          return(
    <SubMenu id={`submenu-vg${index}`}  key={`vg${index}`} title={<span><Icon type={viewGroupIconNameOf('level_two_category',`${groupedMenuItem.viewGroup}`)} style={{marginRight:"20px"}} /><span>{viewGroupDisplayNameOf('level_two_category',`${groupedMenuItem.viewGroup}`)}</span></span>} >
  ...
  getLevelThreeCategorySearch = () => {
    ...
    return connect(state => ({
      rule: state.rule,
      name: window.mtrans('level_three_category','level_two_category.level_three_category_list',false),
      ...
      returnURL: `/levelTwoCategory/${state._levelTwoCategory.id}/workbench`,
  ...
  getLevelThreeCategoryCreateForm = () => {
    ...
    return connect(state => ({
      ...
      metaInfo: state._levelTwoCategory.levelThreeCategoryListMetaInfo,
      count: state._levelTwoCategory.levelThreeCategoryCount,
      returnURL: `/levelTwoCategory/${state._levelTwoCategory.id}/list`,
      currentPage: state._levelTwoCategory.levelThreeCategoryCurrentPageNumber,
      searchFormParameters: state._levelTwoCategory.levelThreeCategorySearchFormParameters,
      loading: state._levelTwoCategory.loading,
      owner: { type: '_levelTwoCategory', id: state._levelTwoCategory.id || this.getOwnerId(), referenceName: 'parentCategory', listName: 'levelThreeCategoryList', ref:state._levelTwoCategory, listDisplayName: appLocaleName(userContext,"List")}, // this is for model namespace and
  ...
  getLevelThreeCategoryUpdateForm = () => {
    ...
    return connect(state => ({
      ...
      role: "levelThreeCategory",
      currentUpdateIndex: state._levelTwoCategory.currentUpdateIndex || 0,
      owner: { type: '_levelTwoCategory', id: state._levelTwoCategory.id || this.getOwnerId(), listName: 'levelThreeCategoryList', ref:state._levelTwoCategory, listDisplayName: appLocaleName(userContext,"List") }, // this is for model namespace and
  ...
  getRequestTypeStepForm = () => {
    ...
    return connect(state => ({
      ...
      owner: { type: '_levelTwoCategory', id: state._levelTwoCategory.id, listName: 'nolist', ref:state._levelTwoCategory, listDisplayName: appLocaleName(userContext,"List") }, // this is for model namespace and
  ...
  buildRouters = () =>{
    const {LevelTwoCategoryWorkbench} = GlobalComponents
    ...
    const routers=[
    {path:"/levelTwoCategory/:id/workbench", component: LevelTwoCategoryWorkbench},
  ...
  handleOpenChange = (openKeys) => {
   ...
   render() {
    ...
    const {searchLocalData}=GlobalComponents.LevelTwoCategoryBase
    ...
    const renderMenuSwitch=()=>{
     ...
     <Layout>
      ...
      {this.state.showSearch&&(
        ...
        <div style={{backgroundColor:'black'}}  onClick={()=>hideSearchResult()}  >{searchLocalData(this.props.levelTwoCategory,this.state.searchKeyword)}</div>
       ...
       <Sider
        ...
        >
         ...
         {this.getNavMenuItems(this.props.levelTwoCategory,"inline","dark")}
