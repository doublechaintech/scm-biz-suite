...
const internalImageListOf = (memberRewardPointRedemption) =>{
...
const internalSettingListOf = (memberRewardPointRedemption) =>{
	const userContext = null
	const optionList = [ 
	]
...
const handleTransferSearch =(targetComponent,filterKey,newRequest)=>{
  ...
  const pageNo = 1;
  const candidateReferenceService = MemberRewardPointRedemptionService[candidateServiceName] 
...
const executeTrans = (memberRewardPointRedemption,targetComponent) =>{
  ...
  validateFieldsAndScroll((error, values) => {
    ...
    const id=memberRewardPointRedemption.id;
    const serviceNameToCall = transferServiceName;

    const payload = {parameters,id,serviceNameToCall}
    
    //targetComponent.setState({transferModalVisiable:false})
    dispatch({type:"_memberRewardPointRedemption/doJob",payload: payload})
...
const buildTransferModal = (memberRewardPointRedemption,targetComponent) => {
...
<Modal title={transferModalTitle}
          ...
          onOk={()=>executeTrans(memberRewardPointRedemption,targetComponent)}
...
const internalRenderExtraHeader = (memberRewardPointRedemption) =>{
	return null
}
const internalRenderExtraFooter = (memberRewardPointRedemption) =>{
	return null
}
...
<Description term="业主">{memberRewardPointRedemption.owner==null?appLocaleName(userContext,"NotAssigned"):memberRewardPointRedemption.owner.displayName}
 <Icon type="swap" onClick={()=>
  showTransferModel(targetComponent,"业主","retailStoreMember","requestCandidateOwner",
	      "transferToAnotherOwner","anotherOwnerId",memberRewardPointRedemption.owner?memberRewardPointRedemption.owner.id:"")} 
...
class MemberRewardPointRedemptionPreference extends Component {
...
}))(Form.create()(MemberRewardPointRedemptionPreference))
