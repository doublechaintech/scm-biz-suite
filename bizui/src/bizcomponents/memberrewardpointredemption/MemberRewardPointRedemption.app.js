...
import styles from './MemberRewardPointRedemption.app.less'
...
class MemberRewardPointRedemptionBizApp extends React.PureComponent {
constructor(props) {
    super(props)
     this.state = {
 ...
 getNavMenuItems = (targetObject, style, customTheme) => {
    ...
    return (
	  <Menu
       ...
       <Menu.Item key="workbench">
        <Link to={`/memberRewardPointRedemption/${this.props.memberRewardPointRedemption.id}/workbench`}><Icon type="solution" style={{marginRight:"20px"}}/><span>工作台</span></Link>
      </Menu.Item>
        ...
        {filteredMenuItemsGroup(targetObject,this).map((groupedMenuItem,index)=>{
          return(
    <SubMenu id={`submenu-vg${index}`}  key={`vg${index}`} title={<span><Icon type={viewGroupIconNameOf('member_reward_point_redemption',`${groupedMenuItem.viewGroup}`)} style={{marginRight:"20px"}} /><span>{viewGroupDisplayNameOf('member_reward_point_redemption',`${groupedMenuItem.viewGroup}`)}</span></span>} >
  ...
  getRequestTypeStepForm = () => {
    ...
    return connect(state => ({
      selectedRows: state._memberRewardPointRedemption.selectedRows,
      role: "cq",
      currentUpdateIndex: state._memberRewardPointRedemption.currentUpdateIndex,
      owner: { type: '_memberRewardPointRedemption', id: state._memberRewardPointRedemption.id, listName: 'nolist', ref:state._memberRewardPointRedemption, listDisplayName: appLocaleName(userContext,"List") }, // this is for model namespace and
  ...
  buildRouters = () =>{
    const {MemberRewardPointRedemptionWorkbench} = GlobalComponents

    const {MemberRewardPointRedemptionDashboard} = GlobalComponents
  	const {MemberRewardPointRedemptionPermission} = GlobalComponents
  	const {MemberRewardPointRedemptionProfile} = GlobalComponents


    const routers=[
    {path:"/memberRewardPointRedemption/:id/workbench", component: MemberRewardPointRedemptionWorkbench},
    
  	{path:"/memberRewardPointRedemption/:id/profile", component: MemberRewardPointRedemptionProfile},
  	{path:"/memberRewardPointRedemption/:id/permission", component: MemberRewardPointRedemptionPermission},
  ...
  handleOpenChange = (openKeys) => {
   ...
   render() {
    ...
    const {searchLocalData}=GlobalComponents.MemberRewardPointRedemptionBase
    ...
    const renderMenuSwitch=()=>{
     ...
     <Layout>
      ...
      {this.state.showSearch&&(
        ...
        <div style={{backgroundColor:'black'}}  onClick={()=>hideSearchResult()}  >{searchLocalData(this.props.memberRewardPointRedemption,this.state.searchKeyword)}</div>
       ...
       <Sider
        ...
        >
         ...
         {this.getNavMenuItems(this.props.memberRewardPointRedemption,"inline","dark")}
