...
import styles from './AccountingDocumentLine.app.less'
...
class AccountingDocumentLineBizApp extends React.PureComponent {
constructor(props) {
    super(props)
     this.state = {
 ...
 getNavMenuItems = (targetObject, style, customTheme) => {
    ...
    return (
	  <Menu
       ...
       <Menu.Item key="workbench">
        <Link to={`/accountingDocumentLine/${this.props.accountingDocumentLine.id}/workbench`}><Icon type="solution" style={{marginRight:"20px"}}/><span>工作台</span></Link>
      </Menu.Item>
        ...
        {filteredMenuItemsGroup(targetObject,this).map((groupedMenuItem,index)=>{
          return(
    <SubMenu id={`submenu-vg${index}`}  key={`vg${index}`} title={<span><Icon type={viewGroupIconNameOf('accounting_document_line',`${groupedMenuItem.viewGroup}`)} style={{marginRight:"20px"}} /><span>{viewGroupDisplayNameOf('accounting_document_line',`${groupedMenuItem.viewGroup}`)}</span></span>} >
  ...
  getRequestTypeStepForm = () => {
    ...
    return connect(state => ({
      selectedRows: state._accountingDocumentLine.selectedRows,
      role: "cq",
      currentUpdateIndex: state._accountingDocumentLine.currentUpdateIndex,
      owner: { type: '_accountingDocumentLine', id: state._accountingDocumentLine.id, listName: 'nolist', ref:state._accountingDocumentLine, listDisplayName: appLocaleName(userContext,"List") }, // this is for model namespace and
  ...
  buildRouters = () =>{
    const {AccountingDocumentLineWorkbench} = GlobalComponents

    const {AccountingDocumentLineDashboard} = GlobalComponents
  	const {AccountingDocumentLinePermission} = GlobalComponents
  	const {AccountingDocumentLineProfile} = GlobalComponents


    const routers=[
    {path:"/accountingDocumentLine/:id/workbench", component: AccountingDocumentLineWorkbench},
    
  	{path:"/accountingDocumentLine/:id/profile", component: AccountingDocumentLineProfile},
  	{path:"/accountingDocumentLine/:id/permission", component: AccountingDocumentLinePermission},
  ...
  handleOpenChange = (openKeys) => {
   ...
   render() {
    ...
    const {searchLocalData}=GlobalComponents.AccountingDocumentLineBase
    ...
    const renderMenuSwitch=()=>{
     ...
     <Layout>
      ...
      {this.state.showSearch&&(
        ...
        <div style={{backgroundColor:'black'}}  onClick={()=>hideSearchResult()}  >{searchLocalData(this.props.accountingDocumentLine,this.state.searchKeyword)}</div>
       ...
       <Sider
        ...
        >
         ...
         {this.getNavMenuItems(this.props.accountingDocumentLine,"inline","dark")}
