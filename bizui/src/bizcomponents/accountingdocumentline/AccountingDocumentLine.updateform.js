...
import styles from './AccountingDocumentLine.updateform.less'
...
class AccountingDocumentLineUpdateForm extends Component {
  state = {
...
>>>>>>> 502e8b8dfc403300a992b5083e79c722e85d1854
        ...
        const accountingDocumentLineId = values.id
        const imagesValues = mapBackToImageValues(convertedImagesValues)
        const parameters = { ...values, accountingDocumentLineId, ...imagesValues }
    ...
    const submitUpdateFormAndContinue = () => {
      validateFieldsAndScroll((error, values) => {
        ...
        dispatch({
          type: `${owner.type}/updateAccountingDocumentLine`,
          payload: {
            id: owner.id,
    ...
    const skipToNext = () => {
      ...
      const newIndex = currentUpdateIndex + 1
      dispatch({
        type: `${owner.type}/gotoNextAccountingDocumentLineUpdateRow`,
        payload: {
          id: owner.id,
          type: 'accountingDocumentLine',
          selectedRows,
...
<<<<<<< HEAD
    ...
    return (
      <PageHeaderLayout
        title={"更新会计凭证行"+(currentUpdateIndex+1)+"/"+selectedRows.length}
        content="更新会计凭证行"
...
=======
	...
	const internalRenderTitle = () =>{
      ...
      return (<div>{linkComp}{appLocaleName(userContext,"Update")}会计凭证行: {(currentUpdateIndex+1)}/{selectedRows.length}</div>)
    }
              ...
              <Col lg={12} md={12} sm={24}>
                <Form.Item label={fieldLabels.direct} {...formItemLayout}>
                  {getFieldDecorator('direct', {
                    initialValue: selectedRow.direct,
                    rules: [{ required: true, message: '请输入直接' }],
                  })(
...
}))(Form.create()(AccountingDocumentLineUpdateForm))
