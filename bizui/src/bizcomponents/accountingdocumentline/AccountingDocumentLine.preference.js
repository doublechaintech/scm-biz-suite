...
const internalImageListOf = (accountingDocumentLine) =>{
<<<<<<< HEAD
...
const internalSettingListOf = (accountingDocumentLine) =>{
<<<<<<< HEAD
...
const handleTransferSearch =(targetComponent,filterKey,newRequest)=>{
  ...
  const pageNo = 1;
  const candidateReferenceService = AccountingDocumentLineService[candidateServiceName] 
...
const executeTrans = (accountingDocumentLine,targetComponent) =>{
  ...
  validateFieldsAndScroll((error, values) => {
    ...
    const id=accountingDocumentLine.id;
    const serviceNameToCall = transferServiceName;

    const payload = {parameters,id,serviceNameToCall}
    
    //targetComponent.setState({transferModalVisiable:false})
    dispatch({type:"_accountingDocumentLine/doJob",payload: payload})
...
const buildTransferModal = (accountingDocumentLine,targetComponent) => {
...
<Modal title={transferModalTitle}
          ...
          onOk={()=>executeTrans(accountingDocumentLine,targetComponent)}
...
const internalRenderExtraFooter = (accountingDocumentLine) =>{
	return null
}
...
=======
<Description term="属于">{accountingDocumentLine.belongsTo==null?appLocaleName(userContext,"NotAssigned"):accountingDocumentLine.belongsTo.displayName}
>>>>>>> 502e8b8dfc403300a992b5083e79c722e85d1854
 <Icon type="swap" onClick={()=>
  showTransferModel(targetComponent,"属于","accountingDocument","requestCandidateBelongsTo",
	      "transferToAnotherBelongsTo","anotherBelongsToId",accountingDocumentLine.belongsTo?accountingDocumentLine.belongsTo.id:"")} 
...
=======
<Description term="会计科目">{accountingDocumentLine.accountingSubject==null?appLocaleName(userContext,"NotAssigned"):accountingDocumentLine.accountingSubject.displayName}
>>>>>>> 502e8b8dfc403300a992b5083e79c722e85d1854
 <Icon type="swap" onClick={()=>
  showTransferModel(targetComponent,"会计科目","accountingSubject","requestCandidateAccountingSubject",
	      "transferToAnotherAccountingSubject","anotherAccountingSubjectId",accountingDocumentLine.accountingSubject?accountingDocumentLine.accountingSubject.id:"")} 
  style={{fontSize: 20,color:"red"}} />
</Description>
...
class AccountingDocumentLinePreference extends Component {
...
}))(Form.create()(AccountingDocumentLinePreference))
