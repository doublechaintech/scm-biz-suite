...
import styles from './KeyPairIdentity.updateform.less'
...
class KeyPairIdentityUpdateForm extends Component {
  state = {
  ...
  render() {
    ...
    const submitUpdateForm = () => {
      validateFieldsAndScroll((error, values) => {
        ...
        const keyPairIdentityId = values.id

        const parameters = { ...values, keyPairIdentityId, }
    ...
    const submitUpdateFormAndContinue = () => {
      validateFieldsAndScroll((error, values) => {
        ...
        dispatch({
          type: `${owner.type}/updateKeyPairIdentity`,
          payload: {
            id: owner.id,
            type: 'keyPairIdentity',
            parameters,
            selectedRows,
    ...
    const skipToNext = () => {
      ...
      const newIndex = currentUpdateIndex + 1
      dispatch({
        type: `${owner.type}/gotoNextKeyPairIdentityUpdateRow`,
        payload: {
          id: owner.id,
          type: 'keyPairIdentity',
          selectedRows,
	...
	const internalRenderTitle = () =>{
      ...
      return (<div>{linkComp}{appLocaleName(userContext,"Update")}秘钥对认证: {(currentUpdateIndex+1)}/{selectedRows.length}</div>)
    }
	...
	return (
      ...
      >
       ...
       <Card title={window.trans('key_pair_identity')} className={styles.card} bordered={false}>
          <Form >
          	<Row gutter={16}>
              ...
              <Col lg={16} md={16} sm={16}>
                <Form.Item>
                  {getFieldDecorator('publicKey', {
                  	initialValue: selectedRow.publicKey,
                    rules: [{  required: true, message: appLocaleName(userContext,"PleaseInput") }],
                  })(
...
}))(Form.create()(KeyPairIdentityUpdateForm))
