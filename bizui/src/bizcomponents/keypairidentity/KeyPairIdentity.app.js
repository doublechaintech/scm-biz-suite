...
import styles from './KeyPairIdentity.app.less'
...
class KeyPairIdentityBizApp extends React.PureComponent {
constructor(props) {
    super(props)
     this.state = {
 ...
 getNavMenuItems = (targetObject, style, customTheme) => {
    ...
    return (
	  <Menu
       ...
       <Menu.Item key="workbench">
        <Link to={`/keyPairIdentity/${this.props.keyPairIdentity.id}/workbench`}><Icon type="solution" style={{marginRight:"20px"}}/><span>工作台</span></Link>
      </Menu.Item>
        ...
        {filteredMenuItemsGroup(targetObject,this).map((groupedMenuItem,index)=>{
          return(
    <SubMenu id={`submenu-vg${index}`}  key={`vg${index}`} title={<span><Icon type={viewGroupIconNameOf('key_pair_identity',`${groupedMenuItem.viewGroup}`)} style={{marginRight:"20px"}} /><span>{viewGroupDisplayNameOf('key_pair_identity',`${groupedMenuItem.viewGroup}`)}</span></span>} >
  ...
  getRequestTypeStepForm = () => {
    ...
    return connect(state => ({
      selectedRows: state._keyPairIdentity.selectedRows,
      role: "cq",
      currentUpdateIndex: state._keyPairIdentity.currentUpdateIndex,
      owner: { type: '_keyPairIdentity', id: state._keyPairIdentity.id, listName: 'nolist', ref:state._keyPairIdentity, listDisplayName: appLocaleName(userContext,"List") }, // this is for model namespace and
  ...
  buildRouters = () =>{
    const {KeyPairIdentityWorkbench} = GlobalComponents

    const {KeyPairIdentityDashboard} = GlobalComponents
  	const {KeyPairIdentityPermission} = GlobalComponents
  	const {KeyPairIdentityProfile} = GlobalComponents


    const routers=[
    {path:"/keyPairIdentity/:id/workbench", component: KeyPairIdentityWorkbench},
    
  	{path:"/keyPairIdentity/:id/profile", component: KeyPairIdentityProfile},
  	{path:"/keyPairIdentity/:id/permission", component: KeyPairIdentityPermission},
  ...
  handleOpenChange = (openKeys) => {
   ...
   render() {
    ...
    const {searchLocalData}=GlobalComponents.KeyPairIdentityBase
    ...
    const renderMenuSwitch=()=>{
     ...
     <Layout>
      ...
      {this.state.showSearch&&(
        ...
        <div style={{backgroundColor:'black'}}  onClick={()=>hideSearchResult()}  >{searchLocalData(this.props.keyPairIdentity,this.state.searchKeyword)}</div>
       ...
       <Sider
        ...
        >
         ...
         {this.getNavMenuItems(this.props.keyPairIdentity,"inline","dark")}
