...
const internalImageListOf = (userWhiteList) =>{
...
const internalSettingListOf = (userWhiteList) =>{
	const userContext = null
	const optionList = [ 
	]
...
const handleTransferSearch =(targetComponent,filterKey,newRequest)=>{
  ...
  const pageNo = 1;
  const candidateReferenceService = UserWhiteListService[candidateServiceName] 
...
const executeTrans = (userWhiteList,targetComponent) =>{
...
const buildTransferModal = (userWhiteList,targetComponent) => {
...
<Modal title={transferModalTitle}
          ...
          onOk={()=>executeTrans(userWhiteList,targetComponent)}
...
class UserWhiteListPreference extends Component {
...
}))(Form.create()(UserWhiteListPreference))
