...
import styles from './RetailStoreProvinceCenter.table.less'
...
class RetailStoreProvinceCenterTable extends PureComponent {
  state = {
    selectedRowKeys: [],
  }
...
<<<<<<< HEAD
 calcDisplayColumns=()=>{
    ...
    const {displayColumns} = RetailStoreProvinceCenterBase
    if(!referenceName){
    ...
    const operationColumn={
      ...
      render: (text, record) => (
        <span>
         ...
         { hasReadPermission(metaInfo)&&<Link to={`/retailStoreProvinceCenter/${record.id}/dashboard`}>{'查看'}</Link>}
    ...
    const operationColumn={
      ...
      render: (text, record) => (
        <span>
          ...
          { hasReadPermission(metaInfo)&&<Link to={`/retailStoreProvinceCenter/${record.id}/dashboard`}>{appLocaleName(userContext,"View")}</Link>}
...
export default RetailStoreProvinceCenterTable
