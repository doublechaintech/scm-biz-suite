...
const internalImageListOf = (retailStoreProvinceCenter) =>{
<<<<<<< HEAD
...
const internalSettingListOf = (retailStoreProvinceCenter) =>{
<<<<<<< HEAD
...
const handleTransferSearch =(targetComponent,filterKey,newRequest)=>{
  ...
  const pageNo = 1;
  const candidateReferenceService = RetailStoreProvinceCenterService[candidateServiceName] 
...
const executeTrans = (retailStoreProvinceCenter,targetComponent) =>{
  ...
  validateFieldsAndScroll((error, values) => {
    ...
    const id=retailStoreProvinceCenter.id;
    const serviceNameToCall = transferServiceName;

    const payload = {parameters,id,serviceNameToCall}
    
    //targetComponent.setState({transferModalVisiable:false})
    dispatch({type:"_retailStoreProvinceCenter/doJob",payload: payload})
...
const buildTransferModal = (retailStoreProvinceCenter,targetComponent) => {
...
<Modal title={transferModalTitle}
          ...
          onOk={()=>executeTrans(retailStoreProvinceCenter,targetComponent)}
...
const internalRenderExtraHeader = (retailStoreProvinceCenter) =>{
	return null
}
const internalRenderExtraFooter = (retailStoreProvinceCenter) =>{
	return null
}
...
class RetailStoreProvinceCenterPreference extends Component {
...
}))(Form.create()(RetailStoreProvinceCenterPreference))
