...
const internalImageListOf = (supplyOrderShipment) =>{
...
const internalSettingListOf = (supplyOrderShipment) =>{
	const userContext = null
	const optionList = [ 
	]
...
const handleTransferSearch =(targetComponent,filterKey,newRequest)=>{
  ...
  const pageNo = 1;
  const candidateReferenceService = SupplyOrderShipmentService[candidateServiceName] 
...
const executeTrans = (supplyOrderShipment,targetComponent) =>{
...
const buildTransferModal = (supplyOrderShipment,targetComponent) => {
...
<Modal title={transferModalTitle}
          ...
          onOk={()=>executeTrans(supplyOrderShipment,targetComponent)}
...
const internalRenderExtraHeader = (supplyOrderShipment) =>{
	return null
}
const internalRenderExtraFooter = (supplyOrderShipment) =>{
	return null
}
...
class SupplyOrderShipmentPreference extends Component {
  ...
  render() {
    ...
    const { id,displayName, consumerOrderCount, supplyOrderCount } = this.props.supplyOrderShipment
...
}))(Form.create()(SupplyOrderShipmentPreference))
