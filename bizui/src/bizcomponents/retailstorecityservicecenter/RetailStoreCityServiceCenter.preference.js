...
const internalImageListOf = (retailStoreCityServiceCenter) =>{
...
const internalSettingListOf = (retailStoreCityServiceCenter) =>{
	const userContext = null
	const optionList = [ 
	]
...
const handleTransferSearch =(targetComponent,filterKey,newRequest)=>{
  ...
  const pageNo = 1;
  const candidateReferenceService = RetailStoreCityServiceCenterService[candidateServiceName] 
...
const executeTrans = (retailStoreCityServiceCenter,targetComponent) =>{
  ...
  validateFieldsAndScroll((error, values) => {
    ...
    const id=retailStoreCityServiceCenter.id;
    const serviceNameToCall = transferServiceName;

    const payload = {parameters,id,serviceNameToCall}
    
    //targetComponent.setState({transferModalVisiable:false})
    dispatch({type:"_retailStoreCityServiceCenter/doJob",payload: payload})
...
const buildTransferModal = (retailStoreCityServiceCenter,targetComponent) => {
...
<Modal title={transferModalTitle}
          ...
          onOk={()=>executeTrans(retailStoreCityServiceCenter,targetComponent)}
...
const internalRenderExtraHeader = (retailStoreCityServiceCenter) =>{
	return null
}
const internalRenderExtraFooter = (retailStoreCityServiceCenter) =>{
	return null
}
...
<Description term="属于">{retailStoreCityServiceCenter.belongsTo==null?appLocaleName(userContext,"NotAssigned"):retailStoreCityServiceCenter.belongsTo.displayName}
 <Icon type="swap" onClick={()=>
  showTransferModel(targetComponent,"属于","retailStoreProvinceCenter","requestCandidateBelongsTo",
...
class RetailStoreCityServiceCenterPreference extends Component {
...
}))(Form.create()(RetailStoreCityServiceCenterPreference))
