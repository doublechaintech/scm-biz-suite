...
<<<<<<< HEAD
 calcDisplayColumns=()=>{
    ...
    const {displayColumns} = GoodsBase
    if(!referenceName){
    ...
    const operationColumn={
      ...
      render: (text, record) => (
        <span>
         ...
         { hasReadPermission(metaInfo)&&<Link to={`/goods/${record.id}/dashboard`}>{'查看'}</Link>}
    ...
    const operationColumn={
      ...
      render: (text, record) => (
        <span>
          ...
          { hasReadPermission(metaInfo)&&<Link to={`/goods/${record.id}/dashboard`}>{appLocaleName(userContext,"View")}</Link>}
