 ...
 getNavMenuItems = (targetObject, style, customTheme) => {
    ...
    return (
	  <Menu
       ...
       <Menu.Item key="workbench">
        <Link to={`/levelOneCategory/${this.props.levelOneCategory.id}/workbench`}><Icon type="solution" style={{marginRight:"20px"}}/><span>工作台</span></Link>
      </Menu.Item>
        ...
        {filteredMenuItemsGroup(targetObject,this).map((groupedMenuItem,index)=>{
          return(
    <SubMenu id={`submenu-vg${index}`}  key={`vg${index}`} title={<span><Icon type={viewGroupIconNameOf('level_one_category',`${groupedMenuItem.viewGroup}`)} style={{marginRight:"20px"}} /><span>{viewGroupDisplayNameOf('level_one_category',`${groupedMenuItem.viewGroup}`)}</span></span>} >
  ...
  getLevelTwoCategorySearch = () => {
    ...
    return connect(state => ({
      rule: state.rule,
      name: window.mtrans('level_two_category','level_one_category.level_two_category_list',false),
      ...
      returnURL: `/levelOneCategory/${state._levelOneCategory.id}/workbench`,
  ...
  getLevelTwoCategoryCreateForm = () => {
    ...
    return connect(state => ({
      ...
      searchFormParameters: state._levelOneCategory.levelTwoCategorySearchFormParameters,
      loading: state._levelOneCategory.loading,
      owner: { type: '_levelOneCategory', id: state._levelOneCategory.id || this.getOwnerId(), referenceName: 'parentCategory', listName: 'levelTwoCategoryList', ref:state._levelOneCategory, listDisplayName: appLocaleName(userContext,"List")}, // this is for model namespace and
  ...
  getLevelTwoCategoryUpdateForm = () => {
    ...
    return connect(state => ({
      ...
      role: "levelTwoCategory",
      currentUpdateIndex: state._levelOneCategory.currentUpdateIndex || 0,
      owner: { type: '_levelOneCategory', id: state._levelOneCategory.id || this.getOwnerId(), listName: 'levelTwoCategoryList', ref:state._levelOneCategory, listDisplayName: appLocaleName(userContext,"List") }, // this is for model namespace and
  ...
  getRequestTypeStepForm = () => {
    ...
    return connect(state => ({
      ...
      owner: { type: '_levelOneCategory', id: state._levelOneCategory.id, listName: 'nolist', ref:state._levelOneCategory, listDisplayName: appLocaleName(userContext,"List") }, // this is for model namespace and
  ...
  buildRouters = () =>{
    const {LevelOneCategoryWorkbench} = GlobalComponents
    ...
    const routers=[
    {path:"/levelOneCategory/:id/workbench", component: LevelOneCategoryWorkbench},
  ...
  handleOpenChange = (openKeys) => {
   ...
   render() {
    ...
    const {searchLocalData}=GlobalComponents.LevelOneCategoryBase
    ...
    const renderMenuSwitch=()=>{
     ...
     <Layout>
      ...
      {this.state.showSearch&&(
        ...
        <div style={{backgroundColor:'black'}}  onClick={()=>hideSearchResult()}  >{searchLocalData(this.props.levelOneCategory,this.state.searchKeyword)}</div>
       ...
       <Sider
        ...
        >
         ...
         {this.getNavMenuItems(this.props.levelOneCategory,"inline","dark")}
