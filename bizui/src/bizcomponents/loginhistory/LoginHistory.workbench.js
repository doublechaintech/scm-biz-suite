...
import styles from './LoginHistory.workbench.less'
...
const internalSummaryOf = (cardsData,targetComponent) =>{
	...
	const loginHistory = cardsData.cardsSource
	const {LoginHistoryService} = GlobalComponents
	const userContext = null
	return (
	<div>
	<DescriptionList className={styles.headerList} size="small" col="4">
<Description term="ID" style={{wordBreak: 'break-all'}}>{loginHistory.id}</Description> 
<Description term="登录时间">{ moment(loginHistory.loginTime).format('YYYY-MM-DD HH:mm')}</Description> 
<Description term="来自IP" style={{wordBreak: 'break-all'}}>{loginHistory.fromIp}</Description> 
<Description term="描述" style={{wordBreak: 'break-all'}}>{loginHistory.description}</Description> 
<Description term="系统用户">{loginHistory.secUser==null?appLocaleName(userContext,"NotAssigned"):`${loginHistory.secUser.displayName}(${loginHistory.secUser.id})`}
</Description>
...
class LoginHistoryWorkbench extends Component {
...
}))(Form.create()(LoginHistoryWorkbench))
