...
import styles from './LoginHistory.updateform.less'
...
class LoginHistoryUpdateForm extends Component {
  state = {
  ...
  getSelectedRow() {
    ...
    const convertiedValues = selectedRows.map((item) => {
      return {
        ...item,
        loginTime: moment(item.loginTime),
  ...
  render() {
    ...
    const submitUpdateForm = () => {
      validateFieldsAndScroll((error, values) => {
        ...
        const parameters = { ...values, loginHistoryId, }
	...
	const internalRenderTitle = () =>{
      ...
      return (<div>{linkComp}{appLocaleName(userContext,"Update")}登录历史: {(currentUpdateIndex+1)}/{selectedRows.length}</div>)
    }
