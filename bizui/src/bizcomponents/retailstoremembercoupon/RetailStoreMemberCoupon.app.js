...
constructor(props) {
 ...
 getNavMenuItems = (targetObject, style, customTheme) => {
    ...
    return (
	  <Menu
       ...
       <Menu.Item key="workbench">
        <Link to={`/retailStoreMemberCoupon/${this.props.retailStoreMemberCoupon.id}/workbench`}><Icon type="solution" style={{marginRight:"20px"}}/><span>工作台</span></Link>
      </Menu.Item>
        ...
        {filteredMenuItemsGroup(targetObject,this).map((groupedMenuItem,index)=>{
          return(
    <SubMenu id={`submenu-vg${index}`}  key={`vg${index}`} title={<span><Icon type={viewGroupIconNameOf('retail_store_member_coupon',`${groupedMenuItem.viewGroup}`)} style={{marginRight:"20px"}} /><span>{viewGroupDisplayNameOf('retail_store_member_coupon',`${groupedMenuItem.viewGroup}`)}</span></span>} >
  ...
  buildRouters = () =>{
    const {RetailStoreMemberCouponWorkbench} = GlobalComponents
    ...
    const routers=[
    {path:"/retailStoreMemberCoupon/:id/workbench", component: RetailStoreMemberCouponWorkbench},
