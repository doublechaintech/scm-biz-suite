...
import styles from './RetailStoreMemberCoupon.workbench.less'
...
const renderSettingMenuItem = (item,cardsData,targetComponent) =>{
  ...
  return (<Menu.Item key={item.name}>
      <Link to={`/retailStoreMemberCoupon/${targetComponent.props.retailStoreMemberCoupon.id}/list/${item.name}/${item.displayName}/`}>
...
const internalSummaryOf = (cardsData,targetComponent) =>{
	...
	const retailStoreMemberCoupon = cardsData.cardsSource
	const {RetailStoreMemberCouponService} = GlobalComponents
	const userContext = null
	return (
	<div>
	<DescriptionList className={styles.headerList} size="small" col="4">
<Description term="ID" style={{wordBreak: 'break-all'}}>{retailStoreMemberCoupon.id}</Description> 
...
<Description term="更新于">{ moment(retailStoreMemberCoupon.lastUpdateTime).format('YYYY-MM-DD HH:mm')}</Description> 
...
class RetailStoreMemberCouponWorkbench extends Component {
 ...
 state = {
  ...
  render() {
    ...
    const cardsData = {cardsName:window.trans('retail_store_member_coupon'),cardsFor: "retailStoreMemberCoupon",
...
}))(Form.create()(RetailStoreMemberCouponWorkbench))
