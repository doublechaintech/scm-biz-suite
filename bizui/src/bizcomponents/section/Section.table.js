...
import styles from './Section.table.less'
...
class SectionTable extends PureComponent {
  ...
  enhanceColumnsWithSorter=()=>{
    const {displayColumns} = SectionBase
    ...
    const operationColumn={
      ...
      render: (text, record) => (
        <span>
          ...
          { hasReadPermission(metaInfo)&&<Link to={`/section/${record.id}/dashboard`}>{appLocaleName(userContext,"View")}</Link>}
