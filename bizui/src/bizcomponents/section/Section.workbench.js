...
import styles from './Section.workbench.less'
...
const optionList =(section)=>{return [
	]}

const buildTransferModal = defaultBuildTransferModal
const showTransferModel = defaultShowTransferModel
const internalRenderSubjectList = defaultRenderSubjectList
const internalSettingListOf = (section) =>defaultSettingListOf(section, optionList)
const internalLargeTextOf = (section) =>{
...
const renderSettingMenuItem = (item,cardsData,targetComponent) =>{
  ...
  return (<Menu.Item key={item.name}>
      <Link to={`/section/${targetComponent.props.section.id}/list/${item.name}/${item.displayName}/`}>
...
const renderSettingMenu = (cardsData,targetComponent) =>{
  ...
  return (<Menu>
    	<Menu.Item key="profile">
  			<Link to={`/section/${targetComponent.props.section.id}/permission`}><Icon type="safety-certificate" theme="twoTone" twoToneColor="#52c41a"/><span>{appLocaleName(userContext,"Permission")}</span></Link>
		</Menu.Item>
		<Menu.Divider />
...
const internalSummaryOf = (cardsData,targetComponent) =>{
	...
	const section = cardsData.cardsSource
...
class SectionWorkbench extends Component {
 ...
 state = {
  ...
  render() {
    ...
    const { id,displayName,  } = this.props.section
    if(!this.props.section.class){
      return null
    }
    const returnURL = this.props.returnURL

    const cardsData = {cardsName:window.trans('section'),cardsFor: "section",
    	cardsSource: this.props.section,returnURL,displayName,
  		subItems: [
