...
const renderItemOfList=(slide, targetComponent, columCount, listName)=>{
  ...
  const userContext = null
  return (
    <Card key={`${listName}-${slide.id}`} style={{marginTop:"10px"}}>
...
const packFormValuesToObject = ( formValuesToPack )=>{
	const {name, displayOrder, linkToUrl, pageId} = formValuesToPack
	const page = {id: pageId, version: 2^31}
	const data = {name, displayOrder, linkToUrl, page}
	return data
}
const unpackObjectToFormValues = ( objectToUnpack )=>{
	const {name, displayOrder, linkToUrl, page} = objectToUnpack
	const pageId = page ? page.id : null
	const data = {name, displayOrder, linkToUrl, pageId}
	return data
}
