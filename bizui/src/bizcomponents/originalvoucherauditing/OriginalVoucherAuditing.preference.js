...
const internalImageListOf = (originalVoucherAuditing) =>{
...
const internalSettingListOf = (originalVoucherAuditing) =>{
...
const handleTransferSearch =(targetComponent,filterKey,newRequest)=>{
  ...
  const pageNo = 1;
  const candidateReferenceService = OriginalVoucherAuditingService[candidateServiceName] 
...
const executeTrans = (originalVoucherAuditing,targetComponent) =>{
  ...
  validateFieldsAndScroll((error, values) => {
    ...
    const id=originalVoucherAuditing.id;
    const serviceNameToCall = transferServiceName;

    const payload = {parameters,id,serviceNameToCall}
    
    //targetComponent.setState({transferModalVisiable:false})
    dispatch({type:"_originalVoucherAuditing/doJob",payload: payload})
...
const buildTransferModal = (originalVoucherAuditing,targetComponent) => {
...
<Modal title={transferModalTitle}
          ...
          onOk={()=>executeTrans(originalVoucherAuditing,targetComponent)}
...
const internalRenderExtraHeader = (originalVoucherAuditing) =>{
	return null
}
const internalRenderExtraFooter = (originalVoucherAuditing) =>{
	return null
}
...
class OriginalVoucherAuditingPreference extends Component {
...
}))(Form.create()(OriginalVoucherAuditingPreference))
