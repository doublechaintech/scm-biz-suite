...
import styles from './RetailStoreOrderPicking.preference.less'
...
const internalImageListOf = (retailStoreOrderPicking) =>{
...
const internalSettingListOf = (retailStoreOrderPicking) =>{
...
const handleTransferSearch =(targetComponent,filterKey,newRequest)=>{
  ...
  const pageNo = 1;
  const candidateReferenceService = RetailStoreOrderPickingService[candidateServiceName] 
...
const executeTrans = (retailStoreOrderPicking,targetComponent) =>{
  ...
  validateFieldsAndScroll((error, values) => {
    ...
    const id=retailStoreOrderPicking.id;
    const serviceNameToCall = transferServiceName;

    const payload = {parameters,id,serviceNameToCall}
    
    //targetComponent.setState({transferModalVisiable:false})
    dispatch({type:"_retailStoreOrderPicking/doJob",payload: payload})
...
const buildTransferModal = (retailStoreOrderPicking,targetComponent) => {
...
<Modal title={transferModalTitle}
          ...
          onOk={()=>executeTrans(retailStoreOrderPicking,targetComponent)}
...
const internalRenderExtraHeader = (retailStoreOrderPicking) =>{
	return null
}
const internalRenderExtraFooter = (retailStoreOrderPicking) =>{
	return null
}
...
class RetailStoreOrderPickingPreference extends Component {
...
}))(Form.create()(RetailStoreOrderPickingPreference))
