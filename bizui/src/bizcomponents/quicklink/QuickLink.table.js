...
import styles from './QuickLink.table.less'
...
class QuickLinkTable extends PureComponent {
  state = {
    selectedRowKeys: [],
  }
  ...
  enhanceColumnsWithSorter=()=>{
    const {displayColumns} = QuickLinkBase
    ...
    const operationColumn={
      ...
      render: (text, record) => (
        <span>
          ...
          { hasReadPermission(metaInfo)&&<Link to={`/quickLink/${record.id}/dashboard`}>{appLocaleName(userContext,"View")}</Link>}
