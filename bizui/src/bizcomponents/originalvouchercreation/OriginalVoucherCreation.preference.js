...
const internalImageListOf = (originalVoucherCreation) =>{
...
const internalSettingListOf = (originalVoucherCreation) =>{
	const userContext = null
	const optionList = [ 
	]
...
const handleTransferSearch =(targetComponent,filterKey,newRequest)=>{
  ...
  const pageNo = 1;
  const candidateReferenceService = OriginalVoucherCreationService[candidateServiceName] 
...
const executeTrans = (originalVoucherCreation,targetComponent) =>{
  ...
  validateFieldsAndScroll((error, values) => {
    ...
    const id=originalVoucherCreation.id;
    const serviceNameToCall = transferServiceName;

    const payload = {parameters,id,serviceNameToCall}
    
    //targetComponent.setState({transferModalVisiable:false})
    dispatch({type:"_originalVoucherCreation/doJob",payload: payload})
...
const buildTransferModal = (originalVoucherCreation,targetComponent) => {
...
<Modal title={transferModalTitle}
          ...
          onOk={()=>executeTrans(originalVoucherCreation,targetComponent)}
...
const internalRenderExtraHeader = (originalVoucherCreation) =>{
	return null
}
const internalRenderExtraFooter = (originalVoucherCreation) =>{
	return null
}
...
class OriginalVoucherCreationPreference extends Component {
  ...
  render() {
    ...
    const { id,displayName, originalVoucherCount } = this.props.originalVoucherCreation
...
}))(Form.create()(OriginalVoucherCreationPreference))
