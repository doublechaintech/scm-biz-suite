...
const internalImageListOf = (retailStoreOrderConfirmation) =>{
...
const internalSettingListOf = (retailStoreOrderConfirmation) =>{
	const userContext = null
	const optionList = [ 
	]
...
const handleTransferSearch =(targetComponent,filterKey,newRequest)=>{
  ...
  const pageNo = 1;
  const candidateReferenceService = RetailStoreOrderConfirmationService[candidateServiceName] 
...
const executeTrans = (retailStoreOrderConfirmation,targetComponent) =>{
  ...
  validateFieldsAndScroll((error, values) => {
    ...
    const id=retailStoreOrderConfirmation.id;
    const serviceNameToCall = transferServiceName;

    const payload = {parameters,id,serviceNameToCall}
    
    //targetComponent.setState({transferModalVisiable:false})
    dispatch({type:"_retailStoreOrderConfirmation/doJob",payload: payload})
...
const buildTransferModal = (retailStoreOrderConfirmation,targetComponent) => {
...
<Modal title={transferModalTitle}
          ...
          onOk={()=>executeTrans(retailStoreOrderConfirmation,targetComponent)}
...
const internalRenderExtraHeader = (retailStoreOrderConfirmation) =>{
	return null
}
const internalRenderExtraFooter = (retailStoreOrderConfirmation) =>{
	return null
}
...
class RetailStoreOrderConfirmationPreference extends Component {
  ...
  render() {
    ...
    const { id,displayName, retailStoreOrderCount } = this.props.retailStoreOrderConfirmation
...
}))(Form.create()(RetailStoreOrderConfirmationPreference))
