...
export default {
  ...
  subscriptions: {
    ...
    setup({ dispatch, history }) { 
      history.listen((location) => {
      	const modelName = 'secUserBlocking'
  ...
  effects: {
    *view({ payload }, { call, put, select }) { 
      ...
      const cachedData = yield select(state => state._secUserBlocking)
    ...
    *gotoCreateForm({ payload }, { put }) {
      ...
      yield put(routerRedux.push(`/secUserBlocking/${id}/list/${role}CreateForm`))
    },
    *gotoUpdateForm({ payload }, { put }) {
      const { id, role, selectedRows, currentUpdateIndex } = payload
      const state = { id, role, selectedRows, currentUpdateIndex }
      const location = { pathname: `/secUserBlocking/${id}/list/${role}UpdateForm`, state }
    ...
    *addSecUser({ payload }, { call, put }) {
      ...
      const data = yield call(SecUserBlockingService.addSecUser, id, parameters)
      if (hasError(data)) {
      ...
      const location = { pathname: `/secUserBlocking/${id}/list/SecUserList/Sec User+${appLocaleName(userContext,'List')}`, state: newState }
      yield put(routerRedux.push(location))
    },
    *updateSecUser({ payload }, { call, put }) {
      ...
      const location = { pathname: `/secUserBlocking/${id}/list/SecUserList/Sec User列表`, state: newPlayload }
      yield put(routerRedux.push(location))
    },
