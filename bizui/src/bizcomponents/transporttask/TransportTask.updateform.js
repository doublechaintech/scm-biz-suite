...
class TransportTaskUpdateForm extends Component {
  state = {
  ...
  render() {
    ...
    const submitUpdateForm = () => {
      validateFieldsAndScroll((error, values) => {
        ...
        const parameters = { ...values, transportTaskId, }
    ...
    const submitUpdateFormAndContinue = () => {
      validateFieldsAndScroll((error, values) => {
        ...
        dispatch({
          type: `${owner.type}/updateTransportTask`,
          payload: {
            id: owner.id,
            type: 'transportTask',
            parameters,
    ...
    const skipToNext = () => {
      ...
      const newIndex = currentUpdateIndex + 1
      dispatch({
        type: `${owner.type}/gotoNextTransportTaskUpdateRow`,
        payload: {
          id: owner.id,
          type: 'transportTask',
          selectedRows,
	...
	const internalRenderTitle = () =>{
      ...
      return (<div>{linkComp}{appLocaleName(userContext,"Update")}运输任务: {(currentUpdateIndex+1)}/{selectedRows.length}</div>)
    }
              ...
              <Col lg={24} md={24} sm={24}>
                <Form.Item label={fieldLabels.start} {...formItemLayout}>
                  {getFieldDecorator('start', {
                    ...
                    <SmallTextInput size="large"  placeholder={fieldLabels.start} />
...
}))(Form.create()(TransportTaskUpdateForm))
