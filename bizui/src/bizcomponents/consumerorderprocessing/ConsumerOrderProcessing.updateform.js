...
import styles from './ConsumerOrderProcessing.updateform.less'
...
class ConsumerOrderProcessingUpdateForm extends Component {
  state = {
  ...
  getSelectedRow() {
    ...
    const convertiedValues = selectedRows.map((item) => {
      return {
        ...item,
        processTime: moment(item.processTime),
...
>>>>>>> 502e8b8dfc403300a992b5083e79c722e85d1854
        ...
        const consumerOrderProcessingId = values.id
        const imagesValues = mapBackToImageValues(convertedImagesValues)
        const parameters = { ...values, consumerOrderProcessingId, ...imagesValues }
    ...
    const submitUpdateFormAndContinue = () => {
      validateFieldsAndScroll((error, values) => {
        ...
        dispatch({
          type: `${owner.type}/updateConsumerOrderProcessing`,
          payload: {
            id: owner.id,
            type: 'consumerOrderProcessing',
            parameters,
            selectedRows,
    ...
    const skipToNext = () => {
      ...
      const newIndex = currentUpdateIndex + 1
      dispatch({
        type: `${owner.type}/gotoNextConsumerOrderProcessingUpdateRow`,
        payload: {
          id: owner.id,
          type: 'consumerOrderProcessing',
          selectedRows,
...
<<<<<<< HEAD
    ...
    return (
      <PageHeaderLayout
        title={"更新消费者的订单处理"+(currentUpdateIndex+1)+"/"+selectedRows.length}
        content="更新消费者的订单处理"
...
=======
	...
	const internalRenderTitle = () =>{
      ...
      return (<div>{linkComp}{appLocaleName(userContext,"Update")}消费者的订单处理: {(currentUpdateIndex+1)}/{selectedRows.length}</div>)
    }
              ...
              <Col lg={12} md={12} sm={24}>
                <Form.Item label={fieldLabels.processTime} {...formItemLayout}>
                  {getFieldDecorator('processTime', {
                    initialValue: selectedRow.processTime,
...
}))(Form.create()(ConsumerOrderProcessingUpdateForm))
