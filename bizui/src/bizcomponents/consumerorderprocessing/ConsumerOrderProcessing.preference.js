...
const internalImageListOf = (consumerOrderProcessing) =>{
...
const internalSettingListOf = (consumerOrderProcessing) =>{
	const userContext = null
	const optionList = [ 
	]
...
const handleTransferSearch =(targetComponent,filterKey,newRequest)=>{
  ...
  const pageNo = 1;
  const candidateReferenceService = ConsumerOrderProcessingService[candidateServiceName] 
...
const executeTrans = (consumerOrderProcessing,targetComponent) =>{
  ...
  validateFieldsAndScroll((error, values) => {
    ...
    const id=consumerOrderProcessing.id;
    const serviceNameToCall = transferServiceName;

    const payload = {parameters,id,serviceNameToCall}
    
    //targetComponent.setState({transferModalVisiable:false})
    dispatch({type:"_consumerOrderProcessing/doJob",payload: payload})
...
const buildTransferModal = (consumerOrderProcessing,targetComponent) => {
...
<Modal title={transferModalTitle}
          ...
          onOk={()=>executeTrans(consumerOrderProcessing,targetComponent)}
...
const internalRenderExtraFooter = (consumerOrderProcessing) =>{
	return null
}
...
class ConsumerOrderProcessingPreference extends Component {
  ...
  render() {
    ...
    const cardsData = {cardsName:"消费者的订单处理",cardsFor: "consumerOrderProcessing",cardsSource: this.props.consumerOrderProcessing,
  		subItems: [
...
}))(Form.create()(ConsumerOrderProcessingPreference))
