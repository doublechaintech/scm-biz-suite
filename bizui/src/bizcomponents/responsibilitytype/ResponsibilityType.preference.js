...
const internalImageListOf = (responsibilityType) =>{
...
const internalSettingListOf = (responsibilityType) =>{
	const userContext = null
	const optionList = [ 
	]
...
const handleTransferSearch =(targetComponent,filterKey,newRequest)=>{
  ...
  const pageNo = 1;
  const candidateReferenceService = ResponsibilityTypeService[candidateServiceName] 
...
const executeTrans = (responsibilityType,targetComponent) =>{
...
const buildTransferModal = (responsibilityType,targetComponent) => {
...
<Modal title={transferModalTitle}
          ...
          onOk={()=>executeTrans(responsibilityType,targetComponent)}
...
const internalRenderExtraHeader = (responsibilityType) =>{
	return null
}
const internalRenderExtraFooter = (responsibilityType) =>{
	return null
}
...
class ResponsibilityTypePreference extends Component {
...
}))(Form.create()(ResponsibilityTypePreference))
