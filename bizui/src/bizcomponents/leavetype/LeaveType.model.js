...
export default {
  ...
  effects: {
    *view({ payload }, { call, put, select }) { 
      ...
      const cachedData = yield select(state => state._leaveType)
      ...
      const data = yield call(LeaveTypeService.view, payload.id)
    ...
    *load({ payload }, { call, put }) { 
      const {LeaveTypeService} = GlobalComponents;
      //yield put({ type: 'showLoading', payload })
      const data = yield call(LeaveTypeService.load, payload.id, payload.parameters)
...
>>>>>>> 502e8b8dfc403300a992b5083e79c722e85d1854
    ...
    *gotoCreateForm({ payload }, { put }) {
      ...
      yield put(routerRedux.push(`/leaveType/${id}/list/${role}CreateForm`))
    },
    *gotoUpdateForm({ payload }, { put }) {
      const { id, role, selectedRows, currentUpdateIndex } = payload
      const state = { id, role, selectedRows, currentUpdateIndex }
      const location = { pathname: `/leaveType/${id}/list/${role}UpdateForm`, state }
      yield put(routerRedux.push(location))
    },
    *goback({ payload }, { put }) {
      const { id, type,listName } = payload
      yield put(routerRedux.push(`/leaveType/${id}/list/${type}List/${listName}`))
    },
...
>>>>>>> 502e8b8dfc403300a992b5083e79c722e85d1854
      ...
      const data = yield call(LeaveTypeService.addEmployeeLeave, id, parameters)
      if (hasError(data)) {
...
<<<<<<< HEAD
      const location = { pathname: `/leaveType/${id}/list/\EmployeeLeaveList/请假记录列表`, state: newState }
      yield put(routerRedux.push(location))
    },
...
>>>>>>> 502e8b8dfc403300a992b5083e79c722e85d1854
      ...
      console.log('get form parameters', parameters)
      const data = yield call(LeaveTypeService.updateEmployeeLeave, id, parameters)
      if (hasError(data)) {
        handleServerError(data)
        return
      }
...
>>>>>>> 502e8b8dfc403300a992b5083e79c722e85d1854
      ...
      const { id, role, parameters, continueNext } = payload
      console.log('get form parameters', parameters)
      const data = yield call(LeaveTypeService.removeEmployeeLeaveList, id, parameters)
      if (hasError(data)) {
        handleServerError(data)
        return
      }
