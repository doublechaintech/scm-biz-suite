 ...
 getNavMenuItems = (targetObject, style, customTheme) => {
    ...
    return (
	  <Menu
       ...
       <Menu.Item key="workbench">
        <Link to={`/leaveType/${this.props.leaveType.id}/workbench`}><Icon type="solution" style={{marginRight:"20px"}}/><span>工作台</span></Link>
      </Menu.Item>
        ...
        {filteredMenuItemsGroup(targetObject,this).map((groupedMenuItem,index)=>{
          return(
    <SubMenu id={`submenu-vg${index}`}  key={`vg${index}`} title={<span><Icon type={viewGroupIconNameOf('leave_type',`${groupedMenuItem.viewGroup}`)} style={{marginRight:"20px"}} /><span>{viewGroupDisplayNameOf('leave_type',`${groupedMenuItem.viewGroup}`)}</span></span>} >
  ...
  getEmployeeLeaveSearch = () => {
    ...
    return connect(state => ({
      rule: state.rule,
      name: window.mtrans('employee_leave','leave_type.employee_leave_list',false),
      role: "employeeLeave",
      ...
      returnURL: `/leaveType/${state._leaveType.id}/workbench`,
  ...
  getEmployeeLeaveCreateForm = () => {
    ...
    return connect(state => ({
      ...
      metaInfo: state._leaveType.employeeLeaveListMetaInfo,
      count: state._leaveType.employeeLeaveCount,
      returnURL: `/leaveType/${state._leaveType.id}/list`,
      currentPage: state._leaveType.employeeLeaveCurrentPageNumber,
      searchFormParameters: state._leaveType.employeeLeaveSearchFormParameters,
      loading: state._leaveType.loading,
      owner: { type: '_leaveType', id: state._leaveType.id || this.getOwnerId(), referenceName: 'type', listName: 'employeeLeaveList', ref:state._leaveType, listDisplayName: appLocaleName(userContext,"List")}, // this is for model namespace and
  ...
  getEmployeeLeaveUpdateForm = () => {
    ...
    return connect(state => ({
      ...
      role: "employeeLeave",
      currentUpdateIndex: state._leaveType.currentUpdateIndex || 0,
      owner: { type: '_leaveType', id: state._leaveType.id || this.getOwnerId(), listName: 'employeeLeaveList', ref:state._leaveType, listDisplayName: appLocaleName(userContext,"List") }, // this is for model namespace and
  ...
  getRequestTypeStepForm = () => {
    ...
    return connect(state => ({
      ...
      owner: { type: '_leaveType', id: state._leaveType.id, listName: 'nolist', ref:state._leaveType, listDisplayName: appLocaleName(userContext,"List") }, // this is for model namespace and
  ...
  buildRouters = () =>{
    const {LeaveTypeWorkbench} = GlobalComponents
    ...
    const routers=[
    {path:"/leaveType/:id/workbench", component: LeaveTypeWorkbench},
  ...
  handleOpenChange = (openKeys) => {
   ...
   render() {
    ...
    const {searchLocalData}=GlobalComponents.LeaveTypeBase
    ...
    const renderMenuSwitch=()=>{
     ...
     <Layout>
      ...
      {this.state.showSearch&&(
        ...
        <div style={{backgroundColor:'black'}}  onClick={()=>hideSearchResult()}  >{searchLocalData(this.props.leaveType,this.state.searchKeyword)}</div>
       ...
       <Sider
        ...
        >
         ...
         {this.getNavMenuItems(this.props.leaveType,"inline","dark")}
