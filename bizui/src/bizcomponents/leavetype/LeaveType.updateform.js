...
import styles from './LeaveType.updateform.less'
...
class LeaveTypeUpdateForm extends Component {
  state = {
  ...
  render() {
    ...
    const submitUpdateForm = () => {
      validateFieldsAndScroll((error, values) => {
        ...
        const leaveTypeId = values.id

        const parameters = { ...values, leaveTypeId, }
    ...
    const submitUpdateFormAndContinue = () => {
      validateFieldsAndScroll((error, values) => {
        ...
        dispatch({
          type: `${owner.type}/updateLeaveType`,
          payload: {
            id: owner.id,
            type: 'leaveType',
            parameters,
    ...
    const skipToNext = () => {
      ...
      const newIndex = currentUpdateIndex + 1
      dispatch({
        type: `${owner.type}/gotoNextLeaveTypeUpdateRow`,
        payload: {
          id: owner.id,
          type: 'leaveType',
          selectedRows,
	...
	const internalRenderTitle = () =>{
      ...
      return (<div>{linkComp}{appLocaleName(userContext,"Update")}请假类型: {(currentUpdateIndex+1)}/{selectedRows.length}</div>)
    }
...
}))(Form.create()(LeaveTypeUpdateForm))
