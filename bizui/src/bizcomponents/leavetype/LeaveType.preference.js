...
const internalImageListOf = (leaveType) =>{
<<<<<<< HEAD
...
const internalSettingListOf = (leaveType) =>{
<<<<<<< HEAD
...
const handleTransferSearch =(targetComponent,filterKey,newRequest)=>{
  ...
  const pageNo = 1;
  const candidateReferenceService = LeaveTypeService[candidateServiceName] 
...
const executeTrans = (leaveType,targetComponent) =>{
  ...
  validateFieldsAndScroll((error, values) => {
    ...
    //targetComponent.setState({transferModalVisiable:false})
    dispatch({type:"_leaveType/doJob",payload: payload})
...
const buildTransferModal = (leaveType,targetComponent) => {
...
<Modal title={transferModalTitle}
          ...
          onOk={()=>executeTrans(leaveType,targetComponent)}
...
const internalRenderExtraHeader = (leaveType) =>{
	return null
}
const internalRenderExtraFooter = (leaveType) =>{
	return null
}
...
class LeaveTypePreference extends Component {
...
}))(Form.create()(LeaveTypePreference))
