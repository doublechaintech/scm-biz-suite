...
import styles from './CandidateContainer.dashboard.less'
...
const imageList =(candidateContainer)=>{return [
	 ]}

const internalImageListOf = (candidateContainer) =>defaultImageListOf(candidateContainer,imageList)

const optionList =(candidateContainer)=>{return [
	]}

const buildTransferModal = defaultBuildTransferModal
const showTransferModel = defaultShowTransferModel
const internalRenderSubjectList = defaultRenderSubjectList
const internalSettingListOf = (candidateContainer) =>defaultSettingListOf(candidateContainer, optionList)
const internalLargeTextOf = (candidateContainer) =>{
...
const renderSettingMenuItem = (item,cardsData,targetComponent) =>{
  ...
  return (<Menu.Item key={item.name}>
      <Link to={`/candidateContainer/${targetComponent.props.candidateContainer.id}/list/${item.name}/${item.displayName}/`}>
...
const internalSummaryOf = (cardsData,targetComponent) =>{
	...
	const candidateContainer = cardsData.cardsSource
	const {CandidateContainerService} = GlobalComponents
	const userContext = null
	return (
	<div>
	<DescriptionList className={styles.headerList} size="small" col="4">
<Description term="ID" style={{wordBreak: 'break-all'}}>{candidateContainer.id}</Description> 
...
class CandidateContainerDashboard extends Component {
 ...
 state = {
    ...
    defaultType: 'candidateContainer'
  ...
  render() {
    ...
    const { id,displayName, candidateElementListMetaInfo, candidateElementCount } = this.props.candidateContainer
    if(!this.props.candidateContainer.class){
      return null
    }
...
}))(Form.create()(CandidateContainerDashboard))
