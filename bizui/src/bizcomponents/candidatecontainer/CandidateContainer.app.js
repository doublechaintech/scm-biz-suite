 ...
 getNavMenuItems = (targetObject, style, customTheme) => {
    ...
    return (
	  <Menu
       ...
       <Menu.Item key="workbench">
        <Link to={`/candidateContainer/${this.props.candidateContainer.id}/workbench`}><Icon type="solution" style={{marginRight:"20px"}}/><span>工作台</span></Link>
      </Menu.Item>
        ...
        {filteredMenuItemsGroup(targetObject,this).map((groupedMenuItem,index)=>{
          return(
    <SubMenu id={`submenu-vg${index}`}  key={`vg${index}`} title={<span><Icon type={viewGroupIconNameOf('candidate_container',`${groupedMenuItem.viewGroup}`)} style={{marginRight:"20px"}} /><span>{viewGroupDisplayNameOf('candidate_container',`${groupedMenuItem.viewGroup}`)}</span></span>} >
  ...
  getCandidateElementSearch = () => {
    ...
    return connect(state => ({
      rule: state.rule,
      name: window.mtrans('candidate_element','candidate_container.candidate_element_list',false),
      ...
      returnURL: `/candidateContainer/${state._candidateContainer.id}/workbench`,
  ...
  getCandidateElementCreateForm = () => {
    ...
    return connect(state => ({
      ...
      searchFormParameters: state._candidateContainer.candidateElementSearchFormParameters,
      loading: state._candidateContainer.loading,
      owner: { type: '_candidateContainer', id: state._candidateContainer.id || this.getOwnerId(), referenceName: 'container', listName: 'candidateElementList', ref:state._candidateContainer, listDisplayName: appLocaleName(userContext,"List")}, // this is for model namespace and
  ...
  getCandidateElementUpdateForm = () => {
    ...
    return connect(state => ({
      ...
      role: "candidateElement",
      currentUpdateIndex: state._candidateContainer.currentUpdateIndex || 0,
      owner: { type: '_candidateContainer', id: state._candidateContainer.id || this.getOwnerId(), listName: 'candidateElementList', ref:state._candidateContainer, listDisplayName: appLocaleName(userContext,"List") }, // this is for model namespace and
  ...
  getRequestTypeStepForm = () => {
    ...
    return connect(state => ({
      ...
      owner: { type: '_candidateContainer', id: state._candidateContainer.id, listName: 'nolist', ref:state._candidateContainer, listDisplayName: appLocaleName(userContext,"List") }, // this is for model namespace and
  ...
  buildRouters = () =>{
    const {CandidateContainerWorkbench} = GlobalComponents
    ...
    const routers=[
    {path:"/candidateContainer/:id/workbench", component: CandidateContainerWorkbench},
  ...
  handleOpenChange = (openKeys) => {
   ...
   render() {
    ...
    const {searchLocalData}=GlobalComponents.CandidateContainerBase
    ...
    const renderMenuSwitch=()=>{
     ...
     <Layout>
      ...
      {this.state.showSearch&&(
        ...
        <div style={{backgroundColor:'black'}}  onClick={()=>hideSearchResult()}  >{searchLocalData(this.props.candidateContainer,this.state.searchKeyword)}</div>
       ...
       <Sider
        ...
        >
         ...
         {this.getNavMenuItems(this.props.candidateContainer,"inline","dark")}
