...
class MoyiSearch extends PureComponent {
  state = {
  ...
  render(){
    ...
    const {MoyiTable} = GlobalComponents;
    const {MoyiSearchForm} = GlobalComponents;
    const {MoyiModalTable} = GlobalComponents;
    ...
    return (
      <PageHeaderLayout title={`${displayName}:${this.props.name}列表`}>
        ...
        {showDeletionDialog(this,MoyiModalTable,"moyiIds")}
...
export default Form.create()(MoyiSearch)
