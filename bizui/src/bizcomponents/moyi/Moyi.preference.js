...
import styles from './Moyi.preference.less'
...
const internalImageListOf = (moyi) =>{
...
const internalSettingListOf = (moyi) =>{
...
const internalLargeTextOf = (moyi) =>{
...
const handleTransferSearch =(targetComponent,filterKey,newRequest)=>{
  const {MoyiService} = GlobalComponents;
  ...
  const pageNo = 1;
  const candidateReferenceService = MoyiService[candidateServiceName] 
...
const executeTrans = (moyi,targetComponent) =>{
  ...
  validateFieldsAndScroll((error, values) => {
    ...
    //targetComponent.setState({transferModalVisiable:false})
    dispatch({type:"_moyi/doJob",payload: payload})
...
const buildTransferModal = (moyi,targetComponent) => {
...
<Modal title={transferModalTitle}
          ...
          onOk={()=>executeTrans(moyi,targetComponent)}
...
const internalRenderExtraHeader = (moyi) =>{
	return null
}
const internalRenderExtraFooter = (moyi) =>{
	return null
}
...
const internalSummaryOf = (moyi,targetComponent) =>{

	return (
	<DescriptionList className={styles.headerList} size="small" col="4">
<Description term="序号">{moyi.id}</Description> 
<Description term="名称">{moyi.name}</Description> 
<Description term="描述">{moyi.description}</Description> 
	
        {buildTransferModal(moyi,targetComponent)}
      </DescriptionList>
	)
...
const renderPermissionSetting = moyi => {
  const {MoyiBase} = GlobalComponents
  return <PermissionSetting targetObject={moyi}  targetObjectMeta={MoyiBase}/>


}


class MoyiPreference extends Component {
  ...
  render() {
    ...
    const { id,displayName,  } = this.props.moyi
    const cardsData = {cardsName:"Moyi",cardsFor: "moyi",cardsSource: this.props.moyi,
  		subItems: [
