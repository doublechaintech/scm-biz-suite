...
import styles from './OriginalVoucherConfirmation.table.less'
...
class OriginalVoucherConfirmationTable extends PureComponent {
  state = {
    selectedRowKeys: [],
  }
...
<<<<<<< HEAD
 calcDisplayColumns=()=>{
    ...
    const {displayColumns} = OriginalVoucherConfirmationBase
    if(!referenceName){
    ...
    const operationColumn={
      ...
      render: (text, record) => (
        <span>
         ...
         { hasReadPermission(metaInfo)&&<Link to={`/originalVoucherConfirmation/${record.id}/dashboard`}>{'查看'}</Link>}
    ...
    const operationColumn={
      ...
      render: (text, record) => (
        <span>
          ...
          { hasReadPermission(metaInfo)&&<Link to={`/originalVoucherConfirmation/${record.id}/dashboard`}>{appLocaleName(userContext,"View")}</Link>}
...
export default OriginalVoucherConfirmationTable
