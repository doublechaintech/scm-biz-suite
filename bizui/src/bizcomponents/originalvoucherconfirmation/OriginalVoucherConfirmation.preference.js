...
import styles from './OriginalVoucherConfirmation.preference.less'
...
const internalImageListOf = (originalVoucherConfirmation) =>{
...
const internalSettingListOf = (originalVoucherConfirmation) =>{
	const userContext = null
	const optionList = [ 
	]
...
const handleTransferSearch =(targetComponent,filterKey,newRequest)=>{
  ...
  const pageNo = 1;
  const candidateReferenceService = OriginalVoucherConfirmationService[candidateServiceName] 
...
const executeTrans = (originalVoucherConfirmation,targetComponent) =>{
  ...
  validateFieldsAndScroll((error, values) => {
    ...
    const id=originalVoucherConfirmation.id;
    const serviceNameToCall = transferServiceName;

    const payload = {parameters,id,serviceNameToCall}
    
    //targetComponent.setState({transferModalVisiable:false})
    dispatch({type:"_originalVoucherConfirmation/doJob",payload: payload})
...
const buildTransferModal = (originalVoucherConfirmation,targetComponent) => {
...
<Modal title={transferModalTitle}
          ...
          onOk={()=>executeTrans(originalVoucherConfirmation,targetComponent)}
...
const internalRenderExtraHeader = (originalVoucherConfirmation) =>{
	return null
}
const internalRenderExtraFooter = (originalVoucherConfirmation) =>{
	return null
}
...
class OriginalVoucherConfirmationPreference extends Component {
...
}))(Form.create()(OriginalVoucherConfirmationPreference))
