...
const menuData = {menuName: window.trans('storage_space'), menuFor: "storageSpace",  internalName: "storage_space",
  		subItems: [
...
const settingMenuData = {menuName: window.trans('storage_space'), menuFor: "storageSpace",  internalName: "storage_space",
  		subItems: [
...
const displayColumns = [
  { title: fieldLabels.id, debugtype: 'string', dataIndex: 'id', width: '6', render: (text, record)=>renderTextCell(text,record,'storageSpace') , sorter: true },
...
const renderActionList=(storageSpace, targetObject, columCount, listName)=>{

	if(!storageSpace){
		return null
	}
	if(!storageSpace.actionList){
		return null
	}
	if(storageSpace.actionList.length === 0){
		return null
	}
	return (
		<div className={styles.overlay}>
			...
			{storageSpace.actionList.map(action=>(<Link key={action.id} to={{pathname: action.actionPath.substring(1), state: {ownerId:targetObject.id,action,selectedRows:[storageSpace]}}} >
...
const renderItemOfList=(storageSpace, targetObject, columCount, listName)=>{
  ...
  const userContext = null
  return (
     <Row key={`${listName}-${storageSpace.id}`} className={styles.itemDesc}>
	...
	<Col span={20}>
      ...
      <DescriptionList  key={storageSpace.id} size="small" col={displayColumnsCount} >
        ...
        <Description term={fieldLabels.warehouse}>{renderReferenceItem(storageSpace.warehouse)}</Description>

        <Description term={fieldLabels.latitude}><div style={{"color":"red"}}>{formatNumber(storageSpace.latitude,6)}</div></Description> 
        <Description term={fieldLabels.longitude}><div style={{"color":"red"}}>{formatNumber(storageSpace.longitude,6)}</div></Description> 
      ...
      {renderActionList(storageSpace,targetObject)}
    </Row>
	)
...
const StorageSpaceBase={unpackObjectToFormValues, menuData,settingMenuData,displayColumns,fieldLabels,renderItemOfList, stepOf, searchLocalData}
