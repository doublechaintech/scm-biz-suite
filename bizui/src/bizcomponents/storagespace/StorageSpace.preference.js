...
const internalImageListOf = (storageSpace) =>{
...
const internalSettingListOf = (storageSpace) =>{
	const userContext = null
	const optionList = [ 
	]
...
const handleTransferSearch =(targetComponent,filterKey,newRequest)=>{
  ...
  const pageNo = 1;
  const candidateReferenceService = StorageSpaceService[candidateServiceName] 
...
const executeTrans = (storageSpace,targetComponent) =>{
...
const buildTransferModal = (storageSpace,targetComponent) => {
...
<Modal title={transferModalTitle}
          ...
          onOk={()=>executeTrans(storageSpace,targetComponent)}
...
const internalRenderExtraHeader = (storageSpace) =>{
	return null
}
const internalRenderExtraFooter = (storageSpace) =>{
	return null
}
...
<Description term="仓库">{storageSpace.warehouse==null?appLocaleName(userContext,"NotAssigned"):storageSpace.warehouse.displayName}
 <Icon type="swap" onClick={()=>
  showTransferModel(targetComponent,"仓库","warehouse","requestCandidateWarehouse",
	      "transferToAnotherWarehouse","anotherWarehouseId",storageSpace.warehouse?storageSpace.warehouse.id:"")} 
...
class StorageSpacePreference extends Component {
...
}))(Form.create()(StorageSpacePreference))
