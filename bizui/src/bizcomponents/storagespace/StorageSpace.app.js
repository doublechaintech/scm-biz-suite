 ...
 getNavMenuItems = (targetObject, style, customTheme) => {
    ...
    return (
	  <Menu
       ...
       <Menu.Item key="workbench">
        <Link to={`/storageSpace/${this.props.storageSpace.id}/workbench`}><Icon type="solution" style={{marginRight:"20px"}}/><span>工作台</span></Link>
      </Menu.Item>
        ...
        {filteredMenuItemsGroup(targetObject,this).map((groupedMenuItem,index)=>{
          return(
    <SubMenu id={`submenu-vg${index}`}  key={`vg${index}`} title={<span><Icon type={viewGroupIconNameOf('storage_space',`${groupedMenuItem.viewGroup}`)} style={{marginRight:"20px"}} /><span>{viewGroupDisplayNameOf('storage_space',`${groupedMenuItem.viewGroup}`)}</span></span>} >
  ...
  getGoodsShelfSearch = () => {
    ...
    return connect(state => ({
      rule: state.rule,
      name: window.mtrans('goods_shelf','storage_space.goods_shelf_list',false),
      role: "goodsShelf",
      ...
      returnURL: `/storageSpace/${state._storageSpace.id}/workbench`,
  ...
  getGoodsShelfCreateForm = () => {
    ...
    return connect(state => ({
      ...
      metaInfo: state._storageSpace.goodsShelfListMetaInfo,
      count: state._storageSpace.goodsShelfCount,
      returnURL: `/storageSpace/${state._storageSpace.id}/list`,
      currentPage: state._storageSpace.goodsShelfCurrentPageNumber,
      searchFormParameters: state._storageSpace.goodsShelfSearchFormParameters,
      loading: state._storageSpace.loading,
      owner: { type: '_storageSpace', id: state._storageSpace.id || this.getOwnerId(), referenceName: 'storageSpace', listName: 'goodsShelfList', ref:state._storageSpace, listDisplayName: appLocaleName(userContext,"List")}, // this is for model namespace and
  ...
  getGoodsShelfUpdateForm = () => {
    ...
    return connect(state => ({
      ...
      role: "goodsShelf",
      currentUpdateIndex: state._storageSpace.currentUpdateIndex || 0,
      owner: { type: '_storageSpace', id: state._storageSpace.id || this.getOwnerId(), listName: 'goodsShelfList', ref:state._storageSpace, listDisplayName: appLocaleName(userContext,"List") }, // this is for model namespace and
  ...
  getRequestTypeStepForm = () => {
    ...
    return connect(state => ({
      ...
      owner: { type: '_storageSpace', id: state._storageSpace.id, listName: 'nolist', ref:state._storageSpace, listDisplayName: appLocaleName(userContext,"List") }, // this is for model namespace and
  ...
  buildRouters = () =>{
    const {StorageSpaceWorkbench} = GlobalComponents

    const {StorageSpaceDashboard} = GlobalComponents
  	const {StorageSpacePermission} = GlobalComponents
  	const {StorageSpaceProfile} = GlobalComponents


    const routers=[
    {path:"/storageSpace/:id/workbench", component: StorageSpaceWorkbench},
    
  	{path:"/storageSpace/:id/profile", component: StorageSpaceProfile},
  	{path:"/storageSpace/:id/permission", component: StorageSpacePermission},
  ...
  handleOpenChange = (openKeys) => {
   ...
   render() {
    ...
    const {searchLocalData}=GlobalComponents.StorageSpaceBase
    ...
    const renderMenuSwitch=()=>{
     ...
     <Layout>
      ...
      {this.state.showSearch&&(
        ...
        <div style={{backgroundColor:'black'}}  onClick={()=>hideSearchResult()}  >{searchLocalData(this.props.storageSpace,this.state.searchKeyword)}</div>
       ...
       <Sider
        ...
        >
         ...
         {this.getNavMenuItems(this.props.storageSpace,"inline","dark")}
