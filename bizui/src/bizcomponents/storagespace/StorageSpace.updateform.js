...
import styles from './StorageSpace.updateform.less'
...
class StorageSpaceUpdateForm extends Component {
  state = {
  ...
  render() {
    ...
    const submitUpdateForm = () => {
      validateFieldsAndScroll((error, values) => {
        ...
        const storageSpaceId = values.id

        const parameters = { ...values, storageSpaceId, }
    ...
    const submitUpdateFormAndContinue = () => {
      validateFieldsAndScroll((error, values) => {
        ...
        dispatch({
          type: `${owner.type}/updateStorageSpace`,
          payload: {
            id: owner.id,
            type: 'storageSpace',
            parameters,
    ...
    const skipToNext = () => {
      ...
      const newIndex = currentUpdateIndex + 1
      dispatch({
        type: `${owner.type}/gotoNextStorageSpaceUpdateRow`,
        payload: {
          id: owner.id,
          type: 'storageSpace',
          selectedRows,
	...
	const internalRenderTitle = () =>{
      ...
      return (<div>{linkComp}{appLocaleName(userContext,"Update")}存货区: {(currentUpdateIndex+1)}/{selectedRows.length}</div>)
    }
...
}))(Form.create()(StorageSpaceUpdateForm))
