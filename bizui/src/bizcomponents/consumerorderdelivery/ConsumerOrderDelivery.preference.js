...
import styles from './ConsumerOrderDelivery.preference.less'
...
const internalImageListOf = (consumerOrderDelivery) =>{
...
const internalSettingListOf = (consumerOrderDelivery) =>{
...
const handleTransferSearch =(targetComponent,filterKey,newRequest)=>{
  ...
  const pageNo = 1;
  const candidateReferenceService = ConsumerOrderDeliveryService[candidateServiceName] 
...
const executeTrans = (consumerOrderDelivery,targetComponent) =>{
  ...
  validateFieldsAndScroll((error, values) => {
    ...
    const id=consumerOrderDelivery.id;
    const serviceNameToCall = transferServiceName;

    const payload = {parameters,id,serviceNameToCall}
    
    //targetComponent.setState({transferModalVisiable:false})
    dispatch({type:"_consumerOrderDelivery/doJob",payload: payload})
...
const buildTransferModal = (consumerOrderDelivery,targetComponent) => {
...
<Modal title={transferModalTitle}
          ...
          onOk={()=>executeTrans(consumerOrderDelivery,targetComponent)}
...
const internalRenderExtraHeader = (consumerOrderDelivery) =>{
	return null
}
const internalRenderExtraFooter = (consumerOrderDelivery) =>{
	return null
}
...
class ConsumerOrderDeliveryPreference extends Component {
  ...
  render() {
    ...
    const cardsData = {cardsName:"消费者订单交货",cardsFor: "consumerOrderDelivery",cardsSource: this.props.consumerOrderDelivery,
  		subItems: [
...
}))(Form.create()(ConsumerOrderDeliveryPreference))
