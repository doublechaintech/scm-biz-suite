...
import styles from './MemberRewardPoint.app.less'
...
class MemberRewardPointBizApp extends React.PureComponent {
constructor(props) {
    super(props)
     this.state = {
 ...
 getNavMenuItems = (targetObject, style, customTheme) => {
    ...
    return (
	  <Menu
       ...
       <Menu.Item key="workbench">
        <Link to={`/memberRewardPoint/${this.props.memberRewardPoint.id}/workbench`}><Icon type="solution" style={{marginRight:"20px"}}/><span>工作台</span></Link>
      </Menu.Item>
        ...
        {filteredMenuItemsGroup(targetObject,this).map((groupedMenuItem,index)=>{
          return(
    <SubMenu id={`submenu-vg${index}`}  key={`vg${index}`} title={<span><Icon type={viewGroupIconNameOf('member_reward_point',`${groupedMenuItem.viewGroup}`)} style={{marginRight:"20px"}} /><span>{viewGroupDisplayNameOf('member_reward_point',`${groupedMenuItem.viewGroup}`)}</span></span>} >
  ...
  getRequestTypeStepForm = () => {
    ...
    return connect(state => ({
      selectedRows: state._memberRewardPoint.selectedRows,
      role: "cq",
      currentUpdateIndex: state._memberRewardPoint.currentUpdateIndex,
      owner: { type: '_memberRewardPoint', id: state._memberRewardPoint.id, listName: 'nolist', ref:state._memberRewardPoint, listDisplayName: appLocaleName(userContext,"List") }, // this is for model namespace and
  ...
  buildRouters = () =>{
    const {MemberRewardPointWorkbench} = GlobalComponents

    const {MemberRewardPointDashboard} = GlobalComponents
  	const {MemberRewardPointPermission} = GlobalComponents
  	const {MemberRewardPointProfile} = GlobalComponents


    const routers=[
    {path:"/memberRewardPoint/:id/workbench", component: MemberRewardPointWorkbench},
    
  	{path:"/memberRewardPoint/:id/profile", component: MemberRewardPointProfile},
  	{path:"/memberRewardPoint/:id/permission", component: MemberRewardPointPermission},
  ...
  handleOpenChange = (openKeys) => {
   ...
   render() {
    ...
    const {searchLocalData}=GlobalComponents.MemberRewardPointBase
    ...
    const renderMenuSwitch=()=>{
     ...
     <Layout>
      ...
      {this.state.showSearch&&(
        ...
        <div style={{backgroundColor:'black'}}  onClick={()=>hideSearchResult()}  >{searchLocalData(this.props.memberRewardPoint,this.state.searchKeyword)}</div>
       ...
       <Sider
        ...
        >
         ...
         {this.getNavMenuItems(this.props.memberRewardPoint,"inline","dark")}
