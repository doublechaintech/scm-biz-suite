...
import styles from './MemberRewardPoint.workbench.less'
...
const renderSettingMenuItem = (item,cardsData,targetComponent) =>{
  ...
  return (<Menu.Item key={item.name}>
      <Link to={`/memberRewardPoint/${targetComponent.props.memberRewardPoint.id}/list/${item.name}/${item.displayName}/`}>
...
const renderSettingMenu = (cardsData,targetComponent) =>{
  ...
  return (<Menu>
    	<Menu.Item key="profile">
  			<Link to={`/memberRewardPoint/${targetComponent.props.memberRewardPoint.id}/permission`}><Icon type="safety-certificate" theme="twoTone" twoToneColor="#52c41a"/><span>{appLocaleName(userContext,"Permission")}</span></Link>
		</Menu.Item>
		<Menu.Divider />
...
const internalSummaryOf = (cardsData,targetComponent) =>{
	...
	const memberRewardPoint = cardsData.cardsSource
	const {MemberRewardPointService} = GlobalComponents
	const userContext = null
	return (
	<div>
	<DescriptionList className={styles.headerList} size="small" col="4">
<Description term="ID" style={{wordBreak: 'break-all'}}>{memberRewardPoint.id}</Description> 
...
class MemberRewardPointWorkbench extends Component {
...
}))(Form.create()(MemberRewardPointWorkbench))
