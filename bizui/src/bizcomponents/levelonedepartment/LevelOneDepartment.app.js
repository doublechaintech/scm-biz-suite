 ...
 getNavMenuItems = (targetObject, style, customTheme) => {
    ...
    return (
	  <Menu
       ...
       <Menu.Item key="workbench">
        <Link to={`/levelOneDepartment/${this.props.levelOneDepartment.id}/workbench`}><Icon type="solution" style={{marginRight:"20px"}}/><span>工作台</span></Link>
      </Menu.Item>
        ...
        {filteredMenuItemsGroup(targetObject,this).map((groupedMenuItem,index)=>{
          return(
    <SubMenu id={`submenu-vg${index}`}  key={`vg${index}`} title={<span><Icon type={viewGroupIconNameOf('level_one_department',`${groupedMenuItem.viewGroup}`)} style={{marginRight:"20px"}} /><span>{viewGroupDisplayNameOf('level_one_department',`${groupedMenuItem.viewGroup}`)}</span></span>} >
  ...
  getLevelTwoDepartmentSearch = () => {
    ...
    return connect(state => ({
      rule: state.rule,
      name: window.mtrans('level_two_department','level_one_department.level_two_department_list',false),
      ...
      returnURL: `/levelOneDepartment/${state._levelOneDepartment.id}/workbench`,
  ...
  getLevelTwoDepartmentCreateForm = () => {
    ...
    return connect(state => ({
      ...
      metaInfo: state._levelOneDepartment.levelTwoDepartmentListMetaInfo,
      count: state._levelOneDepartment.levelTwoDepartmentCount,
      returnURL: `/levelOneDepartment/${state._levelOneDepartment.id}/list`,
      currentPage: state._levelOneDepartment.levelTwoDepartmentCurrentPageNumber,
      searchFormParameters: state._levelOneDepartment.levelTwoDepartmentSearchFormParameters,
      loading: state._levelOneDepartment.loading,
      owner: { type: '_levelOneDepartment', id: state._levelOneDepartment.id || this.getOwnerId(), referenceName: 'belongsTo', listName: 'levelTwoDepartmentList', ref:state._levelOneDepartment, listDisplayName: appLocaleName(userContext,"List")}, // this is for model namespace and
  ...
  getLevelTwoDepartmentUpdateForm = () => {
    ...
    return connect(state => ({
      ...
      role: "levelTwoDepartment",
      currentUpdateIndex: state._levelOneDepartment.currentUpdateIndex || 0,
      owner: { type: '_levelOneDepartment', id: state._levelOneDepartment.id || this.getOwnerId(), listName: 'levelTwoDepartmentList', ref:state._levelOneDepartment, listDisplayName: appLocaleName(userContext,"List") }, // this is for model namespace and
  ...
  getRequestTypeStepForm = () => {
    ...
    return connect(state => ({
      ...
      owner: { type: '_levelOneDepartment', id: state._levelOneDepartment.id, listName: 'nolist', ref:state._levelOneDepartment, listDisplayName: appLocaleName(userContext,"List") }, // this is for model namespace and
  ...
  buildRouters = () =>{
    const {LevelOneDepartmentWorkbench} = GlobalComponents
    ...
    const routers=[
    {path:"/levelOneDepartment/:id/workbench", component: LevelOneDepartmentWorkbench},
  ...
  handleOpenChange = (openKeys) => {
   ...
   render() {
    ...
    const {searchLocalData}=GlobalComponents.LevelOneDepartmentBase
    ...
    const renderMenuSwitch=()=>{
     ...
     <Layout>
      ...
      {this.state.showSearch&&(
        ...
        <div style={{backgroundColor:'black'}}  onClick={()=>hideSearchResult()}  >{searchLocalData(this.props.levelOneDepartment,this.state.searchKeyword)}</div>
       ...
       <Sider
        ...
        >
         ...
         {this.getNavMenuItems(this.props.levelOneDepartment,"inline","dark")}
