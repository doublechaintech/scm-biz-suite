...
import styles from './LevelOneDepartment.workbench.less'
...
const renderSettingMenuItem = (item,cardsData,targetComponent) =>{
  ...
  return (<Menu.Item key={item.name}>
      <Link to={`/levelOneDepartment/${targetComponent.props.levelOneDepartment.id}/list/${item.name}/${item.displayName}/`}>
...
const internalSummaryOf = (cardsData,targetComponent) =>{
	...
	const levelOneDepartment = cardsData.cardsSource
	const {LevelOneDepartmentService} = GlobalComponents
	const userContext = null
	return (
	<div>
	<DescriptionList className={styles.headerList} size="small" col="4">
<Description term="ID" style={{wordBreak: 'break-all'}}>{levelOneDepartment.id}</Description> 
...
class LevelOneDepartmentWorkbench extends Component {
...
}))(Form.create()(LevelOneDepartmentWorkbench))
