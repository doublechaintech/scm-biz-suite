...
const internalImageListOf = (accountingDocumentType) =>{
...
const internalSettingListOf = (accountingDocumentType) =>{
	const userContext = null
	const optionList = [ 
	]
...
const handleTransferSearch =(targetComponent,filterKey,newRequest)=>{
  ...
  const pageNo = 1;
  const candidateReferenceService = AccountingDocumentTypeService[candidateServiceName] 
...
const executeTrans = (accountingDocumentType,targetComponent) =>{
  ...
  validateFieldsAndScroll((error, values) => {
    ...
    const id=accountingDocumentType.id;
    const serviceNameToCall = transferServiceName;

    const payload = {parameters,id,serviceNameToCall}
    
    //targetComponent.setState({transferModalVisiable:false})
    dispatch({type:"_accountingDocumentType/doJob",payload: payload})
...
const buildTransferModal = (accountingDocumentType,targetComponent) => {
...
<Modal title={transferModalTitle}
          ...
          onOk={()=>executeTrans(accountingDocumentType,targetComponent)}
...
const internalRenderExtraHeader = (accountingDocumentType) =>{
	return null
}
const internalRenderExtraFooter = (accountingDocumentType) =>{
	return null
}
...
<Description term="会计期间">{accountingDocumentType.accountingPeriod==null?appLocaleName(userContext,"NotAssigned"):accountingDocumentType.accountingPeriod.displayName}
 <Icon type="swap" onClick={()=>
  showTransferModel(targetComponent,"会计期间","accountSet","requestCandidateAccountingPeriod",
	      "transferToAnotherAccountingPeriod","anotherAccountingPeriodId",accountingDocumentType.accountingPeriod?accountingDocumentType.accountingPeriod.id:"")} 
...
class AccountingDocumentTypePreference extends Component {
...
}))(Form.create()(AccountingDocumentTypePreference))
