...
import styles from './AccountingDocumentType.updateform.less'
...
class AccountingDocumentTypeUpdateForm extends Component {
  state = {
...
>>>>>>> 502e8b8dfc403300a992b5083e79c722e85d1854
        ...
        const accountingDocumentTypeId = values.id
        const imagesValues = mapBackToImageValues(convertedImagesValues)
        const parameters = { ...values, accountingDocumentTypeId, ...imagesValues }
    ...
    const submitUpdateFormAndContinue = () => {
      validateFieldsAndScroll((error, values) => {
        ...
        dispatch({
          type: `${owner.type}/updateAccountingDocumentType`,
          payload: {
            id: owner.id,
    ...
    const skipToNext = () => {
      ...
      const newIndex = currentUpdateIndex + 1
      dispatch({
        type: `${owner.type}/gotoNextAccountingDocumentTypeUpdateRow`,
        payload: {
          id: owner.id,
          type: 'accountingDocumentType',
          selectedRows,
...
<<<<<<< HEAD
    ...
    return (
      <PageHeaderLayout
        title={"更新会计凭证类型"+(currentUpdateIndex+1)+"/"+selectedRows.length}
        content="更新会计凭证类型"
...
=======
	...
	const internalRenderTitle = () =>{
      ...
      return (<div>{linkComp}{appLocaleName(userContext,"Update")}会计凭证类型: {(currentUpdateIndex+1)}/{selectedRows.length}</div>)
    }
...
}))(Form.create()(AccountingDocumentTypeUpdateForm))
