 ...
 getNavMenuItems = (targetObject, style, customTheme) => {
    ...
    return (
	  <Menu
       ...
       <Menu.Item key="workbench">
        <Link to={`/accountingDocumentType/${this.props.accountingDocumentType.id}/workbench`}><Icon type="solution" style={{marginRight:"20px"}}/><span>工作台</span></Link>
      </Menu.Item>
        ...
        {filteredMenuItemsGroup(targetObject,this).map((groupedMenuItem,index)=>{
          return(
    <SubMenu id={`submenu-vg${index}`}  key={`vg${index}`} title={<span><Icon type={viewGroupIconNameOf('accounting_document_type',`${groupedMenuItem.viewGroup}`)} style={{marginRight:"20px"}} /><span>{viewGroupDisplayNameOf('accounting_document_type',`${groupedMenuItem.viewGroup}`)}</span></span>} >
  ...
  getAccountingDocumentSearch = () => {
    ...
    return connect(state => ({
      rule: state.rule,
      name: window.mtrans('accounting_document','accounting_document_type.accounting_document_list',false),
      ...
      returnURL: `/accountingDocumentType/${state._accountingDocumentType.id}/workbench`,
  ...
  getAccountingDocumentCreateForm = () => {
    ...
    return connect(state => ({
      ...
      searchFormParameters: state._accountingDocumentType.accountingDocumentSearchFormParameters,
      loading: state._accountingDocumentType.loading,
      owner: { type: '_accountingDocumentType', id: state._accountingDocumentType.id || this.getOwnerId(), referenceName: 'documentType', listName: 'accountingDocumentList', ref:state._accountingDocumentType, listDisplayName: appLocaleName(userContext,"List")}, // this is for model namespace and
  ...
  getAccountingDocumentUpdateForm = () => {
    ...
    return connect(state => ({
      ...
      role: "accountingDocument",
      currentUpdateIndex: state._accountingDocumentType.currentUpdateIndex || 0,
      owner: { type: '_accountingDocumentType', id: state._accountingDocumentType.id || this.getOwnerId(), listName: 'accountingDocumentList', ref:state._accountingDocumentType, listDisplayName: appLocaleName(userContext,"List") }, // this is for model namespace and
  ...
  getRequestTypeStepForm = () => {
    ...
    return connect(state => ({
      ...
      owner: { type: '_accountingDocumentType', id: state._accountingDocumentType.id, listName: 'nolist', ref:state._accountingDocumentType, listDisplayName: appLocaleName(userContext,"List") }, // this is for model namespace and
  ...
  buildRouters = () =>{
    const {AccountingDocumentTypeWorkbench} = GlobalComponents
    ...
    const routers=[
    {path:"/accountingDocumentType/:id/workbench", component: AccountingDocumentTypeWorkbench},
      ...
      {this.state.showSearch&&(
       ...
       <Sider
        ...
        >
         ...
         {this.getNavMenuItems(this.props.accountingDocumentType,"inline","dark")}
