...
import styles from './EventAttendance.app.less'
...
class EventAttendanceBizApp extends React.PureComponent {
constructor(props) {
    super(props)
     this.state = {
 ...
 getNavMenuItems = (targetObject, style, customTheme) => {
    ...
    return (
	  <Menu
       ...
       <Menu.Item key="workbench">
        <Link to={`/eventAttendance/${this.props.eventAttendance.id}/workbench`}><Icon type="solution" style={{marginRight:"20px"}}/><span>工作台</span></Link>
      </Menu.Item>
        ...
        {filteredMenuItemsGroup(targetObject,this).map((groupedMenuItem,index)=>{
          return(
    <SubMenu id={`submenu-vg${index}`}  key={`vg${index}`} title={<span><Icon type={viewGroupIconNameOf('event_attendance',`${groupedMenuItem.viewGroup}`)} style={{marginRight:"20px"}} /><span>{viewGroupDisplayNameOf('event_attendance',`${groupedMenuItem.viewGroup}`)}</span></span>} >
  ...
  getRequestTypeStepForm = () => {
    ...
    return connect(state => ({
      selectedRows: state._eventAttendance.selectedRows,
      role: "cq",
      currentUpdateIndex: state._eventAttendance.currentUpdateIndex,
      owner: { type: '_eventAttendance', id: state._eventAttendance.id, listName: 'nolist', ref:state._eventAttendance, listDisplayName: appLocaleName(userContext,"List") }, // this is for model namespace and
  ...
  buildRouters = () =>{
    const {EventAttendanceWorkbench} = GlobalComponents

    const {EventAttendanceDashboard} = GlobalComponents
  	const {EventAttendancePermission} = GlobalComponents
  	const {EventAttendanceProfile} = GlobalComponents


    const routers=[
    {path:"/eventAttendance/:id/workbench", component: EventAttendanceWorkbench},
    
  	{path:"/eventAttendance/:id/profile", component: EventAttendanceProfile},
  	{path:"/eventAttendance/:id/permission", component: EventAttendancePermission},
  ...
  handleOpenChange = (openKeys) => {
   ...
   render() {
    ...
    const {searchLocalData}=GlobalComponents.EventAttendanceBase
    ...
    const renderMenuSwitch=()=>{
     ...
     <Layout>
      ...
      {this.state.showSearch&&(
        ...
        <div style={{backgroundColor:'black'}}  onClick={()=>hideSearchResult()}  >{searchLocalData(this.props.eventAttendance,this.state.searchKeyword)}</div>
       ...
       <Sider
        ...
        >
         ...
         {this.getNavMenuItems(this.props.eventAttendance,"inline","dark")}
