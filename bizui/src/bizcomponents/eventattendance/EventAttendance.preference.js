...
const internalImageListOf = (eventAttendance) =>{
...
const internalSettingListOf = (eventAttendance) =>{
	const userContext = null
	const optionList = [ 
	]
...
const handleTransferSearch =(targetComponent,filterKey,newRequest)=>{
  ...
  const pageNo = 1;
  const candidateReferenceService = EventAttendanceService[candidateServiceName] 
...
const executeTrans = (eventAttendance,targetComponent) =>{
...
const buildTransferModal = (eventAttendance,targetComponent) => {
...
<Modal title={transferModalTitle}
          ...
          onOk={()=>executeTrans(eventAttendance,targetComponent)}
...
const internalRenderExtraHeader = (eventAttendance) =>{
	return null
}
const internalRenderExtraFooter = (eventAttendance) =>{
	return null
}
...
<Description term="潜在的客户">{eventAttendance.potentialCustomer==null?appLocaleName(userContext,"NotAssigned"):eventAttendance.potentialCustomer.displayName}
 <Icon type="swap" onClick={()=>
  showTransferModel(targetComponent,"潜在的客户","potentialCustomer","requestCandidatePotentialCustomer",
	      "transferToAnotherPotentialCustomer","anotherPotentialCustomerId",eventAttendance.potentialCustomer?eventAttendance.potentialCustomer.id:"")} 
  style={{fontSize: 20,color:"red"}} />
</Description>
<Description term="城市活动">{eventAttendance.cityEvent==null?appLocaleName(userContext,"NotAssigned"):eventAttendance.cityEvent.displayName}
 <Icon type="swap" onClick={()=>
  showTransferModel(targetComponent,"城市活动","cityEvent","requestCandidateCityEvent",
	      "transferToAnotherCityEvent","anotherCityEventId",eventAttendance.cityEvent?eventAttendance.cityEvent.id:"")} 
  style={{fontSize: 20,color:"red"}} />
</Description>
...
class EventAttendancePreference extends Component {
...
}))(Form.create()(EventAttendancePreference))
