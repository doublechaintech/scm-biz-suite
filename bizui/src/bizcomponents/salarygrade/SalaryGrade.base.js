...
import styles from './SalaryGrade.base.less'
const {
...
const renderItemOfList=(salaryGrade, targetComponent, columCount)=>{
  ...
  const userContext = null
  return (
    <Card key={`salaryGrade-${salaryGrade.id}`} style={{marginTop:"10px"}}>
		
	<Col span={4}>
		<Avatar size={90} style={{ backgroundColor: genColor(), verticalAlign: 'middle' }}>
			{leftChars(salaryGrade.displayName)}
		</Avatar>
	</Col>
	<Col span={20}>
      ...
      <DescriptionList  key={salaryGrade.id} size="small" col={displayColumnsCount} >
...
const SalaryGradeBase={menuData,settingMenuData,displayColumns,fieldLabels,renderItemOfList, stepOf, searchLocalData}
