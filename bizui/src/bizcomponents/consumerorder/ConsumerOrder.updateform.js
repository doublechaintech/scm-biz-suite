...
import styles from './ConsumerOrder.updateform.less'
...
class ConsumerOrderUpdateForm extends Component {
  state = {
...
>>>>>>> 502e8b8dfc403300a992b5083e79c722e85d1854
        ...
        const consumerOrderId = values.id
        const imagesValues = mapBackToImageValues(convertedImagesValues)
        const parameters = { ...values, consumerOrderId, ...imagesValues }
    ...
    const submitUpdateFormAndContinue = () => {
      validateFieldsAndScroll((error, values) => {
        ...
        dispatch({
          type: `${owner.type}/updateConsumerOrder`,
          payload: {
            id: owner.id,
            type: 'consumerOrder',
            parameters,
    ...
    const skipToNext = () => {
      ...
      const newIndex = currentUpdateIndex + 1
      dispatch({
        type: `${owner.type}/gotoNextConsumerOrderUpdateRow`,
        payload: {
          id: owner.id,
          type: 'consumerOrder',
          selectedRows,
...
<<<<<<< HEAD
    ...
    return (
      <PageHeaderLayout
        title={"更新消费者订单"+(currentUpdateIndex+1)+"/"+selectedRows.length}
        content="更新消费者订单"
...
=======
	...
	const internalRenderTitle = () =>{
      ...
      return (<div>{linkComp}{appLocaleName(userContext,"Update")}消费者订单: {(currentUpdateIndex+1)}/{selectedRows.length}</div>)
    }
...
}))(Form.create()(ConsumerOrderUpdateForm))
