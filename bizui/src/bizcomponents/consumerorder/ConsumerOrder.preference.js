...
const internalImageListOf = (consumerOrder) =>{
...
const internalSettingListOf = (consumerOrder) =>{
	const userContext = null
	const optionList = [ 
	]
...
const handleTransferSearch =(targetComponent,filterKey,newRequest)=>{
  ...
  const pageNo = 1;
  const candidateReferenceService = ConsumerOrderService[candidateServiceName] 
...
const executeTrans = (consumerOrder,targetComponent) =>{
...
const buildTransferModal = (consumerOrder,targetComponent) => {
...
<Modal title={transferModalTitle}
          ...
          onOk={()=>executeTrans(consumerOrder,targetComponent)}
...
const internalRenderExtraHeader = (consumerOrder) =>{
	return null
}
const internalRenderExtraFooter = (consumerOrder) =>{
	return null
}
...
<Description term="消费者">{consumerOrder.consumer==null?appLocaleName(userContext,"NotAssigned"):consumerOrder.consumer.displayName}
 <Icon type="swap" onClick={()=>
  showTransferModel(targetComponent,"消费者","retailStoreMember","requestCandidateConsumer",
	      "transferToAnotherConsumer","anotherConsumerId",consumerOrder.consumer?consumerOrder.consumer.id:"")} 
  style={{fontSize: 20,color:"red"}} />
</Description>
<Description term="商场">{consumerOrder.store==null?appLocaleName(userContext,"NotAssigned"):consumerOrder.store.displayName}
 <Icon type="swap" onClick={()=>
  showTransferModel(targetComponent,"商场","retailStore","requestCandidateStore",
...
class ConsumerOrderPreference extends Component {
  ...
  render() {
    ...
    const { id,displayName, consumerOrderLineItemCount, consumerOrderShippingGroupCount, consumerOrderPaymentGroupCount, consumerOrderPriceAdjustmentCount, retailStoreMemberGiftCardConsumeRecordCount } = this.props.consumerOrder
...
}))(Form.create()(ConsumerOrderPreference))
