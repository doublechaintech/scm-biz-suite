...
class RetailStoreOrderLineItemCreateForm extends Component {
  state = {
  ...
  render() {
    ...
    const submitCreateFormAndContinue = () => {
      validateFieldsAndScroll((error, values) => {
        ...
        const parameters = { ...values, ...imagesValues }
        dispatch({
          type: `${owner.type}/addRetailStoreOrderLineItem`,
          payload: { id: owner.id, type: 'retailStoreOrderLineItem', parameters, continueNext: true },
        })
      })
    }
	...
	return (
      <PageHeaderLayout
        title={`${appLocaleName(userContext,"CreateNew")}Retail Store Order Line Item`}
        content={`${appLocaleName(userContext,"CreateNew")}Retail Store Order Line Item`}
...
}))(Form.create()(RetailStoreOrderLineItemCreateForm))
