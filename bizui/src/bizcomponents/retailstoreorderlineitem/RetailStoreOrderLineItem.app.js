...
import styles from './RetailStoreOrderLineItem.app.less'
...
class RetailStoreOrderLineItemBizApp extends React.PureComponent {
constructor(props) {
    super(props)
     this.state = {
 ...
 getNavMenuItems = (targetObject, style, customTheme) => {
    ...
    return (
	  <Menu
       ...
       <Menu.Item key="workbench">
        <Link to={`/retailStoreOrderLineItem/${this.props.retailStoreOrderLineItem.id}/workbench`}><Icon type="solution" style={{marginRight:"20px"}}/><span>工作台</span></Link>
      </Menu.Item>
        ...
        {filteredMenuItemsGroup(targetObject,this).map((groupedMenuItem,index)=>{
          return(
    <SubMenu id={`submenu-vg${index}`}  key={`vg${index}`} title={<span><Icon type={viewGroupIconNameOf('retail_store_order_line_item',`${groupedMenuItem.viewGroup}`)} style={{marginRight:"20px"}} /><span>{viewGroupDisplayNameOf('retail_store_order_line_item',`${groupedMenuItem.viewGroup}`)}</span></span>} >
  ...
  getRequestTypeStepForm = () => {
    ...
    return connect(state => ({
      selectedRows: state._retailStoreOrderLineItem.selectedRows,
      role: "cq",
      currentUpdateIndex: state._retailStoreOrderLineItem.currentUpdateIndex,
      owner: { type: '_retailStoreOrderLineItem', id: state._retailStoreOrderLineItem.id, listName: 'nolist', ref:state._retailStoreOrderLineItem, listDisplayName: appLocaleName(userContext,"List") }, // this is for model namespace and
  ...
  buildRouters = () =>{
    const {RetailStoreOrderLineItemWorkbench} = GlobalComponents

    const {RetailStoreOrderLineItemDashboard} = GlobalComponents
  	const {RetailStoreOrderLineItemPermission} = GlobalComponents
  	const {RetailStoreOrderLineItemProfile} = GlobalComponents


    const routers=[
    {path:"/retailStoreOrderLineItem/:id/workbench", component: RetailStoreOrderLineItemWorkbench},
    
  	{path:"/retailStoreOrderLineItem/:id/profile", component: RetailStoreOrderLineItemProfile},
  	{path:"/retailStoreOrderLineItem/:id/permission", component: RetailStoreOrderLineItemPermission},
  ...
  handleOpenChange = (openKeys) => {
   ...
   render() {
    ...
    const {searchLocalData}=GlobalComponents.RetailStoreOrderLineItemBase
    ...
    const renderMenuSwitch=()=>{
     ...
     <Layout>
      ...
      {this.state.showSearch&&(
        ...
        <div style={{backgroundColor:'black'}}  onClick={()=>hideSearchResult()}  >{searchLocalData(this.props.retailStoreOrderLineItem,this.state.searchKeyword)}</div>
       ...
       <Sider
        ...
        >
         ...
         {this.getNavMenuItems(this.props.retailStoreOrderLineItem,"inline","dark")}
