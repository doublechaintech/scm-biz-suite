...
const menuData = {menuName: window.trans('termination'), menuFor: "termination",  internalName: "termination",
  		subItems: [
...
const settingMenuData = {menuName: window.trans('termination'), menuFor: "termination",  internalName: "termination",
  		subItems: [
...
const renderActionList=(termination, targetObject, columCount, listName)=>{

	if(!termination){
		return null
	}
	if(!termination.actionList){
		return null
	}
	if(termination.actionList.length === 0){
		return null
	}
	return (
		<div className={styles.overlay}>
			...
			{termination.actionList.map(action=>(<Link key={action.id} to={{pathname: action.actionPath.substring(1), state: {ownerId:targetObject.id,action,selectedRows:[termination]}}} >
...
const renderItemOfList=(termination, targetObject, columCount, listName)=>{
  ...
  const userContext = null
  return (
     <Row key={`${listName}-${termination.id}`} className={styles.itemDesc}>
	...
	<Col span={20}>
      ...
      {renderActionList(termination,targetObject)}
    </Row>
	)
...
const packFormValuesToObject = ( formValuesToPack )=>{
	const {reasonId, typeId, comment} = formValuesToPack
	const reason = {id: reasonId, version: 2^31}
	const type = {id: typeId, version: 2^31}
	const data = {reason, type, comment}
	return data
}
const unpackObjectToFormValues = ( objectToUnpack )=>{
	const {reason, type, comment} = objectToUnpack
	const reasonId = reason ? reason.id : null
	const typeId = type ? type.id : null
	const data = {reasonId, typeId, comment}
	return data
}
...
const TerminationBase={unpackObjectToFormValues, menuData,settingMenuData,displayColumns,fieldLabels,renderItemOfList, stepOf, searchLocalData}
