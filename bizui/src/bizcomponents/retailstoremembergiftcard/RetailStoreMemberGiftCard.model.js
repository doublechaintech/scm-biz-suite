...
export default {
  ...
  effects: {
    ...
    *analyze({ payload }, { call, put, select }){
      ...
      const data = yield call(RetailStoreMemberGiftCardService.analyze, payload.id)
    ...
    *addRetailStoreMemberGiftCardConsumeRecord({ payload }, { call, put }) {
      ...
      const location = { pathname: `/retailStoreMemberGiftCard/${id}/list/RetailStoreMemberGiftCardConsumeRecordList/零售门店会员卡消费记录+${appLocaleName(userContext,'List')}`, state: newState }
      yield put(routerRedux.push(location))
    },
    *updateRetailStoreMemberGiftCardConsumeRecord({ payload }, { call, put }) {
      ...
      const location = { pathname: `/retailStoreMemberGiftCard/${id}/list/RetailStoreMemberGiftCardConsumeRecordList/零售门店会员卡消费记录列表`, state: newPlayload }
      yield put(routerRedux.push(location))
    },
