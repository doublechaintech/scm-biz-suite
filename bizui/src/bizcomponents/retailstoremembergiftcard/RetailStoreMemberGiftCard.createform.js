...
class RetailStoreMemberGiftCardCreateForm extends Component {
  state = {
  ...
  render() {
    ...
    const submitCreateFormAndContinue = () => {
      validateFieldsAndScroll((error, values) => {
        ...
        const parameters = { ...values, ...imagesValues }
        dispatch({
          type: `${owner.type}/addRetailStoreMemberGiftCard`,
          payload: { id: owner.id, type: 'retailStoreMemberGiftCard', parameters, continueNext: true },
        })
      })
    }
	...
	return (
      <PageHeaderLayout
        title={`${appLocaleName(userContext,"CreateNew")}Retail Store Member Gift Card`}
        content={`${appLocaleName(userContext,"CreateNew")}Retail Store Member Gift Card`}
...
}))(Form.create()(RetailStoreMemberGiftCardCreateForm))
