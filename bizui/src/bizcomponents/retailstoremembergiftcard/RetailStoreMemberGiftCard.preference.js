...
const internalImageListOf = (retailStoreMemberGiftCard) =>{
<<<<<<< HEAD
...
const internalSettingListOf = (retailStoreMemberGiftCard) =>{
<<<<<<< HEAD
...
const handleTransferSearch =(targetComponent,filterKey,newRequest)=>{
  ...
  const pageNo = 1;
  const candidateReferenceService = RetailStoreMemberGiftCardService[candidateServiceName] 
...
const executeTrans = (retailStoreMemberGiftCard,targetComponent) =>{
  ...
  validateFieldsAndScroll((error, values) => {
    ...
    const id=retailStoreMemberGiftCard.id;
    const serviceNameToCall = transferServiceName;

    const payload = {parameters,id,serviceNameToCall}
    
    //targetComponent.setState({transferModalVisiable:false})
    dispatch({type:"_retailStoreMemberGiftCard/doJob",payload: payload})
...
const buildTransferModal = (retailStoreMemberGiftCard,targetComponent) => {
...
<Modal title={transferModalTitle}
          ...
          onOk={()=>executeTrans(retailStoreMemberGiftCard,targetComponent)}
...
const internalRenderExtraHeader = (retailStoreMemberGiftCard) =>{
	return null
}
const internalRenderExtraFooter = (retailStoreMemberGiftCard) =>{
	return null
}
...
<<<<<<< HEAD
<Description term="业主">{retailStoreMemberGiftCard.owner==null?"未分配":retailStoreMemberGiftCard.owner.displayName}
=======
<Description term="业主">{retailStoreMemberGiftCard.owner==null?appLocaleName(userContext,"NotAssigned"):retailStoreMemberGiftCard.owner.displayName}
>>>>>>> 502e8b8dfc403300a992b5083e79c722e85d1854
 <Icon type="swap" onClick={()=>
  showTransferModel(targetComponent,"业主","retailStoreMember","requestCandidateOwner",
	      "transferToAnotherOwner","anotherOwnerId",retailStoreMemberGiftCard.owner?retailStoreMemberGiftCard.owner.id:"")} 
...
class RetailStoreMemberGiftCardPreference extends Component {
...
}))(Form.create()(RetailStoreMemberGiftCardPreference))
