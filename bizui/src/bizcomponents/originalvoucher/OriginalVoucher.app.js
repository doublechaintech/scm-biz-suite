...
import styles from './OriginalVoucher.app.less'
...
class OriginalVoucherBizApp extends React.PureComponent {
constructor(props) {
    super(props)
     this.state = {
 ...
 getNavMenuItems = (targetObject, style, customTheme) => {
    ...
    return (
	  <Menu
       ...
       <Menu.Item key="workbench">
        <Link to={`/originalVoucher/${this.props.originalVoucher.id}/workbench`}><Icon type="solution" style={{marginRight:"20px"}}/><span>工作台</span></Link>
      </Menu.Item>
        ...
        {filteredMenuItemsGroup(targetObject,this).map((groupedMenuItem,index)=>{
          return(
    <SubMenu id={`submenu-vg${index}`}  key={`vg${index}`} title={<span><Icon type={viewGroupIconNameOf('original_voucher',`${groupedMenuItem.viewGroup}`)} style={{marginRight:"20px"}} /><span>{viewGroupDisplayNameOf('original_voucher',`${groupedMenuItem.viewGroup}`)}</span></span>} >
  ...
  getRequestTypeStepForm = () => {
    ...
    return connect(state => ({
      selectedRows: state._originalVoucher.selectedRows,
      role: "cq",
      currentUpdateIndex: state._originalVoucher.currentUpdateIndex,
      owner: { type: '_originalVoucher', id: state._originalVoucher.id, listName: 'nolist', ref:state._originalVoucher, listDisplayName: appLocaleName(userContext,"List") }, // this is for model namespace and
  ...
  buildRouters = () =>{
    const {OriginalVoucherWorkbench} = GlobalComponents

    const {OriginalVoucherDashboard} = GlobalComponents
  	const {OriginalVoucherPermission} = GlobalComponents
  	const {OriginalVoucherProfile} = GlobalComponents


    const routers=[
    {path:"/originalVoucher/:id/workbench", component: OriginalVoucherWorkbench},
    
  	{path:"/originalVoucher/:id/profile", component: OriginalVoucherProfile},
  	{path:"/originalVoucher/:id/permission", component: OriginalVoucherPermission},
  ...
  handleOpenChange = (openKeys) => {
   ...
   render() {
    ...
    const {searchLocalData}=GlobalComponents.OriginalVoucherBase
    ...
    const renderMenuSwitch=()=>{
     ...
     <Layout>
      ...
      {this.state.showSearch&&(
        ...
        <div style={{backgroundColor:'black'}}  onClick={()=>hideSearchResult()}  >{searchLocalData(this.props.originalVoucher,this.state.searchKeyword)}</div>
       ...
       <Sider
        ...
        >
         ...
         {this.getNavMenuItems(this.props.originalVoucher,"inline","dark")}
