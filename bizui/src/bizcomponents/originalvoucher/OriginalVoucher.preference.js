...
const internalImageListOf = (originalVoucher) =>{
<<<<<<< HEAD
...
const internalSettingListOf = (originalVoucher) =>{
<<<<<<< HEAD
...
const handleTransferSearch =(targetComponent,filterKey,newRequest)=>{
  ...
  const pageNo = 1;
  const candidateReferenceService = OriginalVoucherService[candidateServiceName] 
...
const executeTrans = (originalVoucher,targetComponent) =>{
...
const buildTransferModal = (originalVoucher,targetComponent) => {
...
<Modal title={transferModalTitle}
          ...
          onOk={()=>executeTrans(originalVoucher,targetComponent)}
...
const internalRenderExtraHeader = (originalVoucher) =>{
	return null
}
const internalRenderExtraFooter = (originalVoucher) =>{
	return null
}
...
=======
<Description term="属于">{originalVoucher.belongsTo==null?appLocaleName(userContext,"NotAssigned"):originalVoucher.belongsTo.displayName}
>>>>>>> 502e8b8dfc403300a992b5083e79c722e85d1854
 <Icon type="swap" onClick={()=>
  showTransferModel(targetComponent,"属于","accountingDocument","requestCandidateBelongsTo",
	      "transferToAnotherBelongsTo","anotherBelongsToId",originalVoucher.belongsTo?originalVoucher.belongsTo.id:"")} 
...
class OriginalVoucherPreference extends Component {
...
}))(Form.create()(OriginalVoucherPreference))
