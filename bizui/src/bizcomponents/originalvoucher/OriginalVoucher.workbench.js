...
import styles from './OriginalVoucher.workbench.less'
...
const renderSettingMenuItem = (item,cardsData,targetComponent) =>{
  ...
  return (<Menu.Item key={item.name}>
      <Link to={`/originalVoucher/${targetComponent.props.originalVoucher.id}/list/${item.name}/${item.displayName}/`}>
...
const internalSummaryOf = (cardsData,targetComponent) =>{
	...
	const originalVoucher = cardsData.cardsSource
	const {OriginalVoucherService} = GlobalComponents
	const userContext = null
	return (
	<div>
	<DescriptionList className={styles.headerList} size="small" col="4">
<Description term="ID" style={{wordBreak: 'break-all'}}>{originalVoucher.id}</Description> 
...
class OriginalVoucherWorkbench extends Component {
...
}))(Form.create()(OriginalVoucherWorkbench))
