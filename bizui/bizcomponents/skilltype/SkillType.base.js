...
const menuData = {menuName: window.trans('skill_type'), menuFor: "skillType",  internalName: "skill_type",
  		subItems: [
...
const settingMenuData = {menuName: window.trans('skill_type'), menuFor: "skillType",  internalName: "skill_type",
  		subItems: [
...
const renderActionList=(skillType, targetObject, columCount, listName)=>{

	if(!skillType){
		return null
	}
	if(!skillType.actionList){
		return null
	}
	if(skillType.actionList.length === 0){
		return null
	}
	return (
		<div className={styles.overlay}>
			...
			{skillType.actionList.map(action=>(<Link key={action.id} to={{pathname: action.actionPath.substring(1), state: {ownerId:targetObject.id,action,selectedRows:[skillType]}}} >
...
const renderItemOfList=(skillType, targetObject, columCount, listName)=>{
  ...
  const userContext = null
  return (
     <Row key={`${listName}-${skillType.id}`} className={styles.itemDesc}>
	...
	<Col span={20}>
      ...
      {renderActionList(skillType,targetObject)}
    </Row>
	)
...
const SkillTypeBase={unpackObjectToFormValues, menuData,settingMenuData,displayColumns,fieldLabels,renderItemOfList, stepOf, searchLocalData}
