...
import styles from './Instructor.workbench.less'
...
const internalSummaryOf = (cardsData,targetComponent) =>{
	...
	const instructor = cardsData.cardsSource
	const {InstructorService} = GlobalComponents
	const userContext = null
	return (
	<div>
	<DescriptionList className={styles.headerList} size="small" col="4">
<Description term="ID" style={{wordBreak: 'break-all'}}>{instructor.id}</Description> 
...
<Description term="更新于">{ moment(instructor.lastUpdateTime).format('YYYY-MM-DD HH:mm')}</Description> 
...
class InstructorWorkbench extends Component {
...
}))(Form.create()(InstructorWorkbench))
