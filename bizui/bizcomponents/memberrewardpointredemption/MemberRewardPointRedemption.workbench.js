...
import styles from './MemberRewardPointRedemption.workbench.less'
...
const renderSettingMenuItem = (item,cardsData,targetComponent) =>{
  ...
  return (<Menu.Item key={item.name}>
      <Link to={`/memberRewardPointRedemption/${targetComponent.props.memberRewardPointRedemption.id}/list/${item.name}/${item.displayName}/`}>
...
const renderSettingMenu = (cardsData,targetComponent) =>{
  ...
  return (<Menu>
    	<Menu.Item key="profile">
  			<Link to={`/memberRewardPointRedemption/${targetComponent.props.memberRewardPointRedemption.id}/permission`}><Icon type="safety-certificate" theme="twoTone" twoToneColor="#52c41a"/><span>{appLocaleName(userContext,"Permission")}</span></Link>
		</Menu.Item>
		<Menu.Divider />
...
const internalSummaryOf = (cardsData,targetComponent) =>{
	...
	const memberRewardPointRedemption = cardsData.cardsSource
	const {MemberRewardPointRedemptionService} = GlobalComponents
	const userContext = null
	return (
	<div>
	<DescriptionList className={styles.headerList} size="small" col="4">
<Description term="ID" style={{wordBreak: 'break-all'}}>{memberRewardPointRedemption.id}</Description> 
...
class MemberRewardPointRedemptionWorkbench extends Component {
...
}))(Form.create()(MemberRewardPointRedemptionWorkbench))
