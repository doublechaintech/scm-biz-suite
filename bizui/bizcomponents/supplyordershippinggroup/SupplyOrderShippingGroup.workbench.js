...
import styles from './SupplyOrderShippingGroup.workbench.less'
...
const renderSettingMenuItem = (item,cardsData,targetComponent) =>{
  ...
  return (<Menu.Item key={item.name}>
      <Link to={`/supplyOrderShippingGroup/${targetComponent.props.supplyOrderShippingGroup.id}/list/${item.name}/${item.displayName}/`}>
...
const renderSettingMenu = (cardsData,targetComponent) =>{
  ...
  return (<Menu>
    	<Menu.Item key="profile">
  			<Link to={`/supplyOrderShippingGroup/${targetComponent.props.supplyOrderShippingGroup.id}/permission`}><Icon type="safety-certificate" theme="twoTone" twoToneColor="#52c41a"/><span>{appLocaleName(userContext,"Permission")}</span></Link>
		</Menu.Item>
		<Menu.Divider />
...
const internalSummaryOf = (cardsData,targetComponent) =>{
	...
	const supplyOrderShippingGroup = cardsData.cardsSource
	const {SupplyOrderShippingGroupService} = GlobalComponents
	const userContext = null
	return (
	<div>
	<DescriptionList className={styles.headerList} size="small" col="4">
<Description term="ID" style={{wordBreak: 'break-all'}}>{supplyOrderShippingGroup.id}</Description> 
<Description term="名称" style={{wordBreak: 'break-all'}}>{supplyOrderShippingGroup.name}</Description> 
...
<Description term="金额" style={{wordBreak: 'break-all'}}>{supplyOrderShippingGroup.amount}</Description> 
...
class SupplyOrderShippingGroupWorkbench extends Component {
...
}))(Form.create()(SupplyOrderShippingGroupWorkbench))
