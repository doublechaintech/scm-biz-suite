...
import styles from './WechatMiniappIdentity.updateform.less'
...
class WechatMiniappIdentityUpdateForm extends Component {
  state = {
  ...
  render() {
    ...
    const submitUpdateForm = () => {
      validateFieldsAndScroll((error, values) => {
        ...
        const wechatMiniappIdentityId = values.id

        const parameters = { ...values, wechatMiniappIdentityId, }
    ...
    const submitUpdateFormAndContinue = () => {
      validateFieldsAndScroll((error, values) => {
        ...
        dispatch({
          type: `${owner.type}/updateWechatMiniappIdentity`,
          payload: {
            id: owner.id,
            type: 'wechatMiniappIdentity',
            parameters,
            selectedRows,
    ...
    const skipToNext = () => {
      ...
      const newIndex = currentUpdateIndex + 1
      dispatch({
        type: `${owner.type}/gotoNextWechatMiniappIdentityUpdateRow`,
        payload: {
          id: owner.id,
          type: 'wechatMiniappIdentity',
          selectedRows,
	...
	const internalRenderTitle = () =>{
      ...
      return (<div>{linkComp}{appLocaleName(userContext,"Update")}微信小程序认证: {(currentUpdateIndex+1)}/{selectedRows.length}</div>)
    }
              ...
              <Col lg={24} md={24} sm={24}>
                <Form.Item label={fieldLabels.openId} {...formItemLayout}>
                  {getFieldDecorator('openId', {
                    ...
                    <SmallTextInput size="large"  placeholder={fieldLabels.openId} />
              ...
              <Col lg={24} md={24} sm={24}>
                <Form.Item label={fieldLabels.unionId} {...formItemLayout}>
                  {getFieldDecorator('unionId', {
                    initialValue: selectedRow.unionId,
                    rules: [{ required: false, message: appLocaleName(userContext,"PleaseInput") }],
                  })(
                    <SmallTextInput size="large"  placeholder={fieldLabels.unionId} />
...
}))(Form.create()(WechatMiniappIdentityUpdateForm))
