...
export default {
  ...
  namespace: '_goodsShelfStockCount',
  ...
  subscriptions: {
    ...
    setup({ dispatch, history }) { 
      history.listen((location) => {
      	const modelName = 'goodsShelfStockCount'
  ...
  effects: {
    ...
    *analyze({ payload }, { call, put, select }){
      ...
      const data = yield call(GoodsShelfStockCountService.analyze, payload.id)
    ...
    *view({ payload }, { call, put, select }) { 
      ...
      const cachedData = yield select(state => state._goodsShelfStockCount)
      ...
      const {GoodsShelfStockCountService} = GlobalComponents;
      const data = yield call(GoodsShelfStockCountService.view, payload.id)
    ...
    *load({ payload }, { call, put }) { 
      const {GoodsShelfStockCountService} = GlobalComponents;
      //yield put({ type: 'showLoading', payload })
      const data = yield call(GoodsShelfStockCountService.load, payload.id, payload.parameters)
    ...
    *gotoUpdateForm({ payload }, { put }) {
      ...
      const location = { pathname: `/goodsShelfStockCount/${id}/list/${role}UpdateForm`, state }
      yield put(routerRedux.push(location))
    },
    *goback({ payload }, { put }) {
      const { id, type,listName } = payload
      yield put(routerRedux.push(`/goodsShelfStockCount/${id}/list/${type}List/${listName}`))
    },
    ...
    *addStockCountIssueTrack({ payload }, { call, put }) {
      ...
      const data = yield call(GoodsShelfStockCountService.addStockCountIssueTrack, id, parameters)
      if (hasError(data)) {
    ...
    *updateStockCountIssueTrack({ payload }, { call, put }) {
      ...
      console.log('get form parameters', parameters)
      const data = yield call(GoodsShelfStockCountService.updateStockCountIssueTrack, id, parameters)
      if (hasError(data)) {
        handleServerError(data)
        return
      }
    ...
    *removeStockCountIssueTrackList({ payload }, { call, put }) {
     const userContext = null
      ...
      const { id, role, parameters, continueNext } = payload
      console.log('get form parameters', parameters)
      const data = yield call(GoodsShelfStockCountService.removeStockCountIssueTrackList, id, parameters)
      if (hasError(data)) {
        handleServerError(data)
        return
      }
