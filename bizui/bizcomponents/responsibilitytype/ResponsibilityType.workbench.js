...
import styles from './ResponsibilityType.workbench.less'
...
const renderSettingMenuItem = (item,cardsData,targetComponent) =>{
  ...
  return (<Menu.Item key={item.name}>
      <Link to={`/responsibilityType/${targetComponent.props.responsibilityType.id}/list/${item.name}/${item.displayName}/`}>
...
const renderSettingMenu = (cardsData,targetComponent) =>{
  ...
  return (<Menu>
    	<Menu.Item key="profile">
  			<Link to={`/responsibilityType/${targetComponent.props.responsibilityType.id}/permission`}><Icon type="safety-certificate" theme="twoTone" twoToneColor="#52c41a"/><span>{appLocaleName(userContext,"Permission")}</span></Link>
		</Menu.Item>
		<Menu.Divider />
...
const internalSummaryOf = (cardsData,targetComponent) =>{
	...
	const responsibilityType = cardsData.cardsSource
	const {ResponsibilityTypeService} = GlobalComponents
	const userContext = null
	return (
	<div>
	<DescriptionList className={styles.headerList} size="small" col="4">
<Description term="ID" style={{wordBreak: 'break-all'}}>{responsibilityType.id}</Description> 
...
class ResponsibilityTypeWorkbench extends Component {
 ...
 state = {
  ...
  render() {
    ...
    const cardsData = {cardsName:window.trans('responsibility_type'),cardsFor: "responsibilityType",
    	cardsSource: this.props.responsibilityType,returnURL,displayName,
  		subItems: [
{name: 'employeeList', displayName: window.mtrans('employee','responsibility_type.employee_list',false) ,viewGroup:'__no_group', type:'employee',count:employeeCount,addFunction: true, role: 'employee', metaInfo: employeeListMetaInfo, renderItem: GlobalComponents.EmployeeBase.renderItemOfList},
...
}))(Form.create()(ResponsibilityTypeWorkbench))
