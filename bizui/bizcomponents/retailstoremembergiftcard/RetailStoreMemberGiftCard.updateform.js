...
import styles from './RetailStoreMemberGiftCard.updateform.less'
...
class RetailStoreMemberGiftCardUpdateForm extends Component {
  state = {
  ...
  render() {
    ...
    const submitUpdateForm = () => {
      validateFieldsAndScroll((error, values) => {
        ...
        const retailStoreMemberGiftCardId = values.id

        const parameters = { ...values, retailStoreMemberGiftCardId, }
    ...
    const submitUpdateFormAndContinue = () => {
      validateFieldsAndScroll((error, values) => {
        ...
        dispatch({
          type: `${owner.type}/updateRetailStoreMemberGiftCard`,
          payload: {
            id: owner.id,
            type: 'retailStoreMemberGiftCard',
            parameters,
            selectedRows,
    ...
    const skipToNext = () => {
      ...
      const newIndex = currentUpdateIndex + 1
      dispatch({
        type: `${owner.type}/gotoNextRetailStoreMemberGiftCardUpdateRow`,
        payload: {
          id: owner.id,
          type: 'retailStoreMemberGiftCard',
          selectedRows,
	...
	const internalRenderTitle = () =>{
      ...
      return (<div>{linkComp}{appLocaleName(userContext,"Update")}零售店会员礼品卡: {(currentUpdateIndex+1)}/{selectedRows.length}</div>)
    }
              ...
              <Col lg={24} md={24} sm={24}>
                <Form.Item label={fieldLabels.remain} {...formItemLayout}>
                  {getFieldDecorator('remain', {
                    ...
                    <SmallTextInput size="large" prefix={`${appLocaleName(userContext,"Currency")}`} placeholder={fieldLabels.remain} />
...
}))(Form.create()(RetailStoreMemberGiftCardUpdateForm))
