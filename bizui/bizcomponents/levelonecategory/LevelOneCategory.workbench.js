...
import styles from './LevelOneCategory.workbench.less'
...
const renderSettingMenuItem = (item,cardsData,targetComponent) =>{
  ...
  return (<Menu.Item key={item.name}>
      <Link to={`/levelOneCategory/${targetComponent.props.levelOneCategory.id}/list/${item.name}/${item.displayName}/`}>
...
const renderSettingMenu = (cardsData,targetComponent) =>{
  ...
  return (<Menu>
    	<Menu.Item key="profile">
  			<Link to={`/levelOneCategory/${targetComponent.props.levelOneCategory.id}/permission`}><Icon type="safety-certificate" theme="twoTone" twoToneColor="#52c41a"/><span>{appLocaleName(userContext,"Permission")}</span></Link>
		</Menu.Item>
		<Menu.Divider />
...
const internalSummaryOf = (cardsData,targetComponent) =>{
	...
	const levelOneCategory = cardsData.cardsSource
	const {LevelOneCategoryService} = GlobalComponents
	const userContext = null
	return (
	<div>
	<DescriptionList className={styles.headerList} size="small" col="4">
<Description term="ID" style={{wordBreak: 'break-all'}}>{levelOneCategory.id}</Description> 
...
class LevelOneCategoryWorkbench extends Component {
...
}))(Form.create()(LevelOneCategoryWorkbench))
