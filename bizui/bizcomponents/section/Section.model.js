...
export default {
  ...
  effects: {
    ...
    *analyze({ payload }, { call, put, select }){
      ...
      const data = yield call(SectionService.analyze, payload.id)
    ...
    *view({ payload }, { call, put, select }) { 
      ...
      const cachedData = yield select(state => state._section)
      ...
      const {SectionService} = GlobalComponents;
      const data = yield call(SectionService.view, payload.id)
    ...
    *load({ payload }, { call, put }) { 
      const {SectionService} = GlobalComponents;
      //yield put({ type: 'showLoading', payload })
      const data = yield call(SectionService.load, payload.id, payload.parameters)
    ...
    *gotoCreateForm({ payload }, { put }) {
      ...
      yield put(routerRedux.push(`/section/${id}/list/${role}CreateForm`))
    },
    *gotoUpdateForm({ payload }, { put }) {
      const { id, role, selectedRows, currentUpdateIndex } = payload
      const state = { id, role, selectedRows, currentUpdateIndex }
      const location = { pathname: `/section/${id}/list/${role}UpdateForm`, state }
      yield put(routerRedux.push(location))
    },
    *goback({ payload }, { put }) {
      const { id, type,listName } = payload
      yield put(routerRedux.push(`/section/${id}/list/${type}List/${listName}`))
    },
