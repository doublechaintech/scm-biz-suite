...
import styles from './AccountSet.workbench.less'
...
const internalSummaryOf = (cardsData,targetComponent) =>{
	...
	const accountSet = cardsData.cardsSource
	const {AccountSetService} = GlobalComponents
	const userContext = null
	return (
	<div>
	<DescriptionList className={styles.headerList} size="small" col="4">
<Description term="ID" style={{wordBreak: 'break-all'}}>{accountSet.id}</Description> 
...
<Description term="更新于">{ moment(accountSet.lastUpdateTime).format('YYYY-MM-DD HH:mm')}</Description> 
...
class AccountSetWorkbench extends Component {
...
}))(Form.create()(AccountSetWorkbench))
