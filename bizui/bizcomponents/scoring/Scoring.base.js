...
const menuData = {menuName: window.trans('scoring'), menuFor: "scoring",  internalName: "scoring",
  		subItems: [
...
const settingMenuData = {menuName: window.trans('scoring'), menuFor: "scoring",  internalName: "scoring",
  		subItems: [
...
const displayColumns = [
  { title: fieldLabels.id, debugtype: 'string', dataIndex: 'id', width: '6', render: (text, record)=>renderTextCell(text,record,'scoring') , sorter: true },
  { title: fieldLabels.scoredBy, debugtype: 'string', dataIndex: 'scoredBy', width: '7',render: (text, record)=>renderTextCell(text,record)},
  { title: fieldLabels.score, dataIndex: 'score', className:'money', render: (text, record) => renderNumberCell(text, record, 0), sorter: true  },
...
const renderActionList=(scoring, targetObject, columCount, listName)=>{

	if(!scoring){
		return null
	}
	if(!scoring.actionList){
		return null
	}
	if(scoring.actionList.length === 0){
		return null
	}
	return (
		<div className={styles.overlay}>
			...
			{scoring.actionList.map(action=>(<Link key={action.id} to={{pathname: action.actionPath.substring(1), state: {ownerId:targetObject.id,action,selectedRows:[scoring]}}} >
...
const renderItemOfList=(scoring, targetObject, columCount, listName)=>{
  ...
  const userContext = null
  return (
     <Row key={`${listName}-${scoring.id}`} className={styles.itemDesc}>
	...
	<Col span={20}>
      ...
      <DescriptionList  key={scoring.id} size="small" col={displayColumnsCount} >
        ...
        <Description term={fieldLabels.score}><div style={{"color":"red"}}>{formatNumber(scoring.score,0)}</div></Description> 
        <Description term={fieldLabels.comment} style={{wordBreak: 'break-all'}}>{scoring.comment}</Description> 


      </DescriptionList>
     </Col>
      {renderActionList(scoring,targetObject)}
    </Row>
	)
...
const ScoringBase={unpackObjectToFormValues, menuData,settingMenuData,displayColumns,fieldLabels,renderItemOfList, stepOf, searchLocalData}
