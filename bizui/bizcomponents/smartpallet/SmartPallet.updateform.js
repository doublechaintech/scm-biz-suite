...
import styles from './SmartPallet.updateform.less'
...
class SmartPalletUpdateForm extends Component {
  state = {
  ...
  render() {
    ...
    const submitUpdateForm = () => {
      validateFieldsAndScroll((error, values) => {
        ...
        const smartPalletId = values.id

        const parameters = { ...values, smartPalletId, }
    ...
    const submitUpdateFormAndContinue = () => {
      validateFieldsAndScroll((error, values) => {
        ...
        dispatch({
          type: `${owner.type}/updateSmartPallet`,
          payload: {
            id: owner.id,
            type: 'smartPallet',
            parameters,
    ...
    const skipToNext = () => {
      ...
      const newIndex = currentUpdateIndex + 1
      dispatch({
        type: `${owner.type}/gotoNextSmartPalletUpdateRow`,
        payload: {
          id: owner.id,
          type: 'smartPallet',
          selectedRows,
	...
	const internalRenderTitle = () =>{
      ...
      return (<div>{linkComp}{appLocaleName(userContext,"Update")}智能托盘: {(currentUpdateIndex+1)}/{selectedRows.length}</div>)
    }
...
}))(Form.create()(SmartPalletUpdateForm))
