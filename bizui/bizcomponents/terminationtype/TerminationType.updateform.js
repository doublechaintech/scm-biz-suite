...
import styles from './TerminationType.updateform.less'
...
class TerminationTypeUpdateForm extends Component {
  state = {
  ...
  render() {
    ...
    const submitUpdateForm = () => {
      validateFieldsAndScroll((error, values) => {
        ...
        const terminationTypeId = values.id

        const parameters = { ...values, terminationTypeId, }
    ...
    const skipToNext = () => {
      ...
      const newIndex = currentUpdateIndex + 1
      dispatch({
        type: `${owner.type}/gotoNextTerminationTypeUpdateRow`,
        payload: {
          id: owner.id,
          type: 'terminationType',
          selectedRows,
	...
	const internalRenderTitle = () =>{
      ...
      return (<div>{linkComp}{appLocaleName(userContext,"Update")}雇佣终止类型: {(currentUpdateIndex+1)}/{selectedRows.length}</div>)
    }
...
}))(Form.create()(TerminationTypeUpdateForm))
