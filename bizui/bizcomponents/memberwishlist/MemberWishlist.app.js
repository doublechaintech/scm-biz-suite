 ...
 getNavMenuItems = (targetObject, style, customTheme) => {
    ...
    return (
	  <Menu
       ...
       <Menu.Item key="workbench">
        <Link to={`/memberWishlist/${this.props.memberWishlist.id}/workbench`}><Icon type="solution" style={{marginRight:"20px"}}/><span>工作台</span></Link>
      </Menu.Item>
  ...
  getMemberWishlistProductSearch = () => {
    ...
    return connect(state => ({
      rule: state.rule,
      name: window.mtrans('member_wishlist_product','member_wishlist.member_wishlist_product_list',false),
      ...
      returnURL: `/memberWishlist/${state._memberWishlist.id}/workbench`,
  ...
  getMemberWishlistProductCreateForm = () => {
    ...
    return connect(state => ({
      ...
      metaInfo: state._memberWishlist.memberWishlistProductListMetaInfo,
      count: state._memberWishlist.memberWishlistProductCount,
      returnURL: `/memberWishlist/${state._memberWishlist.id}/list`,
      currentPage: state._memberWishlist.memberWishlistProductCurrentPageNumber,
      searchFormParameters: state._memberWishlist.memberWishlistProductSearchFormParameters,
      loading: state._memberWishlist.loading,
      owner: { type: '_memberWishlist', id: state._memberWishlist.id || this.getOwnerId(), referenceName: 'owner', listName: 'memberWishlistProductList', ref:state._memberWishlist, listDisplayName: appLocaleName(userContext,"List")}, // this is for model namespace and
  ...
  getMemberWishlistProductUpdateForm = () => {
    ...
    return connect(state => ({
      ...
      role: "memberWishlistProduct",
      currentUpdateIndex: state._memberWishlist.currentUpdateIndex || 0,
      owner: { type: '_memberWishlist', id: state._memberWishlist.id || this.getOwnerId(), listName: 'memberWishlistProductList', ref:state._memberWishlist, listDisplayName: appLocaleName(userContext,"List") }, // this is for model namespace and
  ...
  getRequestTypeStepForm = () => {
    ...
    return connect(state => ({
      ...
      owner: { type: '_memberWishlist', id: state._memberWishlist.id, listName: 'nolist', ref:state._memberWishlist, listDisplayName: appLocaleName(userContext,"List") }, // this is for model namespace and
  ...
  buildRouters = () =>{
    const {MemberWishlistWorkbench} = GlobalComponents

    const {MemberWishlistDashboard} = GlobalComponents
  	const {MemberWishlistPermission} = GlobalComponents
  	const {MemberWishlistProfile} = GlobalComponents


    const routers=[
    {path:"/memberWishlist/:id/workbench", component: MemberWishlistWorkbench},
    
  	{path:"/memberWishlist/:id/profile", component: MemberWishlistProfile},
  	{path:"/memberWishlist/:id/permission", component: MemberWishlistPermission},
  ...
  handleOpenChange = (openKeys) => {
   ...
   render() {
    ...
    const {searchLocalData}=GlobalComponents.MemberWishlistBase
    ...
    const renderMenuSwitch=()=>{
     ...
     <Layout>
      ...
      {this.state.showSearch&&(
        ...
        <div style={{backgroundColor:'black'}}  onClick={()=>hideSearchResult()}  >{searchLocalData(this.props.memberWishlist,this.state.searchKeyword)}</div>
       ...
       <Sider
        ...
        >
         ...
         {this.getNavMenuItems(this.props.memberWishlist,"inline","dark")}
