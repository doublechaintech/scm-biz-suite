...
import styles from './SalaryGrade.updateform.less'
...
class SalaryGradeUpdateForm extends Component {
  state = {
  ...
  render() {
    ...
    const submitUpdateForm = () => {
      validateFieldsAndScroll((error, values) => {
        ...
        const salaryGradeId = values.id

        const parameters = { ...values, salaryGradeId, }
    ...
    const submitUpdateFormAndContinue = () => {
      validateFieldsAndScroll((error, values) => {
        ...
        dispatch({
          type: `${owner.type}/updateSalaryGrade`,
          payload: {
            id: owner.id,
            type: 'salaryGrade',
            parameters,
    ...
    const skipToNext = () => {
      ...
      const newIndex = currentUpdateIndex + 1
      dispatch({
        type: `${owner.type}/gotoNextSalaryGradeUpdateRow`,
        payload: {
          id: owner.id,
          type: 'salaryGrade',
          selectedRows,
	...
	const internalRenderTitle = () =>{
      ...
      return (<div>{linkComp}{appLocaleName(userContext,"Update")}工资等级: {(currentUpdateIndex+1)}/{selectedRows.length}</div>)
    }
...
}))(Form.create()(SalaryGradeUpdateForm))
