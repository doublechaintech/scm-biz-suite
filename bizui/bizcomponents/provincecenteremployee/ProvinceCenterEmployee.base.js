...
const menuData = {menuName: window.trans('province_center_employee'), menuFor: "provinceCenterEmployee",  internalName: "province_center_employee",
  		subItems: [
...
const settingMenuData = {menuName: window.trans('province_center_employee'), menuFor: "provinceCenterEmployee",  internalName: "province_center_employee",
  		subItems: [
...
const displayColumns = [
  { title: fieldLabels.id, debugtype: 'string', dataIndex: 'id', width: '6', render: (text, record)=>renderTextCell(text,record,'provinceCenterEmployee') , sorter: true },
...
const renderActionList=(provinceCenterEmployee, targetObject, columCount, listName)=>{

	if(!provinceCenterEmployee){
		return null
	}
	if(!provinceCenterEmployee.actionList){
		return null
	}
	if(provinceCenterEmployee.actionList.length === 0){
		return null
	}
	return (
		<div className={styles.overlay}>
			...
			{provinceCenterEmployee.actionList.map(action=>(<Link key={action.id} to={{pathname: action.actionPath.substring(1), state: {ownerId:targetObject.id,action,selectedRows:[provinceCenterEmployee]}}} >
...
const renderItemOfList=(provinceCenterEmployee, targetObject, columCount, listName)=>{

  if(!provinceCenterEmployee){
  	return null
  }
  if(!provinceCenterEmployee.id){
  	return null
  }
  ...
  const userContext = null
  return (
     <Row key={`${listName}-${provinceCenterEmployee.id}`} className={styles.itemDesc}>
	...
	<Col span={20}>
      ...
      <DescriptionList  key={provinceCenterEmployee.id} size="small" col={displayColumnsCount} >
        ...
        <Description term={fieldLabels.department}>{renderReferenceItem(provinceCenterEmployee.department)}</Description>

        <Description term={fieldLabels.provinceCenter}>{renderReferenceItem(provinceCenterEmployee.provinceCenter)}</Description>



      </DescriptionList>
     </Col>
      {renderActionList(provinceCenterEmployee,targetObject)}
    </Row>
	)
...
const ProvinceCenterEmployeeBase={unpackObjectToFormValues, menuData,settingMenuData,displayColumns,fieldLabels,renderItemOfList, stepOf, searchLocalData}
