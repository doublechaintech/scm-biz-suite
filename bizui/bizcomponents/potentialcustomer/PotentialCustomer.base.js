...
const menuData = {menuName: window.trans('potential_customer'), menuFor: "potentialCustomer",  internalName: "potential_customer",
  		subItems: [
...
const settingMenuData = {menuName: window.trans('potential_customer'), menuFor: "potentialCustomer",  internalName: "potential_customer",
  		subItems: [
...
const displayColumns = [
  { title: fieldLabels.id, debugtype: 'string', dataIndex: 'id', width: '6', render: (text, record)=>renderTextCell(text,record,'potentialCustomer') , sorter: true },
...
const renderActionList=(potentialCustomer, targetObject, columCount, listName)=>{

	if(!potentialCustomer){
		return null
	}
	if(!potentialCustomer.actionList){
		return null
	}
	if(potentialCustomer.actionList.length === 0){
		return null
	}
	return (
		<div className={styles.overlay}>
			...
			{potentialCustomer.actionList.map(action=>(<Link key={action.id} to={{pathname: action.actionPath.substring(1), state: {ownerId:targetObject.id,action,selectedRows:[potentialCustomer]}}} >
...
const renderItemOfList=(potentialCustomer, targetObject, columCount, listName)=>{
  ...
  const userContext = null
  return (
     <Row key={`${listName}-${potentialCustomer.id}`} className={styles.itemDesc}>
	...
	<Col span={20}>
      ...
      {renderActionList(potentialCustomer,targetObject)}
    </Row>
	)
...
const packFormValuesToObject = ( formValuesToPack )=>{
	const {name, mobile, cityServiceCenterId, cityPartnerId, description, lastUpdateTime} = formValuesToPack
	const cityServiceCenter = {id: cityServiceCenterId, version: 2^31}
	const cityPartner = {id: cityPartnerId, version: 2^31}
	const data = {name, mobile, cityServiceCenter, cityPartner, description, lastUpdateTime:moment(lastUpdateTime).valueOf()}
	return data
}
const unpackObjectToFormValues = ( objectToUnpack )=>{
	const {name, mobile, cityServiceCenter, cityPartner, description, lastUpdateTime} = objectToUnpack
	const cityServiceCenterId = cityServiceCenter ? cityServiceCenter.id : null
	const cityPartnerId = cityPartner ? cityPartner.id : null
	const data = {name, mobile, cityServiceCenterId, cityPartnerId, description, lastUpdateTime:moment(lastUpdateTime)}
	return data
}
...
const PotentialCustomerBase={unpackObjectToFormValues, menuData,settingMenuData,displayColumns,fieldLabels,renderItemOfList, stepOf, searchLocalData}
