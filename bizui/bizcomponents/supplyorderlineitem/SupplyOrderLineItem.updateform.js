...
import styles from './SupplyOrderLineItem.updateform.less'
...
class SupplyOrderLineItemUpdateForm extends Component {
  state = {
  ...
  render() {
    ...
    const submitUpdateForm = () => {
      validateFieldsAndScroll((error, values) => {
        ...
        const supplyOrderLineItemId = values.id

        const parameters = { ...values, supplyOrderLineItemId, }
    ...
    const submitUpdateFormAndContinue = () => {
      validateFieldsAndScroll((error, values) => {
        ...
        dispatch({
          type: `${owner.type}/updateSupplyOrderLineItem`,
          payload: {
            id: owner.id,
            type: 'supplyOrderLineItem',
            parameters,
            selectedRows,
    ...
    const skipToNext = () => {
      ...
      const newIndex = currentUpdateIndex + 1
      dispatch({
        type: `${owner.type}/gotoNextSupplyOrderLineItemUpdateRow`,
        payload: {
          id: owner.id,
          type: 'supplyOrderLineItem',
          selectedRows,
	...
	const internalRenderTitle = () =>{
      ...
      return (<div>{linkComp}{appLocaleName(userContext,"Update")}供应订单行项目: {(currentUpdateIndex+1)}/{selectedRows.length}</div>)
    }
...
}))(Form.create()(SupplyOrderLineItemUpdateForm))
