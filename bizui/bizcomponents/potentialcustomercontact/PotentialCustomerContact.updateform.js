...
import styles from './PotentialCustomerContact.updateform.less'
...
class PotentialCustomerContactUpdateForm extends Component {
  state = {
  ...
  getSelectedRow() {
    ...
    const convertiedValues = selectedRows.map((item) => {
      return {
        ...item,
        contactDate: moment(item.contactDate),
  ...
  render() {
    ...
    const submitUpdateForm = () => {
      validateFieldsAndScroll((error, values) => {
        ...
        const potentialCustomerContactId = values.id

        const parameters = { ...values, potentialCustomerContactId, }
    ...
    const submitUpdateFormAndContinue = () => {
      validateFieldsAndScroll((error, values) => {
        ...
        dispatch({
          type: `${owner.type}/updatePotentialCustomerContact`,
          payload: {
            id: owner.id,
            type: 'potentialCustomerContact',
            parameters,
            selectedRows,
    ...
    const skipToNext = () => {
      ...
      const newIndex = currentUpdateIndex + 1
      dispatch({
        type: `${owner.type}/gotoNextPotentialCustomerContactUpdateRow`,
        payload: {
          id: owner.id,
          type: 'potentialCustomerContact',
          selectedRows,
              ...
              <Col lg={24} md={24} sm={24}>
                <Form.Item label={fieldLabels.contactMethod} {...formItemLayout}>
                  {getFieldDecorator('contactMethod', {
                    ...
                    <SmallTextInput size="large"  placeholder={fieldLabels.contactMethod} />
...
}))(Form.create()(PotentialCustomerContactUpdateForm))
