...
import styles from './EmployeeLeave.workbench.less'
...
const renderSettingMenuItem = (item,cardsData,targetComponent) =>{
  ...
  return (<Menu.Item key={item.name}>
      <Link to={`/employeeLeave/${targetComponent.props.employeeLeave.id}/list/${item.name}/${item.displayName}/`}>
...
const internalSummaryOf = (cardsData,targetComponent) =>{
	...
	const employeeLeave = cardsData.cardsSource
	const {EmployeeLeaveService} = GlobalComponents
	const userContext = null
	return (
	<div>
	<DescriptionList className={styles.headerList} size="small" col="4">
<Description term="ID" style={{wordBreak: 'break-all'}}>{employeeLeave.id}</Description> 
...
class EmployeeLeaveWorkbench extends Component {
...
}))(Form.create()(EmployeeLeaveWorkbench))
