...
import styles from './Termination.workbench.less'
...
const renderSettingMenuItem = (item,cardsData,targetComponent) =>{
  ...
  return (<Menu.Item key={item.name}>
      <Link to={`/termination/${targetComponent.props.termination.id}/list/${item.name}/${item.displayName}/`}>
...
const renderSettingMenu = (cardsData,targetComponent) =>{
  ...
  return (<Menu>
    	<Menu.Item key="profile">
  			<Link to={`/termination/${targetComponent.props.termination.id}/permission`}><Icon type="safety-certificate" theme="twoTone" twoToneColor="#52c41a"/><span>{appLocaleName(userContext,"Permission")}</span></Link>
		</Menu.Item>
		<Menu.Divider />
...
const internalSummaryOf = (cardsData,targetComponent) =>{
	...
	const termination = cardsData.cardsSource
	const {TerminationService} = GlobalComponents
	const userContext = null
	return (
	<div>
	<DescriptionList className={styles.headerList} size="small" col="4">
<Description term="ID" style={{wordBreak: 'break-all'}}>{termination.id}</Description> 
...
class TerminationWorkbench extends Component {
...
}))(Form.create()(TerminationWorkbench))
