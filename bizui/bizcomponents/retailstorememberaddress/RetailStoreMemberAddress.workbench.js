...
import styles from './RetailStoreMemberAddress.workbench.less'
...
const renderSettingMenuItem = (item,cardsData,targetComponent) =>{
  ...
  return (<Menu.Item key={item.name}>
      <Link to={`/retailStoreMemberAddress/${targetComponent.props.retailStoreMemberAddress.id}/list/${item.name}/${item.displayName}/`}>
...
const internalSummaryOf = (cardsData,targetComponent) =>{
	...
	const retailStoreMemberAddress = cardsData.cardsSource
	const {RetailStoreMemberAddressService} = GlobalComponents
	const userContext = null
	return (
	<div>
	<DescriptionList className={styles.headerList} size="small" col="4">
<Description term="ID" style={{wordBreak: 'break-all'}}>{retailStoreMemberAddress.id}</Description> 
...
class RetailStoreMemberAddressWorkbench extends Component {
...
}))(Form.create()(RetailStoreMemberAddressWorkbench))
