...
export default {
  ...
  effects: {
    ...
    *analyze({ payload }, { call, put, select }){
      ...
      const data = yield call(SlideService.analyze, payload.id)
    ...
    *view({ payload }, { call, put, select }) { 
      ...
      const cachedData = yield select(state => state._slide)
    ...
    *gotoCreateForm({ payload }, { put }) {
      ...
      yield put(routerRedux.push(`/slide/${id}/list/${role}CreateForm`))
    },
    *gotoUpdateForm({ payload }, { put }) {
      const { id, role, selectedRows, currentUpdateIndex } = payload
      const state = { id, role, selectedRows, currentUpdateIndex }
      const location = { pathname: `/slide/${id}/list/${role}UpdateForm`, state }
      yield put(routerRedux.push(location))
    },
    *goback({ payload }, { put }) {
      const { id, type,listName } = payload
      yield put(routerRedux.push(`/slide/${id}/list/${type}List/${listName}`))
    },
