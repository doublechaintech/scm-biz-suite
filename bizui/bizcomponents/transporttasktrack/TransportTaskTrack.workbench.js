...
import styles from './TransportTaskTrack.workbench.less'
...
const renderSettingMenuItem = (item,cardsData,targetComponent) =>{
  ...
  return (<Menu.Item key={item.name}>
      <Link to={`/transportTaskTrack/${targetComponent.props.transportTaskTrack.id}/list/${item.name}/${item.displayName}/`}>
...
const renderSettingMenu = (cardsData,targetComponent) =>{
  ...
  return (<Menu>
    	<Menu.Item key="profile">
  			<Link to={`/transportTaskTrack/${targetComponent.props.transportTaskTrack.id}/permission`}><Icon type="safety-certificate" theme="twoTone" twoToneColor="#52c41a"/><span>{appLocaleName(userContext,"Permission")}</span></Link>
		</Menu.Item>
		<Menu.Divider />
...
const internalSummaryOf = (cardsData,targetComponent) =>{
	...
	const transportTaskTrack = cardsData.cardsSource
	const {TransportTaskTrackService} = GlobalComponents
	const userContext = null
	return (
	<div>
	<DescriptionList className={styles.headerList} size="small" col="4">
<Description term="ID" style={{wordBreak: 'break-all'}}>{transportTaskTrack.id}</Description> 
...
class TransportTaskTrackWorkbench extends Component {
...
}))(Form.create()(TransportTaskTrackWorkbench))
