...
const menuData = {menuName: window.trans('potential_customer_contact_person'), menuFor: "potentialCustomerContactPerson",  internalName: "potential_customer_contact_person",
  		subItems: [
...
const settingMenuData = {menuName: window.trans('potential_customer_contact_person'), menuFor: "potentialCustomerContactPerson",  internalName: "potential_customer_contact_person",
  		subItems: [
...
const renderActionList=(potentialCustomerContactPerson, targetObject, columCount, listName)=>{

	if(!potentialCustomerContactPerson){
		return null
	}
	if(!potentialCustomerContactPerson.actionList){
		return null
	}
	if(potentialCustomerContactPerson.actionList.length === 0){
		return null
	}
	return (
		<div className={styles.overlay}>
			...
			{potentialCustomerContactPerson.actionList.map(action=>(<Link key={action.id} to={{pathname: action.actionPath.substring(1), state: {ownerId:targetObject.id,action,selectedRows:[potentialCustomerContactPerson]}}} >
...
const renderItemOfList=(potentialCustomerContactPerson, targetObject, columCount, listName)=>{
  ...
  const userContext = null
  return (
     <Row key={`${listName}-${potentialCustomerContactPerson.id}`} className={styles.itemDesc}>
	...
	<Col span={20}>
      ...
      {renderActionList(potentialCustomerContactPerson,targetObject)}
    </Row>
	)
...
const packFormValuesToObject = ( formValuesToPack )=>{
	const {name, mobile, potentialCustomerId, description} = formValuesToPack
	const potentialCustomer = {id: potentialCustomerId, version: 2^31}
	const data = {name, mobile, potentialCustomer, description}
	return data
}
const unpackObjectToFormValues = ( objectToUnpack )=>{
	const {name, mobile, potentialCustomer, description} = objectToUnpack
	const potentialCustomerId = potentialCustomer ? potentialCustomer.id : null
	const data = {name, mobile, potentialCustomerId, description}
	return data
}
...
const PotentialCustomerContactPersonBase={unpackObjectToFormValues, menuData,settingMenuData,displayColumns,fieldLabels,renderItemOfList, stepOf, searchLocalData}
