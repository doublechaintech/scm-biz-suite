...
import styles from './InterviewType.updateform.less'
...
class InterviewTypeUpdateForm extends Component {
  state = {
  ...
  render() {
    ...
    const submitUpdateForm = () => {
      validateFieldsAndScroll((error, values) => {
        ...
        const interviewTypeId = values.id

        const parameters = { ...values, interviewTypeId, }
    ...
    const submitUpdateFormAndContinue = () => {
      validateFieldsAndScroll((error, values) => {
        ...
        dispatch({
          type: `${owner.type}/updateInterviewType`,
          payload: {
            id: owner.id,
            type: 'interviewType',
            parameters,
    ...
    const skipToNext = () => {
      ...
      const newIndex = currentUpdateIndex + 1
      dispatch({
        type: `${owner.type}/gotoNextInterviewTypeUpdateRow`,
        payload: {
          id: owner.id,
          type: 'interviewType',
          selectedRows,
	...
	const internalRenderTitle = () =>{
      ...
      return (<div>{linkComp}{appLocaleName(userContext,"Update")}面试类型: {(currentUpdateIndex+1)}/{selectedRows.length}</div>)
    }
...
}))(Form.create()(InterviewTypeUpdateForm))
