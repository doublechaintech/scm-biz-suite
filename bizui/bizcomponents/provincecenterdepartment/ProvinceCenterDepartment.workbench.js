...
import styles from './ProvinceCenterDepartment.workbench.less'
...
const renderSettingMenuItem = (item,cardsData,targetComponent) =>{
  ...
  return (<Menu.Item key={item.name}>
      <Link to={`/provinceCenterDepartment/${targetComponent.props.provinceCenterDepartment.id}/list/${item.name}/${item.displayName}/`}>
...
const renderSettingMenu = (cardsData,targetComponent) =>{
  ...
  return (<Menu>
    	<Menu.Item key="profile">
  			<Link to={`/provinceCenterDepartment/${targetComponent.props.provinceCenterDepartment.id}/permission`}><Icon type="safety-certificate" theme="twoTone" twoToneColor="#52c41a"/><span>{appLocaleName(userContext,"Permission")}</span></Link>
		</Menu.Item>
		<Menu.Divider />
...
const internalSummaryOf = (cardsData,targetComponent) =>{
	...
	const provinceCenterDepartment = cardsData.cardsSource
	const {ProvinceCenterDepartmentService} = GlobalComponents
	const userContext = null
	return (
	<div>
	<DescriptionList className={styles.headerList} size="small" col="4">
<Description term="ID" style={{wordBreak: 'break-all'}}>{provinceCenterDepartment.id}</Description> 
...
class ProvinceCenterDepartmentWorkbench extends Component {
...
}))(Form.create()(ProvinceCenterDepartmentWorkbench))
