...
import styles from './ConsumerOrderPaymentGroup.dashboard.less'
...
const imageList =(consumerOrderPaymentGroup)=>{return [
	 ]}

const internalImageListOf = (consumerOrderPaymentGroup) =>defaultImageListOf(consumerOrderPaymentGroup,imageList)

const optionList =(consumerOrderPaymentGroup)=>{return [
	]}

const buildTransferModal = defaultBuildTransferModal
const showTransferModel = defaultShowTransferModel
const internalRenderSubjectList = defaultRenderSubjectList
const internalSettingListOf = (consumerOrderPaymentGroup) =>defaultSettingListOf(consumerOrderPaymentGroup, optionList)
const internalLargeTextOf = (consumerOrderPaymentGroup) =>{
...
const renderSettingMenuItem = (item,cardsData,targetComponent) =>{
  ...
  return (<Menu.Item key={item.name}>
      <Link to={`/consumerOrderPaymentGroup/${targetComponent.props.consumerOrderPaymentGroup.id}/list/${item.name}/${item.displayName}/`}>
...
const renderSettingMenu = (cardsData,targetComponent) =>{
  ...
  return (<Menu>
    	<Menu.Item key="profile">
  			<Link to={`/consumerOrderPaymentGroup/${targetComponent.props.consumerOrderPaymentGroup.id}/permission`}><Icon type="safety-certificate" theme="twoTone" twoToneColor="#52c41a"/><span>{appLocaleName(userContext,"Permission")}</span></Link>
		</Menu.Item>
		<Menu.Divider />
...
const internalSummaryOf = (cardsData,targetComponent) =>{
	...
	const consumerOrderPaymentGroup = cardsData.cardsSource
	const {ConsumerOrderPaymentGroupService} = GlobalComponents
	const userContext = null
	return (
	<div>
	<DescriptionList className={styles.headerList} size="small" col="4">
<Description term="ID" style={{wordBreak: 'break-all'}}>{consumerOrderPaymentGroup.id}</Description> 
<Description term="名称" style={{wordBreak: 'break-all'}}>{consumerOrderPaymentGroup.name}</Description> 
...
<Description term="卡号码" style={{wordBreak: 'break-all'}}>{consumerOrderPaymentGroup.cardNumber}</Description> 
...
class ConsumerOrderPaymentGroupDashboard extends Component {
 ...
 state = {
    ...
    defaultType: 'consumerOrderPaymentGroup'
  ...
  render() {
    ...
    if(!this.props.consumerOrderPaymentGroup.class){
      return null
    }
...
}))(Form.create()(ConsumerOrderPaymentGroupDashboard))
