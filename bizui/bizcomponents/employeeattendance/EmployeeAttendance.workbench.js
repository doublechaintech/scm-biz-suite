...
import styles from './EmployeeAttendance.workbench.less'
...
const renderSettingMenuItem = (item,cardsData,targetComponent) =>{
  ...
  return (<Menu.Item key={item.name}>
      <Link to={`/employeeAttendance/${targetComponent.props.employeeAttendance.id}/list/${item.name}/${item.displayName}/`}>
...
const internalSummaryOf = (cardsData,targetComponent) =>{
	...
	const employeeAttendance = cardsData.cardsSource
	const {EmployeeAttendanceService} = GlobalComponents
	const userContext = null
	return (
	<div>
	<DescriptionList className={styles.headerList} size="small" col="4">
<Description term="ID" style={{wordBreak: 'break-all'}}>{employeeAttendance.id}</Description> 
...
class EmployeeAttendanceWorkbench extends Component {
 ...
 state = {
  ...
  render() {
    ...
    const cardsData = {cardsName:window.trans('employee_attendance'),cardsFor: "employeeAttendance",
...
}))(Form.create()(EmployeeAttendanceWorkbench))
