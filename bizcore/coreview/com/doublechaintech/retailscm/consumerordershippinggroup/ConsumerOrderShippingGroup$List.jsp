...
<c:if test="${ empty consumerOrderShippingGroupList}" >
 ...
 <c:if test="${not empty consumerOrderShippingGroupList}" >
    
    
<%

 	SmartList list=(SmartList)request.getAttribute("consumerOrderShippingGroupList"); 
...
<c:set var="currentPageNumber" value="${page.pageNumber}"/>
	...
	<li class="page-item ${classType}">
		<a href='#${ownerBeanName}Manager/load${ownerClassName}/${result.id}/${consumerOrderShippingGroupListName};${consumerOrderShippingGroupListName}CurrentPage=${page.pageNumber};${consumerOrderShippingGroupListName}RowsPerPage=${rowsPerPage}/' 
...
<th>${userContext.localeMap['@action']}</th>
		...
		<tbody>
			...
			<c:forEach var="item" items="${consumerOrderShippingGroupList}">
				<tr currentVersion='${item.version}' id="consumerOrderShippingGroup-${item.id}" ><td><a class="link-action-removed" href="./consumerOrderShippingGroupManager/view/${item.id}/"> ${item.id}</a></td>
<c:if test="${param.referName ne 'name'}">	<td contenteditable='true' class='edit-value'  propertyToChange='name' storedCellValue='${item.name}' prefix='${ownerBeanName}Manager/updateConsumerOrderShippingGroup/${result.id}/${item.id}/'>${item.name}</td>
	...
	<td class="select_candidate_td"
			data-candidate-method="./consumerOrderShippingGroupManager/requestCandidateBizOrder/${ownerBeanName}/${item.id}/"
			data-switch-method="./consumerOrderShippingGroupManager/transferToAnotherBizOrder/${item.id}/"
...
<c:if test="${param.referName ne 'amount'}">	<td contenteditable='true' class='edit-value money'  propertyToChange='amount' storedCellValue='${item.amount}' prefix='${ownerBeanName}Manager/updateConsumerOrderShippingGroup/${result.id}/${item.id}/'><fmt:formatNumber type="currency"  value="${item.amount}" /></td>
</c:if>
				<td>
