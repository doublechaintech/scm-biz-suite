...
<section><h3>生超的订单(${result.id})</h3><table >
<tr><td class='th'>买方</td><td >${result.buyer.displayName}</td><td class='th'>头衔</td><td >${result.title}</td></tr><tr><td class='th'>总金额</td><td >${result.totalAmount}</td><td class='th'>合同</td><td >${result.contract}</td></tr><tr><td class='th'>更新于</td><td ><fmt:formatDate pattern="yyyy-MM-dd HH:mm" value="${result.lastUpdateTime}" /></td><td class='th'></td><td ></td></tr></table>
</section>
<section><c:if test="${not empty result.retailStoreOrderLineItemList}">
	<c:forEach items="${result.retailStoreOrderLineItemList}" var="item">
<section><h3>双链小超订单行项目(${item.id})</h3><table >
...
</c:forEach></c:if><c:if test="${not empty result.retailStoreOrderShippingGroupList}">
<section><h3>生超订单送货分组</h3>
<table><tr>
<tr>
<th>ID</th><th>名称</th><th>订单</th><th>金额</th></tr>	<c:forEach items="${result.retailStoreOrderShippingGroupList}" var="item">
	<tr>
<td>${item.id}</td><td>${item.name}</td><td>${item.bizOrder.displayName}</td><td>${item.amount}</td></tr></c:forEach></table>
</section>
</c:if><c:if test="${not empty result.retailStoreOrderPaymentGroupList}">
<section><h3>生超订单付款组</h3>
<table><tr>
<tr>
<th>ID</th><th>名称</th><th>订单</th><th>卡号码</th></tr>	<c:forEach items="${result.retailStoreOrderPaymentGroupList}" var="item">
	<tr>
