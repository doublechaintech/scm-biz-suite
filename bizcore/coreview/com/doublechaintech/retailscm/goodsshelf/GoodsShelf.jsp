<%@ page import='java.util.*,com.doublechaintech.retailscm.*,com.doublechaintech.retailscm.goodsshelf.GoodsShelf'%>
...
<c:set var="ownerBeanName" value="goodsShelf" scope="request"/>
<c:set var="ownerClassName" value="GoodsShelf" scope="request"/>
...
<div class="row">
	...
	<% GoodsShelf result = (GoodsShelf)request.getAttribute("result");  %>
			<li><a data-toggle="tab" href="#goodsShelfStockCountList" class="disabled"> ${userContext.localeMap['goods_shelf_stock_count']}</a></li>
			<li><a data-toggle="tab" href="#goodsAllocationList" class="disabled"> ${userContext.localeMap['goods_allocation']}</a></li>
...
<div class="row" desc="show parent objects">
	...
	<c:set var="goodsShelf" value="${ result}" scope="request" />
<sky:include page="com/doublechaintech/retailscm/goodsshelf/GoodsShelf$Summary.jsp" />
...
<sky:include page="com/doublechaintech/retailscm/damagespace/DamageSpace$Info.jsp" />
		...
		<c:if test='${not empty userContext.accessTokens["goodsShelfStockCountList"] or ignoreListAccessControl}'>
		<c:set var="goodsShelfStockCountList" value="${result.goodsShelfStockCountList}" scope="request"/>
		<c:set var="goodsShelfStockCountListName" value="goodsShelfStockCountList" scope="request"/>
		<div id="goodsShelfStockCountList" class="tab-pane fade sublist" refer-name="shelf">
			<sky:include page="com/doublechaintech/retailscm/goodsshelfstockcount/GoodsShelfStockCount$List.jsp"
					referName="shelf"/>
		</div>
	</c:if>
	<c:if test='${not empty userContext.accessTokens["goodsAllocationList"] or ignoreListAccessControl}'>
		<c:set var="goodsAllocationList" value="${result.goodsAllocationList}" scope="request"/>
		<c:set var="goodsAllocationListName" value="goodsAllocationList" scope="request"/>
		<div id="goodsAllocationList" class="tab-pane fade sublist" refer-name="goods_shelf">
			<sky:include page="com/doublechaintech/retailscm/goodsallocation/GoodsAllocation$List.jsp"
					referName="goodsShelf"/>
		</div>
	</c:if>
