<%@ page import='java.util.*,com.doublechaintech.retailscm.*,com.doublechaintech.retailscm.report.Report'%>
...
<c:set var="ownerBeanName" value="report" scope="request"/>
<c:set var="ownerClassName" value="Report" scope="request"/>
...
<div class="row">
	...
	<% Report result = (Report)request.getAttribute("result");  %>
			<li><a data-toggle="tab" href="#reportLineList" class="disabled"> ${userContext.localeMap['report_line']}</a></li>
...
<div class="row" desc="show parent objects">
	...
	<c:set var="report" value="${ result}" scope="request" />
<sky:include page="com/doublechaintech/retailscm/report/Report$Summary.jsp" />
...
<sky:include page="com/doublechaintech/retailscm/retailstorecountrycenter/RetailStoreCountryCenter$Info.jsp" />
		...
		<c:if test='${not empty userContext.accessTokens["reportLineList"] or ignoreListAccessControl}'>
		<c:set var="reportLineList" value="${result.reportLineList}" scope="request"/>
		<c:set var="reportLineListName" value="reportLineList" scope="request"/>
		<div id="reportLineList" class="tab-pane fade sublist" refer-name="owner">
			<sky:include page="com/doublechaintech/retailscm/reportline/ReportLine$List.jsp"
					referName="owner"/>
		</div>
	</c:if>
