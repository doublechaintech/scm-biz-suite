...
<c:if test="${ empty shippingSpaceList}" >
 ...
 <c:if test="${not empty shippingSpaceList}" >
    
    
<%

 	SmartList list=(SmartList)request.getAttribute("shippingSpaceList"); 
...
<c:set var="currentPageNumber" value="${page.pageNumber}"/>
	...
	<li class="page-item ${classType}">
		<a href='#${ownerBeanName}Manager/load${ownerClassName}/${result.id}/${shippingSpaceListName};${shippingSpaceListName}CurrentPage=${page.pageNumber};${shippingSpaceListName}RowsPerPage=${rowsPerPage}/' 
...
<c:if test="${param.referName ne 'lastUpdateTime'}">
	<th>${userContext.localeMap['shipping_space.last_update_time']}</th>
</c:if>
<th>${userContext.localeMap['@action']}</th>
		</tr></thead>
		<tbody>
			
			<c:forEach var="item" items="${shippingSpaceList}">
				<tr currentVersion='${item.version}' id="shippingSpace-${item.id}" ><td><a class="link-action-removed" href="./shippingSpaceManager/view/${item.id}/"> ${item.id}</a></td>
<c:if test="${param.referName ne 'location'}">	<td contenteditable='true' class='edit-value'  propertyToChange='location' storedCellValue='${item.location}' prefix='${ownerBeanName}Manager/updateShippingSpace/${result.id}/${item.id}/'>${item.location}</td>
	...
	<td class="select_candidate_td"
			data-candidate-method="./shippingSpaceManager/requestCandidateWarehouse/${ownerBeanName}/${item.id}/"
			data-switch-method="./shippingSpaceManager/transferToAnotherWarehouse/${item.id}/"
...
<c:if test="${param.referName ne 'latitude'}">	<td contenteditable='true' class='edit-value'  propertyToChange='latitude' storedCellValue='${item.latitude}' prefix='${ownerBeanName}Manager/updateShippingSpace/${result.id}/${item.id}/'>${item.latitude}</td>
...
</c:if><c:if test="${param.referName ne 'lastUpdateTime'}">	<td contenteditable='true' class='edit-value'  propertyToChange='lastUpdateTime' storedCellValue='${item.lastUpdateTime}' prefix='${ownerBeanName}Manager/updateShippingSpace/${result.id}/${item.id}/'><fmt:formatDate pattern="yyyy-MM-dd'T'HH:mm:ss" value="${item.lastUpdateTime}" /></td>
</c:if>
				<td>
