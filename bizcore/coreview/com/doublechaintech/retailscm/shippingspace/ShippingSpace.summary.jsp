...
<section><h3>发货区(${result.id})</h3><table >
<tr><td class='th'>位置</td><td >${result.location}</td><td class='th'>联系电话</td><td >${result.contactNumber}</td></tr><tr><td class='th'>总面积</td><td >${result.totalArea}</td><td class='th'>仓库</td><td >${result.warehouse.displayName}</td></tr><tr><td class='th'>纬度</td><td >${result.latitude}</td><td class='th'>经度</td><td >${result.longitude}</td></tr><tr><td class='th'>描述</td><td >${result.description}</td><td class='th'>更新于</td><td ><fmt:formatDate pattern="yyyy-MM-dd HH:mm" value="${result.lastUpdateTime}" /></td></tr></table>
</section>
