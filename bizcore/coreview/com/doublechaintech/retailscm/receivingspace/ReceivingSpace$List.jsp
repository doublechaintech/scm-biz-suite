...
<c:if test="${ empty receivingSpaceList}" >
 ...
 <c:if test="${not empty receivingSpaceList}" >
    
    
<%

 	SmartList list=(SmartList)request.getAttribute("receivingSpaceList"); 
...
<c:set var="currentPageNumber" value="${page.pageNumber}"/>
	...
	<li class="page-item ${classType}">
		<a href='#${ownerBeanName}Manager/load${ownerClassName}/${result.id}/${receivingSpaceListName};${receivingSpaceListName}CurrentPage=${page.pageNumber};${receivingSpaceListName}RowsPerPage=${rowsPerPage}/' 
...
<th>${userContext.localeMap['@action']}</th>
		...
		<tbody>
			...
			<c:forEach var="item" items="${receivingSpaceList}">
				<tr currentVersion='${item.version}' id="receivingSpace-${item.id}" ><td><a class="link-action-removed" href="./receivingSpaceManager/view/${item.id}/"> ${item.id}</a></td>
<c:if test="${param.referName ne 'location'}">	<td contenteditable='true' class='edit-value'  propertyToChange='location' storedCellValue='${item.location}' prefix='${ownerBeanName}Manager/updateReceivingSpace/${result.id}/${item.id}/'>${item.location}</td>
	...
	<td class="select_candidate_td"
			data-candidate-method="./receivingSpaceManager/requestCandidateWarehouse/${ownerBeanName}/${item.id}/"
			data-switch-method="./receivingSpaceManager/transferToAnotherWarehouse/${item.id}/"
