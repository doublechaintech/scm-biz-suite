...
<c:if test="${ empty retailStoreOrderPickingList}" >
 ...
 <c:if test="${not empty retailStoreOrderPickingList}" >
    
    
<%

 	SmartList list=(SmartList)request.getAttribute("retailStoreOrderPickingList"); 
...
<c:set var="currentPageNumber" value="${page.pageNumber}"/>
	...
	<li class="page-item ${classType}">
		<a href='#${ownerBeanName}Manager/load${ownerClassName}/${result.id}/${retailStoreOrderPickingListName};${retailStoreOrderPickingListName}CurrentPage=${page.pageNumber};${retailStoreOrderPickingListName}RowsPerPage=${rowsPerPage}/' 
...
<th>${userContext.localeMap['@action']}</th>
		...
		<tbody>
			...
			<c:forEach var="item" items="${retailStoreOrderPickingList}">
				<tr currentVersion='${item.version}' id="retailStoreOrderPicking-${item.id}" ><td><a class="link-action-removed" href="./retailStoreOrderPickingManager/view/${item.id}/"> ${item.id}</a></td>
