...
<section><h3>城市合伙人(${result.id})</h3><table >
<tr><td class='th'>名称</td><td >${result.name}</td><td class='th'>手机</td><td >${result.mobile}</td></tr><tr><td class='th'>城市服务中心</td><td >${result.cityServiceCenter.displayName}</td><td class='th'>描述</td><td >${result.description}</td></tr><tr><td class='th'>更新于</td><td ><fmt:formatDate pattern="yyyy-MM-dd HH:mm" value="${result.lastUpdateTime}" /></td><td class='th'></td><td ></td></tr></table>
</section>
<section><c:if test="${not empty result.potentialCustomerList}">
...
</c:forEach></c:if><c:if test="${not empty result.potentialCustomerContactList}">
