...
<c:if test="${ empty transportTaskTrackList}" >
 ...
 <c:if test="${not empty transportTaskTrackList}" >
    
    
<%

 	SmartList list=(SmartList)request.getAttribute("transportTaskTrackList"); 
...
<c:set var="currentPageNumber" value="${page.pageNumber}"/>
	...
	<li class="page-item ${classType}">
		<a href='#${ownerBeanName}Manager/load${ownerClassName}/${result.id}/${transportTaskTrackListName};${transportTaskTrackListName}CurrentPage=${page.pageNumber};${transportTaskTrackListName}RowsPerPage=${rowsPerPage}/' 
...
<th>${userContext.localeMap['@action']}</th>
		...
		<tbody>
			...
			<c:forEach var="item" items="${transportTaskTrackList}">
				<tr currentVersion='${item.version}' id="transportTaskTrack-${item.id}" ><td><a class="link-action-removed" href="./transportTaskTrackManager/view/${item.id}/"> ${item.id}</a></td>
...
</c:if><c:if test="${param.referName ne 'movement'}">
	<td class="select_candidate_td"
			data-candidate-method="./transportTaskTrackManager/requestCandidateMovement/${ownerBeanName}/${item.id}/"
			data-switch-method="./transportTaskTrackManager/transferToAnotherMovement/${item.id}/"
			data-link-template="./transportTaskManager/view/${'$'}{ID}/">
		<span class="display_span">
			<c:if test="${not empty  item.movement}">
			<a href='./transportTaskManager/view/${item.movement.id}/'>${item.movement.displayName}</a>
			</c:if>
			<c:if test="${empty  item.movement}">
			<a href='#'></a>
			</c:if>
