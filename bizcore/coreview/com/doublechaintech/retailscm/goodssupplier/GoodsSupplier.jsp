<%@ page import='java.util.*,com.doublechaintech.retailscm.*,com.doublechaintech.retailscm.goodssupplier.GoodsSupplier'%>
...
<c:set var="ownerBeanName" value="goodsSupplier" scope="request"/>
<c:set var="ownerClassName" value="GoodsSupplier" scope="request"/>
...
<div class="row">
	...
	<% GoodsSupplier result = (GoodsSupplier)request.getAttribute("result");  %>
			<li><a data-toggle="tab" href="#supplierProductList" class="disabled"> ${userContext.localeMap['supplier_product']}</a></li>
...
<div class="row" desc="show parent objects">
	...
	<c:set var="goodsSupplier" value="${ result}" scope="request" />
<sky:include page="com/doublechaintech/retailscm/goodssupplier/GoodsSupplier$Summary.jsp" />
	...
	<div class="row">
<c:set var="retailStoreCountryCenter" value="${ result.belongTo}" scope="request" />
<c:set var="referName" value="(belongTo)" scope="request" />
<sky:include page="com/doublechaintech/retailscm/retailstorecountrycenter/RetailStoreCountryCenter$Info.jsp" />
		...
		<c:if test='${not empty userContext.accessTokens["supplierProductList"] or ignoreListAccessControl}'>
		<c:set var="supplierProductList" value="${result.supplierProductList}" scope="request"/>
		<c:set var="supplierProductListName" value="supplierProductList" scope="request"/>
		<div id="supplierProductList" class="tab-pane fade sublist" refer-name="supplier">
			<sky:include page="com/doublechaintech/retailscm/supplierproduct/SupplierProduct$List.jsp"
					referName="supplier"/>
		</div>
	</c:if>
	<c:if test='${not empty userContext.accessTokens["supplyOrderList"] or ignoreListAccessControl}'>
		...
		<div id="supplyOrderList" class="tab-pane fade sublist" refer-name="seller">
	...
	<c:if test='${not empty userContext.accessTokens["accountSetList"] or ignoreListAccessControl}'>
		...
		<div id="accountSetList" class="tab-pane fade sublist" refer-name="goods_supplier">
			<sky:include page="com/doublechaintech/retailscm/accountset/AccountSet$List.jsp"
					referName="goodsSupplier"/>
		</div>
	</c:if>
