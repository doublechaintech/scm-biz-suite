...
<c:if test="${ empty supplierSpaceList}" >
 ...
 <c:if test="${not empty supplierSpaceList}" >
    
    
<%

 	SmartList list=(SmartList)request.getAttribute("supplierSpaceList"); 
...
<c:set var="currentPageNumber" value="${page.pageNumber}"/>
	...
	<li class="page-item ${classType}">
		<a href='#${ownerBeanName}Manager/load${ownerClassName}/${result.id}/${supplierSpaceListName};${supplierSpaceListName}CurrentPage=${page.pageNumber};${supplierSpaceListName}RowsPerPage=${rowsPerPage}/' 
...
<c:if test="${param.referName ne 'lastUpdateTime'}">
	<th>${userContext.localeMap['supplier_space.last_update_time']}</th>
</c:if>
<th>${userContext.localeMap['@action']}</th>
		</tr></thead>
		<tbody>
			
			<c:forEach var="item" items="${supplierSpaceList}">
				<tr currentVersion='${item.version}' id="supplierSpace-${item.id}" ><td><a class="link-action-removed" href="./supplierSpaceManager/view/${item.id}/"> ${item.id}</a></td>
<c:if test="${param.referName ne 'location'}">	<td contenteditable='true' class='edit-value'  propertyToChange='location' storedCellValue='${item.location}' prefix='${ownerBeanName}Manager/updateSupplierSpace/${result.id}/${item.id}/'>${item.location}</td>
	...
	<td class="select_candidate_td"
			data-candidate-method="./supplierSpaceManager/requestCandidateWarehouse/${ownerBeanName}/${item.id}/"
			data-switch-method="./supplierSpaceManager/transferToAnotherWarehouse/${item.id}/"
...
<c:if test="${param.referName ne 'latitude'}">	<td contenteditable='true' class='edit-value'  propertyToChange='latitude' storedCellValue='${item.latitude}' prefix='${ownerBeanName}Manager/updateSupplierSpace/${result.id}/${item.id}/'>${item.latitude}</td>
...
</c:if><c:if test="${param.referName ne 'lastUpdateTime'}">	<td contenteditable='true' class='edit-value'  propertyToChange='lastUpdateTime' storedCellValue='${item.lastUpdateTime}' prefix='${ownerBeanName}Manager/updateSupplierSpace/${result.id}/${item.id}/'><fmt:formatDate pattern="yyyy-MM-dd'T'HH:mm:ss" value="${item.lastUpdateTime}" /></td>
</c:if>
				<td>
