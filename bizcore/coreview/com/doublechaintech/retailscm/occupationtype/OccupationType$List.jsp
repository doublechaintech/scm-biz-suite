...
<c:if test="${ empty occupationTypeList}" >
 ...
 <c:if test="${not empty occupationTypeList}" >
    
    
<%

 	SmartList list=(SmartList)request.getAttribute("occupationTypeList"); 
...
<c:set var="currentPageNumber" value="${page.pageNumber}"/>
	...
	<li class="page-item ${classType}">
		<a href='#${ownerBeanName}Manager/load${ownerClassName}/${result.id}/${occupationTypeListName};${occupationTypeListName}CurrentPage=${page.pageNumber};${occupationTypeListName}RowsPerPage=${rowsPerPage}/' 
...
<th>${userContext.localeMap['@action']}</th>
		...
		<tbody>
			...
			<c:forEach var="item" items="${occupationTypeList}">
				<tr currentVersion='${item.version}' id="occupationType-${item.id}" ><td><a class="link-action-removed" href="./occupationTypeManager/view/${item.id}/"> ${item.id}</a></td>
<c:if test="${param.referName ne 'code'}">	<td contenteditable='true' class='edit-value'  propertyToChange='code' storedCellValue='${item.code}' prefix='${ownerBeanName}Manager/updateOccupationType/${result.id}/${item.id}/'>${item.code}</td>
	...
	<td class="select_candidate_td"
			data-candidate-method="./occupationTypeManager/requestCandidateCompany/${ownerBeanName}/${item.id}/"
			data-switch-method="./occupationTypeManager/transferToAnotherCompany/${item.id}/"
...
<c:if test="${param.referName ne 'description'}">	<td contenteditable='true' class='edit-value'  propertyToChange='description' storedCellValue='${item.description}' prefix='${ownerBeanName}Manager/updateOccupationType/${result.id}/${item.id}/'>${item.description}</td>
