...
<c:if test="${ empty levelTwoDepartmentList}" >
 ...
 <c:if test="${not empty levelTwoDepartmentList}" >
    
    
<%

 	SmartList list=(SmartList)request.getAttribute("levelTwoDepartmentList"); 
...
<c:set var="currentPageNumber" value="${page.pageNumber}"/>
	...
	<li class="page-item ${classType}">
		<a href='#${ownerBeanName}Manager/load${ownerClassName}/${result.id}/${levelTwoDepartmentListName};${levelTwoDepartmentListName}CurrentPage=${page.pageNumber};${levelTwoDepartmentListName}RowsPerPage=${rowsPerPage}/' 
...
<th>${userContext.localeMap['@action']}</th>
		...
		<tbody>
			...
			<c:forEach var="item" items="${levelTwoDepartmentList}">
				<tr currentVersion='${item.version}' id="levelTwoDepartment-${item.id}" ><td><a class="link-action-removed" href="./levelTwoDepartmentManager/view/${item.id}/"> ${item.id}</a></td>
<c:if test="${param.referName ne 'belongsTo'}">
	<td class="select_candidate_td"
			data-candidate-method="./levelTwoDepartmentManager/requestCandidateBelongsTo/${ownerBeanName}/${item.id}/"
			data-switch-method="./levelTwoDepartmentManager/transferToAnotherBelongsTo/${item.id}/"
			data-link-template="./levelOneDepartmentManager/view/${'$'}{ID}/">
		<span class="display_span">
			<c:if test="${not empty  item.belongsTo}">
			<a href='./levelOneDepartmentManager/view/${item.belongsTo.id}/'>${item.belongsTo.displayName}</a>
			</c:if>
...
<c:if test="${param.referName ne 'name'}">	<td contenteditable='true' class='edit-value'  propertyToChange='name' storedCellValue='${item.name}' prefix='${ownerBeanName}Manager/updateLevelTwoDepartment/${result.id}/${item.id}/'>${item.name}</td>
