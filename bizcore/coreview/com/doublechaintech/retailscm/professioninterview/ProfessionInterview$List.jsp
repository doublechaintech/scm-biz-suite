...
<c:if test="${ empty professionInterviewList}" >
 ...
 <c:if test="${not empty professionInterviewList}" >
    
    
<%

 	SmartList list=(SmartList)request.getAttribute("professionInterviewList"); 
...
<c:set var="currentPageNumber" value="${page.pageNumber}"/>
	...
	<li class="page-item ${classType}">
		<a href='#${ownerBeanName}Manager/load${ownerClassName}/${result.id}/${professionInterviewListName};${professionInterviewListName}CurrentPage=${page.pageNumber};${professionInterviewListName}RowsPerPage=${rowsPerPage}/' 
...
<th>${userContext.localeMap['@action']}</th>
		...
		<tbody>
			...
			<c:forEach var="item" items="${professionInterviewList}">
				<tr currentVersion='${item.version}' id="professionInterview-${item.id}" ><td><a class="link-action-removed" href="./professionInterviewManager/view/${item.id}/"> ${item.id}</a></td>
