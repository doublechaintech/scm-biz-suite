...
<c:if test="${ empty potentialCustomerContactPersonList}" >
 ...
 <c:if test="${not empty potentialCustomerContactPersonList}" >
    
    
<%

 	SmartList list=(SmartList)request.getAttribute("potentialCustomerContactPersonList"); 
...
<c:set var="currentPageNumber" value="${page.pageNumber}"/>
	...
	<li class="page-item ${classType}">
		<a href='#${ownerBeanName}Manager/load${ownerClassName}/${result.id}/${potentialCustomerContactPersonListName};${potentialCustomerContactPersonListName}CurrentPage=${page.pageNumber};${potentialCustomerContactPersonListName}RowsPerPage=${rowsPerPage}/' 
...
<th>${userContext.localeMap['@action']}</th>
		...
		<tbody>
			...
			<c:forEach var="item" items="${potentialCustomerContactPersonList}">
				<tr currentVersion='${item.version}' id="potentialCustomerContactPerson-${item.id}" ><td><a class="link-action-removed" href="./potentialCustomerContactPersonManager/view/${item.id}/"> ${item.id}</a></td>
<c:if test="${param.referName ne 'name'}">	<td contenteditable='true' class='edit-value'  propertyToChange='name' storedCellValue='${item.name}' prefix='${ownerBeanName}Manager/updatePotentialCustomerContactPerson/${result.id}/${item.id}/'>${item.name}</td>
	...
	<td class="select_candidate_td"
			data-candidate-method="./potentialCustomerContactPersonManager/requestCandidatePotentialCustomer/${ownerBeanName}/${item.id}/"
			data-switch-method="./potentialCustomerContactPersonManager/transferToAnotherPotentialCustomer/${item.id}/"
...
<c:if test="${param.referName ne 'description'}">	<td contenteditable='true' class='edit-value'  propertyToChange='description' storedCellValue='${item.description}' prefix='${ownerBeanName}Manager/updatePotentialCustomerContactPerson/${result.id}/${item.id}/'>${item.description}</td>
</c:if>
				<td>
