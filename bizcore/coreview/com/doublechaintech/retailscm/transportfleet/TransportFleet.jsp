<%@ page import='java.util.*,com.doublechaintech.retailscm.*,com.doublechaintech.retailscm.transportfleet.TransportFleet'%>
...
<c:set var="ownerBeanName" value="transportFleet" scope="request"/>
<c:set var="ownerClassName" value="TransportFleet" scope="request"/>
...
<div class="row">
	...
	<% TransportFleet result = (TransportFleet)request.getAttribute("result");  %>
			<li><a data-toggle="tab" href="#transportTruckList" class="disabled"> ${userContext.localeMap['transport_truck']}</a></li>
			<li><a data-toggle="tab" href="#truckDriverList" class="disabled"> ${userContext.localeMap['truck_driver']}</a></li>
...
<div class="row" desc="show parent objects">
	...
	<c:set var="transportFleet" value="${ result}" scope="request" />
<sky:include page="com/doublechaintech/retailscm/transportfleet/TransportFleet$Summary.jsp" />
...
<sky:include page="com/doublechaintech/retailscm/retailstorecountrycenter/RetailStoreCountryCenter$Info.jsp" />
		...
		<c:if test='${not empty userContext.accessTokens["transportTruckList"] or ignoreListAccessControl}'>
		<c:set var="transportTruckList" value="${result.transportTruckList}" scope="request"/>
		<c:set var="transportTruckListName" value="transportTruckList" scope="request"/>
		<div id="transportTruckList" class="tab-pane fade sublist" refer-name="owner">
			<sky:include page="com/doublechaintech/retailscm/transporttruck/TransportTruck$List.jsp"
					referName="owner"/>
		</div>
	</c:if>
	<c:if test='${not empty userContext.accessTokens["truckDriverList"] or ignoreListAccessControl}'>
		<c:set var="truckDriverList" value="${result.truckDriverList}" scope="request"/>
		<c:set var="truckDriverListName" value="truckDriverList" scope="request"/>
		<div id="truckDriverList" class="tab-pane fade sublist" refer-name="belongs_to">
			<sky:include page="com/doublechaintech/retailscm/truckdriver/TruckDriver$List.jsp"
					referName="belongsTo"/>
		</div>
	</c:if>
	<c:if test='${not empty userContext.accessTokens["transportTaskList"] or ignoreListAccessControl}'>
		...
		<div id="transportTaskList" class="tab-pane fade sublist" refer-name="belongs_to">
