...
<c:if test="${param.referName ne 'displayOrder'}">
	<th>${userContext.localeMap['page.display_order']}</th>
</c:if>
...
<c:if test="${param.referName ne 'displayOrder'}">	<td contenteditable='true' class='edit-value'  propertyToChange='displayOrder' storedCellValue='${item.displayOrder}' prefix='${ownerBeanName}Manager/updatePage/${result.id}/${item.id}/'>${item.displayOrder}</td>
