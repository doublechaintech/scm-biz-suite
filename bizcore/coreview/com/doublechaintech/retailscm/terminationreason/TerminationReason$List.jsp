...
<c:if test="${ empty terminationReasonList}" >
 ...
 <c:if test="${not empty terminationReasonList}" >
    
    
<%

 	SmartList list=(SmartList)request.getAttribute("terminationReasonList"); 
...
<c:set var="currentPageNumber" value="${page.pageNumber}"/>
	...
	<li class="page-item ${classType}">
		<a href='#${ownerBeanName}Manager/load${ownerClassName}/${result.id}/${terminationReasonListName};${terminationReasonListName}CurrentPage=${page.pageNumber};${terminationReasonListName}RowsPerPage=${rowsPerPage}/' 
...
<th>${userContext.localeMap['@action']}</th>
		...
		<tbody>
			...
			<c:forEach var="item" items="${terminationReasonList}">
				<tr currentVersion='${item.version}' id="terminationReason-${item.id}" ><td><a class="link-action-removed" href="./terminationReasonManager/view/${item.id}/"> ${item.id}</a></td>
<c:if test="${param.referName ne 'code'}">	<td contenteditable='true' class='edit-value'  propertyToChange='code' storedCellValue='${item.code}' prefix='${ownerBeanName}Manager/updateTerminationReason/${result.id}/${item.id}/'>${item.code}</td>
	...
	<td class="select_candidate_td"
			data-candidate-method="./terminationReasonManager/requestCandidateCompany/${ownerBeanName}/${item.id}/"
			data-switch-method="./terminationReasonManager/transferToAnotherCompany/${item.id}/"
...
<c:if test="${param.referName ne 'description'}">	<td contenteditable='true' class='edit-value'  propertyToChange='description' storedCellValue='${item.description}' prefix='${ownerBeanName}Manager/updateTerminationReason/${result.id}/${item.id}/'>${item.description}</td>
</c:if>
				<td>
