...
<c:if test="${ empty supplyOrderShipmentList}" >
 ...
 <c:if test="${not empty supplyOrderShipmentList}" >
    
    
<%

 	SmartList list=(SmartList)request.getAttribute("supplyOrderShipmentList"); 
...
<c:set var="currentPageNumber" value="${page.pageNumber}"/>
	...
	<li class="page-item ${classType}">
		<a href='#${ownerBeanName}Manager/load${ownerClassName}/${result.id}/${supplyOrderShipmentListName};${supplyOrderShipmentListName}CurrentPage=${page.pageNumber};${supplyOrderShipmentListName}RowsPerPage=${rowsPerPage}/' 
...
<th>${userContext.localeMap['@action']}</th>
		...
		<tbody>
			...
			<c:forEach var="item" items="${supplyOrderShipmentList}">
				<tr currentVersion='${item.version}' id="supplyOrderShipment-${item.id}" ><td><a class="link-action-removed" href="./supplyOrderShipmentManager/view/${item.id}/"> ${item.id}</a></td>
