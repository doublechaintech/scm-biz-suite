...
<c:if test="${ empty supplyOrderDeliveryList}" >
 ...
 <c:if test="${not empty supplyOrderDeliveryList}" >
    
    
<%

 	SmartList list=(SmartList)request.getAttribute("supplyOrderDeliveryList"); 
...
<c:set var="currentPageNumber" value="${page.pageNumber}"/>
	...
	<li class="page-item ${classType}">
		<a href='#${ownerBeanName}Manager/load${ownerClassName}/${result.id}/${supplyOrderDeliveryListName};${supplyOrderDeliveryListName}CurrentPage=${page.pageNumber};${supplyOrderDeliveryListName}RowsPerPage=${rowsPerPage}/' 
...
<th>${userContext.localeMap['@action']}</th>
		...
		<tbody>
			...
			<c:forEach var="item" items="${supplyOrderDeliveryList}">
				<tr currentVersion='${item.version}' id="supplyOrderDelivery-${item.id}" ><td><a class="link-action-removed" href="./supplyOrderDeliveryManager/view/${item.id}/"> ${item.id}</a></td>
