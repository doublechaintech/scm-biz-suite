...
<c:if test="${ empty accountingDocumentTypeList}" >
 ...
 <c:if test="${not empty accountingDocumentTypeList}" >
    
    
<%

 	SmartList list=(SmartList)request.getAttribute("accountingDocumentTypeList"); 
...
<c:set var="currentPageNumber" value="${page.pageNumber}"/>
	...
	<li class="page-item ${classType}">
		<a href='#${ownerBeanName}Manager/load${ownerClassName}/${result.id}/${accountingDocumentTypeListName};${accountingDocumentTypeListName}CurrentPage=${page.pageNumber};${accountingDocumentTypeListName}RowsPerPage=${rowsPerPage}/' 
...
<th>${userContext.localeMap['@action']}</th>
		...
		<tbody>
			...
			<c:forEach var="item" items="${accountingDocumentTypeList}">
				<tr currentVersion='${item.version}' id="accountingDocumentType-${item.id}" ><td><a class="link-action-removed" href="./accountingDocumentTypeManager/view/${item.id}/"> ${item.id}</a></td>
<c:if test="${param.referName ne 'name'}">	<td contenteditable='true' class='edit-value'  propertyToChange='name' storedCellValue='${item.name}' prefix='${ownerBeanName}Manager/updateAccountingDocumentType/${result.id}/${item.id}/'>${item.name}</td>
	...
	<td class="select_candidate_td"
			data-candidate-method="./accountingDocumentTypeManager/requestCandidateAccountingPeriod/${ownerBeanName}/${item.id}/"
			data-switch-method="./accountingDocumentTypeManager/transferToAnotherAccountingPeriod/${item.id}/"
			data-link-template="./accountSetManager/view/${'$'}{ID}/">
		<span class="display_span">
			<c:if test="${not empty  item.accountingPeriod}">
			<a href='./accountSetManager/view/${item.accountingPeriod.id}/'>${item.accountingPeriod.displayName}</a>
			</c:if>
