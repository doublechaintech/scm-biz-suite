<%@ page import='java.util.*,com.doublechaintech.retailscm.*,com.doublechaintech.retailscm.goods.Goods'%>
...
<c:set var="ownerBeanName" value="goods" scope="request"/>
<c:set var="ownerClassName" value="Goods" scope="request"/>
...
<div class="row">
	...
	<% Goods result = (Goods)request.getAttribute("result");  %>
			<li><a data-toggle="tab" href="#goodsMovementList" class="disabled"> ${userContext.localeMap['goods_movement']}</a></li>
...
<div class="row" desc="show parent objects">
	...
	<c:set var="goods" value="${ result}" scope="request" />
<sky:include page="com/doublechaintech/retailscm/goods/Goods$Summary.jsp" />
...
<sky:include page="com/doublechaintech/retailscm/retailstoreorder/RetailStoreOrder$Info.jsp" />
		...
		<c:if test='${not empty userContext.accessTokens["goodsMovementList"] or ignoreListAccessControl}'>
		<c:set var="goodsMovementList" value="${result.goodsMovementList}" scope="request"/>
		<c:set var="goodsMovementListName" value="goodsMovementList" scope="request"/>
		<div id="goodsMovementList" class="tab-pane fade sublist" refer-name="goods">
			<sky:include page="com/doublechaintech/retailscm/goodsmovement/GoodsMovement$List.jsp"
					referName="goods"/>
		</div>
	</c:if>
