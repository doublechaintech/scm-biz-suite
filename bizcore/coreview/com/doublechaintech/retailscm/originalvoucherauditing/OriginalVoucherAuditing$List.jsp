...
<c:if test="${ empty originalVoucherAuditingList}" >
 ...
 <c:if test="${not empty originalVoucherAuditingList}" >
    
    
<%

 	SmartList list=(SmartList)request.getAttribute("originalVoucherAuditingList"); 
...
<c:set var="currentPageNumber" value="${page.pageNumber}"/>
	...
	<li class="page-item ${classType}">
		<a href='#${ownerBeanName}Manager/load${ownerClassName}/${result.id}/${originalVoucherAuditingListName};${originalVoucherAuditingListName}CurrentPage=${page.pageNumber};${originalVoucherAuditingListName}RowsPerPage=${rowsPerPage}/' 
...
<th>${userContext.localeMap['@action']}</th>
		...
		<tbody>
			...
			<c:forEach var="item" items="${originalVoucherAuditingList}">
				<tr currentVersion='${item.version}' id="originalVoucherAuditing-${item.id}" ><td><a class="link-action-removed" href="./originalVoucherAuditingManager/view/${item.id}/"> ${item.id}</a></td>
