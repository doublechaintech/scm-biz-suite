...
<c:if test="${ empty salaryGradeList}" >
 ...
 <c:if test="${not empty salaryGradeList}" >
    
    
<%

 	SmartList list=(SmartList)request.getAttribute("salaryGradeList"); 
...
<c:set var="currentPageNumber" value="${page.pageNumber}"/>
	...
	<li class="page-item ${classType}">
		<a href='#${ownerBeanName}Manager/load${ownerClassName}/${result.id}/${salaryGradeListName};${salaryGradeListName}CurrentPage=${page.pageNumber};${salaryGradeListName}RowsPerPage=${rowsPerPage}/' 
...
<th>${userContext.localeMap['@action']}</th>
		...
		<tbody>
			...
			<c:forEach var="item" items="${salaryGradeList}">
				<tr currentVersion='${item.version}' id="salaryGrade-${item.id}" ><td><a class="link-action-removed" href="./salaryGradeManager/view/${item.id}/"> ${item.id}</a></td>
<c:if test="${param.referName ne 'code'}">	<td contenteditable='true' class='edit-value'  propertyToChange='code' storedCellValue='${item.code}' prefix='${ownerBeanName}Manager/updateSalaryGrade/${result.id}/${item.id}/'>${item.code}</td>
	...
	<td class="select_candidate_td"
			data-candidate-method="./salaryGradeManager/requestCandidateCompany/${ownerBeanName}/${item.id}/"
			data-switch-method="./salaryGradeManager/transferToAnotherCompany/${item.id}/"
...
<c:if test="${param.referName ne 'name'}">	<td contenteditable='true' class='edit-value'  propertyToChange='name' storedCellValue='${item.name}' prefix='${ownerBeanName}Manager/updateSalaryGrade/${result.id}/${item.id}/'>${item.name}</td>
