...
<c:if test="${ empty retailStoreMemberList}" >
 ...
 <c:if test="${not empty retailStoreMemberList}" >
    
    
<%

 	SmartList list=(SmartList)request.getAttribute("retailStoreMemberList"); 
...
<c:set var="currentPageNumber" value="${page.pageNumber}"/>
	...
	<li class="page-item ${classType}">
		<a href='#${ownerBeanName}Manager/load${ownerClassName}/${result.id}/${retailStoreMemberListName};${retailStoreMemberListName}CurrentPage=${page.pageNumber};${retailStoreMemberListName}RowsPerPage=${rowsPerPage}/' 
...
<th>${userContext.localeMap['@action']}</th>
		...
		<tbody>
			...
			<c:forEach var="item" items="${retailStoreMemberList}">
				<tr currentVersion='${item.version}' id="retailStoreMember-${item.id}" ><td><a class="link-action-removed" href="./retailStoreMemberManager/view/${item.id}/"> ${item.id}</a></td>
<c:if test="${param.referName ne 'name'}">	<td contenteditable='true' class='edit-value'  propertyToChange='name' storedCellValue='${item.name}' prefix='${ownerBeanName}Manager/updateRetailStoreMember/${result.id}/${item.id}/'>${item.name}</td>
	...
	<td class="select_candidate_td"
			data-candidate-method="./retailStoreMemberManager/requestCandidateOwner/${ownerBeanName}/${item.id}/"
			data-switch-method="./retailStoreMemberManager/transferToAnotherOwner/${item.id}/"
