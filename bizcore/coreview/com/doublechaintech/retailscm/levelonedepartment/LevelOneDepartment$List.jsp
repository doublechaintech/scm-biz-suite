...
<c:if test="${ empty levelOneDepartmentList}" >
 ...
 <c:if test="${not empty levelOneDepartmentList}" >
    
    
<%

 	SmartList list=(SmartList)request.getAttribute("levelOneDepartmentList"); 
...
<c:set var="currentPageNumber" value="${page.pageNumber}"/>
	...
	<li class="page-item ${classType}">
		<a href='#${ownerBeanName}Manager/load${ownerClassName}/${result.id}/${levelOneDepartmentListName};${levelOneDepartmentListName}CurrentPage=${page.pageNumber};${levelOneDepartmentListName}RowsPerPage=${rowsPerPage}/' 
...
<th>${userContext.localeMap['@action']}</th>
		...
		<tbody>
			...
			<c:forEach var="item" items="${levelOneDepartmentList}">
				<tr currentVersion='${item.version}' id="levelOneDepartment-${item.id}" ><td><a class="link-action-removed" href="./levelOneDepartmentManager/view/${item.id}/"> ${item.id}</a></td>
<c:if test="${param.referName ne 'belongsTo'}">
	<td class="select_candidate_td"
			data-candidate-method="./levelOneDepartmentManager/requestCandidateBelongsTo/${ownerBeanName}/${item.id}/"
			data-switch-method="./levelOneDepartmentManager/transferToAnotherBelongsTo/${item.id}/"
			data-link-template="./retailStoreCountryCenterManager/view/${'$'}{ID}/">
		<span class="display_span">
			<c:if test="${not empty  item.belongsTo}">
			<a href='./retailStoreCountryCenterManager/view/${item.belongsTo.id}/'>${item.belongsTo.displayName}</a>
			</c:if>
