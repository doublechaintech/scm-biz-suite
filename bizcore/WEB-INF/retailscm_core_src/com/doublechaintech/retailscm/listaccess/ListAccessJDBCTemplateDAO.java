  ...
  */
  ...
  public ListAccess disconnectFromAll(String listAccessId, int version) throws Exception {
    ...
    ListAccess listAccess =
        loadInternalListAccess(ListAccessTable.withId(listAccessId), emptyOptions());
  ...
  protected ListAccess extractListAccess(AccessKey accessKey, Map<String, Object> loadOptions)
      throws Exception {
    try {
  ...
  protected ListAccess loadInternalListAccess(AccessKey accessKey, Map<String, Object> loadOptions)
      throws Exception {
  ...
  protected ListAccess extractApp(ListAccess listAccess, Map<String, Object> options)
      throws Exception {
  ...
  public SmartList<ListAccess> findListAccessByApp(String userAppId, Map<String, Object> options) {
    ...
    SmartList<ListAccess> resultList =
        queryWith(ListAccessTable.COLUMN_APP, userAppId, options, getListAccessMapper());
    // analyzeListAccessByApp(resultList, userAppId, options);
    return resultList;
  }

  public SmartList<ListAccess> findListAccessByApp(
      String userAppId, int start, int count, Map<String, Object> options) {

    SmartList<ListAccess> resultList =
        queryWithRange(
            ListAccessTable.COLUMN_APP, userAppId, options, getListAccessMapper(), start, count);
    // analyzeListAccessByApp(resultList, userAppId, options);
    return resultList;
  }

  public void analyzeListAccessByApp(
      SmartList<ListAccess> resultList, String userAppId, Map<String, Object> options) {
    if (resultList == null) {
  ...
  public SmartList<ListAccess> saveListAccessList(
    ...
    batchListAccessRemove((List<ListAccess>) lists[REMOVE_LIST_INDEX]);
    batchListAccessRecover((List<ListAccess>) lists[RECOVER_LIST_INDEX]);
    ...
    for (ListAccess listAccess : listAccessList) {
      ...
      if (listAccess.isToRecover() || listAccess.isToRemove()) {
        listAccess.setVersion(-listAccess.getVersion());
      }
    }
  ...
  public SmartList<ListAccess> removeListAccessList(
      SmartList<ListAccess> listAccessList, Map<String, Object> options) {
  ...
  protected List<Object[]> prepareListAccessBatchRecoverArgs(List<ListAccess> listAccessList) {

    List<Object[]> parametersList = new ArrayList<Object[]>();
    for (ListAccess listAccess : listAccessList) {
      if (!listAccess.isToRecover()) {
        continue;
      }
      Object[] parameters = prepareRecoverParameters(listAccess);
      parametersList.add(parameters);
    }
    return parametersList;
  }

  protected List<Object[]> prepareListAccessBatchRemoveArgs(List<ListAccess> listAccessList) {

    List<Object[]> parametersList = new ArrayList<Object[]>();
    for (ListAccess listAccess : listAccessList) {
      if (!listAccess.isToRemove()) {
        continue;
      }
      Object[] parameters = prepareListAccessRemoveParameters(listAccess);
      parametersList.add(parameters);
    }
    return parametersList;
  }
  ...
  protected void batchListAccessRemove(List<ListAccess> listAccessList) {
    String SQL = getRemoveSQL();
    List<Object[]> args = prepareListAccessBatchRemoveArgs(listAccessList);
    int affectedNumbers[] = batchRemove(SQL, args);
    checkBatchReturn(affectedNumbers);
  }

  protected void batchListAccessRecover(List<ListAccess> listAccessList) {
    String SQL = getRecoverSQL();
    List<Object[]> args = prepareListAccessBatchRecoverArgs(listAccessList);
  ...
  protected Object[] splitListAccessList(List<ListAccess> listAccessList) {
    ...
    List<ListAccess> listAccessRemoveList = new ArrayList<ListAccess>();
    List<ListAccess> listAccessRecoverList = new ArrayList<ListAccess>();

    for (ListAccess listAccess : listAccessList) {
      if (listAccess.isToRemove()) {
        listAccessRemoveList.add(listAccess);
        continue;
      }
      if (listAccess.isToRecover()) {
        listAccessRecoverList.add(listAccess);
        continue;
      }
    ...
    return new Object[] {
      listAccessCreateList, listAccessUpdateList, listAccessRemoveList, listAccessRecoverList
    };
  }
  ...
  protected Object[] getSaveListAccessParameters(ListAccess listAccess) {
    if (listAccess.isToRemove()) {
      return prepareListAccessRemoveParameters(listAccess);
    }
    if (listAccess.isToRecover()) {
      return prepareRecoverParameters(listAccess);
    }
  ...
  protected Object[] prepareListAccessRemoveParameters(ListAccess listAccess) {
    return super.prepareRemoveParameters(listAccess);
  }
  ...
  @Override
  public SmartList<ListAccess> findListAccessWithKey(
  ...
  public Map<String, Integer> countListAccessWithGroupKey(
  ...
  @Override
  public CandidateListAccess executeCandidatesQuery(
  ...
  @Override
  protected ListAccessMapper mapperForClazz(Class<?> clazz) {
    return ListAccessMapper.mapperForClass(clazz);
  }
}
