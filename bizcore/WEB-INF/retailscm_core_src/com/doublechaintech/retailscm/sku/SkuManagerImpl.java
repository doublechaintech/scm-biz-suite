...
public class SkuManagerImpl extends CustomRetailscmCheckerManager
    implements SkuManager, BusinessHandler {
  ...
  public BlobObject exportExcelFromList(
    ...
    List<BaseEntity> entityListToNaming = sku.collectReferencesFromLists();
  ...
  protected Sku saveSku(RetailscmUserContext userContext, Sku sku, String[] tokensExpr)
      throws Exception {
  ...
  public Sku loadSku(RetailscmUserContext userContext, String skuId, String[] tokensExpr)
      throws Exception {
  ...
  public Sku searchSku(
      RetailscmUserContext userContext, String skuId, String textToSearch, String[] tokensExpr)
      throws Exception {
  ...
  protected Sku present(RetailscmUserContext userContext, Sku sku, Map<String, Object> tokens)
      throws Exception {
    ...
    List<BaseEntity> entityListToNaming = skuToPresent.collectReferencesFromLists();
    skuDaoOf(userContext).alias(entityListToNaming);
  ...
  protected Sku saveSku(RetailscmUserContext userContext, Sku sku, Map<String, Object> tokens)
      throws Exception {

    return skuDaoOf(userContext).save(sku, tokens);
  }

  protected Sku loadSku(RetailscmUserContext userContext, String skuId, Map<String, Object> tokens)
      throws Exception {
    checkerOf(userContext).checkIdOfSku(skuId);
  ...
  public Sku internalSaveSku(RetailscmUserContext userContext, Sku sku, Map<String, Object> options)
      throws Exception {
    // checkParamsForUpdatingSku(userContext, skuId, skuVersion, property, newValueExpr,
    // tokensExpr);
  ...
  protected void checkParamsForTransferingAnotherProduct(
      RetailscmUserContext userContext, String skuId, String anotherProductId) throws Exception {
  ...
  public Sku transferToAnotherProduct(
      RetailscmUserContext userContext, String skuId, String anotherProductId) throws Exception {
  ...
  public CandidateProduct requestCandidateProduct(
    ...
    SmartList<Product> candidateList =
        productDaoOf(userContext)
            .requestCandidateProductForSku(
  ...
  public Sku forgetByAll(RetailscmUserContext userContext, String skuId, int skuVersion)
      throws Exception {
    return forgetByAllInternal(userContext, skuId, skuVersion);
  }

  protected Sku forgetByAllInternal(RetailscmUserContext userContext, String skuId, int skuVersion)
      throws Exception {
  ...
  public Sku updateGoodsProperties(
      RetailscmUserContext userContext,
      String skuId,
      String id,
      String name,
    ...
    checkParamsForUpdatingGoodsProperties(
        userContext, skuId, id, name, rfid, uom, maxPackage, expireTime, tokensExpr);
    ...
    beforeUpdateGoodsProperties(
        userContext, item, skuId, id, name, rfid, uom, maxPackage, expireTime, tokensExpr);
  ...
  protected void checkParamsForRemovingGoodsList(
      RetailscmUserContext userContext, String skuId, String goodsIds[], String[] tokensExpr)
      throws Exception {
    ...
    synchronized (sku) {
      ...
      goodsManagerOf(userContext)
          .onNewInstanceCreated(
              userContext, (Goods) sku.getFlexiableObjects().get(BaseEntity.COPIED_CHILD));
      return present(userContext, sku, mergedAllTokens(tokensExpr));
    }
  }
  ...
  public Sku updateGoods(
    ...
    checkParamsForUpdatingGoods(
        userContext, skuId, goodsId, goodsVersion, property, newValueExpr, tokensExpr);
    ...
    synchronized (sku) {
      ...
      beforeUpdateGoods(
          userContext, goods, skuId, goodsId, goodsVersion, property, newValueExpr, tokensExpr);
  ...
  */
  ...
  public void onNewInstanceCreated(RetailscmUserContext userContext, Sku newCreated)
      throws Exception {
  ...
  public void sendAllItems(RetailscmUserContext ctx) throws Exception {
    skuDaoOf(ctx).loadAllAsStream().forEach(event -> sendInitEvent(ctx, event));
  }
  ...
  // -----------------------------------// list-of-view 处理 \\-----------------------------------
  protected void enhanceForListOfView(RetailscmUserContext userContext, SmartList<Sku> list)
      throws Exception {
    if (list == null || list.isEmpty()) {
      return;
    }
    List<Product> productList =
        RetailscmBaseUtils.collectReferencedObjectWithType(userContext, list, Product.class);
  ...
  public Object listPageByProduct(
    ...
    SmartList<Sku> list =
        skuDaoOf(userContext).findSkuByProduct(productId, start, count, new HashMap<>());
  ...
  public Object wxappview(RetailscmUserContext userContext, String skuId) throws Exception {
    ...
    propList.add(
        MapUtil.put("id", "4-product")
            ...
            .put(
                "displayMode",
                "{\"brief\":\"origin\",\"imageUrl\":\"picture\",\"name\":\"auto\",\"title\":\"name\",\"imageList\":\"\"}")
            .into_map());
