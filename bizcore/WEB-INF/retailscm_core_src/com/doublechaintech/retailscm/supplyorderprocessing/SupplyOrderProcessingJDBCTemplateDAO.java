	...
	*/
	...
	protected boolean isExtractConsumerOrderListEnabled(Map<String,Object> options){		
 	...
 	protected boolean isAnalyzeConsumerOrderListEnabled(Map<String,Object> options){		 		
 		return SupplyOrderProcessingTokens.of(options).analyzeConsumerOrderListEnabled();
 	}
	...
	protected boolean isExtractSupplyOrderListEnabled(Map<String,Object> options){		
 	...
 	protected boolean isAnalyzeSupplyOrderListEnabled(Map<String,Object> options){		 		
 		return SupplyOrderProcessingTokens.of(options).analyzeSupplyOrderListEnabled();
 	}
	...
	public SmartList<ConsumerOrder> loadOurConsumerOrderList(RetailscmUserContext userContext, List<SupplyOrderProcessing> us, Map<String,Object> options) throws Exception{
		...
		MultipleAccessKey key = new MultipleAccessKey();
		key.put(ConsumerOrder.PROCESSING_PROPERTY, ids.toArray(new String[ids.size()]));
		SmartList<ConsumerOrder> loadedObjs = userContext.getDAOGroup().getConsumerOrderDAO().findConsumerOrderWithKey(key, options);
		Map<String, List<ConsumerOrder>> loadedMap = loadedObjs.stream().collect(Collectors.groupingBy(it->it.getProcessing().getId()));
		us.forEach(it->{
			String id = it.getId();
	...
	public SmartList<SupplyOrder> loadOurSupplyOrderList(RetailscmUserContext userContext, List<SupplyOrderProcessing> us, Map<String,Object> options) throws Exception{
		...
		Set<String> ids = us.stream().map(it->it.getId()).collect(Collectors.toSet());
		MultipleAccessKey key = new MultipleAccessKey();
		key.put(SupplyOrder.PROCESSING_PROPERTY, ids.toArray(new String[ids.size()]));
		SmartList<SupplyOrder> loadedObjs = userContext.getDAOGroup().getSupplyOrderDAO().findSupplyOrderWithKey(key, options);
		Map<String, List<SupplyOrder>> loadedMap = loadedObjs.stream().collect(Collectors.groupingBy(it->it.getProcessing().getId()));
		us.forEach(it->{
			String id = it.getId();
