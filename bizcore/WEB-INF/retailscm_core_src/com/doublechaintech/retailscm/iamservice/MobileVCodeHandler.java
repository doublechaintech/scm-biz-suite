...
public class MobileVCodeHandler extends BaseIdentificationHandler {
	...
	public LoginResult authenticate(RetailscmUserContext userContext, LoginContext loginContext) {
		...
		if (cleanMobile == null) {
			return result.withError("您输入的手机号"+(mobile==null?"(空)":(mobile+"(" + mobile.length()+"位)")+"不是有效的中国大陆手机号码"));
		}
		...
		SmartList<SecUser> sUsers = userContext.getDAOGroup().getSecUserDAO().findSecUserWithKey(key, EO);
		if (sUsers == null) {
	...
	public void bindWithSecUser(RetailscmUserContext userContext, LoginContext loginContext) throws Exception {
		...
		userContext.getManagerGroup().getSecUserManager().internalSaveSecUser(userContext, secUserInDB, EO);
	}
}
