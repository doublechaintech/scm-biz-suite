...
public class UserAllowListJDBCTemplateDAO extends RetailscmBaseDAOImpl implements UserAllowListDAO{
  ...
  public Stream<UserAllowList> loadAllAsStream() {
	...
	protected void throwIfHasException(String userAllowListId,int version,int count) throws Exception{
		if (count == 1) {
	...
	public void delete(String userAllowListId, int version) throws Exception{
		...
		String methodName="delete(String userAllowListId, int version)";
		assertMethodArgumentNotNull(userAllowListId, methodName, "userAllowListId");
		assertMethodIntArgumentGreaterThan(version,0, methodName, "options");


		String SQL=this.getDeleteSQL();
		Object [] parameters=new Object[]{userAllowListId,version};
		int affectedNumber = singleUpdate(SQL,parameters);
		if(affectedNumber == 1){
			return ; //Delete successfully
		}
		if(affectedNumber == 0){
			handleDeleteOneError(userAllowListId,version);
		}
	...
	public UserAllowList disconnectFromAll(String userAllowListId, int version) throws Exception{
		...
		UserAllowList userAllowList = loadInternalUserAllowList(UserAllowListTable.withId(userAllowListId), emptyOptions());
	...
	protected List<Object[]> prepareUserAllowListBatchCreateArgs(List<UserAllowList> userAllowListList){
	...
	protected List<Object[]> prepareUserAllowListBatchUpdateArgs(List<UserAllowList> userAllowListList){
	...
	protected Object[] splitUserAllowListList(List<UserAllowList> userAllowListList){
		...
		return new Object[]{userAllowListCreateList,userAllowListUpdateList};
	}
	...
	public void collectAndEnhance(BaseEntity ownerEntity) {
		List<UserAllowList> userAllowListList = ownerEntity.collectRefsWithType(UserAllowList.INTERNAL_TYPE);
		this.enhanceList(userAllowListList);

	}

	@Override
	public SmartList<UserAllowList> findUserAllowListWithKey(MultipleAccessKey key,
	...
	@Override
	public int countUserAllowListWithKey(MultipleAccessKey key,
			Map<String, Object> options) {
