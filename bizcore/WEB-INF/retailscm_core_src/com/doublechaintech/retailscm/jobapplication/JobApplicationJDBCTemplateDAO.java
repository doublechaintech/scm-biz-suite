	...
	*/
	...
	protected boolean isExtractEmployeeListEnabled(Map<String,Object> options){		
 	...
 	protected boolean isAnalyzeEmployeeListEnabled(Map<String,Object> options){		 		
 		return JobApplicationTokens.of(options).analyzeEmployeeListEnabled();
 	}
	...
	public SmartList<Employee> loadOurEmployeeList(RetailscmUserContext userContext, List<JobApplication> us, Map<String,Object> options) throws Exception{
		...
		MultipleAccessKey key = new MultipleAccessKey();
		key.put(Employee.JOB_APPLICATION_PROPERTY, ids.toArray(new String[ids.size()]));
		SmartList<Employee> loadedObjs = userContext.getDAOGroup().getEmployeeDAO().findEmployeeWithKey(key, options);
		Map<String, List<Employee>> loadedMap = loadedObjs.stream().collect(Collectors.groupingBy(it->it.getJobApplication().getId()));
		us.forEach(it->{
			String id = it.getId();
