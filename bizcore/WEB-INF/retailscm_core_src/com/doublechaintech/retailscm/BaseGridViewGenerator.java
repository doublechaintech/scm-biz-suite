...
public abstract class BaseGridViewGenerator extends BaseEntity{
	...
	public BlobObject exportExcel(BaseEntity targetObjectToExport, String listName) throws IOException{
	...
	protected List<BaseEntity> getListToExport(BaseEntity targetObjectToExport, String listName) {
		...
		if(!(listObject instanceof List)) {
			String message = String.format("The property name '%s' provided for '%s' should be a list",
	...
	private  List<Block> genBlocksForSimpleObject(String pageName, BaseEntity ele) {
		...
		return Arrays.stream(ele.toFlatArray()).map(value
				->wrapValueInCellBlock(pageName,value,1,2,3,4,1)).collect(Collectors.toList());
	...
	protected Block createBlock(String pageName, Object value,int left, int right, int top, int bottom, int color) {
	...
	protected  Block wrapValueInCellBlock(String pageName, Object value,int left, int right, int top, int bottom, int color )
	{
	...
	protected  Block wrapValueInSingleCellBlock(String pageName, Object value,int left,  int top, int color )
	{
