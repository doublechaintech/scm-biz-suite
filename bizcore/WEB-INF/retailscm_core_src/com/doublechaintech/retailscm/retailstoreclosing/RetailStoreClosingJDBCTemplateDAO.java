...
public class RetailStoreClosingJDBCTemplateDAO extends RetailscmBaseDAOImpl implements RetailStoreClosingDAO{
	...
	*/
    ...
    public SmartList<RetailStoreClosing> requestCandidateRetailStoreClosingForRetailStore(RetailscmUserContext userContext, String ownerClass, String id, String filterKey, int pageNo, int pageSize) throws Exception {
		...
		return findAllCandidateByFilter(RetailStoreClosingTable.COLUMN_COMMENT, null, filterKey, pageNo, pageSize, getRetailStoreClosingMapper());
    }
	...
	public SmartList<RetailStore> loadOurRetailStoreList(RetailscmUserContext userContext, List<RetailStoreClosing> us, Map<String,Object> options) throws Exception{
		...
		MultipleAccessKey key = new MultipleAccessKey();
		key.put(RetailStore.CLOSING_PROPERTY, ids.toArray(new String[ids.size()]));
		SmartList<RetailStore> loadedObjs = userContext.getDAOGroup().getRetailStoreDAO().findRetailStoreWithKey(key, options);
		Map<String, List<RetailStore>> loadedMap = loadedObjs.stream().collect(Collectors.groupingBy(it->it.getClosing().getId()));
		us.forEach(it->{
			String id = it.getId();
