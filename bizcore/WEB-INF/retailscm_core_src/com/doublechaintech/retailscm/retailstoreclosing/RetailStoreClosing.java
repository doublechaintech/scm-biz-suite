...
public class RetailStoreClosing extends BaseEntity implements  java.io.Serializable{
  ...
  memberMetaInfoList.add(MemberMetaInfo.referBy(RETAIL_STORE_LIST, "closing", "零售门店列表")
...
return getId();
	...
	public RetailStoreClosing updateId(String id){String oldId = this.id;String newId = trimString(id);if(!shouldReplaceBy(newId, oldId)){return this;}this.id = newId;addPropertyChange(ID_PROPERTY, oldId, newId);this.changed = true;setChecked(false);return this;}
...
return createCriteria(ID_PROPERTY, operator, parameters);
	...
	public RetailStoreClosing ignoreIdCriteria(){super.ignoreSearchProperty(ID_PROPERTY);
return this;
}
...
return getComment();
	...
	public RetailStoreClosing updateComment(String comment){String oldComment = this.comment;String newComment = trimString(comment);if(!shouldReplaceBy(newComment, oldComment)){return this;}this.comment = newComment;addPropertyChange(COMMENT_PROPERTY, oldComment, newComment);this.changed = true;setChecked(false);return this;}
...
return createCriteria(COMMENT_PROPERTY, operator, parameters);
	...
	public RetailStoreClosing ignoreCommentCriteria(){super.ignoreSearchProperty(COMMENT_PROPERTY);
return this;
}
...
return getVersion();
	...
	public RetailStoreClosing updateVersion(int version){int oldVersion = this.version;int newVersion = version;if(!shouldReplaceBy(newVersion, oldVersion)){return this;}this.version = newVersion;addPropertyChange(VERSION_PROPERTY, oldVersion, newVersion);this.changed = true;setChecked(false);return this;}
...
return createCriteria(VERSION_PROPERTY, operator, parameters);
	...
	public RetailStoreClosing ignoreVersionCriteria(){super.ignoreSearchProperty(VERSION_PROPERTY);
return this;
}
...
return this;
  ...
  public  SmartList<RetailStore> retailStoreList(){
	...
	public static RetailStoreClosing createWith(RetailscmUserContext userContext, ThrowingFunction<RetailStoreClosing,RetailStoreClosing,Exception> postHandler, Object ... inputs) throws Exception {

    List<Object> params = inputs == null ? new ArrayList<>() : Arrays.asList(inputs);
    CustomRetailscmPropertyMapper mapper = CustomRetailscmPropertyMapper.of(userContext);
    CreationScene scene = mapper.findParamByClass(params, CreationScene.class);
    RetailscmBeanCreator<RetailStoreClosing> customCreator = mapper.findCustomCreator(RetailStoreClosing.class, scene);
    if (customCreator != null){
      return customCreator.create(userContext, scene, postHandler, params);
    }

    RetailStoreClosing result = new RetailStoreClosing();
    result.setComment(mapper.tryToGet(RetailStoreClosing.class, COMMENT_PROPERTY, String.class,
    ...
    if (result != null){
      userContext.getChecker().checkAndFixRetailStoreClosing(result);
      userContext.getChecker().throwExceptionIfHasErrors(IllegalArgumentException.class);

      
      RetailStoreClosingTokens tokens = mapper.findParamByClass(params, RetailStoreClosingTokens.class);
      if (tokens == null) {
        tokens = RetailStoreClosingTokens.start();
      }
      result = userContext.getManagerGroup().getRetailStoreClosingManager().internalSaveRetailStoreClosing(userContext, result, tokens.done());
