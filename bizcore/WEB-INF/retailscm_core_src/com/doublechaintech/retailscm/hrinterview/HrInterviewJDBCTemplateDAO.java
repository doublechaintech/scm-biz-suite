...
public class HrInterviewJDBCTemplateDAO extends RetailscmNamingServiceDAO implements HrInterviewDAO{
	...
	*/
	...
	protected HrInterview analyzeEmployeeList(HrInterview hrInterview, Map<String,Object> options){
		...
		if(employeeList != null){
			getEmployeeDAO().analyzeEmployeeByHrInterview(employeeList, hrInterview.getId(), options);
	...
	public SmartList<Employee> loadOurEmployeeList(RetailscmUserContext userContext, List<HrInterview> us, Map<String,Object> options) throws Exception{
		...
		MultipleAccessKey key = new MultipleAccessKey();
		key.put(Employee.HR_INTERVIEW_PROPERTY, ids.toArray(new String[ids.size()]));
		SmartList<Employee> loadedObjs = userContext.getDAOGroup().getEmployeeDAO().findEmployeeWithKey(key, options);
		Map<String, List<Employee>> loadedMap = loadedObjs.stream().collect(Collectors.groupingBy(it->it.getHrInterview().getId()));
		us.forEach(it->{
			String id = it.getId();
