...
public class AccountingDocumentCreationJDBCTemplateDAO extends RetailscmNamingServiceDAO implements AccountingDocumentCreationDAO{
	...
	*/
	...
	public SmartList<AccountingDocument> loadOurAccountingDocumentList(RetailscmUserContext userContext, List<AccountingDocumentCreation> us, Map<String,Object> options) throws Exception{
		...
		MultipleAccessKey key = new MultipleAccessKey();
		key.put(AccountingDocument.CREATION_PROPERTY, ids.toArray(new String[ids.size()]));
		...
		Map<String, List<AccountingDocument>> loadedMap = loadedObjs.stream().collect(Collectors.groupingBy(it->it.getCreation().getId()));
		us.forEach(it->{
			String id = it.getId();
