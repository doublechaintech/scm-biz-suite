...
public class OriginalVoucherConfirmationJDBCTemplateDAO extends RetailscmNamingServiceDAO implements OriginalVoucherConfirmationDAO{
	...
	*/
	...
	public SmartList<OriginalVoucher> loadOurOriginalVoucherList(RetailscmUserContext userContext, List<OriginalVoucherConfirmation> us, Map<String,Object> options) throws Exception{
		...
		MultipleAccessKey key = new MultipleAccessKey();
		key.put(OriginalVoucher.CONFIRMATION_PROPERTY, ids.toArray(new String[ids.size()]));
		...
		Map<String, List<OriginalVoucher>> loadedMap = loadedObjs.stream().collect(Collectors.groupingBy(it->it.getConfirmation().getId()));
		us.forEach(it->{
			String id = it.getId();
