...
public class GoodsAllocation extends BaseEntity implements  java.io.Serializable{
  ...
  static{
    ...
    memberMetaInfoList.add(MemberMetaInfo.defineBy(LONGITUDE_PROPERTY, "longitude", "经度")
        .withType("double", "BigDecimal"));
    memberMetaInfoList.add(MemberMetaInfo.defineBy(GOODS_SHELF_PROPERTY, "goods_shelf", "货架")
        .withType("goods_shelf", GoodsShelf.class));
    memberMetaInfoList.add(MemberMetaInfo.defineBy(VERSION_PROPERTY, "version", "版本")
        .withType("version", "int"));

  memberMetaInfoList.add(MemberMetaInfo.referBy(GOODS_LIST, "goodsAllocation", "商品列表")
  ...
  public String[] getPropertyNames(){
    return new String[]{ID_PROPERTY ,LOCATION_PROPERTY ,LATITUDE_PROPERTY ,LONGITUDE_PROPERTY ,GOODS_SHELF_PROPERTY ,VERSION_PROPERTY};
  }
...
return getId();
	...
	public GoodsAllocation updateId(String id){String oldId = this.id;String newId = trimString(id);if(!shouldReplaceBy(newId, oldId)){return this;}this.id = newId;addPropertyChange(ID_PROPERTY, oldId, newId);this.changed = true;setChecked(false);return this;}
...
return createCriteria(ID_PROPERTY, operator, parameters);
	...
	public GoodsAllocation ignoreIdCriteria(){super.ignoreSearchProperty(ID_PROPERTY);
return this;
}
...
return getLocation();
	...
	public GoodsAllocation updateLocation(String location){String oldLocation = this.location;String newLocation = trimString(location);if(!shouldReplaceBy(newLocation, oldLocation)){return this;}this.location = newLocation;addPropertyChange(LOCATION_PROPERTY, oldLocation, newLocation);this.changed = true;setChecked(false);return this;}
...
return createCriteria(LOCATION_PROPERTY, operator, parameters);
	...
	public GoodsAllocation ignoreLocationCriteria(){super.ignoreSearchProperty(LOCATION_PROPERTY);
return this;
}
...
return getLatitude();
	...
	public GoodsAllocation updateLatitude(BigDecimal latitude){BigDecimal oldLatitude = this.latitude;BigDecimal newLatitude = latitude;if(!shouldReplaceBy(newLatitude, oldLatitude)){return this;}this.latitude = newLatitude;addPropertyChange(LATITUDE_PROPERTY, oldLatitude, newLatitude);this.changed = true;setChecked(false);return this;}
...
return createCriteria(LATITUDE_PROPERTY, operator, parameters);
	...
	public GoodsAllocation ignoreLatitudeCriteria(){super.ignoreSearchProperty(LATITUDE_PROPERTY);
return this;
}
...
return getLongitude();
	...
	public GoodsAllocation updateLongitude(BigDecimal longitude){BigDecimal oldLongitude = this.longitude;BigDecimal newLongitude = longitude;if(!shouldReplaceBy(newLongitude, oldLongitude)){return this;}this.longitude = newLongitude;addPropertyChange(LONGITUDE_PROPERTY, oldLongitude, newLongitude);this.changed = true;setChecked(false);return this;}
...
return createCriteria(LONGITUDE_PROPERTY, operator, parameters);
	...
	public GoodsAllocation ignoreLongitudeCriteria(){super.ignoreSearchProperty(LONGITUDE_PROPERTY);
return this;
}
...
return this;
	...
	public void setGoodsShelf(GoodsShelf goodsShelf){GoodsShelf oldGoodsShelf = this.goodsShelf;GoodsShelf newGoodsShelf = goodsShelf;this.goodsShelf = newGoodsShelf;}
...
return getGoodsShelf();
	...
	public GoodsAllocation updateGoodsShelf(GoodsShelf goodsShelf){GoodsShelf oldGoodsShelf = this.goodsShelf;GoodsShelf newGoodsShelf = goodsShelf;if(!shouldReplaceBy(newGoodsShelf, oldGoodsShelf)){return this;}this.goodsShelf = newGoodsShelf;addPropertyChange(GOODS_SHELF_PROPERTY, oldGoodsShelf, newGoodsShelf);this.changed = true;setChecked(false);return this;}
...
return createCriteria(GOODS_SHELF_PROPERTY, operator, parameters);
	...
	public GoodsAllocation ignoreGoodsShelfCriteria(){super.ignoreSearchProperty(GOODS_SHELF_PROPERTY);
return this;
}
...
return getVersion();
	...
	public GoodsAllocation updateVersion(int version){int oldVersion = this.version;int newVersion = version;if(!shouldReplaceBy(newVersion, oldVersion)){return this;}this.version = newVersion;addPropertyChange(VERSION_PROPERTY, oldVersion, newVersion);this.changed = true;setChecked(false);return this;}
...
return createCriteria(VERSION_PROPERTY, operator, parameters);
	...
	public GoodsAllocation ignoreVersionCriteria(){super.ignoreSearchProperty(VERSION_PROPERTY);
return this;
}
...
return this;
  ...
  public  SmartList<Goods> goodsList(){
	...
	public static GoodsAllocation createWith(RetailscmUserContext userContext, ThrowingFunction<GoodsAllocation,GoodsAllocation,Exception> postHandler, Object ... inputs) throws Exception {

    List<Object> params = inputs == null ? new ArrayList<>() : Arrays.asList(inputs);
    CustomRetailscmPropertyMapper mapper = CustomRetailscmPropertyMapper.of(userContext);
    CreationScene scene = mapper.findParamByClass(params, CreationScene.class);
    RetailscmBeanCreator<GoodsAllocation> customCreator = mapper.findCustomCreator(GoodsAllocation.class, scene);
    if (customCreator != null){
      return customCreator.create(userContext, scene, postHandler, params);
    }

    GoodsAllocation result = new GoodsAllocation();
    result.setLocation(mapper.tryToGet(GoodsAllocation.class, LOCATION_PROPERTY, String.class,
        0, true, result.getLocation(), params));
    result.setLatitude(mapper.tryToGet(GoodsAllocation.class, LATITUDE_PROPERTY, BigDecimal.class,
        0, false, result.getLatitude(), params));
    result.setLongitude(mapper.tryToGet(GoodsAllocation.class, LONGITUDE_PROPERTY, BigDecimal.class,
        1, false, result.getLongitude(), params));
    result.setGoodsShelf(mapper.tryToGet(GoodsAllocation.class, GOODS_SHELF_PROPERTY, GoodsShelf.class,
        0, true, result.getGoodsShelf(), params));

    if (postHandler != null) {
      result = postHandler.apply(result);
    }
    if (result != null){
      userContext.getChecker().checkAndFixGoodsAllocation(result);
      userContext.getChecker().throwExceptionIfHasErrors(IllegalArgumentException.class);

      
      GoodsAllocationTokens tokens = mapper.findParamByClass(params, GoodsAllocationTokens.class);
      if (tokens == null) {
        tokens = GoodsAllocationTokens.start();
      }
      result = userContext.getManagerGroup().getGoodsAllocationManager().internalSaveGoodsAllocation(userContext, result, tokens.done());
