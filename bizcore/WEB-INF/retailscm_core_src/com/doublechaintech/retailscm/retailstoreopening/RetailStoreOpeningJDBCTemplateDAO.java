...
public class RetailStoreOpeningJDBCTemplateDAO extends RetailscmNamingServiceDAO implements RetailStoreOpeningDAO{
	...
	*/
	...
	protected void throwIfHasException(String retailStoreOpeningId,int version,int count) throws Exception{
		if (count == 1) {
	...
	public RetailStoreOpening disconnectFromAll(String retailStoreOpeningId, int version) throws Exception{
		...
		RetailStoreOpening retailStoreOpening = loadInternalRetailStoreOpening(RetailStoreOpeningTable.withId(retailStoreOpeningId), emptyOptions());
	...
	protected RetailStoreOpening extractRetailStoreOpening(AccessKey accessKey, Map<String,Object> loadOptions) throws Exception{
		try{
			RetailStoreOpening retailStoreOpening = loadSingleObject(accessKey, getRetailStoreOpeningMapper());
			return retailStoreOpening;
	...
	protected RetailStoreOpening analyzeRetailStoreList(RetailStoreOpening retailStoreOpening, Map<String,Object> options){
		...
		if(retailStoreList != null){
			getRetailStoreDAO().analyzeRetailStoreByOpening(retailStoreList, retailStoreOpening.getId(), options);
	...
	//Using java8 feature to reduce the code significantly
 	protected RetailStoreOpening presentRetailStoreList(
		...
		SmartList<RetailStore> retailStoreList = retailStoreOpening.getRetailStoreList();		
				SmartList<RetailStore> newList= presentSubList(retailStoreOpening.getId(),
				retailStoreList,
				options,
    ...
    public SmartList<RetailStoreOpening> requestCandidateRetailStoreOpeningForRetailStore(RetailscmUserContext userContext, String ownerClass, String id, String filterKey, int pageNo, int pageSize) throws Exception {
		...
		return findAllCandidateByFilter(RetailStoreOpeningTable.COLUMN_COMMENT, filterKey, pageNo, pageSize, getRetailStoreOpeningMapper());
    }
	...
	public SmartList<RetailStore> loadOurRetailStoreList(RetailscmUserContext userContext, List<RetailStoreOpening> us, Map<String,Object> options) throws Exception{
		...
		MultipleAccessKey key = new MultipleAccessKey();
		key.put(RetailStore.OPENING_PROPERTY, ids.toArray(new String[ids.size()]));
		SmartList<RetailStore> loadedObjs = userContext.getDAOGroup().getRetailStoreDAO().findRetailStoreWithKey(key, options);
		Map<String, List<RetailStore>> loadedMap = loadedObjs.stream().collect(Collectors.groupingBy(it->it.getOpening().getId()));
		us.forEach(it->{
			String id = it.getId();
	...
	public void collectAndEnhance(BaseEntity ownerEntity) {
		List<RetailStoreOpening> retailStoreOpeningList = ownerEntity.collectRefsWithType(RetailStoreOpening.INTERNAL_TYPE);
		this.enhanceList(retailStoreOpeningList);
		
	}
	
	@Override
	public SmartList<RetailStoreOpening> findRetailStoreOpeningWithKey(MultipleAccessKey key,
			Map<String, Object> options) {
	...
	@Override
	public int countRetailStoreOpeningWithKey(MultipleAccessKey key,
			Map<String, Object> options) {
