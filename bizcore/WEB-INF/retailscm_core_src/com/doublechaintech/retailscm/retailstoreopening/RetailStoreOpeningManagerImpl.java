...
public class RetailStoreOpeningManagerImpl extends CustomRetailscmCheckerManager
    implements RetailStoreOpeningManager, BusinessHandler {
  ...
  public BlobObject exportExcelFromList(
    ...
    Map<String, Object> tokens =
        RetailStoreOpeningTokens.start().withTokenFromListName(listName).done();
    RetailStoreOpening retailStoreOpening =
        (RetailStoreOpening) this.loadRetailStoreOpening(userContext, id, tokens);
    // to enrich reference object to let it show display name
    List<BaseEntity> entityListToNaming = retailStoreOpening.collectReferencesFromLists();
  ...
  protected RetailStoreOpening saveRetailStoreOpening(
      RetailscmUserContext userContext, RetailStoreOpening retailStoreOpening, String[] tokensExpr)
      throws Exception {
  ...
  protected RetailStoreOpening saveRetailStoreOpeningDetail(
  ...
  public RetailStoreOpening loadRetailStoreOpening(
    ...
    RetailStoreOpening retailStoreOpening =
        loadRetailStoreOpening(userContext, retailStoreOpeningId, tokens);
    // do some calc before sent to customer?
    return present(userContext, retailStoreOpening, tokens);
  }

  public RetailStoreOpening searchRetailStoreOpening(
  ...
  protected RetailStoreOpening present(
      RetailscmUserContext userContext,
      RetailStoreOpening retailStoreOpening,
      Map<String, Object> tokens)
      throws Exception {
    ...
    RetailStoreOpening retailStoreOpeningToPresent =
        retailStoreOpeningDaoOf(userContext).present(retailStoreOpening, tokens);

    List<BaseEntity> entityListToNaming = retailStoreOpeningToPresent.collectReferencesFromLists();
    retailStoreOpeningDaoOf(userContext).alias(entityListToNaming);
  ...
  public RetailStoreOpening loadRetailStoreOpeningDetail(
      RetailscmUserContext userContext, String retailStoreOpeningId) throws Exception {
    RetailStoreOpening retailStoreOpening =
        loadRetailStoreOpening(userContext, retailStoreOpeningId, allTokens());
  ...
  public Object view(RetailscmUserContext userContext, String retailStoreOpeningId)
      throws Exception {
    RetailStoreOpening retailStoreOpening =
        loadRetailStoreOpening(userContext, retailStoreOpeningId, viewTokens());
    markVisited(userContext, retailStoreOpening);
    return present(userContext, retailStoreOpening, viewTokens());
  }

  public Object summaryView(RetailscmUserContext userContext, String retailStoreOpeningId)
      throws Exception {
    RetailStoreOpening retailStoreOpening =
        loadRetailStoreOpening(userContext, retailStoreOpeningId, viewTokens());
  ...
  public Object analyze(RetailscmUserContext userContext, String retailStoreOpeningId)
      throws Exception {
    RetailStoreOpening retailStoreOpening =
        loadRetailStoreOpening(userContext, retailStoreOpeningId, analyzeTokens());
    markVisited(userContext, retailStoreOpening);
  ...
  protected RetailStoreOpening loadRetailStoreOpening(
      RetailscmUserContext userContext, String retailStoreOpeningId, Map<String, Object> tokens)
      throws Exception {
    checkerOf(userContext).checkIdOfRetailStoreOpening(retailStoreOpeningId);
  ...
  protected <T extends BaseEntity> void addActions(
    ...
    // userContext, RetailStoreOpening retailStoreOpening, Map<String, Object> tokens){

  @Override
  public List<RetailStoreOpening> searchRetailStoreOpeningList(
  ...
  protected void checkParamsForUpdatingRetailStoreOpening(
      RetailscmUserContext userContext,
      String retailStoreOpeningId,
  ...
  public RetailStoreOpening clone(RetailscmUserContext userContext, String fromRetailStoreOpeningId)
      throws Exception {
  ...
  public RetailStoreOpening internalSaveRetailStoreOpening(
      ...
      throws Exception {
    // checkParamsForUpdatingRetailStoreOpening(userContext, retailStoreOpeningId,
    // retailStoreOpeningVersion, property, newValueExpr, tokensExpr);
  ...
  public RetailStoreOpening updateRetailStoreOpening(
      ...
      String[] tokensExpr)
      throws Exception {
    checkParamsForUpdatingRetailStoreOpening(
        userContext,
        retailStoreOpeningId,
        retailStoreOpeningVersion,
        property,
        newValueExpr,
        tokensExpr);
  ...
  public RetailStoreOpening updateRetailStoreOpeningProperty(
      RetailscmUserContext userContext,
      String retailStoreOpeningId,
      int retailStoreOpeningVersion,
      String property,
  ...
  public RetailStoreOpening forgetByAll(
      RetailscmUserContext userContext, String retailStoreOpeningId, int retailStoreOpeningVersion)
      throws Exception {
    return forgetByAllInternal(userContext, retailStoreOpeningId, retailStoreOpeningVersion);
  }

  protected RetailStoreOpening forgetByAllInternal(
      RetailscmUserContext userContext, String retailStoreOpeningId, int retailStoreOpeningVersion)
      throws Exception {

    return retailStoreOpeningDaoOf(userContext)
        .disconnectFromAll(retailStoreOpeningId, retailStoreOpeningVersion);
  }
  ...
  public RetailStoreOpening addRetailStore(
    ...
    RetailStoreOpening retailStoreOpening =
        loadRetailStoreOpening(userContext, retailStoreOpeningId, emptyOptions());
    synchronized (retailStoreOpening) {
      // Will be good when the retailStoreOpening loaded from this JVM process cache.
      // Also good when there is a RAM based DAO implementation
      retailStoreOpening.addRetailStore(retailStore);
      retailStoreOpening =
          saveRetailStoreOpening(
              userContext, retailStoreOpening, tokens().withRetailStoreList().done());
  ...
  public RetailStoreOpening updateRetailStoreProperties(
      RetailscmUserContext userContext,
      String retailStoreOpeningId,
      String id,
      String name,
    ...
    RetailStoreOpening retailStoreOpeningToUpdate =
        loadRetailStoreOpening(userContext, retailStoreOpeningId, options);

    if (retailStoreOpeningToUpdate.getRetailStoreList().isEmpty()) {
      throw new RetailStoreOpeningManagerException(
    ...
    // checkParamsForAddingRetailStore(userContext,retailStoreOpeningId,name, code,
    // used,tokensExpr);
    RetailStoreOpening retailStoreOpening =
        saveRetailStoreOpening(
            userContext, retailStoreOpeningToUpdate, tokens().withRetailStoreList().done());
    synchronized (retailStoreOpening) {
      return present(userContext, retailStoreOpening, mergedAllTokens(tokensExpr));
    }
  }
  ...
  public RetailStoreOpening removeRetailStoreList(
      RetailscmUserContext userContext,
      String retailStoreOpeningId,
      String retailStoreIds[],
      String[] tokensExpr)
    ...
    checkParamsForRemovingRetailStoreList(
        userContext, retailStoreOpeningId, retailStoreIds, tokensExpr);
    ...
    synchronized (retailStoreOpening) {
      ...
      retailStoreOpeningDaoOf(userContext)
          .planToRemoveRetailStoreList(retailStoreOpening, retailStoreIds, allTokens());
      retailStoreOpening =
          saveRetailStoreOpening(
              userContext, retailStoreOpening, tokens().withRetailStoreList().done());
  ...
  public RetailStoreOpening removeRetailStore(
    ...
    checkParamsForRemovingRetailStore(
        userContext, retailStoreOpeningId, retailStoreId, retailStoreVersion, tokensExpr);
  ...
  public RetailStoreOpening copyRetailStoreFrom(
      RetailscmUserContext userContext,
      String retailStoreOpeningId,
      String retailStoreId,
      int retailStoreVersion,
    ...
    synchronized (retailStoreOpening) {
      ...
      retailStoreManagerOf(userContext)
          .onNewInstanceCreated(
              userContext,
              (RetailStore) retailStoreOpening.getFlexiableObjects().get(BaseEntity.COPIED_CHILD));
      return present(userContext, retailStoreOpening, mergedAllTokens(tokensExpr));
    }
  }
  ...
  public RetailStoreOpening updateRetailStore(
    ...
    RetailStoreOpening retailStoreOpening =
        loadRetailStoreOpening(userContext, retailStoreOpeningId, loadTokens);
  ...
  */
  ...
  public void sendAllItems(RetailscmUserContext ctx) throws Exception {
    retailStoreOpeningDaoOf(ctx).loadAllAsStream().forEach(event -> sendInitEvent(ctx, event));
  }
  ...
  protected void enhanceForListOfView(
      RetailscmUserContext userContext, SmartList<RetailStoreOpening> list) throws Exception {
    if (list == null || list.isEmpty()) {
      return;
    }
  }
   ...
   * @throws Exception
   */
  public Object wxappview(RetailscmUserContext userContext, String retailStoreOpeningId)
      throws Exception {
    SerializeScope vscope = SerializeScope.EXCLUDE().nothing();
    RetailStoreOpening merchantObj =
        (RetailStoreOpening) this.view(userContext, retailStoreOpeningId);
