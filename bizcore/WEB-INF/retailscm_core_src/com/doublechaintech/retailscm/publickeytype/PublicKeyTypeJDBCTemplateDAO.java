...
public class PublicKeyTypeJDBCTemplateDAO extends RetailscmBaseDAOImpl implements PublicKeyTypeDAO{
	...
	*/
	...
	protected PublicKeyType clone(AccessKey accessKey, Map<String,Object> options) throws Exception{
		...
		newPublicKeyType.setVersion(0);
 		...
 		if(isSaveKeypairIdentifyListEnabled(options)){
 			for(KeypairIdentify item: newPublicKeyType.getKeypairIdentifyList()){
 				item.setVersion(0);
 			}
 		}
	...
	protected boolean isExtractKeypairIdentifyListEnabled(Map<String,Object> options){		
 		return checkOptions(options,PublicKeyTypeTokens.KEYPAIR_IDENTIFY_LIST);
 	}
 	protected boolean isAnalyzeKeypairIdentifyListEnabled(Map<String,Object> options){		 		
 		return PublicKeyTypeTokens.of(options).analyzeKeypairIdentifyListEnabled();
 	}
	...
	protected PublicKeyType loadInternalPublicKeyType(AccessKey accessKey, Map<String,Object> loadOptions) throws Exception{
		...
		if(isExtractKeypairIdentifyListEnabled(loadOptions)){
	 		extractKeypairIdentifyList(publicKeyType, loadOptions);
 		}	
 		
 		
 		if(isAnalyzeKeypairIdentifyListEnabled(loadOptions)){
	 		analyzeKeypairIdentifyList(publicKeyType, loadOptions);
 		}
	...
	protected PublicKeyType extractKeypairIdentifyList(PublicKeyType publicKeyType, Map<String,Object> options){
		...
		SmartList<KeypairIdentify> keypairIdentifyList = getKeypairIdentifyDAO().findKeypairIdentifyByKeyType(publicKeyType.getId(),options);
		if(keypairIdentifyList != null){
			enhanceKeypairIdentifyList(keypairIdentifyList,options);
			publicKeyType.setKeypairIdentifyList(keypairIdentifyList);
		}
	...
	protected PublicKeyType analyzeKeypairIdentifyList(PublicKeyType publicKeyType, Map<String,Object> options){
		...
		SmartList<KeypairIdentify> keypairIdentifyList = publicKeyType.getKeypairIdentifyList();
		if(keypairIdentifyList != null){
			getKeypairIdentifyDAO().analyzeKeypairIdentifyByKeyType(keypairIdentifyList, publicKeyType.getId(), options);
	...
	protected PublicKeyType saveInternalPublicKeyType(PublicKeyType publicKeyType, Map<String,Object> options){
		...
		if(isSaveKeypairIdentifyListEnabled(options)){
	 		saveKeypairIdentifyList(publicKeyType, options);
	 		//removeKeypairIdentifyList(publicKeyType, options);
	...
	public PublicKeyType planToRemoveKeypairIdentifyList(PublicKeyType publicKeyType, String keypairIdentifyIds[], Map<String,Object> options)throws Exception{
		...
		key.put(KeypairIdentify.KEY_TYPE_PROPERTY, publicKeyType.getId());
	...
	public int countKeypairIdentifyListWithSecUser(String publicKeyTypeId, String secUserId, Map<String,Object> options)throws Exception{
		...
		MultipleAccessKey key = new MultipleAccessKey();
		key.put(KeypairIdentify.KEY_TYPE_PROPERTY, publicKeyTypeId);
		key.put(KeypairIdentify.SEC_USER_PROPERTY, secUserId);
	...
	protected PublicKeyType saveKeypairIdentifyList(PublicKeyType publicKeyType, Map<String,Object> options){
	...
	protected PublicKeyType removeKeypairIdentifyList(PublicKeyType publicKeyType, Map<String,Object> options){
	...
	public PublicKeyType present(PublicKeyType publicKeyType,Map<String, Object> options){
		...
		presentKeypairIdentifyList(publicKeyType,options);
	...
	//Using java8 feature to reduce the code significantly
 	protected PublicKeyType presentKeypairIdentifyList(
			PublicKeyType publicKeyType,
			Map<String, Object> options) {

		SmartList<KeypairIdentify> keypairIdentifyList = publicKeyType.getKeypairIdentifyList();		
				SmartList<KeypairIdentify> newList= presentSubList(publicKeyType.getId(),
				keypairIdentifyList,
				options,
				getKeypairIdentifyDAO()::countKeypairIdentifyByKeyType,
				getKeypairIdentifyDAO()::findKeypairIdentifyByKeyType
				);

		
		publicKeyType.setKeypairIdentifyList(newList);
	...
	public SmartList<KeypairIdentify> loadOurKeypairIdentifyList(RetailscmUserContext userContext, List<PublicKeyType> us, Map<String,Object> options) throws Exception{
		...
		MultipleAccessKey key = new MultipleAccessKey();
		key.put(KeypairIdentify.KEY_TYPE_PROPERTY, ids.toArray(new String[ids.size()]));
		...
		Map<String, List<KeypairIdentify>> loadedMap = loadedObjs.stream().collect(Collectors.groupingBy(it->it.getKeyType().getId()));
		us.forEach(it->{
			String id = it.getId();
